...
const definitionFiles = files.filter(name => name !== 'ModelDefinition.ts' && /Definition\.ts$/.test(name));

definitionFiles.forEach(file => {
    const className = file.substring(0, file.length - 'Definition.ts'.length);
    generateInterfaces(className, path.join(definitionDir, file), path.join(definitionDir, className + 'Interfaces.ts'));
});

function generateInterfaces(className, definitionFile, interfaceFile) {
    const definition = require(definitionFile).default;
    const properties = Object.keys(definition.properties).map(key => definition.properties[key]);
