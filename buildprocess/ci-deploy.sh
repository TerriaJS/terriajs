...
http POST "https://api.github.com/repos/${TRAVIS_REPO_SLUG}/statuses/${TRAVIS_PULL_REQUEST_SHA}" "Authorization:token ${GITHUB_TOKEN}" status=pending context=deployment "target_url=http://ci.terria.io/${SAFE_BRANCH_NAME}/"
...
http POST "https://api.github.com/repos/${TRAVIS_REPO_SLUG}/statuses/${TRAVIS_PULL_REQUEST_SHA}" "Authorization:token ${GITHUB_TOKEN}" status=success context=deployment "target_url=http://ci.terria.io/${SAFE_BRANCH_NAME}/"
