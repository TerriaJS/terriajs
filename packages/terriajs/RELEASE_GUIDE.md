...
Run these commands from the standalone public repository root unless a step says otherwise. Publish from the public repository, not the private monorepo’s outer root.
...
- Bump the version number in `packages/terriajs/package.json`.
...
- Review and edit `packages/terriajs/CHANGES.md`.
  - Ensure that entries for new changes are written in the right section by diffing against the last version. Make sure the section name matches the version you set in `packages/terriajs/package.json` above.
  - e.g. `git diff 8.2.25 HEAD -- packages/terriajs/CHANGES.md` (where `8.2.25` is the previous published version)
...
- Bump the version number in `packages/terriajs/package.json`. Follow [semver](http://semver.org/).
- Appropriately update `packages/terriajs/CHANGES.md`, ensuring that changes are listed in their correct sections.
- Update code attributions using `pnpm --filter terriajs exec gulp code-attribution`
...
- `rm -rf packages/terriajs/wwwroot/build`
- Make sure you don't have any changes in your working directory.
- `pnpm --filter terriajs exec gulp lint release`
- `pnpm --dir packages/terriajs publish --publish-branch your-branch --tag your-tag` (for a release not at the tip of `main` a tag must be set, and it must not be `latest` or `next`. For releases on old versions you could use e.g. 6-5-x or 6-x-x as a tag, for upcoming features a short name could be used)
- Ensure that `packages/terriajs/CHANGES.md` on `main` is also updated to list the release and clearly note the nature and reason for release.

Manual publication does not create a Git tag. Create and push the release tag explicitly after a successful publish. The GitHub Actions workflow performs that tagging step automatically.
