...
- Added a map annotation tool: click a point on the map to place a marker and author rich text (using the same TinyMCE editor as Stories). Annotations render in both 2D (Leaflet) and 3D (Cesium) and persist in share links. Their text shows in floating popups anchored to the marker that track the marker as the camera/map moves (fixed screen size regardless of zoom); multiple can be open at once, and which popups are open is persisted so it is restored in share links and story scenes. Clicking a marker toggles its popup; each popup has edit/delete/close controls. Backed by a new `AnnotationCatalogItem` (`type: "annotations"`) plus a new `GlobeOrMap.addScreenAnchor` API that anchors a DOM element to a world position, tracking natively in each viewer (a Leaflet marker in 2D, a `postRender` update in 3D) so there is no lag through the Leaflet zoom animation.
