...
describe("ArcGisPortalCatalogGroup", function() {
  ...
  describe("change some settings - ", function() {
    ...
    it("Spatial Services Gallery group has been filtered from the groups", function() {
      if (portalServerStratum.groups && portalServerStratum.filteredGroups) {
        expect(portalServerStratum.groups.length).toBe(3);
        expect(portalServerStratum.filteredGroups.length).toBe(2);
      }
    });
