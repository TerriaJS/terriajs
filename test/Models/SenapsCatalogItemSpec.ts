...
describe("SenapsLocationsCatalogItem", function () {
  ...
  describe("Can not get any items if not providing base url", async function () {
    ...
    it("- fail to construct locations url", function () {
      expect(item._constructLocationsUrl()).toThrowError("Senaps base url not provided.");
    });

    it("- fail to construct correct streams url", function () {
      expect(item._constructStreamsUrl("123")).toThrowError("Senaps base url not provided.");
    });
  });
