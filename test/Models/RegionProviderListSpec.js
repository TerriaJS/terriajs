...
describe("RegionProviderList", function() {
  ...
  beforeEach(function() {
    ...
    regionProvideListPromise = RegionProviderList.fromUrl(
  ...
  it("matches postcodes with bad and duplicated values", function(done) {
    ...
    tableStructure.loadFromCsv(
      "postcode,value\n3068,1\n2000,2\n5,-1\nfour thousand,-4\n2000,3"
    );
    regionDetails = regionProviderList.getRegionDetails(
    ...
    regionProvider
      ...
      .then(function() {
        ...
        var regionValues = tableStructure.getColumnWithName(
          regionDetail.variableName
        ).values;
        var failedMatches = [];
        var ambiguousMatches = [];
        var indexOfPostcode3068 = regionProvider.regions
          .map(getId)
          .indexOf("3068");
        var indexOfPostcode2000 = regionProvider.regions
          .map(getId)
          .indexOf("2000");
        var regionIndices = regionProvider.mapRegionsToIndicesInto(
          regionValues,
          undefined,
          failedMatches,
          ambiguousMatches
  ...
  it("handles postcodes with leading zeroes", function(done) {
    ...
    regionProvider
      ...
      .then(function() {
        ...
        var indexOfPostcode0800 = regionProvider.regions
          .map(getId)
          .indexOf("0800");
