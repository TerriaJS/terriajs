...
describe('RegionProviderList', function() {
  ...
  it('matches postcodes as expected', function(done) {
    ...
    expect(regionDetails).not.toBe(null);
    expect(regionDetails.regionVariable).toBe('postcode');
    expect(regionDetails.regionProvider.regionType).toBe('POA');
    regionProvider = regionDetails.regionProvider;
    regionProvider.loadRegionIDs().then(function() {
      expect(regionProvider.regions.length).toBeGreaterThan(0);
      var regionValues = tableStructure.getColumnWithName(regionDetails.regionVariable).values;
