...
describe("CzmlCatalogItem", function () {
  ...
  beforeEach(function () {
    ...
    worker.use(
      ...
      http.get("test/CZML/Vehicle.czml", () =>
        HttpResponse.json(vehicleCzml)
      ),
      http.get("test/CZML/simple.czml", () =>
        HttpResponse.json(simpleCzml)
      ),
  ...
  describe("loading a very simple CZML file", function () {
    ...
    it("works by string", async function () {
      czml.setTrait(CommonStrata.user, "czmlString", JSON.stringify(verysimpleCzml));
      await czml.loadMapItems();
      expect(czml.mapItems[0].entities.values.length).toBeGreaterThan(0);
    });
    ...
    it("works by blob", async function () {
      const blob = new Blob([JSON.stringify(verysimpleCzml)], { type: "application/json" }) as File;
  ...
  describe("loading a CZML file with a moving vehicle", function () {
    ...
    it("works by string", async function () {
      czml.setTrait(CommonStrata.user, "czmlString", JSON.stringify(vehicleCzml));
      await czml.loadMapItems();
      expect(czml.mapItems[0].entities.values.length).toBeGreaterThan(0);
    });

    it("works by blob", async function () {
      const blob = new Blob([JSON.stringify(vehicleCzml)], { type: "application/json" }) as File;
      czml.setFileInput(blob);
      await czml.loadMapItems();
      expect(czml.mapItems[0].entities.values.length).toBeGreaterThan(0);
    });
  ...
  describe("error handling", function () {
    ...
    it("fails gracefully when the provided string is not JSON", async function () {
      czml.setTrait(CommonStrata.user, "czmlString", "test string that is not JSON");
      const error = (await czml.loadMapItems()).error;
    ...
    it("fails gracefully when the provided blob is not JSON", async function () {
      const invalidBlob = new Blob(["test string that is not JSON"], { type: "text/plain" }) as File;
