...
export default class TableColumn {
   ...
   */
  @computed
  private get values(): readonly string[] {
    const result: string[] = [];
   ...
   * A token `x` is also added - which corresponds to the value in **this column**.
   * For example:
   * - if `dependencies = ['columnA']`
   * - then a token will be generated for `columnA`
   * - and then we can access the value in an expression (eg `x*columnA` will multiple each value in this column, by the value in `columnA`)
   */
  @computed
  ...
  /**
   * Gets `math-expression-evaluator` expression in "postfix" notation. This is used by `this.valuesAsNumbers` to transform values.
   * This will "bake" tokens from `this.mexpColumnTokens` into the expression.
   * Pairs from `this.mexpColumnPairs` are added each time the expression is evaluated (see `this.valuesAsNumbers`)
   */
  @computed
  get mexpPostfix() {
