...
export default class ViewState {
  constructor(options) {
    ...
    knockout.defineProperty(this,'chartIsOpen', {
      get: function (){
        ...
        const chartableColumnExists = chartableItems.some(item => item.isEnabled && defined(item.chartData()) && !item.dontChartAlone && defined(item.tableStructure) && checkColumn(item));
  ...
  toggleFeaturePrompt(feature, state) {
    ...
    if (state && featureIndexInPrompts < 0) {
