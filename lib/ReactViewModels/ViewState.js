...
export default class ViewState {
    constructor(options) {
      ...
      this._storyPromptHandler = knockout.getObservable(this, 'storyShown').subscribe(storyShown => {
        if(storyShown === false) {
           // only show it once
           if(!defined(this.terria.getLocalProperty('storyPrompted'))){
             this.toggleFeaturePrompt('story', true);
             this.terria.setLocalProperty('storyPrompted', true);
           }
        }
      });
    }
    ...
    toggleFeaturePrompt(feature, state){
      const featureIndexInPrompts = this.featurePrompts.indexOf(feature);
      if(state === true){
        // add feature prompt
        featureIndexInPrompts < 0 && this.featurePrompts.push(feature);
      } else {
        //remove feature prompt
        featureIndexInPrompts > -1 && this.featurePrompts.splice(featureIndexInPrompts, 1);
      }
    }
}
