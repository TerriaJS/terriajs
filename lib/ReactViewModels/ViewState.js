...
export default class ViewState {
  constructor(options) {
    ...
    const updateIsMapFullscreen = () => {
      ...
      if(!defined(this.storyShown)){
        this.storyShown = terria.configParameters.storyEnabled && terria.stories && terria.stories.length;
      }
  ...
  toggleFeaturePrompt(feature, state, persistent= false) {
    ...
    if (state && featureIndexInPrompts < 0 && !this.terria.getLocalProperty(`${feature}Prompted`)) {
