...
export default class ViewState {
  constructor(options) {
    ...
    this.hotspotSummaryEnabled = false;
    this.selectedHotspot = null;
    this.RCSelectedSector = null;
    ...
    // Subscribing crashes an application
    // this._hotspotsFilterHandler = knockout
    //   .getObservable(this, "isHotspotsFiltered")
    //   .subscribe(isHotspotsFiltered => {
    //     if (!isHotspotsFiltered) {
    //       this.terria.nowViewing.items.map(item => {
    ...
    knockout.getObservable(terria, "pickedFeatures").subscribe(() => {
      if (defined(terria.pickedFeatures)) {
        if (terria.pickedFeatures.features.length > 0) {
          const featureProperties =
            terria.pickedFeatures.features[0].properties;
  ...
  dispose() {
    ...
    this._hotspotsFilterHandler.dispose();
