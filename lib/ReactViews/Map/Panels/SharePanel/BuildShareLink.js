...
export function getShareData(terria, includeStories = true) {
  ...
  if (oldShare === true) {
    addViewSettings(terria, initSource);
...
function addStratum(terria, stratumId, initSource) {
  ...
  terria.models.forEach((model, id) => {
    ...
    addModelStratum(model, stratumId, force, initSource);
  });
}
...
function addModelStratum(model, stratumId, force, initSource) {
 ...
 * @private
 */
function addViewSettings(terria, initSource) {
  ...
  initSource.initialCamera = initialCamera;
  initSource.homeCamera = homeCamera;
  initSource.baseMapName = terria.baseMap.name;
