...
import ArcGisFeatureServerCatalogGroup from "../../../../Models/ArcGisFeatureServerCatalogGroup";
import ArcGisFeatureServerCatalogItem from "../../../../Models/ArcGisFeatureServerCatalogItem";
...
export function getShareData(
  ...
  const containsEsriData =
    terria.workbench.items.filter(workbenchItem => {
      return (
        instanceOf(ArcGisMapServerCatalogItem, workbenchItem) ||
        instanceOf(ArcGisFeatureServerCatalog, workbenchItem) ||
        instanceOf(ArcGisFeatureServerCatalogItem, workbenchItem)
      );
    }).length > 0;

  addStratum(terria, CommonStrata.user, initSource);
  // if (containsEsriData) {
  //   addStratum(terria, "mapServer", initSource);
  // }
...
function addModelStratum(terria, model, stratumId, force, initSource) {
  ...
  // If the name of the layer isn't part of the user stratum, then make the name of the model 
  // the default name trait of the user stratum, so the layer name gets saved as part of the share data
  if (stratum && !stratum.name) {
    console.log(stratum);
    runInAction(() => {
      stratum.name = model.name;
    })
  }
