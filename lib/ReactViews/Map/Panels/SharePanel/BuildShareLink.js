...
export function getShareData(terria, includeStories = true) {
  ...
  addUserStratum(terria, initSources);
...
function addUserStratum(terria, initSources) {
  const models = {};

  terria.models.forEach((model, id) => {
    const modelJson = saveModelToJson(model, {
      includeStrata: [CommonStrata.user]
    });
    if (modelJson && modelJson.user) {
      models[id] = modelJson.user;
    }
  });
