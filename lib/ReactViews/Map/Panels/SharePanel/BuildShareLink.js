...
export function getShareData(terria, includeStories = true) {
  ...
  addUserStratum(terria, initSource);
...
function addUserStratum(terria, initSource) {
  initSource.strata = {
    user: {}
  };

  terria.models.forEach((model, id) => {
    addModelUserStratum(model, false, initSource);
  });
}

function addWorkbench(terria, initSource) {
  initSource.workbench = terria.workbench.itemIds;
}

function addModelUserStratum(model, force, initSource) {
  const user = initSource.strata.user;
  ...
  const stratum = model.strata.get(CommonStrata.user);
  const dereferenced = model.dereferenced;
  const dereferencedStratum = dereferenced
    ? dereferenced.strata.get(CommonStrata.user)
    : undefined;
  ...
  user[id] = stratum ? saveStratumToJson(model.traits, stratum) : {};

  if (dereferenced && dereferencedStratum) {
    user[id].dereferenced = saveStratumToJson(
      dereferenced.traits,
      dereferencedStratum
    );
  }
  ...
  if (
    ...
    user[id].knownContainerUniqueIds = model.knownContainerUniqueIds.slice();
  }
}
