...
export default function buildShareLink(terria) {
    // Build the share URL.
    const request = {
        version: '0.0.05',
    ...
    const baseUrl = uri.toString() + '#start=';
    const shortBaseUrl = uri.toString() + '#share=';
    const longUrl = baseUrl + encodeURIComponent(requestString) + generateUserPropertiesQuery();
...
function generateUserPropertiesQuery(terria) {
    return this.userPropWhiteList.reduce((querySoFar, key) => {
