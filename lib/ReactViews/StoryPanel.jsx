...
const StoryPanel = createReactClass({
    ...
    activateStory(id) {
        const story = this.state.stories.find(story => story.id === id);
    ...
    render() {
        return (
            <div className={Styles.storyPanel}>
                {stories.map(story => <div className={Styles.story} key={story.id} onClick={() => this.activateStory(story.id)}>
                     <h3>{story.title}</h3>
                     <p>{story.text}</p>
                </div>)}
            </div>
