  ...
  renderMonthGrid(){
    ...
    renderDayView(){
      ...
      renderHourView(){
        ...
        return <div className={Styles.hourview}><select onChange={(event)=>{this.setState({time: event.target.value}); this.props.onChange(event.target.value)}} value={this.state.time ? this.state.time: ''}><option value=''>Select a time</option> {timeOptions.map(t=> <option key={t.label} value={t.value}>{t.label}</option>)}</select></div>
      },
    ...
    renderDateSummray(time){
      const m = new Date(time);
        return <span><span>{m.getUTCFullYear()}/{m.getUTCMonth()+1}/{m.getUTCDate()}</span> <span>{this.utsTimeDisplay(m)}</span></span>
    },
