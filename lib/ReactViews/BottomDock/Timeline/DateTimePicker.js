...
const DateTimePicker = createReactClass({
    ...
    componentWillReceiveProps(nextProps){
      if(nextProps.value && nextProps.value.getTime() !== this.props.value.getTime()){
  ...
  renderYearGrid(){
    ...
    return <div className={Styles.grid}>{years.map(y=><div className={Styles.gridRow} key={y} onClick={()=>this.setState({year: y})}>
             <span className={Styles.gridLabel}>{y}</span>
             <span className={Styles.gridRowInner}>{monthOfYear.map(m=><span className={data[y][m] ? Styles.activeGrid : ''} key={m} ></span>)}</span></div>)}
           </div>
  },
  ...
  renderMonthGrid(){
    ...
    return <div className={Styles.grid}>
             ...
             <span className={Styles.gridRowInner}>{daysInMonth(i+ 1, year).map(d=><span className={ defined(data[year][i]) && defined(data[year][i][d+1]) ? Styles.activeGrid : ''} key={d} ></span>)}</span></div>)}
           </div>
  },
    ...
    render() {
      if(this.props.dates){
        return (
            ...
            <div><button className={Styles.backbutton} type='button' onClick={()=>this.goBack()}>Back</button></div>
