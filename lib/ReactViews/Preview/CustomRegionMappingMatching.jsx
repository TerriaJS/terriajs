...
const CustomRegionMappingMatching = createReactClass({
  ...
  componentDidMount() {
    RegionProviderList.fromUrl(
      ...
      .then(rpList => rpList.regionProviders.filter(rp => rp.server.getTile))
  ...
  handleGeoJsonRegionProviderChange(evt) {
    this.setState({
      geoJsonRegionProvider: this.state.geoJsonRegionProviders.find(
        rp => rp.regionType === evt.target.value
      )
    });
  },
  ...
  render() {
    return (
      <div>
        <h3>
          Match region column in this file against custom (GeoJSON) region
          definitions
        </h3>
        Select csv column:
        <select
        ...
        >
          <option value="" />
          {this.props.previewed._tableStructure.columns.map(tableColumn => (
