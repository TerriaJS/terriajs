...
const raiseErrorOnRejectedPromise = require("../../Models/raiseErrorOnRejectedPromise");
...
const registerCustomComponentTypes = function(terria) {
  ...
  function tableStructureFromStringData(stringData) {
    ...
    const result = new InlineChart(undefined, terria);

    if (stringData[0] === "[") {
      // Treat as json.
      raiseErrorOnRejectedPromise(
        terria,
        result.loadFromJson(stringData.replace(/&quot;/g, '"'))
      );
    } else if (
      stringData.indexOf("\\n") >= 0 ||
      stringData.indexOf("\n") >= 0
      ...
      raiseErrorOnRejectedPromise(
        terria,
        result.loadFromCsvString(stringData.replace(/\\n/g, "\n"))
      );
    }
