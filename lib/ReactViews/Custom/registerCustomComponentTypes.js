...
const registerCustomComponentTypes = function(terria) {
     ...
     * @private
     */
    function tableStructureFromStringData(stringData, yColumnName) {
        ...
        if (stringData.indexOf('\\n') >=0 || stringData.indexOf('\n') >= 0) {
            // Treat as csv.
            const tableStructure = TableStructure.fromCsv(stringData.replace(/\\n/g, '\n'));

            // Activate the provided y-column, or take a guess.
            if (defined(yColumnName)) {
                const columnToActivate = tableStructure.getColumnWithNameOrId(yColumnName);
                if (columnToActivate) {
                    columnToActivate.toggleActive();
                    return tableStructure;
                }
            }
            const suitableColumns = tableStructure.columnsByType[VarType.SCALAR];
        ...
        furtherProcessing: [
            ...
            //
            {
                ...
                processNode: (context, node, children) => { // eslint-disable-line react/display-name
                    ...
                    return React.createElement(node.name, {key: 'chart', colSpan: 2, className: ChartPreviewStyles.chart}, node.data, revisedChildren);
                }
            },
            {
                shouldProcessNode: node => (
