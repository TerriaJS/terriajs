...
function animateTimer(
  ...
  const opacityTransition = (element, repeatsLeft) => {
    if (repeatsLeft <= 0) {
      return;
    }

    // Use the element's existing opacity as the maximum opacity.
    const max = parseFloat(backgroundElement.style("opacity"));
    ...
    element
      ...
      .on("end", () => opacityTransition(element, repeatsLeft - 1)); // start cycle again
  };

  // Start our opacity animation.
  const repeats =
    Math.ceil(interval / options.opacityAnimationInterval) - 1;

  opacityTransition(backgroundElement, repeats);
  opacityTransition(elapsedTimeElement, repeats);
}
