...
function animateTimer(
  ...
  elapsedTimeElement
    ...
    .attrTween("d", () => t =>
      arcGenerator(radius)({ endAngle: angleInterpolator(t) })
    );

  // Rather than check if the opacity transitions are already operating (which they might be, because they use `
  // interval` to run repeatedly), we just re-add them regardless. Because they have the same d3 transition name, the
  // new transition replaces the previous one.
