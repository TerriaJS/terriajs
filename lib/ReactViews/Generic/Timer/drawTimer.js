 ...
 * @param {DOMElement} [options.elapsed=0] How much time has already passed.
 */
function animateTimer(
  ...
  elapsedTimeElement
    ...
    .transition("arc")
    .duration((interval - options.elapsed) * 1000)
    .ease(d3EaseLinear)
    ...
    .attrTween("d", () => t =>
      arcGenerator(radius)({ endAngle: angleInterpolator(t, startAngle) })
    );
  ...
  function allOpacityTransitions() {
    ...
    const backgroundMinOpacity =
      backgroundMaxOpacity - options.deltaOpacity > options.minOpacity
        ? backgroundMaxOpacity - options.deltaOpacity
        : options.minOpacity;
    const elapsedTimeMaxOpacity = parseFloat(
      elapsedTimeElement.style("opacity")
    );
    const elapsedTimeMinOpacity =
      elapsedTimeMaxOpacity - options.deltaOpacity > options.minOpacity
        ? elapsedTimeMaxOpacity - options.deltaOpacity
        : options.minOpacity;
    ...
    opacityTransition(
      ...
      backgroundMaxOpacity,
      "backgroundIn",
      options.opacityAnimationInterval / 2
    );

    // Elapsed time
    opacityTransition(
      elapsedTimeElement,
      elapsedTimeMaxOpacity,
      elapsedTimeMinOpacity,
      "timeOut"
    );
    opacityTransition(
      elapsedTimeElement,
  ...
  const animationInterval = d3Interval(() => {
    // We don't want to keep animating if the timer isn't actually in the DOM anymore, so we try and select an element
    // to check.
    const cssClass = backgroundElement.attr("class");
    const reselectElement = d3Select("." + cssClass);
    if (!defined(reselectElement) || reselectElement.empty()) {
