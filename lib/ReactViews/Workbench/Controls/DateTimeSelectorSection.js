...
const DateTimeSelectorSection = createReactClass({
    ...
    changeDateTime(time) {
        ...
        // Give this item focus on the timeline, so that the user can select all available dates for this item.
    ...
    render() {
        ...
        if (!item.canUseOwnClock || !defined(item.availableDates) || (item.availableDates.length === 0)) {
            return null;
        }
        return (
