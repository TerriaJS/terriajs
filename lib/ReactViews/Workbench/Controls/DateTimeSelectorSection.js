...
const DateTimeSelectorSection = createReactClass({
    ...
    goToPrevDate(availableDates, currentIndex){
      const prevDate = availableDates[currentIndex - 1];
      this.changeDateTime(prevDate);
    },

    goToNxtDate(availableDates, currentIndex){
      const prevDate = availableDates[currentIndex + 1];
      this.changeDateTime(prevDate);
    },
        ...
        return (
            <div className={Styles.datetimeSelector}>
                ...
                <div className={Styles.datetimeAndPicker}>
                    <button className={Styles.datetimePrev} disabled={currentIndex === 0} onClick={this.goToPrevDate.bind(this, availableDates, currentIndex)}><Icon glyph={Icon.GLYPHS.prev}/></button>
                    <span className={Styles.currentDate}>{defined(currentDate) && formatDateTime(currentDate)}</span>
                    <button className={Styles.datetimeNxt} disabled={currentIndex === availableDates.length - 1} onClick={this.goToNxtDate.bind(this, availableDates, currentIndex)}><Icon glyph={Icon.GLYPHS.next}/></button>
