            ...
            <div className={Styles.datetimeSelector}>
                ...
                <div className={Styles.datetimeSelectorInner}>
                  <div className={Styles.datetimeAndPicker}>
                      <button className={Styles.datetimePrevious} disabled={!item.previousTimeAvaliable()} onClick={item.previousTime.bind(item)}><Icon glyph={Icon.GLYPHS.previous}/></button>
                      <span className={Styles.currentDate}>{defined(discreteTime) && formatDateTime(discreteTime)}</span>
                      <button className={Styles.datetimeNext} disabled={!item.nextTimeAvaliable()} onClick={item.nextTime.bind(item)}><Icon glyph={Icon.GLYPHS.next}/></button>
                  </div>
