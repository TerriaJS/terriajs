...
const DateTimeSelectorSection = createReactClass({
    ...
    componentWillMount() {
        ...
        const that = this;
        if (!this._useOwnClockSubscription) {
            this._useOwnClockSubscription = knockout.getObservable(item, 'useOwnClock').subscribe(() => {
                that.updateClockSubscriptions();
            }, this);
        }
        this.updateClockSubscriptions();
    },

    updateClockSubscriptions(closing) {
        const item = this.props.item;

        //default(closing, false);

        if (closing || item.useOwnClock) {
            if (this._removeTerriaClockTickEvent) {
                this._removeTerriaClockTickEvent();
            }
        }
        if (closing || !item.useOwnClock) {
            if (this._itemClockSubscription) {
                this._itemClockSubscription.dispose();
            }
        }
        ...
        if (!closing) {
            if (!item.useOwnClock) {
                if (!this._removeTerriaClockTickEvent) {
                    this._removeTerriaClockTickEvent = item.terria.clock.onTick.addEventListener(clock => {
                ...
                if (!this._itemClockSubscription) {
                    this._itemClockSubscription = knockout.getObservable(item, 'clock').subscribe(clock => {
                        if (item.useOwnClock && !JulianDate.equals(clock.currentTime, this.state.currentTime)) {
                            this.setStateToCurrentTime();
                        }
                    }, this);
    ...
    componentWillUnmount() {
        this.updateClockSubscriptions(true);
        this._useOwnClockSubscription.dispose();
    },
