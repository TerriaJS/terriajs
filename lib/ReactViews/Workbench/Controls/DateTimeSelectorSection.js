            ...
            <div className={Styles.datetimeSelector}>
                ...
                <div className={Styles.datetimeSelectorInner}>
                  <div className={Styles.datetimeAndPicker}>
                      <button className={Styles.datetimePrevious} disabled={!item.isPreviousTimeAvaliable()} onClick={item.moveToPreviousTime.bind(item)}><Icon glyph={Icon.GLYPHS.previous}/></button>
                      <span className={Styles.currentDate}>{defined(discreteTime) && formatDateTime(discreteTime)}</span>
                      <button className={Styles.datetimeNext} disabled={!item.isNextTimeAvaliable()} onClick={item.moveToNextTime.bind(item)}><Icon glyph={Icon.GLYPHS.next}/></button>
                  </div>
