 ...
 */
function combineValueArrays(valueArrays, nameArrays) {
    ...
    const columnSource = []; // The n-th column of the result comes from columnArrays[columnSource[n].i][columnSource[n].j].
    ...
    const combinedNameArrays = combinedValueArrays.map(function(values, n) {
        return nameArrays[columnSource[n].i][columnSource[n].j];
    });

    return {values: combinedValueArrays, names: combinedNameArrays};
}
...
onmessage = function(event) {
    const valueArrays = event.data.values;
    const nameArrays = event.data.names;
    const combined = combineValueArrays(valueArrays, nameArrays);
    ...
    //     }
    // }
    const rows = toArrayOfRows(combined.values, combined.names);
