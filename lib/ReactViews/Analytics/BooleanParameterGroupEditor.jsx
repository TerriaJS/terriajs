...
const BooleanParameterGroupEditor = createReactClass({
    ...
    toggleAll: function(inputArgs, ev) {
        ...
        if (OneForAllValue === false) {
            ParamElementArray.forEach(function(Parameter) {
                ...
                if (thisButton.children[0].children[0].href.baseVal.split("-")[1] === "off") {
    ...
    renderCheckboxGroup() {
        ...
        let name;
        ((this.props.parameter.name) ? name = this.props.parameter.name + "_Group" : name = this.props.parameter.id + "_Group");
                ...
                <fieldset>
                    <legend>
                        <div 
                          ...
                          onClick={groupClick}
                        >
                          <MoreOrLess initialopen={whichIcon} 
                            myclass={Styles.btnRadio} />
                        </div>
                        <div id={OneForAllDivName} 
                             style={{display: 'inline-block'}} 
                             onClick={allClick}>
                    ...
                    <div id={name} style={whichIcon? ({display: 'block'}) : ({display: 'none'})}>
