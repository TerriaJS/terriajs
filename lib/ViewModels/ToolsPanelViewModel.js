...
var loadImage = require('../../Cesium/Source/Core/loadImage');
var loadWithXhr = require('../../Cesium/Source/Core/loadWithXhr');
var Rectangle = require('../../Cesium/Source/Core/Rectangle');
var throttleRequestByServer = require('../../Cesium/Source/Core/throttleRequestByServer');
...
function requestAbsData(requests) {
    ...
    var popup = PopupMessageViewModel.open('ui', {
        ...
        message: '<div>Caching ABS responses for ' + requests.length + ' data sources</div>'
    });
    ...
    var sendNext = function(currentIndex) {
        ...
        if (elPopup === null || currentIndex >= requests.length) {
            popup.message += '<h1>Completed ' + currentIndex + ' requests</h1>';
            return;
        }

        popup.message += '<h1>Caching ' + requests[currentIndex].item.name + '</h1>';
        if (elPopup !== null) {
            elPopup.scrollTop = elPopup.scrollHeight - elPopup.offsetHeight;
        } 
