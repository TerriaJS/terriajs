...
ToolsPanelViewModel.prototype.countDatasets = function() {
        ...
        for (var i = 0; i < group.items.length; ++i) {
            ...
            if (typeof item.items !== 'undefined') {
                ...
                if (defined(loadPromise) && item.isLoading) {
                    promises.push(loadPromise.then(recurseAndUpdateTotals.bind(undefined, item, stats, childStats)).otherwise(reportLoadError.bind(undefined, item)));
                } else {
                    recurseAndUpdateTotals(item, stats, childStats);
                }
            } else {
                ++stats.items;
            }
    ...
    function recurseAndUpdateTotals(item, stats, childStats) {
        var promise = counter(item, childStats).then(function() {
    ...
    function reportLoadError(item) {
        PopupMessageViewModel.open('ui', {
            title: 'Error loading group',
            message: 'Group "' + item.name + '" failed to load.'
        });
    }
