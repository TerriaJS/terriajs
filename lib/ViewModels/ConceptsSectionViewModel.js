...
var ConceptsSectionViewModel = function(catalogMember) {
    this.catalogMember = catalogMember;
    this.svgArrowDown = svgArrowDown;
    this.svgArrowRight = svgArrowRight;
};

ConceptsSectionViewModel.createForCatalogMember = function(catalogMember) {
    if (!defined(catalogMember.concepts)) {
        return undefined;
    }

    return new ConceptsSectionViewModel(catalogMember);
};
