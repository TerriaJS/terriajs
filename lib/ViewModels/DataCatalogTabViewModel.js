...
DataCatalogTabViewModel.prototype.clickEnabled = function(item) {
    ...
    if (defined(item.initialMessage) && !item.isEnabled && !item.initialMessageViewed) {
        handleInitialMessage(item, item.toggleEnabled);
    } else {
        item.toggleEnabled();
    }
};
...
DataCatalogTabViewModel.prototype.clickOpen = function(item) {
    ...
    if (defined(item.initialMessage) && !item.isOpen && !item.initialMessageViewed) {
        handleInitialMessage(item, item.toggleOpen);
    } else {
        item.toggleOpen();
    }
};
...
function handleInitialMessage(item, successCallback) {
