...
DataCatalogTabViewModel.prototype.clickEnabled = function(item) {
    ...
    if (!item.isEnabled && defined(item.initialMessage) && item.initialMessage.length > 0) {
        PopupMessageViewModel.open('ui', {
            title: 'Data Set Information',
            message: '<div>'+ item.initialMessage +'</div>'
        });
        item.initialMessage = undefined;
    }
...
DataCatalogTabViewModel.prototype.clickOpen = function(item) {
    ...
    if (!item.isOpen && defined(item.initialMessage) && item.initialMessage.length > 0) {
        PopupMessageViewModel.open('ui', {
