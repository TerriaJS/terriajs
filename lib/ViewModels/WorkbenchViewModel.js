...
var CatalogBrowserViewModel = require('./CatalogBrowserViewModel');
...
var WorkbenchTabViewModel = function(options) {
    ...
    this.noDataText = defaultValue(options.noDataText, "Your map does not yet have any data.  Click the link above to add some.");
    ...
    knockout.getObservable(this, 'isActive').subscribe(function(newValue) {
        ...
        if (newValue) {
            var WorkbenchItems = that.nowViewing.items;

            var oneIsOpen = false;
            for (var i = 0; !oneIsOpen && i < WorkbenchItems.length; ++i) {
                oneIsOpen = WorkbenchItems[i].isLegendVisible;
            }

            if (!oneIsOpen && WorkbenchItems.length > 0) {
                WorkbenchItems[0].isLegendVisible = true;
            }
        }
    });
...
inherit(ExplorerTabViewModel, WorkbenchTabViewModel);

WorkbenchTabViewModel.prototype.show = function(container) {
    loadView(require('fs').readFileSync(__dirname + '/../Views/Workbench.html', 'utf8'), container, this);
};

WorkbenchTabViewModel.prototype.showInfo = function(item) {
...
WorkbenchTabViewModel.prototype.urlIsImage = function(url) {
...
WorkbenchTabViewModel.prototype.dragStart = function(viewModel, e) {
...
WorkbenchTabViewModel.prototype.dragEnd = function(viewModel, e) {
    if (this._itemDropped) {
        var draggedItemIndex = this._draggedItem.getAttribute('WorkbenchIndex') | 0;
        var placeholderIndex = this._dragPlaceholder.getAttribute('WorkbenchIndex') | 0;
...
WorkbenchTabViewModel.prototype.dragEnter = function(viewModel, e) {
    if (!defined(this._draggedItem)) {
        return;
    }
    ...
    if (insertBefore && placeholderIndex !== targetIndex - 1) {
        ...
        this._dragPlaceholder.setAttribute('WorkbenchIndex', nodeToInsertBefore.getAttribute('WorkbenchIndex'));
        doInsert = true;
        ...
        if (!nodeToInsertBefore || !defined(nodeToInsertBefore.getAttribute)) {
            ...
            this._dragPlaceholder.setAttribute('WorkbenchIndex', this.nowViewing.items.length);
        } else {
            this._dragPlaceholder.setAttribute('WorkbenchIndex', nodeToInsertBefore.getAttribute('WorkbenchIndex'));
        }
        doInsert = true;
...
WorkbenchTabViewModel.prototype.selectStart = function(viewModel, e) {
...
WorkbenchTabViewModel.prototype.getItemControls = function(item) {
    var right = CatalogItemControl.rightSideItemControls(item);
    var left = CatalogItemControl.leftSideItemControls(item);
    left.push.apply(left, right);
    return left;
};

WorkbenchTabViewModel.prototype.addData = function() {
    CatalogBrowserViewModel.open('ui', {
        catalog: this.nowViewing.terria.catalog
    });
};

module.exports = WorkbenchTabViewModel;
