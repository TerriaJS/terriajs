...
var ExplorerPanelViewModel = function(options) {
    ...
    var activeTabIndex = this.terria.getUserProperty('activeTabIndex');

    if (!activeTabIndex || activeTabIndex >= this.tabs.length) {
        activeTabIndex = 0;
        this.terria.userProperties.activeTabIndex = activeTabIndex;
    }

    if (this.tabs.length > 0) {
        this.activateTab(this.tabs[activeTabIndex], false);
    }
...
ExplorerPanelViewModel.prototype.activateTab = function(tab, openPanel) {
    ...
    this.terria.userProperties.activeTabIndex = this.tabs.indexOf(tab);
};
...
ExplorerPanelViewModel.prototype.getActiveTabIndex = function() {
    return this.terria.getUserProperty('activeTabIndex');
};
