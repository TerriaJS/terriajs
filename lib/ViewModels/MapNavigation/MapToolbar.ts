...
interface ToolButtonOptions<Props = {}> extends BaseButtonOptions {
  tool: {
    name: string;
    component: () =>
      | React.ComponentType<Props>
      | Promise<React.ComponentType<Props>>;
    props?: Omit<Props, "closeTool">;
  };
}
...
interface ToolButton<ToolProps = any> {
  /**
   * Removes the button from the toolbar
   */
  removeButton: () => void;
   ...
   *
   * @param props props to pass to the tool in addition to props passed when
   * creating the tool button in `addToolButton` call. This is useful, for
   * example, if you want to open the tool in a specific mode.
   */
  openTool: (props?: Partial<ToolProps>) => void;
}

export function addToolButton<ToolProps = {}>(
  viewState: ViewState,
  optionsFn: (button: any) => ToolButtonOptions<ToolProps>
  ...
  const options = optionsFn(toolButton);
