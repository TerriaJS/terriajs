...
/**
 * Specifies the set up components that may be shown in a feature info section.  Only components that are compatible with the feature
 * are shown for that feature.  Shown components appear in the same order that they appear in this array.
 * @type {Array}
 */
FeatureInfoPanelSectionViewModel.components = [];
...
FeatureInfoPanelSectionViewModel.prototype.getComponents = function(feature) {
    var result = [];
    for (var i = 0; i < FeatureInfoPanelSectionViewModel.components.length; ++i) {
        var component = FeatureInfoPanelSectionViewModel.components[i].createForFeature(this.catalogItem, this.feature);
