...
// A Mustache function used to remove commas from numbers
function noCommas(text, render) {
...
function generateCsvData(data) {
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        if (type === 'function') {
            // Ignore template functions we've added here, such as 'nocommas'.
            continue;
        }
...
function propertyValues(properties, clock) {
    ...
    if (!defined(result.nocommas)) {
        result.nocommas = function() { return noCommas; };
    }
    return result;
}
