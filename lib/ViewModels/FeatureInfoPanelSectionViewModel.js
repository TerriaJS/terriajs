...
function propertyValues(properties, clock) {
    // Replace '.' and '#' in property keys with _, since Mustache cannot reference keys with these characters.
    // In future, we will check each property for a getValue function here too; if it exists, use it to get the current value.
    var badCharacters = new RegExp('[.#]', 'g');
    var result = {};
    for (var key in properties) {
        if (properties.hasOwnProperty(key)) {
            var cleanKey = key.replace(badCharacters, '_');
            result[cleanKey] = properties[key];
        }
    }
    return result;
}
