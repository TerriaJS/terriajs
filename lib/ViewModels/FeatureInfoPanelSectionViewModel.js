...
FeatureInfoPanelSectionViewModel.prototype.renderCustomTags = function() {
    var chartViewModel;
    for (var i = this.customTags.length - 1; i >= 0; i--) {
        chartViewModel = this.previewChartViewModelFromAttributes(this.customTags[i].attributes);
        chartViewModel.show(document.getElementById(this.customTags[i].id));
    };
};
...
function setInfoFromDescription(viewModel, description) {
    ...
    function walkAndReplaceCustomTags(element) {
        ...
        if (element.tagName.toLowerCase() === 'chart') {
            ...
            var id = createGuid();
            viewModel.customTags.push({tag: 'chart', attributes: attributesObject, id: id});
            var replacement = document.createElement('span');
            replacement.id = id;
            replacement.className = 'chart-wrapper';
            var parent = element.parentElement;
            // if this chart is in a table item (TD),
            ...
            // replace the element with a span element, with an id and wrapper class
