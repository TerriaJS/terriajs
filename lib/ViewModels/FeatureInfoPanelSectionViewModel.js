...
*/
var FeatureInfoPanelSectionViewModel = function(featureInfoPanelViewModel, feature, enableJSONViewer, catalogItem) {
    ...
    var position = defaultValue(this.feature.position || this.terria.pickedFeatures.pickPosition);
    if (defined(position)) {
        var latLngInRadians = Ellipsoid.WGS84.cartesianToCartographic(this.terria.pickedFeatures.pickPosition);
        data.coords = {
            lat: Math.round(CesiumMath.toDegrees(latLngInRadians.latitude) * 1000) / 1000,
            lng: Math.round(CesiumMath.toDegrees(latLngInRadians.longitude) * 1000) / 1000,
        };
    }
    ...
    this.enableJSONViewer = enableJSONViewer;
    ...
    knockout.defineProperty(this, 'showJsonTreeView', {
        get: function() {
            return this.enableJSONViewer && typeof this.rawData === 'object';
        }
    });
};
...
FeatureInfoPanelSectionViewModel.prototype._updateContent = function(data) {
    ...
    if (defined(this.feature.description)) {
        ...
        if(typeof this.rawData === "object" && defined(this.rawData.properties)) {
            this.rawData = this.rawData.properties;
        }
