...
var NominatimSearchProviderViewModel = function(options) {
  ...
  this.countryCodes = defined(options.countryCodes)
    ? "&countrycodes=" + options.countryCodes // eslint-disable-line i18next/no-literal-string
    : "";

  this._geocodeInProgress = undefined;

  this.name = "Nominatim"; // eslint-disable-line i18next/no-literal-string
  this.url = defaultValue(options.url, "//nominatim.openstreetmap.org/"); // eslint-disable-line i18next/no-literal-string
...
NominatimSearchProviderViewModel.prototype.search = function(searchText) {
  ...
  if (defined(this.terria.cesium)) {
    ...
    if (defined(posUL) && defined(posLR)) {
      ...
      bboxStr =
        "&viewbox=" + // eslint-disable-line i18next/no-literal-string
  ...
  var promiseBounded = loadJson(
    this.url +
    "search?q=" + // eslint-disable-line i18next/no-literal-string
    searchText +
    bboxStr +
    "&bounded=1&format=json" + // eslint-disable-line i18next/no-literal-string
    this.countryCodes +
    "&limit=" + // eslint-disable-line i18next/no-literal-string
      this.limitBounded
  );
  var promiseOthers = loadJson(
    ...
    "search?q=" + // eslint-disable-line i18next/no-literal-string
    searchText +
    "&format=json" + // eslint-disable-line i18next/no-literal-string
    this.countryCodes +
    "&limit=" + // eslint-disable-line i18next/no-literal-string
      this.limitOthers
  );
