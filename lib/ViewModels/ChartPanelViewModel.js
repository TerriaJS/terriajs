...
var ChartPanelViewModel = function(options) {
    ...
    this.charts = knockout.computed(function() {
        // For now, assume the x variable is always the first Time variable in the same file as the selected data variable
        var timeVariable,
            ys = [];
        for (var i = chartDataGroup.items.length - 1; i >= 0; i--) {
            var dataCatalogItem = chartDataGroup.items[i];
            if (dataCatalogItem.isEnabled) {
                // doesn't get triggered by changes in dataCatalogItem.dataTable.selected.data
                var selected = dataCatalogItem.dataTable.selected;
                timeVariable = dataCatalogItem.dataTable.variables[selected.time];
                var dataVariable = dataCatalogItem.dataTable.variables[selected.data];
                if (defined(dataVariable)) {
                    ys.push(dataVariable.vals);
                }
            }
        }
        ...
        this.isVisible = (ys.length > 0) || this.isLoading;
        if (ys.length > 0) {
            var x = timeVariable.vals.map(function(d) { return new Date(d) }); // temp - turn string dates into Date
            return [
                new ChartViewModel(this, this, zipXYs(x, ys), {
