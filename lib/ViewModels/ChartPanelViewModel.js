    ...
    this.charts = knockout.computed(function() {
        ...
        for (var i = chartDataGroup.items.length - 1; i >= 0; i--) {
            ...
            if (dataCatalogItem.isEnabled) {
                var xColumn = dataCatalogItem.xColumn;
                var yColumns = dataCatalogItem.yColumns;
                if (defined(yColumns) && defined(xColumn)) {
                    var x = xColumn.values.map(datify); // temp - turn string dates into Date
                    for (var j = 0; j < dataCatalogItem.yColumns.length; j++) {
                        dataPairArrays.push(zip(x, dataCatalogItem.yColumns[j].values));
                        var color = '#ABCDEF';
                        colors.push(color);
                    }
                }
