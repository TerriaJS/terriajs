...
var ChartPanelViewModel = function(options) {
    ...
    var activeVariables = knockout.computed(function() {
        var result = [];
        for (var i = chartDataGroup.items.length - 1; i >= 0; i--) {
            var dataCatalogItem = chartDataGroup.items[i];
            if (dataCatalogItem.isEnabled) {
                // doesn't get triggered by changes in dataCatalogItem.dataTable.selected.data - why? even this doesn't work:
                var selected = dataCatalogItem.dataTable.selected;
                var timeVariable = selected.time;
                var dataVariable = dataCatalogItem.dataTable.variables[selected.data];
                if (defined(dataVariable)) {
                    result.push(dataVariable);
                }
            }
        }
        console.log('found these variables to chart:', result);
        this.isVisible = (result.length > 0);
        return result;
    }, this);
