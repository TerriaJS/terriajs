...
InvokeFunctionPanelViewModel.prototype.invokeFunction = function() {
    var that = this;
    try {
        var promise = when(this.catalogFunction.invoke(this.parameterValues)).otherwise(function(modelError) {
            if (modelError instanceof ModelError) {
                that.catalogFunction.terria.error.raiseEvent(modelError);
            }
        });
        this.close();
