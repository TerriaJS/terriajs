...
function updateForNewTopLayer(animationViewModel) {
    ...
    {
        setClockCurrentTime(animationViewModel);
    }
 ...
 * @param {Terria} terria instance to set current time on
 * @param {JulianDate} date to set
 * @private
 */
function _setTimeIfInRange(terria, timeToSet)
{
    if (JulianDate.lessThan(timeToSet, terria.clock.startTime)) {
        terria.clock.currentTime = terria.clock.startTime;
    }
    else if (JulianDate.greaterThan(timeToSet, terria.clock.stopTime)) {
        terria.clock.currentTime = terria.clock.stopTime;

    } else {
        terria.clock.currentTime = timeToSet;
    }
}
 ...
 *     timesliderInitTime: "present"                            // closest to today's date
 *     timesliderInitTime: "start"                              // start of time range of animation
 *     timesliderInitTime: "end"                                // end of time range of animation
 *     timesliderInitTime: An ISO8601 date e.g. "2015-08-08"    // specified date or nearest if date is outside range
 * @param {AnimationViewModel} view model to set current time on
 */
function setClockCurrentTime(animationViewModel)
{
    var terria = animationViewModel.terria;

    // This is our default. Start at the nearest instant in time.
    var now = JulianDate.now();
    _setTimeIfInRange(terria, now);

    if (defined(terria.timeSeriesStack.topLayer.timesliderInitTime))
    {
        var startTime = terria.timeSeriesStack.topLayer.timesliderInitTime;
        switch(startTime)
        {
            case "start":
                terria.clock.currentTime = terria.clock.startTime;
            ...
            default:
                ...
                var timestamp;
                try {
                    timestamp = JulianDate.fromIso8601(startTime);
                }
                catch (e) {
                    throw new DeveloperError('No good timesliderInitTime specified in config file: ' + startTime);
                }
                if (defined(timestamp))
                {
                    _setTimeIfInRange(terria, timestamp);
                }
        }
    }
}
