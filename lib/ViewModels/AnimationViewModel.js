        ...
        if (this.showAnimation) {
            ...
            if (defined(topLayer.intervals)) {
                for (var i = 1; i < topLayer.intervals._intervals.length; i++) {
                    ...
                    var intervalRange = that.timeline.addHighlightRange('rgba(0, 0, 255, 0.9)', 27, 0);
                    intervalRange.setRange(topLayer.intervals._intervals[i - 1].start, topLayer.intervals._intervals[i - 1].stop);

                    if (JulianDate.compare(gapEnd, gapStart) > 0) {
                        var range = that.timeline.addHighlightRange('rgba(0, 0, 0, 0.5)', 27, 0);
                        //var range = that.timeline.addHighlightRange('rgba(255, 0, 0, 0.9)', 3, 5);
