    ...
    if ((terria.viewerMode === ViewerMode.CesiumTerrain || terria.viewerMode === ViewerMode.CesiumEllipsoid) && !this._useWebGL) {
        if (this._slowWebGLAvailable) {
            PopupMessageViewModel.open(this._uiContainer, {
                ...
                message : 'Your web browser reports that performance with WebGL, a technology used for the 3D view in ' + terria.appName + ', \
is likely to be very poor, so you will see a limited, 2D-only experience.  Consider trying a different web browser, upgrading your video \
drivers, or upgrading your operating system.  Alternatively, you may dismiss this message and continue to use ' + terria.appName + ' in 2D.'
            });
        } else {
            PopupMessageViewModel.open(this._uiContainer, {
...
function supportsWebGL() {
    ...
    if (!gl) {
        // We could get a WebGL context without a major performance caveat.  Let's see if we can get one at all.
