...
us via email at '+that.terria.supportEmail+'.'
    ...
    // if both terriajs default credit and cecium ion credits are required
    if(defined(this._defaultTerriaCredit) && defined(this._bumpyTerrainCredit)){
      // This line worked for sometimes, then stopped working. Therefore I also use CSS to hide it in case it doesn't work
      scene.frameState.creditDisplay.removeDefaultCredit(this._bumpyTerrainCredit);
      const combinedCredit = new Credit(`<div id='logo-credit'>${this._defaultTerriaCredit._html} ${this._bumpyTerrainCredit._html}</div>`);
      scene.frameState.creditDisplay.addDefaultCredit(combinedCredit);
    } else if(defined(this._defaultTerriaCredit)){
        scene.frameState.creditDisplay.addDefaultCredit(this._defaultTerriaCredit);
    }
   ...
   //create leaflet viewer
    ...
    if (d) {
        map.attributionControl.setPrefix(d.element.outerHTML +
            this._defaultTerriaCredit._html +
...
function initializeTerrainProvider(terriaViewer) {
    ...
    if (!terriaViewer.terria.configParameters.hideTerriaLogo){
      const terriaLogo = require('../../wwwroot/images/terria-watermark-onLight.svg');
      terriaViewer._defaultTerriaCredit = new Credit('<a href="https://terria.io/" target="_blank" rel="noopener noreferrer" ><img src="' + terriaLogo + '" title="Cesium ion"/></a>', true);
      CreditDisplay._terriaCredit = terriaViewer._defaultTerriaCredit;
    } else {
      CreditDisplay._terriaCredit = undefined;
    }
}
