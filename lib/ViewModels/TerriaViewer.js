...
Your web browser does not appear to support WebGL, so you will see a limited, 2D-only experience.'
  ...
  this.observeSubscriptions.push(
    knockout.getObservable(this.terria, "quality").subscribe(function() {
      changeQuality(this);
    }, this)
  );
};
...
function changeQuality(viewer) {
  if (defined(viewer.terria.cesium)) {
    const terria = viewer.terria;

    // 0 - defaults
    // 1 - devicePixelRatio as resolutionScale
    // 2 - increase maximumScreenSpaceError
    switch (viewer.terria.quality) {
      case 1:
        terria.cesium.viewer.resolutionScale = 1.0 / devicePixelRatio;
        terria.cesium.viewer.scene.globe.maximumScreenSpaceError = 1;
        break;
      case 2:
        terria.cesium.viewer.resolutionScale = 1.0;
        terria.cesium.viewer.scene.globe.maximumScreenSpaceError = 1;
        break;
      default:
        terria.cesium.viewer.resolutionScale = 1.0 / devicePixelRatio;
        terria.cesium.viewer.scene.globe.maximumScreenSpaceError = 2;
        break;
    }
