...
var SharePopupViewModel = function(options) {
    ...
    this.enableShortenUrl = defined(this.terria.shortenShareUrl);
...
function setShareUrl(viewModel) {
    ...
    if (!viewModel.shortenUrl) {
        ...
        loadWithXhr({
            url : viewModel.terria.shortenShareUrl,
            method : "POST",
            ...
            if (hashIndex === -1 || hashIndex >= result.id.length) {
                console.log('Unexpected url shortening result');
            }
            else {
                viewModel._shortUrl = viewModel._baseUrl + result.id.substring(hashIndex+1);
                setUrlAndEmbed(viewModel._shortUrl);
            }
        }).otherwise(function() {
