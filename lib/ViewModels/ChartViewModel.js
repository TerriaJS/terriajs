...
var DataCatalogItem = require('../Models/DataCatalogItem');
...
ChartViewModel.prototype.expand = function() {
    ...
    var dataCatalogItem = new DataCatalogItem(terria, this.data, this.options);
    dataCatalogItem.load().then(function() {
        console.log('success', dataCatalogItem);
        var group = new CatalogGroup(terria);
        group.name = 'Chart Data';
        group.description = 'A group for chart data.';
        group.add(dataCatalogItem);
        dataCatalogItem.toggleEnabled();
        terria.catalog.group.add(group);
    }).otherwise(function () {
        console.log('failed', dataCatalogItem);
    });
};
