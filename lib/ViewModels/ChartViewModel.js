...
var ChartViewModel = function(chartPanelViewModel, containerViewModel, data, options) {
    ...
    this.height = defaultValue(this.options.colors, defaultOptions.colors);
...
ChartViewModel.prototype.expand = function() {
    ...
    fullOptions.name = this.containerViewModel.name + ': ' + this.options.title;
    var dataCatalogItem = new DataCatalogItem(terria, this.options.src || this.data, fullOptions);
    var group = terria.catalog.chartDataGroup;
    group.isOpen = true;
    group.add(dataCatalogItem);
    dataCatalogItem.isEnabled = true;  // this loads it as well
};
