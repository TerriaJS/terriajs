...
GazetteerSearchProviderViewModel.prototype.search = function(searchText) {
  ...
  var url = this.url + "?q=name:*" + searchText + "*"; // eslint-disable-line i18next/no-literal-string
  // filter out bores, built structures, caves, landmarks, trig stations
  url += "%20-class_code:(D%20E%20G%20J%20T)%20-feature_code:PRS"; // eslint-disable-line i18next/no-literal-string
  url += "&rows=200"; // eslint-disable-line i18next/no-literal-string
          ...
          that.searchResults.push(
            new SearchResultViewModel({
              ...
              isImportant: !!result.feature_code.match(
                "^(CNTY|CONT|DI|PRSH|STAT|LOCB|LOCU|SUB|URBN)$" // eslint-disable-line i18next/no-literal-string
              ),
