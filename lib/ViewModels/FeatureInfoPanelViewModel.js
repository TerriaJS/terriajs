...
FeatureInfoPanelViewModel.prototype.showFeatures = function(features) {
    ...
    when(features.allFeaturesAvailablePromise, function() {
        ...
        if (defined(feature.description)) {
            ...
            var data = feature.description.getRawValue(that.terria.clock.currentTime),
                template = that.terria.infoTemplate;
            that.html = template ? readTemplate(data, template) : feature.description.getValue();
        } else {
            that.html = '';
        }
...
function configureHtmlUpdater(viewModel, featureDescription) {
    ...
    if (defined(featureDescription) && !featureDescription.isConstant) {
        viewModel._clockSubscription = viewModel.terria.clock.onTick.addEventListener(function(clock) {
            ...
            var data = featureDescription.getRawValue(clock.currentTime),
                template = viewModel.terria.infoTemplate;
            viewModel.html = template ? readTemplate(data, template):featureDescription.getValue(clock.currentTime);
        });
    }
}
