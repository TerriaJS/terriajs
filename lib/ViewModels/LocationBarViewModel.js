...
function updateCoordinatesFromCesium(viewModel, position) {
    ...
    if (defined(pickedTriangle)) {
        ...
        var maxDiff = Math.max(approximateHeight - minHeight - viewModel.geoidModel.minimumHeight, maxHeight + viewModel.geoidModel.maximumHeight - approximateHeight);
        errorBar = Math.min(errorBar, maxDiff);
        errorBar = Math.max(errorBar, Math.abs(viewModel.geoidModel.minimumHeight));
        errorBar = Math.max(errorBar, Math.abs(viewModel.geoidModel.maximumHeight));
