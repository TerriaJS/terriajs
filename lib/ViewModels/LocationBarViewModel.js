...
var LocationBarViewModel = function(options) {
    ...
    this.projection = defaultValue(options.projection, new WebMercatorProjection());
    ...
    knockout.track(this, ['latitude', 'longitude', 'elevation', 'useProjection']);
...
function cartographicToFields(viewModel, coordinates, errorBar) {
    if (viewModel.useProjection) {
        var projected = viewModel.projection.project(coordinates);
        viewModel.latitude = projected.y.toFixed(3) + 'm';
        viewModel.longitude = projected.x.toFixed(3) + 'm';
    } else {
