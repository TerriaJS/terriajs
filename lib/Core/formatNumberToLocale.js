...
* and polyfills the maximumFractionDigits option, with rounding.
*
...
function formatNumberToLocale(number, options) {
    ...
    if (idx !== -1) {
        ...
        if (frac.length > options.maximumFractionDigits + 1) {
            frac = ((+frac).toFixed(options.maximumFractionDigits));
            // frac now start with 0.xxx or 1.xxx instead of just .xxx. If it's 1.xxx, need to add 1 to str.
            if (frac[0] === '1') {
                str = (number + 1).toString();
                idx = str.indexOf('.');
            }
            // Note with maximumFractionDigits = 1, this gives 9.99 => "10.0", whereas formatter gives "10".
