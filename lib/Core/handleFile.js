...
function handleFile(e, terria, _fileType, callback) {
        ...
        for (let i = 0; i < files.length; ++i) {
            ...
            if (file.name.toUpperCase().indexOf('.JSON') !== -1) {
                raiseErrorOnRejectedPromise(terria, readJson(file).then((json)=>{
                    ...
                    promises.push(addUserCatalogMember(terria, createCatalogItemFromFileOrUrl(terria, file, dataType.value, true)));
                }));
            } else {
                promises.push(addUserCatalogMember(terria, createCatalogItemFromFileOrUrl(terria, file, dataType.value, true)));
            }
