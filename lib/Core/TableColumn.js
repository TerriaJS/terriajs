...
var TableColumn = function(name, values, options) {
    ...
    if ((!this.isNumeric) && this._type === VarType.SCALAR) {
        this._type = VarType.ENUM;
    }

    this._uniqueValues = undefined;
    if (!this.isNumeric) {
        this._uniqueValues = getUniqueValues(this._values);
    }

    this._displayVariableTypes = options.displayVariableTypes;
    if (defined(this._displayVariableTypes)) {
        this.isVisible = (this._displayVariableTypes.indexOf(this._type) >= 0);
    }
...
defineProperties(TableColumn.prototype, {
    ...
    type: {
        ...
        set: function(type) {
            // Could also update this.dates etc when set to VarType.TIME (if needed).
            this._type = type;
            if (!this.isNumeric) {
                this._uniqueValues = getUniqueValues(this._values);
            }
    ...
    /**
     * Returns whether this column has any numeric values.
     * @memberOf TableColumn.prototype
     * @type {Boolean}
     */
    isNumeric: {
        get: function() {
            return !isNaN(this._minimumValue);
        }
    },
