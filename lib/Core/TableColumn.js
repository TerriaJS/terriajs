...
* @param {TableStructure} [tableStructure] The table structure this column belongs to. Required so that only one column is selected at a time.
* @param {VarType} [type] The variable type (eg. VarType.SCALAR). If not present, an educated guess is made based on the name.
*/
var TableColumn = function(name, values, tableStructure, type) {
    VariableConcept.call(this, name, tableStructure);  // VariableConcept = function(name, parent, active, color)
