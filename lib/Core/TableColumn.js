...
function calculateFinishDates(julianDates) {
    ...
    // Calculate the average spacing of the unique dates, to use as the final duration (with one second shaved off).
    ...
    if (n > 1) {
        finalDurationSeconds = JulianDate.secondsDifference(revisedDates[n - 1], revisedDates[0]) / (n - 1) - 1;
    }
