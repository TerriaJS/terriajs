...
var typeHintSet = [
    ...
    { hint: /^(.*[_ ])?(time|date|year)/i, type: VarType.TIME },
 ...
 *
 * @param  {Array} values The column's values.
...
function convertToDates(values) {
    ...
    if ((firstPositionMaximum >= 1850) && (firstPositionMaximum <= 2100) && areAllIntegers(values)) {
        subtype = VarSubType.YEAR;
        dateParsers = function(v) {
            var jsDate = new Date('01-01-' + v);
