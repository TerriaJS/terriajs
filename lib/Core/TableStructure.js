...
function describeRow(rowObject, infoFields) {
        ...
        if (infoFields.hasOwnProperty(key)) {
            ...
            if (defined(value)) {
                ...
                if (key.substring(0, 2) === '__') {
                    continue;
                // } else if (value instanceof JulianDate) { // not used
                //     value = JulianDate.toDate(value).toDateString();
                }
                if (value === null) {
                    value = '';
                } else if (typeof value === 'object') {
                    value = describeRow(value, infoFields);
                } else {
...
TableStructure.prototype.toRowDescriptions = function(featureInfoFields) {
    ...
    return this.toRowObjects().map(function(rowObject) { return describeRow(rowObject, infoFields); });
};
