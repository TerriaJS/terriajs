 ...
 */
var TableStructure = function(name) {
...
TableStructure.fromJson = function(json, result) {
    ...
    for (var columnNumber = 0; columnNumber < columnNames.length; columnNumber++) {
        ...
        result.columns.push(new TableColumn(name, values, result));
    }
...
function setColumnsByType(structure) {
    ...
    for (var varType in VarType) {
        if (VarType.hasOwnProperty(varType)) {
            ...
            structure._columnsByType[v] = [];
        }
    }
    for (var i = 0; i < structure.columns.length; i++) {
        var column = structure.columns[i];
        structure._columnsByType[column.type].push(column);
    }
}
