...
var ConsoleAnalytics = function() {
  ...
  // or pass true in `configParameters.logToConsole` for verbose app analytics
...
ConsoleAnalytics.prototype.start = function(configParameters) {
  ...
  if (defined(configParameters.logToConsole)) {
    console.log(i18next.t("core.consoleAnalytics.started"));
    this.logToConsole = configParameters.logToConsole;
  } else if (
    !defined(configParameters.logToConsole) ||
    !configParameters.logToConsole
  ) {
