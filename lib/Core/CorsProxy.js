...
var DEFAULT_BASE_PROXY_WITH_KEY_PATH = 'proxywithkey/';
...
function CorsProxy(overrideLoadJson) {
    ...
    /**
     * The base URL of the TerriaJS server proxy when a key may need to be added, to which requests will be appended.
     * In most cases this is the server's host + '/proxywithkey'.
     * @type {String}
     */
    this.baseProxyWithKeyUrl = undefined;
    /**
 ...
 * @param {String} baseProxyWithKeyUrl The base URL to proxy with when a key needs to be inserted by the server - this will default to 'proxywithkey/'
 * @returns {Promise} A promise that resolves when initialisation is complete.
 */
CorsProxy.prototype.init = function(serverConfig, baseProxyUrl, proxyDomains, baseProxyWithKeyUrl) {
    this.baseProxyUrl = defaultValue(baseProxyUrl, DEFAULT_BASE_PROXY_PATH);
    this.baseProxyWithKeyUrl = defaultValue(baseProxyWithKeyUrl, DEFAULT_BASE_PROXY_WITH_KEY_PATH);
...
/**
 * Proxies a URL by appending it to {@link CorsProxy#baseProxyWithKeyUrl} (when a keyType is provided), or
 * {@link CorsProxy#baseProxyUrl} (otherwise).
 * Either way, optionally inserts a proxyFlag that will override the cache headers of the response,
 * allowing for caching to be added where it wouldn't otherwise.
 *
 ...
 * @param {String} [keyType] A key type available in the serverconfig, in its 'keys' property.
 * @returns {String} The proxied URL, using the baseProxyWithKeyUrl endpoint instead of the baseProxyUrl.
 */
CorsProxy.prototype.getURL = function (resource, proxyFlag, keyType) {
    var baseUrl;
    if (defined(keyType)) {
        baseUrl = this.baseProxyWithKeyUrl + keyType + '/';
    } else {
        baseUrl = this.baseProxyUrl;
    }
    var flag = (proxyFlag === undefined) ? '' : '_' + proxyFlag + '/';
    return baseUrl + flag + resource;
};
 ...
 */
CorsProxy.prototype.getURLProxyIfNecessary = function (resource, proxyFlag, keyType) {
    if (this.shouldUseProxy(resource)) {
        return this.getURL(resource, proxyFlag, keyType);
    }
