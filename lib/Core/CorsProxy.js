...
/**
 * Initialises values with config from the proxy server. This is the recommended way to use this object as it ensures
 * the options will be correct for the proxy server it's configured to call, but this can be skipped and the values it
 * initialises set manually if desired. Note that if the request to get config from the proxy server fails, an error
 ...
 *
 * @param {String} proxyableDomainsUrl The url to call on the terriajs server in order to get proxyable domains and other config
 * @param {String} baseProxyUrl The base URL to proxy with - this will default to 'proxy/'
 * @param {Array[string]} proxyDomains Initial value for proxyDomains to which proxyable domains from the server will be appended -
 *      defaults to an empty array.
 * @returns {Promise} A promise that resolves when initialisation is complete.
 */
CorsProxy.prototype.init = function(proxyableDomainsUrl, baseProxyUrl, proxyDomains) {
    this.baseProxyUrl = defaultValue(baseProxyUrl, DEFAULT_BASE_PROXY_PATH);
    this.proxyDomains = defaultValue(proxyDomains, []);

    return this.loadJson(proxyableDomainsUrl).then(function(proxyableDomains) {
        if (proxyableDomains.proxyAllDomains) {
            this.isOpenProxy = true;
        }
        this.proxyDomains = this.proxyDomains.concat(proxyableDomains.proxyableDomains);
    }.bind(this)).otherwise(function(e) {
        console.error('Failed to resolve proxyableDomains from ' + proxyableDomainsUrl);
 ...
 * @param {Array[String]} domains The array of domains to look in
