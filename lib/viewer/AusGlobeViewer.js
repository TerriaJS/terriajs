...
function isTimelineVisible(viewer) {
    if (!defined(viewer)) {
        ...
        return viewer.myAnimation._container.style.visibility === 'visible';
    }
}
...
AusGlobeViewer.prototype.selectViewer = function(bCesium) {
    ...
    if (!bCesium) {
       ...
       //create leaflet viewer
        ...
        this.createTimeline(map, map.clock);

        Clock.clone(previousClock, map.clock);
        map.myTimeline.zoomTo(map.clock.startTime, map.clock.stopTime);
        ...
        this.createTimeline(this.viewer, this.viewer.clock);
...
AusGlobeViewer.prototype.createTimeline = function(viewer, clock) {
    ...
    viewer.myAnimation = new Animation(animationContainer, new AnimationViewModel(clockViewModel));
    ...
    viewer.myTimeline = timeline;

    var that = this;
    timeline.scrubFunction = function(e) {
        if (defined(that.map) && that.map.dragging.enabled()) {
...
AusGlobeViewer.prototype.removeTimeline = function() {
    ...
    if (defined(viewer.myAnimation)) {
        viewerContainer.removeChild(viewer.myAnimation.container);
        viewer.myAnimation = viewer.myAnimation.destroy();
    }

    if (defined(viewer.myTimeline)) {
        viewer.myTimeline.removeEventListener('settime', viewer.myTimeline.scrubFunction, false);
        viewerContainer.removeChild(viewer.myTimeline.container);
        viewer.myTimeline = viewer.myTimeline.destroy();
    }
};

function showTimeline(viewer) {
    viewer.myAnimation._container.style.visibility = 'visible';
    viewer.myTimeline.container.style.visibility = 'visible';
...
function hideTimeline(viewer) {
    ...
    viewer.myAnimation._container.style.visibility = 'hidden';
    viewer.myTimeline.container.style.visibility = 'hidden';
...
AusGlobeViewer.prototype.updateTimeline = function(start, finish, cur, run) {
    ...
    var timeline = viewer.myTimeline;
