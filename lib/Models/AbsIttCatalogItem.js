...
var AbsIttCatalogItem = function(terria) {
    ...
    knockout.track(this, ['url', 'dataSetID', 'regionTypeConcept', 'regionConcept', 'filter', 'tableStyle', '_absDataset', 'opacity', 'displayPercent']);
...
function updateAbsResults(absItem) {
    if (!defined(absItem._absDataset)) {
        return;
    }
...
function updateAbsDataCsvText(absItem, serializeCalls) {
    ...
    function appendActiveCodes(parent, idxConcept, conceptCode) {
        for (var i = 0; i < parent.items.length; i++) {
            ...
            if (node.isActive) {
                ...
                if (parent.code === absItem.regionTypeConcept) {
                    if (absItem.regionType !== node.code) {
    ...
    if (!defined(absItem._absDataTable)) {
        ...
        if (absItem.dataSetID === 'FILE') {
            promises.push(loadText(absItem.url + '_' + absItem.regionType + '_short.csv').then( function(text) {
        ...
        promises.push(loadText('data/2011Census_TOT_' + absItem.regionType + '.csv').then( function(text) {
