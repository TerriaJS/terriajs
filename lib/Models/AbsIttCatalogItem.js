...
function rebuildDataReady(item) {
    if (!defined(item._dataSource)) {
        // This can happen when you open a shared URL with displayRegionPercent false. Not sure why.
        return when();
    }
    item._dataSource._dataReady = when().then(function() {
        ...
        buildTableDataSource(item, tableStructuresAndCombinations);
        return when();
    });
...
// It is synchronous and has no return value.
function buildTableDataSource(item, tableStructuresAndCombinations) {
    ...
    newColumns.push(new TableColumn('Total selected percent', percentValues, {
        active: item.displayPercent
    }));
