...
AbsIttCatalogItem.prototype._load = function() {
    ...
    loadConceptIdsAndConceptNameMap(this).then(function() {
        loadConcepts(item).then(function() {
            loadDataFiles(item).then(function(csvArray) {
                csvArray.forEach(function(csvText, index) {
                    if (index < item._concepts.length) {
                        console.log('loaded file for ', item._concepts[index], csvText);
                    } else {
                        console.log('loaded total populations', csvText);
                    }
                });
            });
        });
