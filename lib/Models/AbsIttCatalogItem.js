...
var AbsIttCatalogItem = function(terria, url) {
    ...
    // _concepts (via concepts) is both set and read in rebuildDataReady.
    // A solution to this would be to make concepts a Promise, but that would require changing the UI side.
    knockout.track(this, ['datasetId', 'displayPercent', '_concepts']);
    ...
    knockout.getObservable(this, 'displayPercent').subscribe(rebuildDataReady.bind(null, this), this);
...
AbsIttCatalogItem.defaultSerializers.filter = function(item, json) {
    ...
    var nestedFilter = item._concepts.map(function(concept) { return concept.toFilter(); });
    json.filter = nestedFilter.reduce(function(a, b) { return a.concat(b); }, []);
};
...
AbsIttCatalogItem.prototype._load = function() {
    ...
    // Cannot do this asynchronously, because CsvCatalogItem.enable() will refer to its enable method.
    this._dataSource = new TableDataSource(this);
    this._dataSource._tableStyle = this.tableStyle;
...
function throwNoRegionError(item, regionColumnName) {
    throw new ModelError({
        sender: item,
        title: 'Region not recognised as csv-geo-au',
        message: '\
An error occurred while invoking GetGenericData on the ABS ITT server: the region name ' + regionColumnName + ' was not recognised as csv-geo-au. \
<p>Please report this error by \
sending an email to <a href="mailto:' + item.terria.supportEmail + '">' + item.terria.supportEmail + '</a>.</p>'
    });
}
...
// Among other things, rebuildDataReady is triggered when the active items are changed.
...
function rebuildDataReady(item) {
    var regionColumnName;
    item._dataSource._dataReady = when().then(function() {
        ...
        regionColumnName = buildTableDataSource(item, tableStructuresAndCombinations);
        return when();
    });

    // Once _dataReady has been set, we can now update the region column type.
    // (Cannot do this earlier because TableDataSource's buildRegionPromise depends on _dataReady.)
    // Note this must happen before enabling the item.
    item._dataSource._regionPromise = undefined;  // Wipe the cached region promise.
    item._dataSource.regionPromise.then(function(regionDetails) {
        if (regionDetails) {
            TableDataSource.setRegionColumnType(regionDetails);
        } else {
            throwNoRegionError(item, regionColumnName);
        }
    });

    return item._dataSource._dataReady;
}
...
function loadTableStructure(item, csvString) {
    // Do not worry about trying to interpret the Time column as a VarType.TIME, as we don't use it as such.
    var tableStructure = new TableStructure('ABS_ITT', {unallowedTypes: [VarType.TIME]});
...
function loadDataFiles(item) {
    ...
    return when.all(loadDataPromises).then(function(tableStructures) {
        return {tableStructures: tableStructures, activeCombinations: activeCombinations};
    });
}
...
function buildValueColumns(item, tableStructures, activeCombinations) {
    ...
    return tableStructures.map(function(tableStructure, index) {
        ...
        var valueName = absCodes
            .filter(function(absCode) { return absCode.concept.id !== item.regionTypeConcept; })
            .map(function(absCode) { return absCode.name.trim(); })
...
// This rebuilds the table data source.
// It is synchronous and returns the region column name (not a Promise).
function buildTableDataSource(item, tableStructuresAndCombinations) {
    ...
    if (!defined(tableStructuresAndCombinations)) {
        ...
        dataSource._tableStructure.columns = [];  // Remove current columns.
        return;
    }
    ...
    var totalSelectedColumn = new TableColumn('Total selected', TableColumn.sumValues(newColumns), {active: !item.displayPercent});
    ...
    var totalPopColumn = new TableColumn('Total population', totalPopColumns[totalPopColumns.length - 1].values);
    ...
    var loadedRegionCode = activeCombinations[0].filter(function(absCode) { return absCode.concept.id === item.regionTypeConcept})[0];
    var regionColumnName = regionTypeToCsvGeo(loadedRegionCode.code, timeColumn.values[0]);
    ...
    // Replace the dataSource's columns.
    dataSource._tableStructure.columns = newColumns;

    return regionColumnName;
}

function regionTypeToCsvGeo(regionType, year) {
    // aus is left as aus, but all others (sa1, sa2, sa3, sa4, ste etc)
    // are converted to sa4_code_2011 or similar.
    var lowerCaseRegionType = regionType.toLowerCase();
    if (lowerCaseRegionType === 'aus') {
        return lowerCaseRegionType;
    } else {
        return lowerCaseRegionType + '_code_' + year;
    }
}
