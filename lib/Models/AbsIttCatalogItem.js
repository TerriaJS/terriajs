...
defineProperties(AbsIttCatalogItem.prototype, {
    ...
    propertiesForSharing : {
        get : function() {
            return AbsCatalogItem.defaultPropertiesForSharing;
        }
    }
...
// Replace the dataSource's columns and ensure that the next time the region promise is requested, it recalculates the region mapping.
function replaceColumns(dataSource, columns) {
    dataSource._regionPromise = undefined;  // Wipe the cached region promise.
    dataSource._tableStructure.columns = columns;  // Replace the columns.
}
...
function buildTableDataSource(item, tableStructuresAndCombinations) {
    ...
    if (!defined(tableStructuresAndCombinations)) {
        ...
        replaceColumns(dataSource, []);  // Remove current columns.
        return;// when();
    }
    ...
    // Replace the dataSource's columns and set the region column type.
    replaceColumns(dataSource, newColumns);
