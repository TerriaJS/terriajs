...
var AbsIttCatalogItem = function(terria, url) {
    ...
    knockout.track(this, ['datasetId', 'opacity', 'displayPercent', '_concepts']);
...
function rebuildDataReady(item) {
    ...
    }).then(function(tableStructures) {
        return buildTableDataSource(item, tableStructures);
    });
}
...
// Returns the active regiontype code, eg. SA4, or undefined if none active.
...
function loadDataFiles(item) {
    // If no region has been selected, there are no files to load.
...
function buildTableDataSource(item, tableStructuresAndCombinations) {
    ...
    if (!defined(tableStructures)) {
        // No data files were requested, eg. because no region was selected.
        return;// when();
    }
    ...
    // The other elements of the tableStructures are the data files, one per activeCombinations.
    // They have columns ["Time", "Value", "REGION", "Description"].
    // Pull out (and shallow clone) the "Value" columns, and rename them to reflect the data codes pertaining to this data file.
    var newColumns = tableStructures.map(function(tableStructure, index) {
        ...
        // Pull out and clone the 'Value' column, and rename it to match all the codes except the region type,
        // eg. Persons 0-4 years  TODO: it would be nice to specify a preferred order of codes here.
        var valueColumn = clone(tableStructure.columns[columnNames.indexOf('Value')]);
        valueColumn.name = absCodes
    ...
    var timeColumn = clone(firstTableStructure.columns[0]);
    var regionColumn = clone(firstTableStructure.columns[2]);
    ...
    // And use the first time from the time column.
    // TODO: probably won't work if no region selected.
    var loadedRegionCode = activeCombinations[0].filter(function(absCode) { return absCode.concept.id === item.regionTypeConcept})[0];
    regionColumn.name = regionTypeToCsvGeo(loadedRegionCode.code, timeColumn.values[0]);

    // Put these columns to the front of the value columns.
    newColumns.unshift(timeColumn);
    newColumns.unshift(regionColumn);
    // And add total population to the end (and check it has the right rows).
    // TODO: write this
    console.log('new columns', newColumns);

    var dataSource = item._dataSource;
    dataSource._regionPromise = undefined;  // Wipe the cache - would be better as a function
    dataSource._tableStyle = item.tableStyle;
    var tableStructure = dataSource._tableStructure;
    tableStructure.columns = newColumns;

    // TODO: if I return this, it never shows up on the map - why?
    dataSource.regionPromise.then(function(regionDetails) {
        if (regionDetails) {
            TableDataSource.setRegionColumnType(regionDetails);
        } else {
            // TODO: Beef this up into a real error.
            console.log('The region was not recognised as csv-geo-au');
        }
    });
}
