    ...
    function updateConceptName(concept) {
        ...
        if (typeof conceptNameMap[concept.name] === 'string') {
            ...
            for (var j = 0; j < concept.items.length; j++) {
                if (defined(codeMap[concept.items[j]])) {
                    concept.name = codeMap[concept.items[j]];
                    return;
                }
            }
        }
...
AbsIttCatalogItem.prototype._cache = function() {
    ...
    function setAllCodesActive(parent) {
        for (var i = 0; i < parent.items.length; i++) {
            if (parent.items[i].isCode) {
                parent.items[i].isActive = true;
            }
            setAllCodesActive(parent.items[i]);
        }
    }
    setAllCodesActive(this.absDataset);
    
    return when(updateAbsDataCsvText(this)).then(function() {
        //clean up
    });
};
