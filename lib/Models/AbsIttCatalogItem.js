...
defineProperties(AbsIttCatalogItem.prototype, {
    ...
    concepts : {
        get : function() {
            if (defined(this.absDataset)) {
                return this.absDataset.items;
            }
            return undefined;
        }
    }
    ...
    return when.all(loadPromises).then(function() {
        ...
        function addConceptCodes(concept, json) {
            ...
            function addTree(parent, codes) {
                // sort age columns by the first integer (so 5 doesn't come after 49)
                function ageToNum(ageDesc) {
                    return Number((ageDesc + "-999").match(/\d+/)[0]);
                }
                if (parent.code === 'AGE') {
                    codes = codes.sort(function(a,b) { return ageToNum(a.description) - ageToNum(b.description) });
                }
