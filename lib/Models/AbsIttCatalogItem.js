...
function loadTable(item, text) {
    var dataSource = item._dataSource;
    dataSource.load(text, item._tableStyle);
}
...
function explodeCombinations(listOfLists) {
    ...
    listOfLists[0].forEach(function(subElement) {
        var tailCombinations = explodeCombinations(listOfLists.slice(1));
        if (defined(tailCombinations)) {
            ...
            console.log(listOfLists, headAndTails);
...
function loadAbsData(item) {
    ...
    var allActiveCombinations = explodeCombinations(activeCodesPerConcept);

    // Construct the 'and' part of the requests by combining concept.code and the AbsCode.name,
    // eg. and=REGIONTYPE.SA4,AGE.A04,MEASURE.3
    // TODO: rename concept.code to concept.id, so it's not so confusing.
    var andParameters = allActiveCombinations.map(function(codes) {
        return codes.map(function(code, index) {
            return item._concepts[index].code + '.' + code.code;
        }).join(',');
    });
    ...
    var loadDataPromises = andParameters.map(function(andParameter) {
        ...
        // Note in the original there was a lot of caching stuff... but not sure if it was being used?
        loadText(url).then(function(text) {
            console.log('loaded abs data', url, text);
        });
    });
