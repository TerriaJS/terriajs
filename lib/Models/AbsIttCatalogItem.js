...
function rebuildDataReady(item) {
    item._dataSource._dataReady = when().then(function() {
        ...
        return buildTableDataSource(item, tableStructuresAndCombinations);
    });
}
...
function buildTableDataSource(item, tableStructuresAndCombinations) {
    ...
    if (!defined(tableStructuresAndCombinations)) {
        // No regions selected
        dataSource._regionPromise = undefined; // Wipe the cached region promise.
        dataSource._tableStructure.columns = []; // Wipe the old data file.
        dataSource.regionPromise;  // Should we return this instead?
        return;// when();
    }
