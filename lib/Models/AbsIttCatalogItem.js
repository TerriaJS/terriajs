...
var AbsIttCatalogItem = function(terria, url) {
    ...
    /**
     * Gets an array of currently active/selected concepts.
     * @return {Array[]} Array of arrays of active/selected concepts, indexed by concept, then code.
     */
    // knockout.defineProperty(this, 'activeItems', {
    //     get: function() {
    //         console.log('get ABS activeItems triggered');
    //         return this._concepts.map(function(concept) { return concept.activeItems });
    //     }
    // });
...
AbsIttCatalogItem.prototype._load = function() {
    ...
    var item = this;
    // TODO: need to cache.
    // TODO: Do we need to "return" each of these?
...
function activeItemsChanged(item, concept, activeItems) {
    console.log('active items changed', item, concept, activeItems);
    // TODO: for now just redo the whole load process...!
    item._dataSource._dataReady = when().then(function() {
        return loadConceptIdsAndConceptNameMap(item);
    }).then(function() {
        return loadConcepts(item);
        ...
        var promises = item._conceptIds.filter(function(conceptId) {
            ...
            return loadJson(conceptCodesUrl).then(function(json) {
                ...
                var concept = new AbsConcept({
                    ...
                    activeItemsChangedCallback: activeItemsChanged.bind(null, item)
                });
...
function buildTableDataSource(item, tableStructures) {
    ...
    if (item.displayPercent) {
        var percentValues = TableColumn.divideValues(totalSelectedColumn, totalPopColumn, 0).map(function(fraction) { return 100 * fraction; });
        newColumns.push(new TableColumn('Total selected percent', percentValues, {active: true}));
    }
    ...
    var loadedRegionCode = item._activeCombinations[0].filter(function(absCode) { return absCode.concept.id === item.regionTypeConcept})[0];
    regionColumn.name = loadedRegionCode.code.toLowerCase() + '_code_' + timeColumn.values[0];
