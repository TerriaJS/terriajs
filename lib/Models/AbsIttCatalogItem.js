...
function buildTableDataSource(item, tableStructures) {
    ...
    var newColumns = tableStructures.map(function(tableStructure, index) {
        ...
        valueColumn.name = absCodes
            ...
            .map(function(absCode) { return absCode.name; })
            .join(' ');
        return valueColumn;
    });
    var totalSelectedColumn = new TableColumn('Total selected', TableColumn.sumValues(newColumns), {active: true});
    newColumns.push(totalSelectedColumn);
    console.log('value columns', newColumns);
    ...
    console.log('renaming region column to', regionColumn.name);
    ...
    var synthesizedTableStructure = new TableStructure('ABS ITT', []);
    synthesizedTableStructure.columns = newColumns;
    console.log(synthesizedTableStructure);

    var dataSource = item._dataSource;
    dataSource._tableStructure = synthesizedTableStructure;
    dataSource._tableStyle = item.tableStyle;
    dataSource.regionPromise.then(function(regionDetails) {
        if (regionDetails) {
            dataSource.setRegionColumnType(regionDetails);
        } else {
