...
AbsIttCatalogItem.prototype._load = function() {
    ...
    item._dataSource._dataReady = when().then(function() {
        return loadConceptIdsAndConceptNameMap(item).then(function() {
            return loadConcepts(item).then(function() {
                return loadDataFiles(item).then(function(tableStructures) {
                    return buildTableDataSource(item, tableStructures);
                });
            });
        });
    });
    item._dataSource._dataReady.then(item._dataSource.updateDisplay);
};
