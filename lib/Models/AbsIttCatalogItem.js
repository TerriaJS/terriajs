...
function updateAbsDataCsvText(absItem, serializeCalls) {
        ...
        for (var i = 0; i < parent.items.length; i++) {
            ...
            if (node.isActive) {
                ...
                if (parent.code === 'REGIONTYPE') {
                    if (regionType !== node.code) {
                        regionType = node.code;
    ...
    if (regionChanged || !defined(absItem._absTotalTable)) {
        ...
        if (absItem.dataSetID === 'FILE') {
            promises.push(loadText(absItem.url + '_' + regionType + '_short.csv').then( function(text) {
        ...
        promises.push(loadText('data/2011Census_TOT_' + regionType + '.csv').then( function(text) {
