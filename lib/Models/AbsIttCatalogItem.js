...
function loadConceptIdsAndConceptNameMap(item) {
    if (!defined(item._loadConceptIdsAndNameMapPromise)) {
        ...
        var loadDatasetConceptsPromise = loadJson(datasetConceptsUrl).then(function(json) {
            ...
            if (json.concepts.indexOf('REGION') === -1 || json.concepts.indexOf('REGIONTYPE') === -1) {
                throw new DeveloperError('datasetId ' + item.datasetId + ' concepts [' + json.concepts.join(', ') + '] do not include "REGION" and "REGIONTYPE".');
            }
