...
var ImageryLayerCatalogItem = function(terria) {
    ...
    knockout.defineProperty(this, 'availableDates', {
        get: function() {
            ...
            console.log('Available dates!', intervals);
...
ImageryLayerCatalogItem.prototype._show = function() {
    ...
    if (this.showOnChart) {
        // do we need to check that there is more than one time slice? 
        this.xAxis = 'time'; //TODO is this really the right approach?
...
function updateAvailabilityFromLocalizedAvailabilityService(item) {
    ...
    function getBbox() {
        ...
        const posRads = Ellipsoid.WGS84.cartesianToCartographic(item.terria.selectedFeature.position.getValue());
    ...
    if (!defined(item.terria.selectedFeature) || !defined(item.terria.selectedFeature.position)) {
    ...
    loadJson(uri).then(featureCollection => {
        try {
            const dates = featureCollection.features[0].properties.data_available_for_dates;
