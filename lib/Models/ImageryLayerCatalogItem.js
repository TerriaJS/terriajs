...
ImageryLayerCatalogItem.enableLayer = function(catalogItem, imageryProvider, opacity, layerIndex, globeOrMap) {
    function onLoadError(tileProviderError) {
        ...
        var hasErrorCode = defined(tileProviderError.error) && defined(tileProviderError.error.statusCode);
        // by default, we're going to give up and display a message
        // in theory tPE.retry is a boolean, but it gets treated as function by TileProviderError.handleError.
        tileProviderError.retry = when.reject(tileProviderError.error);
        if (!hasErrorCode && defined(tileProviderError.x) && defined(tileProviderError.y) && defined(tileProviderError.level)) {
            ...
            // So re-do the request (ultimately with XMLHttpRequest) so we can get more details of the failure.  We need to
            ...
            if (tileUrl) {
                tileProviderError.retry = Resource.fetchImage({
                    url: tileUrl,
                    preferBlob: true
                    // do we want this?
                    // headers: { 'Accept': catalogItem.format + ',*/*' },
                });
            }
        }

        if (catalogItem.handleTileError) {
            // The item has its own error handler, so let's give that a chance. It could be handling either the first request, or a re-attempt.
        ...
        tileProviderError.retry = tileProviderError.retry
            ...
            // SB - I don't understand this logic.
            // .then(function() { })
            .otherwise(e => detailsRequestError(e || {}, tileProviderError));
    }
    ...
    function detailsRequestError (e, tileProviderError) {
        function isToleratedError() {
            ...
            if (e.statusCode === 404) {
                ...
                // no status code? Could be a bad server name, or a CORS issue, or...?
                return true;
            }
        }
        ...
        // So, we have a failure that is not ok.  We allow a few "not ok" failures and then turn off the layer and
        // display a message to the user. We can't count on the TileProviderError's accounting of the times retried because it will
        ...
        if (defined(layer) && globeOrMap.isImageryLayerShown({layer})) {
            if (catalogItem === catalogItem.terria.baseMap ||
                ...
                var errorType = e && e.constructor && e.constructor.name;
                var msg;
                if (errorType === 'Event') {
                    // something went wrong displaying the image
                    msg = 'Unable to display image tiles for catalogue item ' + catalogItem.name + '.  They do not appear to be of a supported image type. ';
                } else if (errorType === 'RequestErrorEvent' && e.statusCode) {
                    msg = 'An error occurred while attempting to download tiles for catalogue item ' + catalogItem.name + ' due to a problem with the server '+
                    '(error code ' + e.statusCode + ').';
                } else  {
                    // probably something went badly wrong in fetch, like CORS fail, bad server name, or something unknown.
                    msg = 'An error occurred while attempting to download tiles for catalogue item ' + catalogItem.name + '.  This may indicate that there is a ' +
                ...
                globeOrMap.terria.error.raiseEvent(new TerriaError({
                    ...
                    message: msg + 
                        ' The catalogue item has been hidden from the map.  You may re-show it in the Now Viewing panel to try again.  Further technical details may be found below.<br/><br/>' +
                        '<pre>' + formatError(e) + '</pre>'
                }));
    ...
    let tileFailures = 0, ignoredFailures = 0;
