...
var ImageryLayerCatalogItem = function(terria) {
    ...
    this._intervals = undefined;           // All imagery availability intervals for the whole layer
    this._localizedIntervals = undefined;  // Imagery availability intervals at a current location
    this._localizedPosition = undefined;   // The location that imagery localization is currently based off.
     ...
     * is not time-dynamic, property is undefined. Uses localized availability if possible. This property is observable.
     * @type {ImageryLayerInterval[]}
     * @default undefined
     */
    knockout.defineProperty(this, 'intervals', {
        get: function() {
            return defaultValue(this._localizedIntervals, this._intervals);
        }, set: function(newIntervals) {
    ...
    /**
     * Gets javascript dates describing the discrete datetimes available for this item.
     * This is primarily used for charting,  
...
ImageryLayerCatalogItem.defaultSerializers.intervals = function(catalogItem, json, propertyName) {
    if (defined(catalogItem._intervals)) {
        var result = [];
        for (var i = 0; i < catalogItem._intervals.length; ++i) {
            var interval = catalogItem._intervals.get(i);
            result.push({
...
ImageryLayerCatalogItem.prototype._enable = function(layerIndex) {
    ...
    var isTimeDynamic = defined(this._intervals) && (this._intervals.length > 0) && defined(this.clock);
...
ImageryLayerCatalogItem.prototype._show = function() {
    ...
    const needUpdate = this.localizedAvailabilityService && !this._localizedIntervals;
...
function updateAvailabilityFromLocalizedAvailabilityService(item) {
    function datesToTimeIntervalCollection(dates, hourOffset = 0)  {
        ...
        // this method: each interval starts at date, ends just before next
        ...
        // this method: each interval starts halfway between a date and its predecessor, ends halfway between date and its successor        
        let currentMiddle, nextMiddle;
        dates.forEach((date, i) => {
            if (!defined(currentMiddle)) {
                currentMiddle = JulianDate.fromIso8601(date);
            }
            if (defined(dates[i + 1])) {
                const current = JulianDate.fromIso8601(date);
                const next = JulianDate.fromIso8601(dates[i+1]);
                nextMiddle = JulianDate.addSeconds(current, JulianDate.secondsDifference(next, current) / 2, new JulianDate());
            } else {
                nextMiddle = JulianDate.addHours(JulianDate.fromIso8601(date), 24, new JulianDate());
            }
    ...
    return loadJson(uri).then(featureCollection => {
        try {
            ...
            item._localizedIntervals = datesToTimeIntervalCollection(dates, las.hourOffset);
        } catch (e) {
            // This kind of error is probably not worth bothering the user about.
            item._localizedIntervals = undefined;
            console.error(e);
        }
    });
}
