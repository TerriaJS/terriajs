  ...
  const layer = globeOrMap.addImageryProvider({
      ...
      operation.attempt(function() {
        ...
        maybeXhr
          .then(function() {
            ...
            if (
              defined(tileProviderError.provider._resource) &&
              defined(tileProviderError.provider._resource.queryParameters) &&
              defined(tileProviderError.provider._resource.queryParameters.time)
            ) {
              key += `T${tileProviderError.provider._resource.queryParameters.time.replace(
                /-/g,
                ""
              )}`;
            }
          ...
          .otherwise(function(e) {
            ...
            if (e.statusCode >= 400 && e.statusCode < 500) {
              ...
              failTile({
                name: "Tile Error",
                message:
                  "Got a 200 (OK) response from URL:\n" +
                  getUrlForImageryTile(
                    imageryProvider,
                    tileProviderError.x,
                    tileProviderError.y,
    ...
    onProjectionError: function() {
      ...
      globeOrMap.terria.error.raiseEvent({
        ...
        message:
          catalogItem.name +
          '" cannot be shown in 2D because it does not support the standard Web Mercator (EPSG:3857) projection.  ' +
