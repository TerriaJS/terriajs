...
var ImageryLayerCatalogItem = function(terria) {
    ...
    // TODO: Need some way to call updateSplitDirection(this) when terria.showSplitter is toggled, but not on every catalog item.
    // knockout.getObservable(this.terria, 'showSplitter').subscribe(function() {
    //     updateSplitDirection(this);
    // }, this);
};
...
ImageryLayerCatalogItem.prototype._enable = function(layerIndex) {
    ...
    this._splitterEventListener = this.terria.splitterEvent.addEventListener(function() {updateSplitDirection(item);});
};
...
ImageryLayerCatalogItem.prototype._disable = function() {
    ...
    if (defined(this._splitterEventListener)) {
        this._splitterEventListener();  // Removes the event listener.
    }
};
...
function updateSplitDirection(item) {
    // This should be called whenever the split direction is changed,
    // the splitter is turned on or off,
    // or the viewer is changed (because some viewers potentially can't show a splitter - though both Cesium and Leaflet can).
    if (defined(item._imageryLayer) && item.isEnabled && item.isShown) {
        item._imageryLayer.splitDirection = (item.terria.showSplitter && item.terria.currentViewer.canShowSplitter) ? item.splitDirection : ImagerySplitDirection.NONE;
        item.terria.currentViewer.notifyRepaintRequired();
    }
}
...
ImageryLayerCatalogItem.enableLayer = function(catalogItem, imageryProvider, opacity, layerIndex, globeOrMap) {
    ...
    const layer = globeOrMap.addImageryProvider({
        ...
        splitDirection: catalogItem.splitDirection,
