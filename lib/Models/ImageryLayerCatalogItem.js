...
ImageryLayerCatalogItem.prototype._show = function() {
    ...
    // TODO only add as chartable if:
    // - there is more than one time slice
    // - the item has been specially configured that way? 
    ...
    return new ChartData(points,
        {
            ...
            onClick: (time) => {
                // Could be problematic if other layers also want to snap the time...
                this.currentTime = ImageryLayerCatalogItem.snapNearestAvailableTime(this.intervals, JulianDate.fromDate(time));
                showDataForTime(this, this.currentTime);
            }
        });
};
