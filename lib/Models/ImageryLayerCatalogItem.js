...
ImageryLayerCatalogItem.enableLayer = function(catalogItem, imageryProvider, opacity, layerIndex, globeOrMap) {
    function onLoadError(tileProviderError) {
        ...
        console.log(`${tileProviderError.timesRetried}, ${tileFailures} (${ignoredFailures} ok)`);
        ...
        if (!hasErrorCode && defined(tileProviderError.x) && defined(tileProviderError.y) && defined(tileProviderError.level)) {
            ...
            if (tileProviderError.timesRetried - ignoredFailures < 10) {
                ...
                if (tileUrl) {
                    ...
                    if (catalogItem.format) {
                        options.headers = { 'Accept': catalogItem.format };
                    }
                    tileProviderError.retry = loadWithXhr(options);
                }
            } else {
                // We seem to have done the "details request" thing many times, and yet, it's not translating into either
                // ignored failures (eg, 404s), nor other kinds of errors (eg, 500). One possible situation at this point is
                // a misconfigured server sending back something which isn't an error, like an XML page with a 200 code.
                // So let's stop fetching.
            }
        }
