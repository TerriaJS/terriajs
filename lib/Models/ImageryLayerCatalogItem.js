...
var ImageryLayerCatalogItem = function(terria) {
    ...
    knockout.track(this, ['_clock', 'opacity', 'treat404AsError', 'ignoreUnknownTileErrors', 'intervals', 'clipToRectangle', 'splitDirection', 'useOwnClock']);
...
function updateSplitDirection(item) {
    // This is called whenever the split direction is changed, the splitter is turned on or off, or the viewer is changed.
    if (defined(item._imageryLayer) && item.isEnabled && item.isShown) {
        item._imageryLayer.splitDirection = item.terria.showSplitter ? item.splitDirection : ImagerySplitDirection.NONE;
        item.terria.currentViewer.notifyRepaintRequired();
    }
}
...
function forceUpdateToClockSubscription(catalogItem) {
    ...
    if (catalogItem.isShown && defined(catalogItem.clock)) {
        ...
        if (catalogItem.useOwnClock) {
            ...
            // Instead, explicitly update the imagery layer.
