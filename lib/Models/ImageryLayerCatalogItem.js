...
ImageryLayerCatalogItem.prototype.createImageryProvider = function(time) {
    ...
    result.tileDownloadTimeout = 5000;
    result.tileDownloadTimeoutMessage = this.name + ': The data provider\'s server took more than 5 seconds to return this tile, so the request was cancelled.';
    return result;
};
...
is invalid.  The catalogue item has been hidden from the map.  You may re-show it in the Now Viewing panel to try again.'
        ...
        result.errorEvent.addEventListener(function(sender, message) {
            ...
            if (message === imageryProvider.tileDownloadTimeoutMessage) {
                catalogItem.terria.error.raiseEvent({
                    sender: catalogItem,
                    title: catalogItem.name,
                    message: '\
The data provider\'s server took more than 5 seconds to return a tile for this dataset, so the dataset was disabled.  You can try again by re-enabling it in the Now Viewing tab.'
                });
            } else {
                catalogItem.terria.error.raiseEvent({
                    sender: catalogItem,
