...
ImageryLayerCatalogItem.enableLayer = function(catalogItem, imageryProvider, opacity, layerIndex, globeOrMap) {
    ...
    const layer = globeOrMap.addImageryProvider({
        ...
        onLoadError: function(tileProviderError) {
            ...
            // With this deferred:
            //  [Reject] means: stop trying to load this tile
            //  [Resolve] means: retry loading this tile
            ...
            function failTile(e) {
                // Give up loading this tile and possibly give up on this layer entirely.
                ++tileFailures;
            ...
            operation.attempt(function() {
                ...
                let promise = defined(error) && defined(error.statusCode) ? when.reject(error) : fetchImage();
                if (catalogItem.handleTileError !== undefined) {
                    // Give the catalog item a chance to handle this error.
                    promise = catalogItem.handleTileError(promise, imageryProvider, tileProviderError.x, tileProviderError.y, tileProviderError.level);
                }

                promise.then(function() {
                    // Succeeded! So stop our retry operation and resolve the deferred so that Cesium or Leaflet retries this tile.
                    ...
                    const key = `L${tileProviderError.leve}X${tileProviderError.x}Y${tileProviderError.y}`;
                    if (tileRetriesByMap[key] === undefined) {
                        tileRetriesByMap[key] = 1;
                    } else if (++tileRetriesByMap[key] > 5) {
                        failTile({
                            name: 'Tile Error',
                    ...
                    if (e.statusCode >= 400 && e.statusCode < 500) {
                        ...
                        if (catalogItem.ignoreUnknownTileErrors) {
                            ...
                            failTile({
                                ...
                                message: 'This probably indicates a Cross-Origin Resource Sharing (CORS) problem.\n' +
                                         'If this is your dataset or application, see:\n' +
