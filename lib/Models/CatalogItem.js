...
CatalogItem.prototype.useClock = function() {
    ...
    return when(this.load(), function() {
        if (!defined(that.clock)) {
            that.terria.cesium.viewer.myTimeline.container.style.visibility = 'hidden';
            document.getElementsByClassName('animation-controls')[0].style.visibility = 'hidden';
            document.getElementsByClassName('cesium-widget-credits')[0].style.bottom = '0px';
            document.getElementsByClassName('cesium-widget-credits')[0].style.left = '0px'
            document.getElementsByClassName('distance-legend')[0].style.bottom = '30px';
            document.getElementsByClassName('location-bar')[0].style.bottom = '30px';
            return;
        }
        ...
        if (defined(that.terria.cesium)) {
            that.terria.cesium.viewer.myTimeline.container.style.visibility = 'visible';
            document.getElementsByClassName('animation-controls')[0].style.visibility = 'visible';
            document.getElementsByClassName('cesium-widget-credits')[0].style.bottom = '27px';
            document.getElementsByClassName('cesium-widget-credits')[0].style.left = '168px'
            document.getElementsByClassName('distance-legend')[0].style.bottom = '55px';
            document.getElementsByClassName('location-bar')[0].style.bottom = '55px';
            that.terria.cesium.viewer.myTimeline.zoomTo( that.terria.clock.startTime,  that.terria.clock.stopTime);
            that.terria.cesium.viewer.forceResize();
        }

        if (defined(that.terria.leaflet)) {
//            that.terria.leaflet.map.myAnimation._container.style.visibility = 'visible';
            that.terria.leaflet.map.myTimeline.container.style.visibility = 'visible';
            that.terria.leaflet.map.myTimeline.zoomTo( that.terria.clock.startTime,  that.terria.clock.stopTime);
        }
    });
};
