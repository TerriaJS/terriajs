...
var CatalogItem = function(terria) {
    ...
    knockout.track(this, ['rectangle', 'legendUrl', 'dataCustodian', 'attribution',
                          'metadataUrl', '_isEnabled', 'isShown', 'isLegendVisible', 'clock',
    ...
    knockout.defineProperty(this, 'isEnabled', {
        get : function() {
            return this._isEnabled;
        },
        set : function(newValue) {
            if (newValue && !this._isEnabled && defined(this.initialMessage)) {
                this.terria.disclaimerEvent.raiseEvent(this, function() {
...
CatalogItem.prototype.load = function() {
    ...
    this._loadingPromise = runLater(function() {
        ...
        if (loadResult instanceof CatalogItem) {
            that.nowViewingCatalogItem = loadResult;
            loadResult.creatorCatalogItem = that;
        }
...
function isEnabledChanged(catalogItem) {
    ...
    if (catalogItem.isEnabled) {
        ...
        if (catalogItem.isShown) {
            // If item already had the isShown flag but wasn't enabled, it wasn't actually shown, so call isShownChanged
            // to update the view.
            isShownChanged(catalogItem);
        } else {
            // If item is enabled, automatically show it.
