...
CatalogItem.prototype.useClock = function() {
    ...
    return when(this.load(), function() {
        ...
        if (defined(that.terria.cesium)) {
            that.terria.cesium.viewer.animation._container.style.visibility = 'visible';
            that.terria.cesium.viewer.timeline.container.style.visibility = 'visible';
            that.terria.cesium.viewer.timeline.zoomTo( that.terria.clock.startTime,  that.terria.clock.stopTime);
            that.terria.cesium.viewer.forceResize();
        }

        if (defined(that.terria.leaflet)) {
            that.terria.leaflet.map.animation._container.style.visibility = 'visible';
            that.terria.leaflet.map.timeline.container.style.visibility = 'visible';
            that.terria.leaflet.map.timeline.zoomTo( that.terria.clock.startTime,  that.terria.clock.stopTime);
        }
    });
};
