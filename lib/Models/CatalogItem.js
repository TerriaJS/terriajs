...
function updateCurrentTime(catalogItem, updatedTime) {
    // Notionally we perform the following, but we replace the clock rather then updating it so that all clients subscribed with DataSourceClock.definitionChanged are notified.
    //JulianDate.clone(terriaClock.currentTime, that.clock.currentTime);
    const newTime = new JulianDate();
    JulianDate.clone(updatedTime, newTime);
    catalogItem.clock.currentTime = newTime;
}
