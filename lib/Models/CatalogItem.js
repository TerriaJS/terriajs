...
var CatalogItem = function(terria) {
     ...
     *
     * @member {JulianDate} The CatalogItems current time.
     * @memberOf CatalogItem.prototype
     */
    ...
    /**
     * Gets the CatalogItems current time as the discrete time that time the catalogItem has information for.
     * Returns undefined if it is not possible to query the time (i.e. the item doesn't have a clock or canUseOwnClock is false).
     *
     * @member {Date} The CatalogItems discrete current time.
     * @memberOf CatalogItem.prototype
     */
    knockout.defineProperty(this, 'discreteTime', {
        get : function() {
            if (!defined(this.currentTime) || !defined(this.availableDates) || !defined(this.intervals) || (this.intervals.length === 0)) {
                return undefined;
            }

            return this.availableDates[this.intervals.indexOf(this.currentTime)];
        },
    });
    ...
    knockout.defineProperty(this, '_watchTerriaClock', {
        get: function() {
    ...
    knockout.getObservable(this, '_watchTerriaClock').subscribe(function(newValue) {
...
defineProperties(CatalogItem.prototype, {
    ...
    clockForDisplay : {
        get : function() {
            // Temporarily make this a synonim for .clock to simplify refactor, before we refactor to remove this completely.
...
function updateTerriaClockWatcher(catalogItem) {
    if (catalogItem._watchTerriaClock && !defined(catalogItem._removeTerriaClockWatch)) {
    ...
    } else if (!catalogItem._watchTerriaClock && defined(catalogItem._removeTerriaClockWatch)) {
 ...
 * This function doe this so that all clients subscribed to the items clock with DataSourceClock.definitionChanged are notified.
 *
...
function useOwnClockChanged(catalogItem) {
    ...
    if (catalogItem._lastUseOwnClock !== catalogItem.useOwnClock) {
        if (catalogItem.useOwnClock) {
            ...
            updateCurrentTime(catalogItem, catalogItem.clock.currentTime);
        } else {
            const newTime = new JulianDate();
            JulianDate.clone(catalogItem.clock.currentTime, newTime);
            catalogItem.terria.clock.currentTime = newTime;
        }
    }
