...
var CatalogItem = function(terria) {
     ...
     * @type {String} Html template that can be used to display info
     */
    // this.infoTemplate = undefined;
    this.infoTemplate = "Test template";
...
function isEnabledChanged(catalogItem) {
    ...
    if (catalogItem.isEnabled) {
        ...
        if (!defined(catalogItem._loadForEnablePromise)) {
            ...
            var loadPromise = when(catalogItem.load(), function() {
                if (catalogItem.isEnabled) {
                    ...
                    catalogItem.terria.infoTemplate.push(catalogItem.infoTemplate);
                }
            });
        ...
        if (!defined(catalogItem._loadForEnablePromise)) {
            ...
            var index = catalogItem.terria.infoTemplate.indexOf(catalogItem.infoTemplate);
            catalogItem.terria.infoTemplate.splice(index, 1);
        }
