...
var CatalogItem = function(terria) {
    ...
    /**
     * Gets or sets the unique id of the item from which this item was copied (if any).
     * Copying is useful so that the same item with different settings can be shown on either side of a splitter.
     * However, once an item has been duplicated, it cannot be duplicated again.
     * (This is only possible if your subclass defines the `splitDirection` property and your `terria.currentViewer` has `canShowSplitter` true.)
     * Do not keep a reference to the copied item because it's too tricky to serialize and update.
     * Note terria.catalog.shareKeyIndex keeps a flat list of all unique ids, so you can find the corresponding item in this list if needed.
     * Default undefined (ie. not copied).
     * @type {String}
     */
    this.copiedFromUniqueId = undefined;
...
CatalogItem.defaultPropertiesForSharing.push('canUseOwnClock');
CatalogItem.defaultPropertiesForSharing.push('copiedFromUniqueId');
