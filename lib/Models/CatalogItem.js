...
var CatalogItem = function(terria) {
    ...
    /**
     * Gets or sets an the unique id of a duplicate of this item.
     * Duplication is useful so that the same item with different settings can be shown on either side of a splitter.
     ...
     * We keep the duplicate's id, so when either is removed, the remaining item can be duplicated again.
     * Do not keep a reference to the duplicate item to avoid circularity.
     * Default undefined (ie. no duplicate item).
     * @type {String}
     */
    this.duplicateItemUniqueId = undefined;
...
CatalogItem.defaultPropertiesForSharing.push('duplicateItemUniqueId');
