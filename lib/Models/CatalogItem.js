...
var CatalogItem = function(terria) {
    ...
    knockout.track(this, ['rectangle', 'legendUrl', 'dataUrlType', 'dataUrl', 'dataCustodian', 'attribution',
                          'metadataUrl', 'isEnabled', 'isShown', 'isLegendVisible', 'clock',
                          'isLoading', 'isMappable', 'showsInfo', 'customProperties',
						  'nowViewingMessage', 'url', '_legendUrls']);
...
CatalogItem.defaultUpdaters.url = function(catalogItem, json, prototypeName) {
    catalogItem.url = json.url;
    if(!defined(json.dataUrl)){
           catalogItem.dataUrl = json.url;
        }
    if(!defined(json.dataUrlType)){
        catalogItem.dataUrlType = 'direct';
    }
};
...
CatalogItem.defaultSerializers.url = function(catalogItem, json, prototypeName) {
    if(defined(catalogItem.url)){
           json.url = catalogItem.url;
        }
};

CatalogItem.defaultSerializers.dataUrl = function(catalogItem, json, prototypeName) {
    if(defined(catalogItem.dataUrl)){
           json.dataUrl = catalogItem.dataUrl;
        }
};

CatalogItem.defaultSerializers.dataUrlType = function(catalogItem, json, prototypeName) {
    if(defined(catalogItem.dataUrlType)){
           json.dataUrlType = catalogItem.dataUrlType;
        }
};
