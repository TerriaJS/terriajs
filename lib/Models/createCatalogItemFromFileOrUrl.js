...
function loadItem(newCatalogItem, name, fileOrUrl) {
    // Strip url protocol & use as name if it's a web link
    var httpProtocol = name.indexOf('http');
    var lastSlashIndex = name.indexOf('//');
    if (httpProtocol === 0 && lastSlashIndex > -1) {
        name = name.substring(lastSlashIndex + 2);
    }
