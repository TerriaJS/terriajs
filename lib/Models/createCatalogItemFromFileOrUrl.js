...
function loadItem(newCatalogItem, name, fileOrUrl) {
    ...
    // When loading in a WMS server at a root path, a string will be empty
    // - let the deferred loading read the title from the server instead of a calculated path of 'nothing'("")
    if (name.length > 0) {
