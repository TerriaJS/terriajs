...
var CatalogFunctionJob = require("./CatalogFunctionJob");
...
const callWebCoverageService = function(wmsCatalogItem) {
  ...
  const pickPointMode = new MapInteractionMode({
    message:
      "Press the SHIFT key and hold down the left mouse button to draw a rectangle to download",
    drawRectangle: true,
    onCancel: () => {
...
function launch(wmsCatalogItem, bbox) {
  ...
  const asyncResult = new CatalogFunctionJob(wmsCatalogItem.terria);
  asyncResult.name = wmsCatalogItem.name + " " + timestamp;
  asyncResult.description = i18next.t("models.wcs.asyncResultDescription", {
