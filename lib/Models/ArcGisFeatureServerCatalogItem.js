...
ArcGisFeatureServerCatalogItem.prototype._load = function() {
  ...
  if (this.inheritStyleInformation) {
    this._metadata = loadMetadata(this);
  }
...
function parseStyleInformation(item) {
  if (defined(item._metadata.drawingInfo)) {
    item._geoJsonItem.style = {};
    const geojsonStyle = item._geoJsonItem.style;
    const defaultFill = item._metadata.renderer.defaultSymbol.fill.color;
    geojsonStyle.fill = rgbToHex(
      defaultFill[0],
      defaultFill[1],
      defaultFill[2]
    );
    geojsonStyle["fill-opacity"] = defaultFill[3];
  }
}
