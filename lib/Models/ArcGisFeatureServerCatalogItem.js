  ...
  if (this.inheritStyleInformation) {
    return loadMetadata(this).then(function(val) {
      ...
      if (that._metadata.drawingInfo.renderer.type === "simple") {
        ...
        that._geoJsonItem.style = combine(geojsonStyle, style);
      }

      return that._geoJsonItem.load().then(function() {
        that.rectangle = that._geoJsonItem.rectangle;
        console.log(that._geoJsonItem._dataSource.entities);
...
function convertEsriRendererToStyle(symbol) {
  ...
  if (defined(fill)) {
    styleObj.fill = new Color.fromBytes(
      fill[0],
      fill[1],
      fill[2]
    ).toCssColorString();
  ...
  if (defined(symbol.imageData)) {
    styleObj["marker-url"] = `data:${symbol.contentType};base64,${
      symbol.imageData
    }`;
    styleObj["marker-height"] = symbol.height;
    styleObj["marker-width"] = symbol.width;
    styleObj["marker-angle"] = symbol.angle;
  }
