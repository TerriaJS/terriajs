...
function calculateImageryLayerIntervals(timeColumn, currentTime, animateForwards) {
    ...
    var beforePredicate = animateForwards ? JulianDate.lessThan : JulianDate.greaterThan;
    var afterPredicate = animateForwards ? JulianDate.greaterThan : JulianDate.lessThan;
    var beforeOrEqualsPredicate = animateForwards ? JulianDate.lessThanOrEquals : JulianDate.greaterThanOrEquals;
    var afterOrEqualsPredicate = animateForwards ? JulianDate.greaterThanOrEquals : JulianDate.lessThanOrEquals;
    ...
    timeColumn.timeIntervals.forEach(function(interval) {
        ...
        [[interval.start, interval.isStartIncluded], [interval.stop, interval.isStopIncluded]].forEach(([time, isIncluded]) => {
            const isBefore = isIncluded ? beforeOrEqualsPredicate : beforePredicate;
            const isAfter  = isIncluded ? afterOrEqualsPredicate  : afterPredicate;
