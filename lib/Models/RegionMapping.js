...
function setNewRegionImageryLayer(regionMapping, layerIndex, regionIndices) {
    ...
    switch (regionDetail.regionProvider.server.type) {
    case "WMS":
    // Recolor the regions, and add feature descriptions.
        regionImageryProvider = new WebMapServiceImageryProvider({
            url: proxyCatalogItemUrl(catalogItem, regionDetail.regionProvider.server.url),
    ...
    case "MVT":
        regionImageryProvider = new MapboxVectorTileImageryProvider({
            ...
            subdomains: regionDetail.regionProvider.server.subdomains,
    ...
    default:
        throw new TerriaError({
            title: "Invalid serverType " + regionDetail.regionProvider.server.type + " in regionMapping.json",
            message: '<div>Expected serverType to be "WMS" or "vector" but got "' + regionDetail.regionProvider.server.type + '"</div>'
        });
    }
}
