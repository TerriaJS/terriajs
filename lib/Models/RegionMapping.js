...
RegionMapping.prototype.loadRegionDetails = function() {
    ...
    return RegionProviderList.fromUrl(regionMapping._regionMappingDefinitionsUrl, this._catalogItem.terria.corsProxy).then(function(regionProviderList) {
        ...
        if (defined(regionMapping._tableStyle)) {
            if (defined(regionMapping._tableStyle.regionsGeoJsonUrl) && defined(regionMapping._tableStyle.regionsGeoJsonProperty)) {
                return loadJson(proxyCatalogItemUrl(regionMapping._catalogItem, regionMapping._tableStyle.regionsGeoJsonUrl, '1d')).then(geoJson => {
                    return createRegionProviderFromGeoJson(regionProviderList, geoJson, regionMapping._tableStyle.regionsGeoJsonProperty, regionMapping._catalogItem.terria)
                }).then(rp => ({
                    regionProviderList,
...
function displayFailedAndAmbiguousMatches(regionMapping, failedMatches, ambiguousMatches) {
    ...
    if ((ambiguousMatches.length > 0) && !defined(timeColumn)) {
        ...
        '<samp>' + uniq(ambiguousNames).join('</samp>, <samp>') + '</samp>' +
        '<br/><br/>';
    }
    if (msg) {
