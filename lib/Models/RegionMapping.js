...
/**
 * This is now deprecated - use RegionMapping.prototype.setRegionColumnType instead.
 *
...
RegionMapping.setRegionColumnType = function(regionDetails) {
    deprecationWarning('RegionMapping.setRegionColumnType', 'RegionMapping.setRegionColumnType is deprecated. Use RegionMapping.prototype.setRegionColumnType instead.\
 regionMapping.regionDetails[].column and .disambigColumn have also been deprecated; use .columnName and disambigColumnName.');
    var regionDetail = regionDetails[0];
    regionDetail.column.type = VarType.REGION;  // regionDetail.column is deprecated too - see loadRegionIds below.
    ...
    return when.all(promises).then(function() {
        ...
        regionMapping._regionDetails = rawRegionDetails.map(function(rawRegionDetail) {
            return {
                ...
                // DeprecationWarning: remove the following references to the actual columns.
                // Currently kept only so that the deprecated RegionMapping.setRegionType continues to work.
    ...
    function getRegionRowPropertiesPropertyCallbackForId(uniqueId) {
        ...
        return function regionRowPropertiesPropertyCallback(time, result) {
            ...
            if (defined(rowNumbersForThisRegion)) {
                properties._terria_getChartData = function() {
                    return tableStructure.toCsvString('source', rowNumbersForThisRegion, false);
                };
            }
            return properties;
        };
        ...
        return function constructMVTFeatureInfo(feature) {
            ...
            if (!regionMapping.hasActiveTimeColumn && !regionMapping.isPolled) {
                ...
                if (defined(cRowObject)) {
                    ...
                    imageryLayerFeatureInfo.properties._terria_numericalProperties = cRowObject.number;
                } else {
