...
function reviseLegendHelper(regionMapping) {
    ...
    regionMapping._legendHelper = new LegendHelper(activeColumn, regionMapping._tableStyle, regionProvider);
...
function changedActiveItems(regionMapping) {
    ...
    if (defined(regionMapping._regionDetails)) {
        if (defined(regionMapping._regionImageryLayer) || defined(regionMapping._hadImageryAtLayerIndex)) {
            redisplayRegions(regionMapping);
            if (defined(regionMapping._regionImageryLayer) && !defined(regionMapping._regionDetails)) {
                ...
                // regionMapping._regionImageryLayer._layerIndex is not always defined, so use "true" in that case.
                regionMapping._hadImageryAtLayerIndex = regionMapping._regionImageryLayer._layerIndex;  // Would prefer not to access an internal variable of imageryLayer.
                if (!defined(regionMapping._hadImageryAtLayerIndex)) {
                    regionMapping._hadImageryAtLayerIndex = true;
                }
                ImageryLayerCatalogItem.hideLayer(regionMapping._catalogItem, regionMapping._regionImageryLayer);
                ImageryLayerCatalogItem.disableLayer(regionMapping._catalogItem, regionMapping._regionImageryLayer);
                regionMapping._regionImageryLayer = undefined;
            }
        }
    }
}
 ...
 * @param {Clock} [clock] The terria clock. (NOT regionMapping._clock, which does not show the same time (and is a DataSourceClock).
...
function setNewRegionImageryLayer(regionMapping, layerIndex, regionIndices) {
    ...
    if (!defined(regionIndices)) {
        ...
        if (!regionMapping._hasDisplayedFeedback) {
    ...
    regionMapping._regionImageryLayer = ImageryLayerCatalogItem.enableLayer(catalogItem, regionImageryProvider, catalogItem.opacity, layerIndex);
    if (defined(catalogItem.terria.leaflet) && colorFunction) {
        regionMapping._regionImageryLayer.setFilter(function () {
...
function redisplayRegions(regionMapping, regionIndices) {
    if (defined(regionMapping._regionDetails)) {
        ...
        if (defined(regionMapping._regionImageryLayer)) {
            layerIndex = regionMapping._regionImageryLayer._layerIndex;  // Would prefer not to access an internal variable of imageryLayer.
            ImageryLayerCatalogItem.hideLayer(regionMapping._catalogItem, regionMapping._regionImageryLayer);
            ImageryLayerCatalogItem.disableLayer(regionMapping._catalogItem, regionMapping._regionImageryLayer);
            regionMapping._regionImageryLayer = undefined;
        }
        setNewRegionImageryLayer(regionMapping, layerIndex, regionIndices);
        ImageryLayerCatalogItem.showLayer(regionMapping._catalogItem, regionMapping._regionImageryLayer);
    }
}
...
function updateClockSubscription(regionMapping) {
    ...
    if (!defined(regionMapping._clock)) {
        return;
    }
    var catalogItem = regionMapping._catalogItem;
...
function displayFailedAndAmbiguousMatches(regionMapping, failedMatches, ambiguousMatches) {
    ...
    var regionDetail = regionMapping._regionDetails[0];
    var regionColumnValues = regionDetail.column.values;
    var timeColumn = regionMapping._tableStructure.columnsByType[VarType.TIME][0];
