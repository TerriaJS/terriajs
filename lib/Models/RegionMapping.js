...
function calculateNextImageryLayerInterval(regionMapping, currentTime) {
    // Loop over time intervals and generate a time interval for the next imagery layer,
    //  or return null if there is no such next interval
    var timeColumn = regionMapping._tableStructure.activeTimeColumn;
    var nextInterval = new TimeInterval();
    nextInterval.start = null;
    nextInterval.stop = null;
    for (const interval of timeColumn.timeIntervals) {
        for (const time of [interval.start, interval.stop]) {
            if (JulianDate.greaterThan(time, currentTime)) {
                if (nextInterval.start === null || JulianDate.lessThan(time, nextInterval.start)) {
                    nextInterval.start = time;
                } else if (JulianDate.greaterThan(time, nextInterval.start) && (nextInterval.stop === null || JulianDate.lessThan(time, nextInterval.stop))) {
                    // Stop must always be after start
                    nextInterval.stop = time;
                }
            }
        }
    }
    console.log(`nextInterval generated. Start: ${nextInterval.start}, stop: ${nextInterval.stop}`);
    return (nextInterval.start !== null && nextInterval.stop !== null) ? nextInterval : null;
}
...
function setNewRegionImageryLayer(regionMapping, layerIndex, regionIndices) {
    ...
    var nextInterval = calculateNextImageryLayerInterval(regionMapping, currentTime);
