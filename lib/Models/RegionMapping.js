...
function calculateRegionIndices(regionMapping, time, failedMatches, ambiguousMatches) {
    ...
    if (defined(timeColumn) && timeColumn.timeIntervals) {
...
function addDescriptionAndProperties(regionMapping, regionIndices, regionImageryProvider) {
    ...
    var idColumnNames = [regionDetail.column.name];
    if (defined(regionDetail.disambigColumn)) {
        idColumnNames.push(regionDetail.disambigColumn.name);
    }
    ...
    function getRegionRowDescriptionPropertyCallbackForId(uniqueId) {
        ...
        return function regionRowDescriptionPropertyCallback(time, result) {
            ...
            return timeSpecificRegionRowDescriptions[uniqueId] || 'No data for the selected date.';
        };
    }
    ...
    function getRegionRowPropertiesPropertyCallbackForId(uniqueId) {
        ...
        return function regionRowPropertiesPropertyCallback(time, result) {
            if (!regionMapping.hasActiveTimeColumn) {
                ...
                var constantProperties = regionMapping._constantRegionRowObjects[uniqueId] || {};
            ...
            var properties = timeSpecificRegionRowObjects[uniqueId] || {};
            properties._terria_columnAliases = columnAliases;
            var idStringForRowNumber = tableStructure.getIdStringForRowNumber(timeSpecificRegionIndices[0], idColumnNames); // Id is the same for any row.
            properties._terria_rowNumbers = rowNumbersMap[idStringForRowNumber];
            var timeColumnValues = regionMapping._tableStructure.activeTimeColumn.values;
            properties._terria_rowNumbers.sort((a, b) => timeColumnValues[a] - timeColumnValues[b]);
            return properties;
        };
    }
    ...
    ImageryProviderHooks.addPickFeaturesHook(regionImageryProvider, function(imageryLayerFeatureInfos) {
        ...
        for (var i = 0; i < imageryLayerFeatureInfos.length; ++i) {
            ...
            if (!regionMapping.hasActiveTimeColumn && !regionMapping.isPolled) {
                ...
                imageryLayerFeatureInfo.properties = regionMapping._constantRegionRowObjects[uniqueId];
