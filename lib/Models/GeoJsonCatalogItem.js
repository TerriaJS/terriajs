...
*/
    ...
    return dataSource.load(geoJsonItem._readyData, options).then(function() {
        ...
        for (var i = 0; i < entities.length; ++i) {
            ...
            if (defined(entity.polygon) && polygonHasWideOutline(entity.polygon) && !polygonIsFilled(entity.polygon) && !polygonHasHoles(entity.polygon)) {
...
function polygonHasHoles(polygon) {
    if (!defined(polygon.hierarchy)) {
        return false;
    }

    var hierarchy = polygon.hierarchy.getValue();
    return defined(hierarchy.holes) && hierarchy.holes.length > 0;
}
