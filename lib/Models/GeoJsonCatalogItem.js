  ...
  return dataSource.load(geoJsonItem._readyData, options).then(function() {
    ...
    try {
      for (var i = 0; i < entities.length; ++i) {
        ...
        if (!properties.networkconnection === true) continue;

        console.log(entity);
        //const coordinates = entity.geometry.coordinates;
        var startLon = properties.source_x;
        var startLat = properties.source_y;
        var stopLon = properties.target_x;
        var stopLat = properties.target_y;
        ...
        var startPosition = Cartesian3.fromDegrees(startLon, startLat, 0);
        property.addSample(startTime, startPosition);
        var stopPosition = Cartesian3.fromDegrees(stopLon, stopLat, 0);
        ...
        property.addSample(startTime, startOffsetPosition);
        property.addSample(midTime, movedPoint);
        property.addSample(stopTime, stopOffsetPosition);
        property.setInterpolationOptions({
          interpolationDegree: 5,
          interpolationAlgorithm: LagrangePolynomialApproximation
        });

        const color1 = new Color(0.2, 0.4, 1.0, 0.35);
        const color2 = new Color(0.15, 0.3, 1.0, 0.8);
