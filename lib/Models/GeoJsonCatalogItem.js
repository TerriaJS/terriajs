...
function updateModelFromData(geoJsonItem, geoJson) {
    ...
    // Detect whether this "geojson" is secretly topojson
    if(defined(geoJson.type) && geoJson.type === "Topology") {
        for(var object in geoJson.objects) {
            if (geoJson.objects.hasOwnProperty(object)) {
              // Unroll the topojson object into corresponding geojson and continue
              geoJson = topojson.feature(geoJson, geoJson.objects[object]);
              // TODO(yuri): Properly handle Topojson files with multiple objects
              break;
            }
        }
    }
