...
function loadGeoJson(geoJsonItem) {
    ...
    */
    ...
    if (geoJsonItem.name !== 'Unnamed Item') { // Don't add bounding box from preview
        ...
        // IE11 incompatible
        var uniqueProperties = properties.filter(p => new Set(geoJsonItem._readyData.features.map(f => f.properties[p])).size === geoJsonItem._readyData.features.length);
        if (uniqueProperties.length > 0) {
            // Region-mappable
            geoJsonCounter++;
            var tileIndex = geojsonvt(geoJsonItem._readyData);
            var urls = Object.keys(RegionProviderList.metaList); // Probably only one URL
            if (urls.length === 1) {
                RegionProviderList.metaList[urls[0]].then(rpList => {
                    var regionProviders = uniqueProperties.map(prop => {
                        var rp = new RegionProvider(`geojson_${geoJsonCounter}`, {
                ...
                console.error(`Expected one region provider url, but there are ${urls.length} (${urls.join(', ')})`);
            }
        }
    }
