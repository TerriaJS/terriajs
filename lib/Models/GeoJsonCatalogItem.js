...
function loadGeoJson(geoJsonItem) {
    ...
    */
    ...
    if (defined(geoJsonItem.terria.cesium)) {
        var result = new ImageryLayer(MapboxVectorTileImageryProvider.fromGeoJson(geoJsonItem._readyData, options), {
            show : true,
        ...
        var scene = geoJsonItem.terria.cesium.scene;
        scene.imageryLayers.add(result);

    } else if (defined(geoJsonItem.terria.leaflet)) {
        var map = geoJsonItem.terria.leaflet.map;
        var tileLayerOptions = {
            async: true,
            opacity: options.fill.alpha,

        };

        if (defined(map.options.maxZoom)) {
            tileLayerOptions.maxZoom = map.options.maxZoom;
        }

        var layer = new MapboxVectorCanvasTileLayer(MapboxVectorTileImageryProvider.fromGeoJson(geoJsonItem._readyData, options), tileLayerOptions);
        layer.addTo(map);
        layer.bringToFront();
    }
