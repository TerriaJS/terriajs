...
var GeoJsonCatalogItem =  function(terria, url) {
    ...
    /**
     * The fields/attributes associated with data.
     * @type {Array}
     */
...
GeoJsonCatalogItem.prototype.searchDataForMatchingFeatures = function(searchText, regex) {
    // check id property to avoid searching the search result that has been added to the workbench
    if (this.id !== 'WorkbenchSearchResult' && defined(this._readyData) && this._readyData.features.length > 0) {
        if (!defined(this.fields)) this.fields = Object.keys(this._readyData.features[0].properties);
        return searchGeoJson(this, regex);
    }
    return [];
};


function searchGeoJson(geojson, regex) {
    const matches = [];
    geojson._readyData.features.forEach(f => {
        for (var ii = 0; ii < geojson.fields.length; ii++) {
            if (regex.test(f.properties[geojson.fields[ii]])) {
                matches.push({
                    matchType: geojson.type,
                    workbenchItem: geojson,
                    feature: f,
                    matchFieldText: f.properties[geojson.fields[ii]]
                });
                break;
            }
        }
