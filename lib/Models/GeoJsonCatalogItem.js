...
function loadGeoJson(geoJsonItem) {
    ...
    */
    ...
    if (geoJsonItem.name !== 'Unnamed Item') { // Don't add bounding box from preview
        var properties = Object.keys(geoJsonItem._readyData.features[0].properties);
        var uniqueProperties = properties.filter(prop =>
            // Filter out JSON properties since RegionProvider expects number or string
            geoJsonItem._readyData.features.every(f => ["number", "string"].includes(typeof f.properties[prop]))
        ).filter(p =>
            // Filter out properties that aren't unique per row
            new Set(geoJsonItem._readyData.features.map(f => f.properties[p])).size === geoJsonItem._readyData.features.length
        );
        console.log(`Unique properties are: ${uniqueProperties.join(',')}`)
        if (uniqueProperties.length > 0) {
            // Region-mappable
            var fid = 0;
            geoJsonItem._readyData.features.forEach(f => f.properties['FID'] = fid++);
            var tileIndex = geojsonvt(geoJsonItem._readyData);
            RegionProviderList.fromUrl(geoJsonItem.terria.configParameters.regionMappingDefinitionsUrl, geoJsonItem.terria.corsProxy).then(rpList => {
                var regionProviders = uniqueProperties.map(prop => {
                    geoJsonCounter++;
                    var rp = new RegionProvider(`geojson_prop_${geoJsonCounter}`, {
                        nameProp: uniqueProperties[0],
                        regionProp: prop,
                        layerName: '',
                        server: tileIndex,
                        serverType: 'MVT',
                        serverMaxZoom: 24,
                        aliases: [`${geoJsonCounter}_${geoJsonItem.name}_${prop}`]
                    });
                    console.log(`Aliased as ${geoJsonCounter}_${geoJsonItem.name}_${prop}`);
                    RegionProvider.processRegionIds(rp, geoJsonItem._readyData.features.map(f => f.properties[prop]), undefined, 'serverReplacements');
                    return rp;
                });
                rpList.regionProviders.push(...regionProviders);
            });
        }
    }
