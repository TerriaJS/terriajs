...
var PolylineGlowMaterialProperty = require("terriajs-cesium/Source/DataSources/PolylineGlowMaterialProperty")
  .default;
var PolylineOutlineMaterialProperty = require("terriajs-cesium/Source/DataSources/PolylineOutlineMaterialProperty")
  .default;
var PolylineArrowMaterialProperty = require("terriajs-cesium/Source/DataSources/PolylineArrowMaterialProperty")
  .default;
var StripeMaterialProperty = require("terriajs-cesium/Source/DataSources/StripeMaterialProperty")
  .default;
var StripeOrientation = require("terriajs-cesium/Source/DataSources/StripeOrientation")
  .default;
var LinearApproximation = require("terriajs-cesium/Source/Core/LinearApproximation")
  .default;
var LagrangePolynomialApproximation = require("terriajs-cesium/Source/Core/LagrangePolynomialApproximation")
  .default;
var Cartographic = require("terriajs-cesium/Source/Core/Cartographic").default;
var VelocityOrientationProperty = require("terriajs-cesium/Source/DataSources/VelocityOrientationProperty")
  .default;
var LabelStyle = require("terriajs-cesium/Source/Scene/LabelStyle").default;
var Cesium = require("terriajs-cesium/Source/Core/ScreenSpaceEventHandler")
  .default;
...
const { default: ArcType } = require("terriajs-cesium/Source/Core/ArcType");
const { default: Ellipsoid } = require("terriajs-cesium/Source/Core/Ellipsoid");
...
GeoJsonCatalogItem.prototype.loadGeoJson = function(geoJsonItem) {
  loadGeoJson(geoJsonItem);
};
...
function loadGeoJson(geoJsonItem) {
  ...
  return dataSource.load(geoJsonItem._readyData, options).then(function() {
    ...
    for (var i = 0; i < entities.length; ++i) {
      ...
      // Regenerate entities for network connections
      if (
        properties.networkconnection &&
        properties.networkconnection.getValue() === true
      ) {
        // Get connection end points
        var startPosition = entity.polyline.positions.getValue()[0];
        var stopPosition = entity.polyline.positions.getValue()[1];

        // Times are required for the interpolation
        var startTime = JulianDate.now();
        var startOffsetTime = JulianDate.addSeconds(
          startTime,
          1000,
          new JulianDate()
        );
        var midTime = JulianDate.addSeconds(startTime, 43200, new JulianDate());
        var midTime2 = JulianDate.addSeconds(
          startTime,
          43300,
          new JulianDate()
        );
        var stopOffsetTime = JulianDate.addSeconds(
          startTime,
          85400,
          new JulianDate()
        );
        ...
        var posProperty = new SampledPositionProperty();
        posProperty.addSample(startTime, startPosition);
        posProperty.addSample(stopTime, stopPosition);
        const distance = Cartesian3.distance(startPosition, stopPosition);

        const timeToCartesian = (time, height) => {
          const cartographicPoint = Cartographic.fromCartesian(
            posProperty.getValue(time)
          );
          cartographicPoint.height = height;
        ...
        // Calculate the midpoint of the straight path and move it to the side
        const midPosition = timeToCartesian(midTime, 0);
        const midPos2 = timeToCartesian(midTime2, 0);
        const vdir = Cartesian3.subtract(
          midPos2,
          midPosition,
          new Cartesian3()
        );
        const perp = Cartesian3.cross(vdir, midPosition, new Cartesian3());
        const normPerp = Cartesian3.normalize(perp, new Cartesian3());
        const scaledPerp = Cartesian3.multiplyByScalar(
          normPerp,
          -distance / 10,
          new Cartesian3()
        );
        const movedPoint = Cartesian3.add(
          midPosition,
          scaledPerp,
          new Cartesian3()
        );

        // Calculate start/stop offset points
        const startOffsetPosition = timeToCartesian(startOffsetTime, 0);
        const stopOffsetPosition = timeToCartesian(stopOffsetTime, 0);

        // Redo the path to be the new arc.
        posProperty = new SampledPositionProperty();
        posProperty.addSample(startTime, startOffsetPosition);
        posProperty.addSample(midTime, movedPoint);
        posProperty.addSample(stopTime, stopOffsetPosition);
        ...
        dataSource.entities.add({
          ...
          path: {
            ...
            material: new StripeMaterialProperty({
              evenColor: new Color(12 / 255, 112 / 255, 167 / 255, 0.4), // target #0C70A7
              oddColor: new Color(238 / 255, 118 / 255, 85 / 255, 0.4), // source  #EE7755
              repeat: 1,
              offset: 0.5,
              orientation: StripeOrientation.VERTICAL
            }),
            width: Math.sqrt(properties.value.getValue()) / 10,
            leadTime: 1e10,
            trailTime: 1e10
          }
        });
