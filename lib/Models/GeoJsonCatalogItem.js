  ...
  return dataSource.load(geoJsonItem._readyData, options).then(function() {
    ...
    try {
      for (var i = 0; i < entities.length; ++i) {
        ...
        var qTime1 = JulianDate.addSeconds(startTime, 21600, new JulianDate());
        var midTime = JulianDate.addSeconds(startTime, 43200, new JulianDate());
        var qTime3 = JulianDate.addSeconds(startTime, 64800, new JulianDate());
        ...
        var midPoint = Cartographic.fromCartesian(property.getValue(midTime));
        midPoint.height = 1200000;
        var midPosition = Ellipsoid.WGS84.cartographicToCartesian(
          midPoint,
          new Cartesian3()
        );

        var qPoint1 = Cartographic.fromCartesian(property.getValue(qTime1));
        qPoint1.height = 150000;
        var qPosition1 = Ellipsoid.WGS84.cartographicToCartesian(
          qPoint1,
          new Cartesian3()
        );

        var qPoint3 = Cartographic.fromCartesian(property.getValue(qTime3));
        qPoint3.height = 1500000;
        var qPosition3 = Ellipsoid.WGS84.cartographicToCartesian(
          qPoint3,
          new Cartesian3()
        );
        ...
        property.addSample(startTime, startPosition);
        //      property.addSample(qTime1, qPosition1);
        property.addSample(midTime, midPosition);
        //      property.addSample(qTime3, qPosition3);
        property.addSample(stopTime, stopPosition);
        ...
        dataSource.entities.add({
          ...
          path: {
            ...
            width: 2 * Math.log(properties.value.getValue() + 2),
            leadTime: 1e10,
            trailTime: 1e10
          }
        });
