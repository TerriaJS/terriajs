...
var colorPalette = require('../Core/colorPalette');
...
function getRandomCssColor(palette, name) {
    var index = hashFromString(name || '') % palette.length;
    ...
    */
    ...
    function defaultColor(colorString, name) {
        if (colorString === undefined) {
            var color = Color.fromCssColorString(getRandomCssColor(colorPalette, name));
            color.alpha = 0.6;
            return color;
        } else {
