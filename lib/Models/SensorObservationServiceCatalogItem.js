...
var SensorObservationServiceCatalogItem = function(terria, url) {
     ...
     * An array of objects with keys 'identifier', 'title' and (optionally) 'defaultDuration', eg.
     *     [{
...
function buildConcepts(item) {
    ...
    if (item.procedures.length > 1) {
        ...
        item._previousProcedureIdentifier = concept.items[0].id;
    }
    if (item.observableProperties.length > 1) {
        ...
        concepts.push(concept);
        item._previousObservablePropertyIdentifier = concept.items[0].id;
    }
    return concepts;
}
...
function changedActiveItems(item) {
    ...
    loadObservationData(item).then(function() {
        ...
        // if (defined(item._previousProcedureIdentifier)) {
        ...
        //     var old = parentConcept.items.filter(concept => !concept.isActive && (concept.id === item._previousProcedureIdentifier))[0];
        //     if (defined(old)) {
        //         old.toggleActive();
        //     }
        // }
        // if (defined(item._previousObservablePropertyIdentifier)) {
        //     parentConcept = item._concepts.filter(concept => concept.id === 'observableProperties')[0];
        //     old = parentConcept.items.filter(concept => !concept.isActive && (concept.id === item._previousObservablePropertyIdentifier))[0];
        //     if (defined(old)) {
        //         old.toggleActive();
        //     }
        // }
