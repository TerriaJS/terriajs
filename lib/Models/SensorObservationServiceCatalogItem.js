...
var SensorObservationServiceCatalogItem = function(terria, url) {
     ...
     * Defaults to "Procedures", but eg. for BoM, "Frequency" would be better.
     * @type {String}
     */
    this.proceduresName = 'Procedures';

    /**
     * Gets or sets the name seen by the user for the list of observable properties.
     * Defaults to "Properties", but eg. for BoM, "Observations" would be better.
     * @type {String}
     */
    this.observablePropertiesName = 'Properties';
...
function changedActiveItems(item) {
    ...
    loadObservationData(item).then(function() {
        ...
        item._recentProcedureName = procedureName;
        item._recentObservablePropertyName = observablePropertyName;
    }).otherwise(function(e) {
        // var parentConcept;
