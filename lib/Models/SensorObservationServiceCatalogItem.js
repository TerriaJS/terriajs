...
SensorObservationServiceCatalogItem.prototype.loadIntoTableStructure = function(url) {
    ...
    return loadSoapBody(sosItem, templateContext).then(function(body) {
        ...
        if (defined(observationData)) {
            ...
            var timeColumn = new TableColumn('date', dateValues);
            var valueColumn = new TableColumn('value', valueValues);
            var featureColumn = new TableColumn('feature id', featureValues);
            var procedureColumn = new TableColumn('procedure', procedureValues);
            var tableStructure = new TableStructure('observations');
            tableStructure.columns = [timeColumn, valueColumn, featureColumn, procedureColumn];
            console.log('Observation table', tableStructure);
...
function loadFeaturesOfInterest(item) {
    ...
    return loadSoapBody(item, templateContext).then(function(body) {
        ...
        var featuresOfInterest = item.tableStructure.getColumnWithName('identifier').values;
        item.loadIntoTableStructure(featuresOfInterest);
