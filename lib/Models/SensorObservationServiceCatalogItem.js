...
var SensorObservationServiceCatalogItem = function(terria, url) {
    ...
    this.procedures = [
        // {
        //     name: 'http://bom.gov.au/waterdata/services/tstypes/Pat4_C_B_1_DailyMean',
    ...
    /**
     * A start date in ISO8601 format. All requests filter to this start date. Set to undefined for none.
     * @type {String}
     */
    this.startDate = '2014-01-01T00:00:00Z';

    /**
     * An end date in ISO8601 format. All requests filter to this end date. Set to undefined for none.
     * @type {String}
     */
    this.endDate = '2016-12-31T23:59:59Z';
...
function querySos(item, options) {
    var url = new URI(item.url)
        .query({ service: 'SOS', version: '2.0' })
        .addQuery(options)
        .toString();

    return loadText(proxyCatalogItemUrl(item, url, '1d'))
        .then(xml2json);
}
...
SensorObservationServiceCatalogItem.prototype._load = function() {
    ...
    return when.all([
        loadFeaturesOfInterest(that),
        loadCapabilities(that)
...
function getTemporalFiltersContext(item) {
    if (item.startDate && item.endDate) {
        return [{index: 1, startDate: item.startDate, endDate: item.endDate}];
    }
}
