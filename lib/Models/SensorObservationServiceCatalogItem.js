...
function observationResponsesToTableStructure(item, procedure, observableProperty, responses) {
    ...
    function extractValues(response) {
        var observationData = response.GetObservationResponse && response.GetObservationResponse.observationData;
        if (defined(observationData)) {
            ...
            observations.forEach(observation => {
                ...
                dateValues.push(...measurements.map(measurement =>
                    (typeof measurement.time === 'object' ? null : measurement.time)
                ));
                valueValues.push(...measurements.map(measurement =>
                ...
                observedPropertyValues.push(...measurements.map(_ => observableProperty.identifier ));
            });
        }
    }
    var dateValues = [], valueValues = [], featureValues = [], procedureValues = [], observedPropertyValues = [];
...
SensorObservationServiceCatalogItem.prototype.loadIntoTableStructure = function(featureOfInterestIdentifiers) {
    ...
    return when.all(requests)
        .then(responses => observationResponsesToTableStructure(item, procedure, observableProperty, responses))
        .otherwise(function(e) {
    ...
    return loadSoapBody(item, templateContext).then(function(body) {
        ...
        if (item.stationIdWhitelist) {
            featureMembers = featureMembers.filter(m => 
                m.MonitoringPoint && item.stationIdWhitelist.indexOf(String(m.MonitoringPoint.identifier)) >= 0);
        }
        if (item.stationIdBlacklist) {
            featureMembers = featureMembers.filter(m => 
                m.MonitoringPoint && !item.stationIdBlacklist.indexOf(String(m.MonitoringPoint.identifier)) >= 0);
        }
