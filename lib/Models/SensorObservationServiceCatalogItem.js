...
SensorObservationServiceCatalogItem.prototype._load = function() {
    ...
    return when.all([ loadObservations(this), loadDescription(this) ]).otherwise(function(e) {
...
function loadObservations(sosItem) {
    ...
    loadWithXhr({
        url : proxyCatalogItemUrl(sosItem, sosItem.url, '1d'),
            ...
            if (!defined(json.Body) || !defined(json.Body.GetObservationResponse)) {
                throw new TerriaError({
                    sender: sosItem,
                    title: sosItem.name,
                    message: 'The SOS server responded with an unknown format.'
                });
            }
        ...
        if (defined(observationResponse.observationData)) {
