...
var SensorObservationServiceCatalogItem = function(terria, url) {
     ...
     * Note the BoM server can return >1 timeseries/feature identifier (eg. ...stations/41001702)
     * So it can be sensible to set this below the true limit.
     * @type {Integer}
     */
    this.responseSizeLimit = 225;
...
function loadObservationData(item) {
    ...
    if (featuresOfInterest.length > item.responseSizeLimit * item.requestNumberLimit) {
        // Too many features to load observations. Just show their positions without data.
