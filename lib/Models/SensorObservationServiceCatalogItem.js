...
SensorObservationServiceCatalogItem.prototype._load = function() {
    ...
    return when.all([
            loadSos(observationsOpts, processObservations),
            loadSos(availabilityOpts, processAvailability),
            loadSos(describeOpts, processDescription)
...
function processDescription(sosItem, sensormljson) {
    //var description = sensormljson.description.SensorDescription.data.SensorML.member; // doesn't really contain anything useful. Maybe we don't need to do this.
    console.log('Sensor description: ' , sensormljson);
}
...
// http://www.bom.gov.au/waterdata/services?datasource=0&service=SOS&request=GetObservation&temporalFilter=phenomenonTime,2016-07-01/2016-07-02&offering=http://bom.gov.au/waterdata/services/tstypes/Year.Mean
/*
    Observations are the actual data points, buried in a huge pile of XML. Unfortunately BOM's server doesn't support the much cleaner GetRequest.
 */
