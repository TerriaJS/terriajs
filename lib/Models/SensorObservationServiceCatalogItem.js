...
function changedActiveItems(item) {
    ...
    loadObservationData(item).then(function() {
        ...
        if (defined(item._recentProcedureName)) {
            var parentConcept = item._concepts.filter(concept => concept.id === 'procedures')[0];
            parentConcept.items.forEach(concept => {
                concept.isActive = (concept.id === item._recentProcedureName);
                // TODO: can we do this without triggering a reload?
            });
        }
        if (defined(item._recentObservablePropertyName)) {
            ...
            parentConcept.items.forEach(concept => {
                concept.isActive = (concept.id === item._recentObservablePropertyName);
                // TODO: can we do this without triggering a reload?
            });
        }
