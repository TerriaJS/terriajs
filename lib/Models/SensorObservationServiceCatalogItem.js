...
var SensorObservationServiceCatalogItem = function(terria, url) {
    ...
    this._recentProcedureName = undefined;
    this._recentObservablePropertyName = undefined;
     ...
     * Servers may have a Response Size Limit, eg. 250. Note the number of responses may be different to the number requested.
     * (The BoM server can return > 1 timeseries/feature identifier, eg. ...stations/41001702.)
     * So it can be sensible to set this below the response size limit.
     * @type {Integer}
     */
    this.requestSizeLimit = 225;
    ...
    knockout.getObservable(this, 'activeConcepts').subscribe(function() {
        // TODO: what if you change these while another one is loading?
        if (!this.isLoading) {
...
SensorObservationServiceCatalogItem.prototype._load = function() {
    ...
    // loadCapabilities(that); // TODO: just for testing
...
function getObjectCorrespondingToSelectedConcept(item, conceptIdAndItemKey) {
    if (item[conceptIdAndItemKey].length === 1) {
        ...
        if (activeConceptIndices.length === 1) {
            var name = activeConceptIndices[0].id;
            var matches = item[conceptIdAndItemKey].filter(element => element.name === name);
            return matches[0];
        }
    }
}
...
SensorObservationServiceCatalogItem.prototype.loadIntoTableStructure = function(url) {
    ...
    if (!defined(procedure.name) || (!defined(observableProperty.name))) {
        return when();
    }
    console.log('There are ' + featureOfInterestIdentifiers.length + ' features.');
    for (var startFeatureNumber = 0; startFeatureNumber < featureOfInterestIdentifiers.length; startFeatureNumber += this.requestSizeLimit) {
        ...
        var paramArray = convertObjectToNameValueArray({
            ...
            observedProperty:  observableProperty.name,
    ...
    return when.all(promises).then(function(bodies) {
        ...
        var valueColumn = new TableColumn(observableProperty.title + ' ' + procedure.title, valueValues, columnOptions);
        ...
        console.log(e);
        throw e;  // Could improve this to dynamically adjust the requestSizeLimit, using
...
function loadObservationData(item) {
    ...
    if (!item.tryToLoadObservationData || featuresOfInterest.length > item.requestSizeLimit * item.requestNumberLimit) {
        // Too many features to load observations (or we've been asked not to try). Just show their positions without data.
        var tableStructure = item._tableStructure;
        if (!defined(tableStructure)) {
            tableStructure = new TableStructure(item.name);
        }
        var columns = createColumnsFromMapping(item._featureMapping, tableStructure);
        tableStructure.columns = columns;
        if (!defined(item._tableStructure)) {
            item.initializeFromTableStructure(tableStructure);
        } else {
            tableStructure.columns[1].isActive = true; // TODO: SUPER TEMP
    ...
    return item.loadIntoTableStructure(featuresOfInterest).then(function(observationTableStructure) {
        ...
        observationTableStructure.idColumnNames = ['identifier'];
        if (!defined(item._tableStructure)) {
            ...
            observationTableStructure.columns[1].isActive = true; // TODO: SUPER TEMP
            item._tableStructure.columns = observationTableStructure.columns;
            console.log(item, item._tableStructure);
 ...
 * and sets item._recentProcedureName and _recentObservablePropertyName.
 * @private
 */
function buildConcepts(item) {
    ...
    if (item.procedures.length > 1) {
        ...
        item._recentProcedureName = concept.items[0].id;
    }
    if (item.observableProperties.length > 1) {
        ...
        concepts.push(concept);
        item._recentObservablePropertyName = concept.items[0].id;
    }
    return concepts;
}
...
var getUniqueValues = require('../Core/getUniqueValues');  // TODO: temp
/**
...
function createColumnsFromMapping(featureMapping, observationTableStructure) {
    ...
    if (defined(identifierColumn)) {
        ...
        console.log('Got ' + getUniqueValues(identifiers).length + ' unique feature identifiers.');
    } else {
...
function changedActiveItems(item) {
    ...
    loadObservationData(item).then(function() {
        ...
        item._recentProcedureName = procedure.name;
        item._recentObservablePropertyName = observableProperty.name;
    }).otherwise(function(e) {
        // var parentConcept;
        // // By waiting until after we reset the active concepts to set isLoading=false, we do not retrigger a load here.
        // if (defined(item._recentProcedureName)) {
        ...
        //     var old = parentConcept.items.filter(concept => !concept.isActive && (concept.id === item._recentProcedureName))[0];
        //     if (defined(old)) {
        //         old.toggleActive();
        //     }
        // }
        // if (defined(item._recentObservablePropertyName)) {
        //     parentConcept = item._concepts.filter(concept => concept.id === 'observableProperties')[0];
        //     old = parentConcept.items.filter(concept => !concept.isActive && (concept.id === item._recentObservablePropertyName))[0];
        //     if (defined(old)) {
        //         old.toggleActive();
        //     }
        // }
...
// // TODO: THESE FUNCTIONS ARE JUST FOR TESTING
...
//     return loadText(proxyCatalogItemUrl(sosGroup, url, '1d'))  // TODO: won't want 1d in the longrun
//         .then(xml2json);
// }
// /**
