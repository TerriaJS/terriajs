...
var UserDrawing = function(options) {
    ...
    // HACK: assume Cesium for now!
    //this.dragHelper = new CesiumDragPoints(options.terria, options.terria.cesium.scene, options.terria.cesium.viewer);
    this.dragHelper = new LeafletDragPoints(options.terria);
};
    ...
    knockout.getObservable(pickPointMode, 'pickedFeatures').subscribe(function(pickedFeatures) {
        when(pickedFeatures.allFeaturesAvailablePromise, function() {
            if (defined(pickedFeatures.pickPosition)) {
                ...
                if (that.dragHelper.dragCount < 5 && !that._clickedExistingPoint(pickedFeatures.features)) {
                    // No existing point was picked, so add a new point
                    that._addPointToPointEntities('Another Point', pickedPoint);
                } else {
                    that.dragHelper.dragCount = 0;
                }
                that._prepareToAddNewPoint();
            }
        });
...
UserDrawing.prototype._clickedExistingPoint = function(features) {
    ...
    features.forEach((feature)=> {
        ...
        if (index === -1) {
            ...
            this._removePointFromPointEntities(feature);
...
/**
 * Remove point from list of pointEntities, which essentially deletes the point
 * @private
 * @param TODO
 */
UserDrawing.prototype._removePointFromPointEntities = function(feature) {
    // We can't just 'remove' the feature from the entities list, because it's not necessarily the same entity,
    // even though the id is the same.
    var pointEntity;
    for (var i=0; i<this.pointEntities.entities.values.length; i++) {
        var pointFeature = this.pointEntities.entities.values[i];
        if (pointFeature.id === feature.id) {
            pointEntity = pointFeature;
            break;
        }
    }
    if (defined(pointEntity)) {
        this.pointEntities.entities.remove(pointEntity);
    }
};
 ...
 * @param TODO
 */
UserDrawing.prototype._addPointToPointEntities = function(name, pickedPoint) {
    ...
    this.dragHelper.updateDraggableObjects(this.pointEntities.entities.values);
