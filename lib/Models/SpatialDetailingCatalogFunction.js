...
SpatialDetailingCatalogFunction.prototype.invoke = function() {
    ...
    var request = {
        ...
        boundaries_name: this._coarseDataRegionTypeParameter.value.regionType.replace(/_2011$/, ''),
        region_codes: value.regionCodes,
        columns: value.columnHeadings[0],
        table: value.singleSelectValues,
        parameters: {
            disagg_boundaries_name: this._regionTypeToPredictParameter.value.regionType.replace(/_2011$/, ''),
    ...
    return invokeTerriaAnalyticsService(this.terria, name, this.url, request).then(function(invocationResult) {
        ...
        var csv = that._regionTypeToPredictParameter.value.aliases[0] + ',Predicted ' + predictedCharacteristic + ' (Quality: ' + result.Quality + '),Lower bound of 95% confidence interval,Upper bound of 95% confidence interval\n';
