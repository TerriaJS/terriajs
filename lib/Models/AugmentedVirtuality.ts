...
//   /* These are the heights that we can toggle through (in meters - above the
//    * surface height). */
//   static readonly PRESET_HEIGHTS = [1000, 250, 20];

//   @observable private _manualAlignment = false;
...
//  * roll, pitch and heading which can be used to set the cameras orientation.
//  *
//  * @param screenOrientation The screen orientation in degrees. Note:
//  * This field is optional, if supplied this value will be used for the screen
//  * orientation, otherwise the screen orientation will be obtained during the
//  * execution of this function.

//  * @return A object with the roll, pitch and heading stored into the
//  * orientation.  @private
//  */
//   private getCurrentOrientation(screenOrientation?: number) {
//     const alpha = this.alpha;
...
//   /**
//    * Moves the viewer to a specified height, setting the orientation so that it
//    * matches the correct Augmented Virtuality orientation.
//    *
//    * @param height The height in Meters above the globe surface. Note: If height is below
//    *                       {@link AugmentedVirtuality.MINIMUM_HOVER_HEIGHT} the height will be set to
//    *                       {@link AugmentedVirtuality.MINIMUM_HOVER_HEIGHT} to avoid visual artifacts
//    *                        when the viewer becomes to close to the surface.
//    * @param [position] The location to hover over. If not specified the current
//    *                   camera location will be used.
//    * @param [flyTo=true] Whether to fly to the location (true) or whether to jump to the location (false).
//    */
//   private async hover(
//     height: number,
...
//     flyTo: boolean = true
//   ) {
//     // Get access to the camera...if it is not avaliable we can't set the new
//     // height so just return now.
//     const camera = this.terria.cesium && this.terria.cesium.scene.camera;
//     if (!camera) return;

//     if (!position) position = camera.positionCartographic.clone();

//     // Clamp the minimum hover height (heights below this value could lead to
//     // poor visual artifacts).
//     height = Math.max(height, AugmentedVirtuality.MINIMUM_HOVER_HEIGHT);
...
//       if (flyTo) camera.flyTo(pose);
//       else camera.setView(pose);
...
//     } else {
//       // Get the ground surface height at this location and offset the height by it.
//       const updatedPosition = await sampleTerrainMostDetailed(terrainProvider, [
//         position
//       ]);
//       flyToHeight(updatedPosition[0].height);
//     }
...
//  *
//  * @return The current screen orientation in degrees.
//  * @private
//  */
...
//  * suitable for specifying the Terria camera orientation.
//  *
//  * @param alpha The alpha value of the device orientation in degrees
//  *              (this is the alpha value in the device's frame of reference).
//  * @param beta The beta value of the device orientation in degrees (this is the
//  *         beta value in the device's frame of reference).
//  * @param gamma The gamma value of the device orientation in degrees (this is
//  *        the gamma value in the device's frame of reference).
...
//   const rotation = Matrix3.clone(Matrix3.IDENTITY);
//   let rotationIncrement: Matrix3;
...
//   rotationIncrement = Matrix3.fromRotationZ(
//     CesiumMath.toRadians(screenOrientation)
//   );
//   Matrix3.multiply(rotation, rotationIncrement, rotation);

//   // Pitch - Align the device orientation frame with the ceasium orientation frame.
//   rotationIncrement = Matrix3.fromRotationX(CesiumMath.toRadians(90));
//   Matrix3.multiply(rotation, rotationIncrement, rotation);

//   // Roll - Apply the deivce roll.
//   rotationIncrement = Matrix3.fromRotationZ(CesiumMath.toRadians(gamma));
//   Matrix3.multiply(rotation, rotationIncrement, rotation);

//   // Pitch - Apply the deivce pitch.
//   rotationIncrement = Matrix3.fromRotationX(CesiumMath.toRadians(-beta));
//   Matrix3.multiply(rotation, rotationIncrement, rotation);

//   // Heading - Apply the incremental deivce heading (from when start was last triggered).
//   rotationIncrement = Matrix3.fromRotationY(
//     CesiumMath.toRadians(-(alpha - realignAlpha))
//   );
//   Matrix3.multiply(rotation, rotationIncrement, rotation);
...
//   // Simplify notation for readability:
//   const r10 = rotation[Matrix3.COLUMN1ROW0];
//   const r11 = rotation[Matrix3.COLUMN1ROW1];
//   const r02 = rotation[Matrix3.COLUMN0ROW2];
//   const r12 = rotation[Matrix3.COLUMN1ROW2];
//   const r22 = rotation[Matrix3.COLUMN2ROW2];
