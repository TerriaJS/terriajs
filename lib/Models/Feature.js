...
Feature.fromEntityCollectionOrEntity = function(entity) {
    ...
    function getHasMatchingIdFunction(entityId) {
        return function(member) { return member.id === entityId; };
    }
    var feature;
    if (entity.entityCollection) {
        var matches = entity.entityCollection.values.filter(getHasMatchingIdFunction(entity.id));
        if (matches.length > 0) {
            feature = matches[0];
        }
    }
