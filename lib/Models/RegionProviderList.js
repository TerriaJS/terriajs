...
/**
 * Fetch JSON file and create ordered list of region providers from it. 
 * @returns Promise.
 */
RegionProviderList.prototype.init = function(url) {
    if (this._initialized) {
        return when(true);
    }
    if (!defined(url)) {
        url = this.terria.regionMappingDefinitionsUrl;
    }

    var that = this;
    return loadJson(url)
        .then(function(o) { 
            that.initFromObject(o);
        });
};
...
RegionProviderList.prototype.initFromObject = function(obj) {
    if (this._initialized) {
        throw new DeveloperError ("RegionProviderList already initialized.");
    }
    var that = this;
...
/**
 * Has RegionProviderList finished loading config file?
 * @returns {Boolean}
 */
RegionProviderList.prototype.ready = function() {
    ...
    for (i = 0; i < this.regionProviders.length; i++) {
        ...
        if (idx !== -1) {
            return { 
                ...
                disambigVariable: this.regionProviders[i].findDisambigVariable(variables)
            };
        }
    }
