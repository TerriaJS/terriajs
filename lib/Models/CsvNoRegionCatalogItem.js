...
var CsvCatalogItem = function(terria, url) {
    ...
    // A promise that resolves to a "region" object with regionProvider, column and disambigColumn properties,
    // once the region IDs have been loaded.
    this._regionIDsPromise = undefined;

    this._region = undefined;  // once the above promise resolves, save the region object directly.
...
defineProperties(CsvCatalogItem.prototype, {
    ...
    supportsReordering: {
        get: function() {
            return defined(this._region) && !this.keepOnTop;
        }
    },
...
function loadTable(csvItem, text) {
    ...
    if (dataSource.hasLatitudeAndLongitude) {
        ...
        // Check if any columns are aliases for regions.
        // Since the region mapping URL is only hit when needed, this is asynchronous.
        csvItem.regionIDsPromise = dataSource.regionPromise.then(function(region) {
            if (region) {
                console.log('Found match based on region', region);
                return region.regionProvider.loadRegionIDs()
                    .then(function(requiredReload) {
                        csvItem._region = region;
                        console.log('Loaded region ids', requiredReload, region);
                        return region;
                    });
            } else {
...
CsvCatalogItem.prototype._enable = function(layerIndex) {
    var that = this;
    this.regionIDsPromise.then(function(region) {
        if (region) {
...
function createRegionImageryProvider(catalogItem) {
    var wmsImageryProvider = new WebMapServiceImageryProvider({
        url: proxyCatalogItemUrl(catalogItem, catalogItem._region.regionProvider.server),
        layers: catalogItem._region.regionProvider.layerName,
    ...
    ImageryProviderHooks.addRecolorFunc(wmsImageryProvider, function(id) { return [0, 200, 100, 255]; });
    // ImageryProviderHooks.addPickFeaturesHook(wmsImageryProvider, function(results) {
    //     if (!defined(results) || results.length === 0) {
    //         return;
    //     }

    //     for (var i = 0; i < results.length; ++i) {
    //         // var uniqueId = results[i].data.properties[catalogItem._region.regionProvider.uniqueIdProp];
