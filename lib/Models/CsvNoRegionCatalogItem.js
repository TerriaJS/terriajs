...
var CsvCatalogItem = function(terria, url) {
    ...
    this._region = undefined;  // for region mapping
    this._imageryLayer = undefined;  // also for region mapping
...
function loadTable(csvItem, text) {
    ...
    if (dataSource.hasLatitudeAndLongitude) {
        ...
        dataSource.regionPromise.then(function(region) {
            if (region) {
                console.log('Found match based on region', region);
                region.regionProvider.loadRegionIDs()
                    .then(function(requiredReload) {
                        csvItem._region = region; // once all the ids are loaded, save the region so we can use its data later.
...
CsvCatalogItem.prototype._enable = function(layerIndex) {
    if (this._region) {
        this._imageryLayer = ImageryLayerCatalogItem.enableLayer(this, createRegionImageryProvider(this), this.opacity, layerIndex);
        this.terria.currentViewer.notifyRepaintRequired();
    }
};
...
CsvCatalogItem.prototype._show = function() {
    if (this._region) {
        ...
        // TODO: why not use a simple boolean for this?
