...
var CsvCatalogItem = function(terria, url) {
    ...
    overrideProperty(this, 'legendUrl', {
        get: function() {
            // Whenever the legend changes, we need to repaint the features. Is there a more direct way to trigger this?
            this.terria.currentViewer.notifyRepaintRequired();
            // And update the legendUrl
            if (defined(this._dataSource) && defined(this._dataSource) && defined(this._dataSource._legend)) {
                return this._dataSource._legend.asPngUrl();
            }
        }
    });
...
function loadTable(csvItem, text) {
    ...
    if (!dataSource.hasLocationData) {
        ...
        // Choose a reasonable column as the default selected column. Choose the first SCALAR, ENUM, or TIME column.
        var columnsByType = dataSource._tableStructure.columnsByType;
        var column = columnsByType[VarType.SCALAR][0] || columnsByType[VarType.ENUM][0] || columnsByType[VarType.TIME][0];
        if (column) {
            column.toggleActive();
        }
    }
