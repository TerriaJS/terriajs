...
var CsvCatalogItem = function(terria, url) {
     ...
     * @default false
     */
    this.pollReplaceData = undefined;
...
function setupTableStructure(item, tableStructure) {
    ...
    if (tableStructure.hasLatitudeAndLongitude) {
        ...
        item._dataSource = new TableDataSource(tableStructure, item._tableStyle, item.name, defined(item.pollSeconds));
