...
CsvCatalogItem.prototype.setChartable = function() {
    ...
    tableStructure.disableOthers = this.disableOthers.bind(this);
...
/**
 * Finds the other table structure that does not have the same type on xColumn and disable them.
 */
CsvCatalogItem.prototype.disableOthers = function() {
    var tableStructure = this._tableStructure;
    var xColumn = this.timeColumn || (this.tableStructure.columnsByType[VarType.SCALAR][0]);
    var catalog = this._terria.catalog;

    catalog.chartableItems.map(function(item) {
        if(item.tableStructure !== tableStructure){
            item.tableStructure.columns.forEach(column => {
                let xColumnOthers = item.timeColumn || (item.tableStructure && item.tableStructure.columnsByType[VarType.SCALAR][0]);
