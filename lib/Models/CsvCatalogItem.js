...
CsvCatalogItem.prototype.disableOthers = function() {
    var tableStructure = this._tableStructure;
    var xOrigin = this.timeColumn || (this.tableStructure.columnsByType[VarType.SCALAR][0]);
    var catalog = this._terria.catalog;

    catalog.chartableItems.map(function(item) {
        if(item.tableStructure !== tableStructure){
            item.tableStructure.columns.forEach(column => {
                let x = item.timeColumn || (item.tableStructure && item.tableStructure.columnsByType[VarType.SCALAR][0]);
                if(x.type !== xOrigin.type){
