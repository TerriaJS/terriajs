...
function showFeedBack(multimatches, failedmatches, successes, totalrows, regionVariable, csvItem) {
    ...
    var error = new ModelError({ 
            title: "Issues loading CSV file: " + csvItem.name,
            message: '<div>'+ msg +'</div>'
        });
    if (successes === 0) {
...
function createRegionLookupFunc(csvItem) {
    ...
    for (var i = 0; i < (userecs ? csvItem.recs.length : codes.length); i++) {
        ...
        if (typeof codes[row] === "number") {
            ...
            if (index < 0 && (defined(rd.serverRegexFind) || defined(rd.dataRegexFind))) {
                for (var j = 0; j < ids.length; j++) {
                    // no harm if serverRegexFind is undefined.
                    var s = ids[j].replace(RegExp(rd.serverRegexFind.toLowerCase(), 'gi'), defaultValue(rd.serverRegexReplace, '$1'));
                    var c = codes[row].toLowerCase().replace(RegExp(rd.dataRegexFind.toLowerCase(), 'gi'), defaultValue(rd.dataRegexReplace, '$1'));
                    if (s === c) {
                        index = j; break;
                    }
                }
...
function determineRegionVariable(varNames, aliases) {
...
function determineRegionType(dataset, regionWmsMap) {
    ...
    for (region in regionWmsMap) {
        if (regionWmsMap.hasOwnProperty(region)) {
            var idx = determineRegionVariable(varNames, regionWmsMap[region].aliases);
            if (idx !== -1) {
                regionType = region;
