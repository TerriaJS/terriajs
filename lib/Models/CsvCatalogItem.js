...
var CsvCatalogItem = function(terria, url) {
    ...
    overrideProperty(this, 'clock', {
        get: function() {
            var timeColumn = this._tableStructure && this._tableStructure.columnsByType[VarType.TIME][0];
...
function loadTableFromCsv(item, csvString) {
    var tableStructure = new TableStructure(item.name, {displayDuration: item._tableStyle.displayDuration});
