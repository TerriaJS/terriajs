...
function parseIdsXML (text, regionProp) {
    ...
    if (!defined(obj.member)) {
        console.log(text);
        throw new ModelError("Couldn't load region boundaries for region " + regionProp);
        return;
    }
    ...
    if (obj.member instanceof Array) {
        for (var i = 0; i < obj.member.length; i++) {
            ids.push(obj.member[i][regionProp]);
        }
    } else {
        ids.push(obj.member[regionProp]);
    }
    return ids;
}
...
function loadRegionIDs(regionDescriptor) {
    ...
    var baseurl = regionDescriptor.server + '?service=wfs&version=2.0&request=getPropertyValue';
    baseurl += '&typenames=' + regionDescriptor.layerName;
    baseurl += '&valueReference=';
    var url = corsProxy.getURL(baseurl + regionDescriptor.regionProp);
    // get the list of IDs that we will attempt to match against for this column
    var p = [loadText(url).then(function(text) {
            regionDescriptor.idMap = parseIdsXML(text, regionDescriptor.regionProp);
            }, function(err) {
                console.log(err);
            })];
    // if this column might be ambiguous then fetch the disambiguating values for each column as well (usually State)
    var dab = regionDescriptor.disambigColumns;
    if (defined(dab)) {
        
        url = corsProxy.getURL(baseurl + dab[0].regionProp);
        p.push(loadText(url).then(function(text) {
            if (!defined(regionDescriptor.dabIds)) {
                regionDescriptor.dabIds = {};
            }
            regionDescriptor.dabIds[dab[0].regionProp] = parseIdsXML(text, dab[0].regionProp);
        }));
    }
    return when.all(p);
}

function showFeedBack(multimatches, failedmatches, successes, totalrows, regionVariable, csvItem) {
    var msg = "";
    if (Object.keys(failedmatches).length > 0) {
        msg += 'These region names were <span class="warning-text">not recognised</span>: <br><br/>' + 
        '<samp>' + Object.keys(failedmatches).join('</samp>, <samp>') + '</samp>' + 
        '<br/><br/>';
    }
    if (Object.keys(multimatches).length > 0) {
        msg += 'These regions had <span class="warning-text">more than one value</span>: <br/><br/>' + 
        '<samp>' + Object.keys(multimatches).join("</samp>, <samp>") + '</samp>' + 
        '<br/><br/>';
    }
    if (!msg) {
        console.log(successes  + ' out of ' + totalrows + ' "' + regionVariable + '" regions matched successfully in ' + csvItem);
        return;
    }
    msg = "" + successes + " out of " + totalrows + " '<samp>" + regionVariable + "</samp>' regions matched.<br/><br/>" + msg;
    ...
    var error = new ModelError({ 
            title: "Issues loading CSV file: " + csvItem.name.slice(0,20), // Long titles mess up the message body
            message: '<div>'+ msg +'</div>'
        });
    if (successes === 0) {
        ...
        csvItem.terria.error.raiseEvent(error);
    }
...
- DONE (regexes in regionmapping.json) "State" is ambiguous, and assuming it's code, or it's a name, is probably sometimes wrong
- DONE Support multiple find/replace pairs on server and local
- Semi-done (exact matching happens first) LGA is ambiguous if we ignore lga type, and still has a Campbelltown ambiguity if we don't. 
- DONE Would like to allow multi-column matches: if lga_name is used, look for a state as well.
...
// replacement can contain '$1' etc.
    ...
    if (defined(replacements)) {
        replacements.forEach(function(rep) {
            r = r.replace(RegExp(rep[0].toLowerCase(), 'gi'), rep[1].toLowerCase());
        });
    }
...
function codeMatchesRegionID(code, id, row, idnum, rd) {
    if (applyReplacements(id, rd.serverReplacements) !== applyReplacements(code, rd.dataReplacements)) {
        return false; // failed match, even after best efforts
    }
    if (!defined(rd.disambigColumns) || rd.disambigColumns.length === 0) {
    ...
    var r = codeMatchesRegionID(dabcode, dabid, row, idnum, dabrd);
    return r;
    ...
    for (var i = 0; i < (userecs ? csvItem.recs.length : codes.length); i++) {
        ...
        totalrows++;
        if (typeof codes[row] === "number") {
            index = ids.indexOf(codes[row]);
        } else {
            index = ids.indexOf(String(codes[row]).toLowerCase());
            ...
            if (index < 0 && (defined(rd.serverReplacements) || defined(rd.dataReplacements))) {
                for (var j = 0; j < ids.length; j++) {
                    if (codeMatchesRegionID(codes[row], ids[j], row, j, rd)) {
                        index = j; break;
                    }
                }
            }
...
function findRegionVariable(varNames, aliases, regionTextCodes, varTextCodes) {
    for (var j = 0; j < aliases.length; j++) {
        for (var i = 0; i < varNames.length; i++) {
            //check that it is the right type of code
            if (regionTextCodes !== varTextCodes[i]) {
                continue;
            }
...
function determineRegionType(dataset, regionWmsMap) {
    ...
    var varNameLC = varNames.map(function(name) {
        return name.toLowerCase();
    });
    var varTextCodes = varNames.map(function(name) {
        return defined(dataset.getVariableEnumList(name));
    });   
    ...
    for (var region in regionWmsMap) {
        if (regionWmsMap.hasOwnProperty(region)) {
            var idx = findRegionVariable(varNames, regionWmsMap[region].aliases, regionWmsMap[region].textCodes, varTextCodes);
            if (idx !== -1) {
                return { regionType: region, regionVariable: varNames[idx] }; 
            }
