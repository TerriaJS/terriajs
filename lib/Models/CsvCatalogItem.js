...
function finishTableLoad(csvItem) {
    ...
    if (varNames.length > 1 && !csvItem.disableUserChanges) {
        ...
        csvDataset.updateFunction = function (varName) { 
                //set new variable anc clear var specific styling
...
*  3. Contact WFS server, fetch IDs for all regions of that type
...
function findRegionVariable(varNames, aliases) {
    for (var j = 0; j < aliases.length; j++) {
        for (var i = 0; i < varNames.length; i++) {
            var varName = varNames[i].toLowerCase();
            if (varName === aliases[j]) {
                return i; // ###SB why did I change this?
            }
        }
    }
    return -1;
...
function determineRegionType(dataset, regionWmsMap) {
    ...
    for (region in regionWmsMap) {
        if (regionWmsMap.hasOwnProperty(region)) {
            var idx = findRegionVariable(varNames, regionWmsMap[region].aliases);
            if (idx !== -1) {
                regionType = region;
