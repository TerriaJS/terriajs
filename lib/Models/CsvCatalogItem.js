...
function loadTableFromCsv(item, csvString) {
    var tableStructure = new TableStructure({displayDuration: item._tableStyle.displayDuration});
    ...
    if (tableStructure.hasLatitudeAndLongitude) {
        ...
        item._dataSource = TableDataSource.create(tableStructure, item._tableStyle);
        return when(true); // We're done - nothing to wait for.
    }
    // Return a promise which resolves once we've set up region mapping, if any.
    var regionMapping = new RegionMapping(item, tableStructure);
