    ...
    if (defined(this.data)) {
        ...
        return loadText(proxyUrl( that.terria, that.url)).then(function(text) {
            ...
            throw new ModelError({
                ...
                message: 'An error occurred while retrieving CSV data from the provided link.\n\n' + e.message
            });
        });
    }
};
...
function initClockFromDataSource(dataSource) {
    var newClock = new DataSourceClock();
    newClock.startTime = dataSource.dataset.getTimeMinValue();
    newClock.stopTime = dataSource.dataset.getTimeMaxValue();
...
function finishTableLoad(csvItem) {
    ...
    } else if (!defined(csvItem.clock)) {
        if (defined(source) && defined(source.dataset) && source.dataset.hasTimeData()) {
            csvItem.clock = initClockFromDataSource(source);
        } else {
            csvItem.clock = undefined;
        }
    }
...
function chooseDataVariable(csvItem) {
    ...
    if (tds.dataVariable !== undefined) {
        return;
    }

    var vars = tds.dataset.getDataVariableList(true);
    if (vars.length === 0) {
        return;
    }
    var dv;
    // if the region variable is the first column, then the data variable has to be the second column. 
    if (defined(csvItem.tableStyle.regionVariable) && csvItem.tableStyle.regionVariable === vars[0]) {
        if (vars.length > 1) {
            dv = vars[1];
        } else {
            return;
    ...
    csvItem.tableStyle.dataVariable = dv;
}


/* Initialise region map properties. */
function styleRegionMap(csvItem, regionProviderList) {
    ...
    if (defined(tableStyle.regionType) && defined(tableStyle.regionVariable)) {
    ...
    } else if (!defined(tableStyle.regionProvider) || !defined(tableStyle.regionVariable)) {
        // we don't know that region to match, so take an educated guess.
        var result = regionProviderList.chooseRegionProvider(dataset.getVariableNames());
        ...
        tableStyle.disambigVariable = result.disambigVariable;
    }
