...
function setupTableStructure(item, tableStructure) {
    ...
    {
        var addressConverter = item.terria.batchGeocoder;
        return addressConverter.bulkConvertAddresses(tableStructure, item.terria.corsProxy).then(function() {
            var timeTaken = JulianDate.secondsDifference(JulianDate.now(), addressConverter.startTime);
            var message = "Bulk geocode of " + addressConverter.numberOfAddressesConverted + " addresses took "
                          + timeTaken.toFixed(2) + " seconds, which is "
                          + (addressConverter.numberOfAddressesConverted/timeTaken).toFixed(2)
                          + " addresses/s, or " + (timeTaken/addressConverter.numberOfAddressesConverted).toFixed(2)
                          + " s/address.\n";

            var missingAddressesMessage = "";
            if (addressConverter.missingAddresses.length > 0 || addressConverter.nullAddresses > 0)
            {
                if (addressConverter.missingAddresses.length > 0) {
                    missingAddressesMessage = "\nThe CSV contains addresses, but " + addressConverter.missingAddresses.length
                        + " can\'t be located on the map:\n"
                        + addressConverter.missingAddresses.join(", ") + "\n";
                }
                if (addressConverter.nullAddresses > 0) {
                    missingAddressesMessage += addressConverter.nullAddresses + " addresses are missing from the CSV.";
                }
            }
            item.terria.error.raiseEvent(new TerriaError({
                sender: this,
                title: 'Bulk Geocoder Information',
                message: message + missingAddressesMessage
            }));
