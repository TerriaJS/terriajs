...
*
* Voilà - very fast client-side chorolplething with no user data sent to servers, and no vector boundaries sent to client. */
...
function showFeedBack(multimatches, failedmatches, successes, totalrows, regionVariable) {
    var msg = "";
    if (Object.keys(failedmatches).length > 0) {
        msg += "These region names weren't recognised: <br><br/><b/>" + Object.keys(failedmatches).join(", ") + "</b><br/><br/>";
    }
    if (Object.keys(multimatches).length > 0) {
        msg += "These regions had more than one value: <br/><br/><b>" + Object.keys(multimatches).join(", ") + "</b><br/><br/>";
    }
    if (msg) {
        msg = "" + successes + " out of " + totalrows + " regions matched.<br/><br/>" + msg;
        ...
        title: "Issues loading CSV file: " + regionVariable,
        message: '<div>'+ msg +'</div>'
    });
...
function createRegionLookupFunc(csvItem) {
    ...
    for (var i = 0; i < (userecs ? csvItem.recs.length : codes.length); i++) {
        ...
        totalrows++;
        index = ids.indexOf(codes[row].toLowerCase());
        if (index < 0) {
    ...
    showFeedBack(multimatches, failedmatches, successes, totalrows, regionVariable);
