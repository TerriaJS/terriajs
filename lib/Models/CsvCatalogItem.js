...
CsvCatalogItem.prototype._createImageryProvider = function(time) {
    ...
    ImageryProviderHooks.addPickFeaturesHook(imageryProvider, function(results) {
        ...
        for (var i = 0; i < results.length; ++i) {
            var uniqueId = results[i].data.properties[that._tableStyle.regionProvider.uniqueIdProp];
...
function initRegionMapStyle(csvItem, regionProviderList) {
    ...
    if (defined(tableStyle.regionType) && defined(tableStyle.regionVariable)) {
        ...
        tableStyle.regionProvider = dataset.getRegionProvider();
...
function showFeedback (results, regionVariable, itemName, terria) {
    ...
    csvItem._colorFunc = regionProvider.getColorLookupFunc(results.regionValues, csvItem._tableDataSource._mapValue2Color.bind(csvItem._tableDataSource));
