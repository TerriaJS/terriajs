...
function loadTable(csvItem, text) {
    ...
    if (!csvItem._tableDataSource.dataset.hasLocationData()) {
        ...
        return loadJson('data/RegionMapping.json').then(function(obj) {
    ...
    a column to user for region mapping.'
                    });
                }
                else {
...
function setRegionVariable(csvItem, regionVariable, regionType) {
    if (!(csvItem._tableDataSource instanceof TableDataSource) || !defined(csvItem._regionWmsMap[regionType])) {
        return;
    }

    csvItem.regionVariable = regionVariable;
    csvItem.regionType = regionType;
    csvItem._regionDescriptor = csvItem._regionWmsMap[regionType];

    console.log('Region var:', csvItem.regionVariable, 'Region type:', csvItem.regionType);
...
function setRegionDataVariable(csvItem, newVar) {
    ...
    csvItem._tableDataSource.setDataVariable(newVar);
    createRegionLookupFunc(csvItem);
