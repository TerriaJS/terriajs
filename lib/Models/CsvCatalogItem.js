...
CsvCatalogItem.prototype.startPolling = function() {
    ...
    if (defined(polling.seconds) && polling.seconds > 0) {
        ...
        this._pollTimeout = setTimeout(function() {
            if (item.isEnabled) {
                if (defined(polling.url)) {
                    loadIntoTableStructure(polling.url).then(function(newTable) {
                        console.log('polled polling.url', polling.url, newTable);
                    ...
                    loadIntoTableStructure(item.url).then(function(newTable) {
                        console.log('polled item.url', item.url, newTable);
