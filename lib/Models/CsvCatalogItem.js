...
CsvCatalogItem.regenerateChartItem = function(itemJson, terria) {
  ...
  if (itemJson.sourceCatalogItemId) {
    const item = terria.catalog.shareKeyIndex[itemJson.sourceCatalogItemId];
    ...
    if (
      ...
      when(item.load()).then(() => {
        newItem.data = terria.catalog.shareKeyIndex[
          itemJson.sourceCatalogItemId
        ].loadIntoTableStructure(itemJson.url, {
          procedure: itemJson.regenerationOptions.procedure
        });
      });
    }
  }
