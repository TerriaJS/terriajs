...
function setupTableStructure(item, tableStructure) {
    ...
    if (!tableStructure.hasLatitudeAndLongitude && tableStructure.hasAddress && defined(item.terria.batchGeocoder))
    {
        ...
        return addressConverter.bulkConvertAddresses(tableStructure, item.terria.corsProxy).then(function(missingAddresses) {
            ...
            if (missingAddresses.length > 0 || addressConverter.nullAddresses > 0)
            {
                if (missingAddresses.length > 0) {
                    missingAddressesMessage = "\nThe CSV contains addresses, but " + missingAddresses.length
