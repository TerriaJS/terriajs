...
var CsvCatalogItem = function(terria, url) {
   ...
   /**
    ...
    knockout.track(this, ['url', 'data', 'dataSourceUrl', 'tableStyle', 'opacity', 'keepOnTop', '_regionMapped', '_csvDataset']);
    ...
    knockout.getObservable(this, 'tableStyle').subscribe(function(value) {
...
function finishTableLoad(csvItem) {
    ...
    var varNames = source.dataset.getVariableNamesByType([VarType.SCALAR,VarType.ENUM]);
    ...
    csvDataset.updateFunction = function (varName) { 
        if (!csvItem._regionMapped) {
            source.setDataVariable(varName);
        }
        else {
            setRegionDataVariable(csvItem, varName);
        }
        csvItem.tableStyle.dataVariable = source.dataset.getDataVariable();
