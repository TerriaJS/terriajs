...
var CsvCatalogItem = function(terria, url) {
   ...
   /**
    ...
    knockout.track(this, ['url', 'data', 'dataSourceUrl', 'tableStyle', 'opacity', '_regionMapped', '_csvDataset']);
...
CsvCatalogItem.prototype._createImageryProvider = function(time) {
    var imageryProvider = new WebMapServiceImageryProvider({
        url: proxyUrl( this.terria, this.regionServer),
...
function finishTableLoad(csvItem) {
    ...
    var varNames = csvItem._tableDataSource.dataset.getVariableNames();
    for (var i = 0; i < varNames.length; i++) {
        csvDataset.items.push(new CsvVariable(varNames[i], csvDataset));
    }
    csvDataset.setSelected(csvItem._tableDataSource.dataset.getDataVariable());
    csvDataset.updateFunction = function (varName) { 
        if (!csvItem._regionMapped) {
            csvItem._tableDataSource.setDataVariable(varName);
        }
        else {
            setRegionDataVariable(csvItem, varName);
        }
        csvItem.tableStyle.dataVariable = csvItem._tableDataSource.dataset.getDataVariable();
