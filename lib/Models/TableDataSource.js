...
function setOneFeaturePerRow(dataSource, tableStructure, specialColumns) {
    ...
    for (var i = 0; i < dataSource._rowObjects.length; i++) {
        ...
        addPointToEntity(feature, tableColumnStyle, scale, color, show);
 ...
 * @param {[type]} tableStructure [description]
 * @param {[type]} specialColumns [description]
 */
function setOneFeaturePerId(dataSource, tableStructure, specialColumns) {
    ...
    for (var key in rowNumbers) {
        if (rowNumbers.hasOwnProperty(key)) {
            ...
            // TODO: shift these inside the i-loop too.
            var value = defined(specialColumns.value) ? specialColumns.value.indicesOrValues[rowNumbers[key][0]] : undefined;
            var color = legendHelper.getColorFromValue(value);
            var scale = legendHelper.getScaleFromValue(value);
            var show = calculateShow(feature.availability);
            addPointToEntity(feature, tableColumnStyle, scale, color, show);
