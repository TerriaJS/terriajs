...
var TableDataSource = function(catalogItem) {
    ...
    knockout.getObservable(this._tableStructure, 'activeItems').subscribe(this.updateDisplay.bind(this), this);
};
...
TableDataSource.prototype.loadFromCsv = function(csvString, tableStyle) {
    ...
    if (that.hasLatitudeAndLongitude) {
        ...
        that.regionPromise.then(function(regionDetails) {
            if (regionDetails) {
                that.setRegionColumnType(regionDetails);
 ...
 */
TableDataSource.prototype.setRegionColumnType = function(regionDetails) {
 ...
 */
TableDataSource.prototype.updateDisplay = function() {
    var that = this;
    reviseLegendHelper(that);
    updateEntitiesAndExtent(that);  // Only does anything if there are lat & lon columns.
