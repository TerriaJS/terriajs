...
function getPositionOfRowNumber(specialColumns, rowNumber) {
        ...
        defined(specialColumns.height) ? specialColumns.height.values[rowNumber] : undefined
    );
}

function setOneFeaturePerRow(dataSource, tableStructure, specialColumns) {
    // These two are only used for POLLED csv items.
    // If tableStructure.idColumnNameOrIds (from csvItem.idColumns) is not specified, use the row number to link rows.
    // TODO: Currently using idColumnNameOrIds is not implemented.
    ...
    function getRowPropertiesPropertyCallbackForRow(rowNumber) {
        return function callback() {
            var properties = dataSource._rowObjects[rowNumber];
    ...
    dataSource._entityCollection.removeAll(); // Remove existing entities first.

    for (var i = 0; i < dataSource._rowObjects.length; i++) {
        ...
        var rowObject = dataSource._rowObjects[i];
        var feature = new Feature({
            name: rowObject.title || rowObject[fallbackNameField] || defaultFeatureName
        });
        ...
        if (!dataSource._isUpdating) {
            ...
            feature.properties._terria_columnAliases = tableStructure.getColumnAliases();
        } else {
        ...
        var value = defined(specialColumns.value) ? specialColumns.value.indicesOrValues[i] : undefined;
...
function setOneFeaturePerId(dataSource, tableStructure, specialColumns) {
    ...
    var tableColumnStyle = legendHelper.tableColumnStyle;
    var fallbackNameField = chooseFallbackNameField(tableStructure.getColumnNames());
    var rowNumbers = tableStructure.getIdMapping();
    var columnAliases = tableStructure.getColumnAliases();
    var isSampled = tableStructure.isSampled;
    var shouldInterpolateColorAndSize = isSampled && (defined(specialColumns.value) && !specialColumns.value.usesIndicesIntoUniqueValues);
    dataSource._entityCollection.removeAll(); // Remove existing entities first.

    for (var key in rowNumbers) {
        if (rowNumbers.hasOwnProperty(key)) {
            var firstRow = dataSource._rowObjects[rowNumbers[key][0]];
            var feature = new Feature({
            ...
            if (isSampled) {
                feature.position = new SampledPositionProperty();
            } else {
                feature.position = new TimeIntervalCollectionPositionProperty();
            }
            ...
            for (var i = 0; i < rowNumbers[key].length; i++) {
                ...
                feature.description.intervals.addInterval(descriptionInterval);

                // Add the feature position.
                if (isSampled) {
                    if (defined(point)) {
                        feature.position.addSample(specialColumns.time.julianDates[rowNumber], point);
                    }
                } else {
                    var positionInterval = interval.clone();
                    positionInterval.data = point;
                    feature.position.intervals.addInterval(positionInterval);
                }

                // Add the feature color, scale and pixelSize.
                var value = defined(specialColumns.value) ? specialColumns.value.indicesOrValues[rowNumber] : undefined;
                if (shouldInterpolateColorAndSize) {
...
function updateEntitiesAndExtent(dataSource) {
    ...
    if (defined(specialColumns.longitude) && defined(specialColumns.latitude)) {
        ...
        if (defined(specialColumns.time) && defined(tableStructure.idColumnNames) && !dataSource.isUpdating) {
            console.warn('Csvs with a time column, idColumns and polling are only partially supported.');
        }
        if (defined(specialColumns.time) && defined(tableStructure.idColumnNames) && !dataSource._isUpdating) {
