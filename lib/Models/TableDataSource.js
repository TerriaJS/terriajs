...
var TableDataSource = function(catalogItem) {
    ...
    knockout.getObservable(this._tableStructure, 'activeItems').subscribe(function(activeItems) {
        ...
        if (that._regionImageryLayer) {
            that.regionPromise.then(function() {
                redisplayRegions(that);
            });
        }
    }, this);
};
   ...
   errorEvent : {
    ...
    regionPromise: {
        get: function() {
            ...
            // No pre-existing region promise, so build one.
            // It is safe to call this multiple times.
            return buildRegionPromise(this);
        }
    },
...
TableDataSource.fromCsv = function(csvString, tableStyle) {
    ...
    dataSource.loadFromCsv(csvString, tableStyle);
};
 ...
 * If desired, chooses a sensible default active column.
 * @param {Boolean} [shouldActivateAColumn=false] Pass true to make sure at least a default column is activated.
 */
TableDataSource.prototype.setup = function(shouldActivateAColumn) {
    var dataSource = this;
    var tableStyle = dataSource._tableStyle;
    ...
    if (dataSource.tableStructure.columnsByType[VarType.TIME].length > 0) {
        var timeColumn = dataSource.tableStructure.columnsByType[VarType.TIME][0];
        dataSource._availabilities = timeColumn.values.map(function(value, index) {
            return calculateAvailability(index, timeColumn, tableStyle);
        });
        var availabilityCollection = new TimeIntervalCollection();
        dataSource._availabilities.forEach(function(availability) {
            availabilityCollection.addInterval(availability);
        });
        updateClock(dataSource, availabilityCollection);
    }

    var columnToActivate;
    if (defined(tableStyle) && defined(tableStyle.dataVariable)) {
        columnToActivate = dataSource._tableStructure.getColumnWithName(tableStyle.dataVariable);
        if (columnToActivate) {
    ...
    if (dataSource.hasLatitudeAndLongitude) {
        ...
        if (!defined(columnToActivate)) {
            chooseAndActivateAColumn(dataSource._tableStructure);
        }
    } else {
...
function buildRegionPromise(dataSource) {
    var regionProviderListPromise = RegionProviderList.fromUrl(dataSource._regionMappingDefinitionsUrl);
...
function redisplayRegions(dataSource, regionIndices) {
    dataSource.regionPromise.then(function(regionDetails) {
        if (regionDetails) {
            var layerIndex = dataSource._regionImageryLayer._layerIndex;  // Would prefer not to access an internal variable of imageryLayer.
