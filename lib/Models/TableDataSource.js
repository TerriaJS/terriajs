...
var TableDataSource = function(tableStructure, tableStyle, name, isUpdating) {
    ...
    this._rowObjects = undefined;  // The most recent properties and descriptions are saved here if the dataSource isUpdating.
...
function updateEntitiesAndExtent(dataSource) {
    ...
    // TODO: for now we are using the row number to link rows - better to specify polling.idColumns.
    ...
    if (defined(longitudeColumn) && defined(latitudeColumn)) {
        ...
        for (var i = 0; i < rowObjects.length; i++) {
            ...
            if (!dataSource._isUpdating) {
                feature.description = rowDescriptions[i];
                setEntityProperties(feature, rowObject, tableStructure.getColumnAliases());
            } else {
                feature.description = new CallbackProperty(getRowDescriptionPropertyCallbackForRow(i), false);
                if (feature.propertyNames.indexOf('properties') === -1) {
                    feature.addProperty('properties');  // Not defined yet, but could be in future.
                }
            ...
            feature.availability = timeColumn && timeColumn.availabilities && timeColumn.availabilities[i];
