...
function setOneFeaturePerRow(dataSource, tableStructure, specialColumns) {
    ...
    for (var i = 0; i < dataSource._rowObjects.length; i++) {
        ...
        feature.position = Cartesian3.fromDegrees(
            specialColumns.longitude.values[i],
            specialColumns.latitude.values[i],
            defined(specialColumns.height) ? specialColumns.height.values[i] : undefined
        );
        ...
        feature.availability = specialColumns.time && specialColumns.time.availabilities && specialColumns.time.availabilities[i];
