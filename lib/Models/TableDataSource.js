...
function setOneFeaturePerId(dataSource, tableStructure, specialColumns) {
    ...
    for (var key in rowNumbersMap) {
        if (rowNumbersMap.hasOwnProperty(key)) {
            var firstRow = dataSource._rowObjects[rowNumbersMap[key][0]];
            var feature = new Feature({
                id: dataSource._guid + '-' + key,
            ...
            for (var i = 0; i < rowNumbersMap[key].length; i++) {
                var rowNumber = rowNumbersMap[key][i];
