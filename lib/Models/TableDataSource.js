...
function setOneFeaturePerRow(dataSource, tableStructure, specialColumns) {
    ...
    for (i = 0; i < dataSource._rowObjects.length; i++) {
        ...
        //feature._properties["Aristotle Metadata: Name"] = tableStructure.columns[i].meta_name;
        //feature._properties["Aristotle Metadata: Definition"] = tableStructure.columns[i].meta_definition;
        //feature._properties["Aristotle Metadata: DataType"] = tableStructure.columns[i].meta_datatype;
...
function setOneFeaturePerId(dataSource, tableStructure, specialColumns) {
    ...
    function getChartDataFunction(rowNumbersForThisFeatureId) {
        return function() {
            return tableStructure.toCsvString('source', rowNumbersForThisFeatureId, false);
        };
    }
        ...
        if (rowNumbersMap.hasOwnProperty(key)) {
            ...
            for (var i = 0; i < rowNumbersMap[key].length; i++) {
                ...
                if (defined(rowNumbersForThisFeatureId)) {
                    propertiesInterval.data._terria_getChartData = getChartDataFunction(rowNumbersForThisFeatureId);
                }
