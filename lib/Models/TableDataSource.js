...
function setOneFeaturePerId(dataSource, tableStructure, specialColumns) {
    ...
    function getChartDataFunction(tableStructure, rowNumbersForThisFeatureId) {
        return function() {
    ...
    for (var key in rowNumbersMap) {
        if (rowNumbersMap.hasOwnProperty(key)) {
            ...
            getChartDataFunction(tableStructure, rowNumbersForThisFeatureId)(); // TODO: temp, just to check if any errors
            for (var i = 0; i < rowNumbersForThisFeatureId.length; i++) {
                ...
                if (defined(rowNumbersForThisFeatureId)) {
                    propertiesInterval.data._terria_getChartData = getChartDataFunction(tableStructure, rowNumbersForThisFeatureId);
                }
