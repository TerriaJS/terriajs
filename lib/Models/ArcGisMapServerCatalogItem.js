...
ArcGisMapServerCatalogItem.prototype._load = function() {
    if (!defined(this._mapServerData) || !defined(this._layersData)) {
        ...
        var serviceUrl = proxyCatalogItemUrl(this, baseUri.clone().addQuery('f', 'json').toString());
        var layersUrl = proxyCatalogItemUrl(this, baseUri.clone().segment(layers).addQuery('f', 'json').toString());
        var legendUrl = proxyCatalogItemUrl(this, baseUri.clone().segment('legend').addQuery('f', 'json').toString());

        var that = this;

        var serviceMetadata = this.mapServerData || loadJson(serviceUrl);
        var layersMetadata = this.mapServerLayersData || loadJson(layersUrl);
        var legendData = loadJson(legendUrl).then(this.loadLegendFromJson.bind(this)).then(function(legendImageUrl) {
