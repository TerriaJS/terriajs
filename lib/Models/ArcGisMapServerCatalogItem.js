...
ArcGisMapServerCatalogItem.prototype._load = function() {
    ...
    function getJson(uri) {
        return loadJson(proxyCatalogItemUrl(that, uri.addQuery('f', 'json').toString()));
    }
    if (!defined(this._mapServerData) || !defined(this._layersData)) {
        ...
        if (uri.segment(-1).match(/\d+/)) {
            ...
            baseUri = uri.clone().segment(-1, '');
            layers = uri.segment(-1);

            this.layers = layers; // ## is this ok to do?
        }
        var serviceUri = baseUri.clone();
        var layersUri = baseUri.clone().segment(layers); // either 'layers' or a number

        var serviceMetadata = this.mapServerData || getJson(serviceUri);
        var layersMetadata = this.mapServerLayersData || getJson(layersUri);
        
        var legendData;
        // if catalog contains a hand-crafted legend image, we respect it.
        if (!defined(that.legendUrl)) {
            var legendUri = baseUri.clone().segment('legend');
            legendData = getJson(legendUri).
                then(this.loadLegendFromJson.bind(this)).
                then(function(legendImageUrl) {
                    that._legendUrl = legendImageUrl;
                    return that._legendurl;
                });
        }

        return when.all([serviceMetadata, layersMetadata, legendData]).then(function(results) {
