...
ArcGisMapServerCatalogItem.prototype._load = function() {
    if (!defined(this._mapServerData) || !defined(this._layersData)) {
        ...
        var reparts = /^(.*\/)([0-9]+)\/?/.exec(this.url);
        var serviceUrl, layersUrl;
        if (reparts) {
            // URL is a single REST layer, like .../arcgis/rest/services/Society/Society_SCRC/MapServer/16
            serviceUrl = cleanAndProxyUrl(terria, reparts[1], this.forceProxy) + '?f=json';
            layersUrl = cleanAndProxyUrl(terria, this.url, this.forceProxy) + '?f=json';
        } else {
            serviceUrl = cleanAndProxyUrl(terria, this.url, this.forceProxy) + '?f=json';
            layersUrl = cleanAndProxyUrl(terria, this.url, this.forceProxy) + '/layers?f=json';
        }
