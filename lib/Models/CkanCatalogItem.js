...
CkanCatalogItem.prototype._load = function() {
  ...
  if (!defined(this.resourceId) && !defined(this.datasetId)) {
    throw new TerriaError({
      ...
      message:
        "CkanCatalogItem requires that either resourceId or datasetId be specified."
    });
  }
    ...
    datasetIdPromise = loadJson(resourceUrl).then(function(resourceJson) {
      ...
      if (
        ...
        throw new TerriaError({
          ...
          message:
            "The resource returned from the CKAN server does not appear to have a package_id."
        });
      }
    ...
    return loadJson(datasetUrl).then(function(json) {
      ...
      throw new TerriaError({
        ...
        message: defined(that.resourceId)
          ? "The CKAN dataset does not have a resource with the ID " +
            that.resourceId +
