...
function CkanCatalogItem(terria) {
     ...
     * is {@see CkanCatalogItem#datasetID} must be specified.
     * @type {String}
     */
...
CkanCatalogItem.prototype._load = function() {
    ...
    return datasetIDPromise.then(function(datasetID) {
        ...
        return loadJson(datasetUrl).then(function(json) {
            ...
            var itemProperties = combine(that.itemProperties, {
                rectangle: that.rectangle,
                legendUrl: that._legendUrl,
                legendUrls: that._legendUrls,
                attribution: that.attribution,
                clock: that.clock,
                featureInfoTemplate: that.featureInfoTemplate,
                maximumShownFeatureInfos: that.maximumShownFeatureInfos !== that.terria.configParameters.defaultMaximumShownFeatureInfos ? that.maximumShownFeatureInfos : undefined,
                dataUrl: that._dataUrl,
                dataUrlType: that._dataUrlType
            });

            for (var i = 0; i < resources.length; ++i) {
                // If we're looking for a particular resource ID, skip all others.
                if (defined(that.resourceID) && resources[i].id !== that.resourceID) {
                    continue;
                }
            ...
            throw new TerriaError({
                ...
                message: defined(this.resourceID)
                            ? 'The CKAN dataset does not have a resource with the ID ' + this.resourceID + ' or it does not have a supported format.'
