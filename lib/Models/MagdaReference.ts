...
export default class MagdaReference extends UrlMixin(
    ...
    private static createMemberFromRecord(
        ...
        if (
            isJsonObject(aspects.group) &&
            Array.isArray(aspects.group.members)
        ) {
        ...
        if (isJsonObject(aspects["dcat-dataset-strings"])) {
            ...
            if (
                !distributions.every(distribution => isJsonObject(distribution))
            ) {
        ...
        if (
            ...
            const ids = members.map(member => {
                ...
                if (!model) {
                    ...
                    if (
                        terria.getModelById(BaseModel, member.id) === undefined
                    ) {
                        terria.addModel(ref);
                    }
                    return ref.uniqueId;
        ...
        for (let i = 0; i < distributionFormats.length; ++i) {
            ...
            for (let j = 0; j < distributions.length; ++j) {
                ...
                if (isJsonObject(dcatJson)) {
                    ...
                    if (
                        url === undefined &&
                        typeof dcatJson.accessURL === "string"
                    ) {
