...
TableCatalogItem.prototype.initializeFromTableStructure = function(tableStructure) {
    ...
    // Apart from using tableStyle to set the active time column, we could also use it to set:
    // units, type, and name. Note active (and name?) are handled separately (I think).
    if (defined(tableStyle.columns)) {
        for (const nameOrIndex in tableStyle.columns) {
            if (tableStyle.columns.hasOwnProperty(nameOrIndex)) {
                ...
                if (defined(column)) {
                    if (defined(columnStyle.units) && !defined(column.units)) {
                        column.units = columnStyle.units;
                    }
                    if (defined(columnStyle.type) && !defined(column.type)) {
