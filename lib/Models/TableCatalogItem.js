...
TableCatalogItem.defaultUpdaters.tableStyle = function(
  ...
  // sync up columns
  if (defined(item._tableStyle) && defined(item.tableStructure)) {
    item.applyTableStyleColumnsToStructure(
      item._tableStyle,
      item.tableStructure
    );
  }
  // if there's no dataVariable or columns, make sure things aren't selected
  if (
    json[propertyName] &&
    !defined(json[propertyName].columns) &&
    !defined(json[propertyName].dataVariable) &&
    defined(item.tableStructure)
    // !item.tableStructure.requireSomeActive
    // not sure if we check this, should updateFromJson override these checks?
  ) {
    item.tableStructure.items.map(tableStructureItem => {
...
TableCatalogItem.prototype.syncActiveColumns = function(
  ...
  if (!defined(tableStyle)) {
    console.error("syncActiveColumns was called without a tableStyle?");
    return;
  } else if (!defined(tableStructure)) {
    console.error("syncActiveColumns was called without a tableStructure?");
    return;
  }
  if (!(tableStructure instanceof TableStructure)) {
    console.error("table structure passed in isn't a TableStructure");
  }
  if (
        ...
        if (
          ...
          if (addressGeocoderData.missingAddresses.length > 0) {
            missingAddressesMessage =
              "\nThe CSV contains addresses, but " +
              addressGeocoderData.missingAddresses.length +
          ...
          if (addressGeocoderData.nullAddresses > 0) {
            missingAddressesMessage +=
              addressGeocoderData.nullAddresses +
              " addresses are missing from the CSV.";
          }
      ...
      .otherwise(function(e) {
        item.terria.error.raiseEvent(
          new TerriaError({
            ...
            message:
              "Unable to map addresses to lat-long coordinates, as an error occurred while retrieving address coordinates. Please check your internet connection or try again later."
          })
        );
...
TableCatalogItem.prototype.setChartable = function() {
  ...
  if (
    !defined(this.tableStyle.dataVariable) &&
    !defined(this.tableStyle.columns)
  ) {
