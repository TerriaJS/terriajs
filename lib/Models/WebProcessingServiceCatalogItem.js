...
WebProcessingServiceCatalogItem.prototype._load = function() {
    ...
    return wpsResponsePromise.then(function(xmlOrJson) {
        ...
        for (var i = 0; i < outputs.length; ++i) {
            ...
            report += '*' + outputTitle + '*: ' + formatOutputValue(outputTitle, outputs[i].Data.LiteralData) + '\n\n';
        }
...
function formatOutputValue(title, value) {
    if (defined(value) && value.match(/[.\/](png|jpg|jpeg|gif|svg)/i)) {
        return '<a href="' + value + '"><img src="' + value + '" alt="' + title + '" /></a>';
    } else if (value.indexOf('http:') === 0 || value.indexOf('https:') === 0) {
        var uri = new URI(value);
        return '<a href="' + value + '">' + uri.filename() + '</a>';
    } else {
        return value;
    }
}
