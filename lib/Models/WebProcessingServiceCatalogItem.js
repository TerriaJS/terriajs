        ...
        return wpsResponsePromise.then(function(xmlOrJson) {
            ...
            for (var i = 0; i < outputs.length; ++i) {
                ...
                if (defined(outputs[i].Data.LiteralData)) {
                    ...
                    {
                        var colors = '';
                        if (defined(outputs[i].colors)) {
                            colors = 'colors="' + outputs[i].colors + '"';
                        }
                        content = '<collapsible title="' + outputs[i].Title + '" open="' + (i === 0 ? 'true' : 'false') + '">';
                        content += '<chart ' + colors + ' can-download="true" hide-buttons="false" title="' + outputs[i].Title + '" data=\'' + outputs[i].Data.ComplexData + '\' styling="histogram"></chart>';
                        content += '</collapsible>';
                    }
            ...
            var outputsSection =
                ...
                (outputs || []).reduce(function(previousValue, output) {
                    ...
                    if (defined(output.Data.LiteralData)) {
                        ...
                        {
                            var colors = '';
                            if (defined(output.colors)) {
                                colors = 'colors="' + output.colors + '"';
                            }
                            content = '<chart ' + colors + ' can-download="true" hide-buttons="false" title="' + output.Title + '" data=\'' + output.Data.ComplexData + '\' styling: "feature-info">';
                        }
