...
var AugmentedVirtuality = function(terria) {
    ...
    this._hover_level = AugmentedVirtuality.PRESET_HEIGHTS.length-1;
    ...
    knockout.track(this, ['_event_loop_state', '_manual_alignment', '_maximum_updates_per_second',
                          '_realign_alpha', '_realign_heading', '_hover_level']);
...
AugmentedVirtuality.prototype.toggleHoverHeight = function() {
    this._hover_level = (this._hover_level + 1) % AugmentedVirtuality.PRESET_HEIGHTS.length;

    this.hover(AugmentedVirtuality.PRESET_HEIGHTS[this._hover_level]);
};
...
AugmentedVirtuality.prototype.hover = function(height) {
    ...
    const original_position = camera.positionCartographic.clone();

    // Reset the viewer height.
    var flyToHeight = function(surface_height) {
    ...
    if ((!defined(this._terria.cesium) ||
        ...
        let terrainProvider = this._terria.cesium.scene.terrainProvider;
        const promise = sampleTerrainMostDetailed(terrainProvider, [original_position]);
