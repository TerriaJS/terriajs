...
import freezeObject from 'terriajs-cesium/Source/Core/freezeObject';
 ...
 */
AugmentedVirtuality.MINIMUM_HOVER_HEIGHT = freezeObject(20.0);
 ...
 *                       AugmentedVirtuality#MINIMUM_HOVER_HEIGHT the height will be set to
 *                       AugmentedVirtuality#MINIMUM_HOVER_HEIGHT to avoid visual artifacts when the viewer becomes to
 *                       close to the surface.
 * @param {Cartographic} position Optional, the location to hover over. If not specified the current camera location will be used.
 */
AugmentedVirtuality.prototype.hover = function(height, position) {
    const that = this;
    ...
    let flyToHeight = function(surfaceHeight) {
    ...
    if ((!defined(this._terria.cesium) ||
        ...
        const promise = sampleTerrainMostDetailed(terrainProvider, [position]);
 ...
 * @param {Float} maximumHeight Optional: The maximum height (in meters) to cap the current camera height to (if this
 *                              value is specified and the viewer is above this height the camera will be restricted
 *                              to this height).
 */
AugmentedVirtuality.prototype.moveTo = function(position, maximumHeight) {
    const that = this;
    ...
    let cameraPosition = camera.positionCartographic.clone();
    let viewerHeight = cameraPosition.height;

    // Reset the viewer height.
    let moveToLocation = function(surfaceHeight) {
        if (!defined(surfaceHeight)) {
            surfaceHeight = 0;
        }
    ...
    if ((!defined(this._terria.cesium) ||
        ...
        const terrainProvider = this._terria.cesium.scene.terrainProvider;
        const promise = sampleTerrainMostDetailed(terrainProvider, [cameraPosition]);
        when(promise, function(updatedPosition) {
