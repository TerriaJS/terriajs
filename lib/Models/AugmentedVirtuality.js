...
AugmentedVirtuality.prototype.hover = function(height) {
    ...
    const originalPosition = camera.positionCartographic.clone();

    // Reset the viewer height.
    var flyToHeight = function(surfaceHeight) {
        if (defined(surfaceHeight)) {
            height += surfaceHeight;
        }

        const newPosition = CesiumCartesian3.fromRadians(originalPosition.longitude, originalPosition.latitude, height);
    ...
    if ((!defined(this._terria.cesium) ||
        ...
        const promise = sampleTerrainMostDetailed(terrainProvider, [originalPosition]);
