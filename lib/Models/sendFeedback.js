...
function sendFeedback(options) {
  ...
  const shareLinkPromise = options.sendShareURL
    ? BuildShareLink.canShorten(terria)
      ...
      : Promise.resolve(BuildShareLink.buildShareLink(terria))
...
function raiseError(terria) {
  terria.error.raiseEvent(
    new TerriaError({
      ...
      message:
        "This is really embarrassing, but an error occurred while attempting to send your feedback.  Please email it to " +
        terria.supportEmail +
        " instead."
    })
  );
