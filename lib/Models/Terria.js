...
Terria.prototype.updateFromStartData = function(startData, includePersistantInitSources = true) {
    var initSources = includePersistantInitSources ? this.initSources.slice() : this.initSources.slice(1);
...
function interpretStartData(terria, startData, persistentInitSources, temporaryInitSources) {
    ...
    if (defined(startData.initSources)) {
        for (var i = 0; i < startData.initSources.length; ++i) {
            ...
            if (temporaryInitSources.indexOf(initSource) < 0 && !(temporaryInitSources[0]&&temporaryInitSources[0].initFragment && initSource.initFragment && temporaryInitSources[0].initFragment === initSource.initFragment)) {
