...
Terria.prototype.start = function(options) {
    ...
    return loadJson5(options.configUrl).then(function(config) {
        ...
        return that.serverConfig.init().then(function() {
            ...
            that.corsProxy.init(serverConfig, cp.corsProxyBaseUrl, config.proxyDomains);
            ...
            return that.updateFromApplicationUrl(applicationUrl, that.urlShortener);
        }).then(function () {
            ...
            console.error('Error from updateFromApplicationUrl: ',  e);
            that.error.raiseEvent({
 ...
 */
Terria.prototype.updateFromApplicationUrl = function(newUrl) {
