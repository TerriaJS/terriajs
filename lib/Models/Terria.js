...
Terria.prototype.addInitSource = function(initSource) {
    ...
    if (defined(initSource.pickedFeatures)) {
        promise.then(function() {
            var removeViewLoadedListener = this.afterViewerChanged.addEventListener(function() {
                removeViewLoadedListener();

                var result = this.currentViewer.pickLocation(initSource.pickedFeatures.pickCoords, initSource.pickedFeatures.providerCoords);
                result.allFeaturesAvailablePromise = result.allFeaturesAvailablePromise.then(function() {
                    var features = defaultValue(this.pickedFeatures.features, []);

                    var featureIndex = {byId: {}, byName: {}};
                    ...
                    if (initSourceEntities) {
                        this.nowViewing.items
                            ...
                            .forEach(function(entity) {
                                featureIndex.byId[entity.id] = entity;
                                featureIndex.byName[entity.name] = entity;
                            });

                        features = features.concat(initSourceEntities.map(function(initSourceEntity) {
                           return featureIndex.byId[initSourceEntity.id] || featureIndex.byName[initSourceEntity.name];
                        }).filter(function(entity) {
                            return defined(entity);
                        }));
                    }

                    features.forEach(function(entity) {
                        featureIndex.byId[entity.id] = entity;
                        featureIndex.byName[entity.name] = entity;
                    });

                    this.selectedFeature = featureIndex.byId[initSource.pickedFeatures.current.id] ||
                        featureIndex.byName[initSource.pickedFeatures.current.name];

                    this.pickedFeatures.features = features;
                }.bind(this));

                this.pickedFeatures = result;
            }.bind(this));
        }.bind(this));
    }
