...
Terria.prototype.start = function(options) {
    ...
    return loadJson5(options.configUrl).then(function(config) {
        ...
        // Init the proxy THEN update from application URL as updating the application URL might have to expand
        ...
        return that.corsProxy.init(cp.proxyableDomainsUrl, cp.corsProxyBaseUrl, config.proxyDomains)
            .then(function () {
                ...
                that.error.raiseEvent({
                    title: 'Failed to initialize proxy',
                    message: 'This will cause some layers to be unavailable, or all layers on some older browsers.'
                });
            });
    });
};
