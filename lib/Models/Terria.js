...
var Terria = function(options) {
  ...
  /**
   * Gets or sets the  Sector that is currently selected.
   */
  ...
  knockout.defineProperty(this, "pickedFeatures", {
    get: () => {
      return this._pickedFeatures;
    },
    set: value => {
      this._pickedFeatures = value;
      if (value.features) {
        this.onFeaturePicked(value.features);
      }
    }
  });
  knockout.defineProperty(this, "selectedSector", {
    get: () => {
  ...
  this.onSectorSelected = value => {
    ...
    const val = this.nowViewing.items[0]._readyData.features.filter(feature => {
      return Object.values(feature.properties).includes("agriculture");
    });
    console.log(this.nowViewing.items[0]._readyData.features);

    // this.nowViewing.items[0]._readyData.features.splice(0, this.nowViewing.items[0]._readyData.features.length, val)
    // console.log(this.nowViewing.items[0]._readyData.features)
  };
  ...
  this.onFeaturePicked = features => {
    if (
      ...
      features[0].properties["story-id"]
    ) {
