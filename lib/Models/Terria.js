...
var Terria = function(options) {
    ...
    /**
     * Gets or sets the path to POST url shorting calls for share links.
     * @type {String}
     */
    this.shortenShareUrl = options.shortenShareUrl;

    /**
     * Gets or sets the path to GET url expanding calls for share links.
     * @type {String}
     */
    this.expandShareUrl = options.expandShareUrl;
    ...
    for (var property in hashProperties) {
        if (hashProperties.hasOwnProperty(property)) {
            ...
            if (property === 'clean') {
                ...
                var promise;
                if (propertyValue.trim()[0] !== '{') {
                    var path = generateUrlFromHash(terria, propertyValue);
                    promise = loadJson(path);
                }
                return when(promise).then( function(json) {
                    if (defined(json) && defined(json.longUrl)) {
                        propertyValue = getPropertyValueFromUrl(json.longUrl)
                    }
...
function generateUrlFromHash(terria, hash) {
    return terria.expandShareUrl + hash;
}

function getPropertyValueFromUrl(longUrl) {
    var startIndex = longUrl.indexOf('start=');
    var encodedText = longUrl.substring(startIndex+6);
