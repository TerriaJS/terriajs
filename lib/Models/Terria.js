...
Terria.prototype.addInitSource = function(initSource, fromStory = false) {
  ...
  directInitSourceProperties.forEach(function(propertyName) {
    ...
    if(initSource[propertyName]){
      console.log('initProperty', initSource[propertyName]);
      console.log('basemap name', that[propertyName]);
      console.log('basemap', that.baseMap);
    }
    if (propertyName === "baseMapName" && defined(initSource[propertyName])) {
      ...
      if (defined(that.baseMap) && (that.baseMap.name === initSource[propertyName])){
        return;
      } else if (
