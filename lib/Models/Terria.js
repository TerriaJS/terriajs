  ...
  return loadJson5(options.configUrl).then(function(config) {
    ...
    return that.serverConfig
      ...
      .then(function() {
        ...
        // Check url params on application init
        // @terria pass "that" as terria
        // TODO
        window.addEventListener("popstate", event => {
          // The URL changed...
          console.log("🎹🎹🎹🎹🎹🎹🎹", event.currentTarget.location, that);
        });

        if (receipt.getUrlParams()) {
          setTimeout(
            () => receipt.launchStory(receipt.getUrlParams(), that),
            1000
          );
        }
      });
