...
Terria.prototype.updateApplicationUrl = function(newUrl) {
    ...
    return when(promise).then(function() {
        ...
        const stories = initSources.filter(init => defined(init.stories))[0];
        if(stories) that.stories = stories.stories;
        return loadInitSources(that, initSources);
    });
};

Terria.prototype.updateFromStartData = function(startData, fromStory=false) {
    var initSources = fromStory? [] : this.initSources.slice();
    interpretStartData(this, startData, this.initSources, initSources);
    return loadInitSources(this, initSources, fromStory);
};
...
Terria.prototype.addInitSource = function(initSource, fromStory = false) {
    ...
    directInitSourceProperties.forEach(function(propertyName) {
        if (defined(initSource[propertyName]) && initSource[propertyName] !== that[propertyName]) {
    ...
    if (defined(initSource.viewerMode) && !defined(this.userProperties.map)) {
        ...
        if (desiredMode === '2d') {
          if(this.viewerMode !== ViewerMode.Leaflet){
            this.viewerMode = ViewerMode.Leaflet;
            if(defined(viewerChangeListener)) viewerChangeListener();
          }
            
        } else if (desiredMode === '3d') {
          if(this.viewerMode !== ViewerMode.CesiumTerrain){
            this.viewerMode = ViewerMode.CesiumTerrain;
            if(defined(viewerChangeListener)) viewerChangeListener();
          }
        } else if (desiredMode === '3dsmooth') {
          if(this.viewerMode !== ViewerMode.CesiumEllipsoid){
            this.viewerMode = ViewerMode.CesiumEllipsoid; 
            if(defined(viewerChangeListener)) viewerChangeListener();
          }
        }
      if(fromStory === true){
        viewerChangeListener = this.afterViewerChanged.addEventListener(function(){
          const initView = that.initialView;
          that.initialView = initView;
        });
      }
    }
...
function interpretStartData(terria, startData, persistentInitSources, temporaryInitSources) {
    ...
    if (defined(startData.initSources)) {
        for (var i = 0; i < startData.initSources.length; ++i) {
            ...
            if (temporaryInitSources.indexOf(initSource) < 0 && !initFragmentExists(temporaryInitSources, initSource)) {
...
function initFragmentExists(temporaryInitSources, initSource){
  if(initSource.initFragment && typeof initSource.initFragment === 'string'){
    if(findIndex(temporaryInitSources, init=> init.initFragment === initSource.initFragment) >= 0){
      return true;
    }
  }
  return false;
...
function loadInitSources(terria, initSources, fromStory = false) {
        ...
        return promiseSoFar
            ...
            .then(function(initSource) {
                if (defined(initSource)) {
                    return terria.addInitSource(initSource, fromStory);
                }});
    }, when());
}
