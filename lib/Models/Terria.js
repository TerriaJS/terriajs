...
var {addMarker} = require ('./LocationMarkerUtils');
    ...
    return loadJson5(options.configUrl).then(function(config) {
        ...
        if (defined(initializationUrls)) {
            for (var i = 0; i < initializationUrls.length; i++) {
                that.initSources.push(generateInitializationUrl(baseUri, initializationUrls[i]));
            }
        }
        ...
        return that.serverConfig.init(cp.serverConfigUrl).then(function() {
            ...
            return that.updateApplicationUrl(applicationUrl, baseUri);
        }).then(function () {
 ...
 */
Terria.prototype.updateApplicationUrl = function(newUrl, baseUri) {
    var uri = new URI(newUrl);
    var hash = uri.fragment();
    var hashProperties = queryToObject(hash);
    baseUri = defaultValue(baseUri, uri.clone().search('').fragment(''));

    var initSources = this.initSources.slice();
    var promise = interpretHash(this, hashProperties, this.userProperties, this.initSources, initSources, baseUri);
...
function interpretHash(terria, hashProperties, userProperties, persistentInitSources, temporaryInitSources, baseUri) {
    var promise;
    ...
    return when(promise, function(shareProps) {
        Object.keys(hashProperties).forEach(function(property) {
            ...
            if (property === 'clean') {
                ...
                var initSourceFile = generateInitializationUrl(baseUri, property);
...
function generateInitializationUrl(baseUri, url) {
...
function loadInitSource(terria, source) {
    ...
    if (typeof source === 'string') {
        ...
        loadSource = fragmentPaths.map(path => buildInitUrlFromFragment(path, source)).join(', ');

        loadPromise = loadJson5(terria.corsProxy.getURLProxyIfNecessary(buildInitUrlFromFragment(fragmentPaths[0], source)));

        for (let i = 1; i < fragmentPaths.length; ++i) {
            loadPromise = loadPromise.otherwise(function() {
                return loadJson5(terria.corsProxy.getURLProxyIfNecessary(buildInitUrlFromFragment(fragmentPaths[i], source)));
            });
        }
    }
...
function buildInitUrlFromFragment(path, fragmentObject) {
