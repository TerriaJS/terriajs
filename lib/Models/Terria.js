...
var Terria = function(options) {
    ...
    /**
     * Gets or sets the URL for the TerriaJS-Server OGR2OGR conversion service.
     * @type {String}
     */
    this.conversionServiceBaseUrl = options.conversionServiceBaseUrl;

    /**
     * Gets or sets the URL for the TerriaJS-Server proj4 lookup service.
     * @type {String}
     */
    this.proj4Service4BaseUrl = options.proj4ServiceBaseUrl;

    this.corsProxy.baseProxyUrl = defaultValue(options.corsProxyBaseUrl, this.corsProxy.baseProxyUrl); 
...
Terria.prototype.start = function(options) {
    ...
    return loadJson(options.configUrl).then(function(config) {
        ...
        corsProxy.baseProxyUrl = defaultValue(config.corsProxyBaseUrl, corsProxy.baseProxyUrl);
        that.conversionServiceBaseUrl = config.conversionServiceBaseUrl || that.conversionServiceBaseUrl || 'convert';
        that.proj4ServiceBaseUrl = config.proj4ServiceBaseUrl || that.proj4ServiceBaseUrl ||  'proj4def/';
