...
var SceneMode = require("terriajs-cesium/Source/Scene/SceneMode").default;
...
var Terria = function(options) {
  ...
  knockout.getObservable(this, "viewerMode").subscribe(function() {
    if (this.cesium === undefined) {
      return;
    }
    if (this.viewerMode === ViewerMode.Cesium2D) {
      this.cesium.scene.mode = SceneMode.SCENE2D;
    } else if (this.viewerMode !== ViewerMode.Leaflet) {
      this.cesium.scene.mode = SceneMode.SCENE3D;
    }
  }, this);
...
Terria.prototype.start = function(options) {
  ...
  return loadJson5(options.configUrl).then(function(config) {
          ...
          if (isCommonMobilePlatform() && !defined(that.userProperties.map)) {
            ...
            if (
              ...
              if (mobileDefault === "3dterrain") {
                ...
                that.viewerMode = ViewerMode.Leaflet;
              } else if (mobileDefault === "2dcesium") {
                that.viewerMode = ViewerMode.Cesium2D;
              }
            }
          }
    ...
    // TODO RECEIPT activate if you want to see urls params errors
    // .otherwise(function(e) {
...
Terria.prototype.addInitSource = function(
  initSource,
  fromStory = false,
  resetZoom = true
) {
  ...
  if (defined(initSource.homeCamera) && resetZoom) {
    this.homeView = CameraView.fromJson(initSource.homeCamera);
  }

  if (defined(initSource.initialCamera) && resetZoom) {
...
Terria.prototype.resetSourcesAndCatalog = function() {
  // Depopulate the initSources.
  if (defined(this.initSources)) {
    this.initSources = [];
  }

  // Depopulate the list of services.
  if (defined(this.services)) {
    this.services = new Services(this);
  }

  // Depopulate the catalog
  if (defined(this.catalog)) {
    this.catalog = new Catalog(this);
  }

  if (defined(this.sharedCatalogMembers)) {
    this.sharedCatalogMembers = [];
  }

  if (defined(this.nowViewing)) {
    this.nowViewing.destroy();
    this.nowViewing = new NowViewing(this);
  }
};
...
Terria.prototype.interpretHash = interpretHash;
...
Terria.prototype.loadInitSources = loadInitSources;
