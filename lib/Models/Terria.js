...
Terria.prototype.addInitSource = function(initSource) {
    ...
    if (defined(initSource.pickedFeatures)) {
        promise.then(function() {
            var onViewerLoaded = function() {
                this.afterViewerChanged.removeEventListener(onViewerLoaded);
                this.currentViewer.pickLocation(initSource.pickedFeatures.pickCoords, initSource.pickedFeatures.providerCoords).then(function() {
                    ...
                    // Match by id if possible and fall back to name.
                    for (var i = 0; i < features.length; i++) {
                        if (features[i].id === initSource.pickedFeatures.id) {
                            return features[i];
                        }
                    }
                    for (i = 0; i < features.length; i++) {
                        if (features[i].name === initSource.pickedFeatures.name) {
                            return features[i];
                        }
                    }
                }.bind(this)).then(function(feature) {
                    this.selectedFeature = feature;
                }.bind(this));
            }.bind(this);

            this.afterViewerChanged.addEventListener(onViewerLoaded);
        }.bind(this));
    }
