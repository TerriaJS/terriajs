...
function interpretHash(hashProperties, userProperties, persistentInitSources, temporaryInitSources) {
    for (var property in hashProperties) {
        if (hashProperties.hasOwnProperty(property)) {
            ...
            if (property === 'start' && propertyValue.substring(0,3) !== '%7B') {
                var initSourceFile = getInitSourcePathFromHash(propertyValue);
...
function loadInitSource(terria, source) {
    if (typeof source === 'string') {
        ...
        return loadJson(source).then(function(initSource) {
            if (defined(initSource.longUrl)) {
                initSource = getInitSourceFromHash(initSource.longUrl)
            }
...
function getInitSourceFromHash(longUrl) {
...
function getInitSourcePathFromHash(hash) {
