...
Terria.prototype.updateFromStartData = function(startData, includePersistantInitSources = true) {
    var initFragmentIndex = this.initSources.findIndex(init => init.initFragment === 'terria');
    var initSources = includePersistantInitSources ? this.initSources.slice() : this.initSources.slice(initFragmentIndex + 1);
...
Terria.prototype.addInitSource = function(initSource) {
    ...
    if(defined(initSource.timelineSettings)){
      this.clock.shouldAnimate = initSource.timelineSettings.shouldAnimate;
      this.clock.multiplier = initSource.timelineSettings.multiplier;
    }
...
function interpretStartData(terria, startData, persistentInitSources, temporaryInitSources) {
    ...
    if (defined(startData.initSources)) {
        for (var i = 0; i < startData.initSources.length; ++i) {
            ...
            if (temporaryInitSources.indexOf(initSource) < 0 && !initFragmentExist(temporaryInitSources, initSource)) {
...
function initFragmentExist(temporaryInitSources, initSource){
  if(initSource.initFragment && typeof initSource.initFragment === 'string'){
    if(temporaryInitSources.findIndex(init=> init.initFragment === initSource.initFragment) >= 0){
      return true;
    }
  }
  return false;
        ...
        return promiseSoFar
            ...
            .then(function(initSource) {
                if (defined(initSource)) {
                  ...
                  // if previous init source fails to load, then we return
                  // empty promise
                  return when();
                }
            });
