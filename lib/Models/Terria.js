...
var Terria = function(options) {
    ...
    /**
     * An event that is raised when the splitter is toggled on or off or the current viewer is changed.
     * The alternative is for every catalog item that can implement splitting (eg. all the ImageryCatalogItems) to knockout track
     * `item.terria.showSplitter` and `item.terria.currentViewer`.
     * With hundreds of items in the catalog, this can hang the UI for some seconds, even if no action is taken.
     * This way, catalog items can opt in to listen when they are enabled.
     * Subscribers are passed the {@link Terria} instance.
     * @type {CesiumEvent}
     */
    this.splitterEvent = new CesiumEvent();

    // TODO: Why track showTimeline, which doesn't appear anywhere in the code (in this file or any other)?
    knockout.track(this, ['viewerMode', 'baseMap', 'baseMapName', 'fogSettings', '_initialView', 'homeView', 'pickedFeatures', 'selectedFeature', 'mapInteractionModeStack', 'configParameters', 'showTimeline', 'catalog', 'selectBox', 'currentViewer', 'showSplitter']);
    ...
    knockout.getObservable(this, 'showSplitter').subscribe(function() {
        this.splitterEvent.raiseEvent(this);
    }, this);

    knockout.getObservable(this, 'currentViewer').subscribe(function() {
        this.splitterEvent.raiseEvent(this);
    }, this);
};
