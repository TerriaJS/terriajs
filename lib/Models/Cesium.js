...
var Cesium = function(terria, viewer) {
  ...
  this._showSplitterSubscription = knockout
    ...
    .subscribe(function() {
      var terrainItem = this.terria.nowViewing.items.filter(item =>
        defined(item._terrainProvider)
      )[0];
      if (this.terria.showSplitter) {
        if (
          terrainItem &&
          terrainItem._terrainProvider === this.scene.globe.terrainProvider
        ) {
    ...
    function(scene, error) {
      this.terria.error.raiseEvent(
        new TerriaError({
          ...
          message:
            "\
<p>An error occurred while rendering in 3D.  This probably indicates a bug in " +
            terria.appName +
...
error by sending an email to <a href=\"mailto:" +
            terria.supportEmail +
            '">' +
            terria.supportEmail +
            "</a> with the \
technical details below.  Thank you!</p><pre>" +
            formatError(error) +
            "</pre>"
        })
      );
