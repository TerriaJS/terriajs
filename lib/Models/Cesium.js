...
technical details below.  Thank you!</p><pre>' + formatError(error) + '</pre>'
    ...
    this._workbenchMapItemsSubscription = autorun(() => {
        ...
        imageryLayers.forEach((mapItem, modelIndex) => {
            // ImageryLayerCollection has index order opposite of et
            const reversedIndex = imageryLayers.length - modelIndex - 1;
            const index = this.scene.imageryLayers.indexOf(mapItem);
            if (index === -1) {
                this.scene.imageryLayers.add(mapItem, reversedIndex);
            } else if (index > reversedIndex) {
                for (let j = index; j > reversedIndex; j--) {
