...
var Cesium = function(terria, viewer) {
    ...
    setImagerySplitPosition(this.scene, this.terria.splitPosition);
...
Cesium.prototype.updateItemForSplitter = function(item) {
    if (!defined(item.splitDirection) || !defined(item._imageryLayer)) {
        return;
    }
    ...
    if (terria.showSplitter) {
        item._imageryLayer.splitDirection = item.splitDirection;
    } else {
        item._imageryLayer.splitDirection = ImagerySplitDirection.NONE;
    }

    // Also update the next layer, if any.
    if (item._nextLayer) {
        item._nextLayer.splitDirection = item._imageryLayer.splitDirection;
    }
...
Cesium.prototype.pauseMapInteraction = function() {
    ...
    if (this._pauseMapInteractionCount === 1) {
        this.scene.readonlyscreenSpaceCameraController.enableInputs = false;
    }
};
...
Cesium.prototype.resumeMapInteraction = function() {
    ...
    if (this._pauseMapInteractionCount === 0) {
        setTimeout(() => {
            if (this._pauseMapInteractionCount === 0) {
                this.scene.readonlyscreenSpaceCameraController.enableInputs = true;
            }
        }, 0);
    }
};
...
function setImagerySplitPosition(scene, splitPosition) {
    // The terria splitPosition property is expressed in CSS pixels.
    // Cesium's imagerySplitPosition wants drawing buffer pixels.
    // These may not be the same.
    const percentage = splitPosition / scene.canvas.clientWidth;
    const drawingBufferPixels = percentage * scene.drawingBufferWidth;
    scene.imagerySplitPosition = drawingBufferPixels;
}
