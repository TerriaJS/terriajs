...
function selectFeature(cesium) {
    ...
    var tileset = cesium.viewer.scene.primitives.add(new Cesium3DTileset({
        url : 'build/TerriaJS/data/Cesium3DTiles/Tilesets/Tileset'
    }));
    console.log("Added tileset!");
    console.log(tileset);
    return when(tileset.readyPromise).then(function(tileset) {
        var boundingBox = tileset._root._boundingVolume;
        console.log("bounding box " + boundingBox);
        var centre = Rectangle.center(boundingBox.rectangle);
        console.log("centre " + centre);
        var height = boundingBox.maximumHeight + 500.0;
        console.log("height " + height);
        cesium.viewer.camera.setView({
            destination : Cartesian3.fromRadians(centre.longitude, centre.latitude, height)
        });
    });
}
