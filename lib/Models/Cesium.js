...
Cesium.prototype.pickLocation = function(latlng, imageryLayerCoords, existingFeatures) {
    var result = new PickedFeatures();
    result.features = defaultValue(existingFeatures, []);
    ...
    var promises = [];
    var imageryLayers = [];
    for (var i = 0; i < this.scene.imageryLayers.length; i++) {
    ...
    result.allFeaturesAvailablePromise = when.all(promises).then(function(imageryLayerFeatures) {
        result.isLoading = false;

        result.features = imageryLayerFeatures.reduce(function(resultFeatures, imageryLayerFeatures, i) {
            if (!defined(imageryLayerFeatures)) {
                return resultFeatures;
            }

            return resultFeatures.concat(imageryLayerFeatures.map(function(feature) {
...
function pickObject(cesium, position, after) {
    var result = new PickedFeatures();

    result.allFeaturesAvailablePromise = defaultValue(after, when()).then(function() {
        ...
        var picked = cesium.scene.drillPick(position);
        ...
        return {
            pickRay: pickRay,
            pickPositionCartographic: pickPositionCartographic
        };
    }).then(function(args) {
        var deferred = when.defer();

        result.providerCoords = {};
        for (var i = 0; i < cesium.scene.imageryLayers.length; i++) {
            var imageryProvider = cesium.scene.imageryLayers.get(i).imageryProvider;
            imageryProvider.pickFeatures = function(imageryProvider, oldPick, x, y, level, longitude, latitude) {
                var featuresPromise = oldPick.call(imageryProvider, x, y, level, longitude, latitude);

                // Use url to identify providers because wtf else can we do?
                if (imageryProvider.url) {
                    result.providerCoords[imageryProvider.url] = {
                        x: x,
                        y: y,
                        level: level
                    };
            ...
            }.bind(undefined, imageryProvider, imageryProvider.pickFeatures);
        }

        var pickRasterPromise = cesium.scene.imageryLayers.pickImageryLayerFeatures(args.pickRay, cesium.scene);

        // If the position somehow gets rotated off-screen before this point, pickRasterPromise will return undefined.
        if (defined(pickRasterPromise)) {
            pickRasterPromise.then(function(features) {
        ...
        for (var i = 0; i < features.length; ++i) {
            ...
            // This at least avoids major parallax effects on the selection indicator.
            if (!defined(feature.position.height) || feature.position.height === 0.0) {
                feature.position.height = args.pickPositionCartographic.height;
            }
