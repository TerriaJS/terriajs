...
Cesium.prototype.pickLocation = function(latlng, imageryLayerCoords, existingFeatures) {
    ...
    result.allFeaturesAvailablePromise = when.all(promises).then(function(allFeatures) {
...
function pickObject(cesium, position, after) {
    ...
    for (var j = 0; j < cesium.scene.imageryLayers.length; j++) {
        var imageryProvider = cesium.scene.imageryLayers.get(j).imageryProvider;
    ...
    var pickRasterPromise = cesium.scene.imageryLayers.pickImageryLayerFeatures(pickRay, cesium.scene);

    // If the position somehow gets rotated off-screen before this point, pickRasterPromise will return undefined.
    result.allFeaturesAvailablePromise = when(pickRasterPromise).then(function(features) {
        result.isLoading = false;
