...
Cesium.prototype.getRealWorldViewDimensions = function() {
    ...
    var left = this.scene.camera.getPickRay(new Cartesian2((width / 2) | 0, (height / 2) | 0));
    var right = this.scene.camera.getPickRay(new Cartesian2(1 + (width / 2) | 0, (height / 2) | 0));
    ...
    return {
        height: height * pixelDistance,
        width: width * pixelDistance
    };
};

Cesium.prototype.getBoundingBoxCoords = function() {
    var width = this.scene.canvas.clientWidth;
    var height = this.scene.canvas.clientHeight;

    var topLeftRay = this.scene.camera.getPickRay(new Cartesian2(0, 0));
    var bottomRightRay = this.scene.camera.getPickRay(new Cartesian2(width, height));

    var topLeftPosition = this.scene.globe.pick(topLeftRay, this.scene);
    var bottomRightPosition = this.scene.globe.pick(bottomRightRay, this.scene);

    if (!defined(topLeftPosition) || !defined(bottomRightPosition)) {
        return;
    }

    var topLeftRads = this.scene.globe.ellipsoid.cartesianToCartographic(topLeftPosition);
    var bottomRightRads = this.scene.globe.ellipsoid.cartesianToCartographic(bottomRightPosition);

    return {
        topLeft: {
            latitude: CesiumMath.toDegrees(topLeftRads.latitude),
            longitude: CesiumMath.toDegrees(topLeftRads.longitude)
        },
        bottomRight: {
            latitude: CesiumMath.toDegrees(bottomRightRads.latitude),
            longitude: CesiumMath.toDegrees(bottomRightRads.longitude)
        }
    };
};
