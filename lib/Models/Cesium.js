...
Cesium.prototype.pickFromScreenPosition = function(screenPosition) {
    ...
    if (this.terria.showSplitter) {
        ...
        var side = this.terria.getSplitterSideForScreenPosition(screenPosition);
        result.allFeaturesAvailablePromise = result.allFeaturesAvailablePromise.then(function() {
            result.features = result.features.filter(function(feature) {
                return ( feature.imageryLayer.splitDirection === ImagerySplitDirection.NONE ||
                         feature.imageryLayer.splitDirection === side );
            });
        });
    }
