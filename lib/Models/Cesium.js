...
Cesium.prototype.pickLocation = function(latlng) {
    var coords = SceneTransforms.wgs84ToWindowCoordinates(this.scene,
        this.scene.globe.ellipsoid.cartographicToCartesian(Cartographic.fromDegrees(latlng.lng, latlng.lat, latlng.height)));

    if (this.stoppedRendering) {
        return pickObject(this, coords);
    } else {
        var deferred = when.defer();
        var removeListener = this.scene.globe.tileLoadProgressEvent.addEventListener(function(count) {
            if (count === 0) {
                removeListener();
                deferred.resolve();
            }
        }.bind(this));
        pickObject(this, coords, deferred.promise);
    }
};
