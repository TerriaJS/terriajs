...
function selectFeature(cesium) {
    ...
    if (defined(cesium._removeHighlightCallback)) {
        cesium._removeHighlightCallback();
        cesium._removeHighlightCallback = undefined;
    }
    ...
    if (defined(feature)) {
        ...
        if (defined(feature.polygon)) {
            ...
            feature.polygon.outlineColor = Color.WHITE;
            feature.polygon.material = Color.WHITE.withAlpha(0.75);

            cesium._removeHighlightCallback = function() {
        ...
        if (defined(feature.polyline)) {
            ...
            feature.polyline.material = Color.WHITE;
        ...
        if (!hasGeometry) {
            ...
            if (defined(geoJson) && defined(geoJson.geometry) && geoJson.geometry.type !== 'Point') {
                var catalogItem = new GeoJsonCatalogItem(cesium.terria);
                ...
                var removeCallback = cesium._removeHighlightCallback = function() {
                    catalogItem._hide();
                    catalogItem._disable();
                };

                catalogItem.load().then(function() {
                    if (removeCallback !== cesium._removeHighlightCallback) {
                        return;
                    }
