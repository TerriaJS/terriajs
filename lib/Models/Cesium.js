...
Cesium.prototype.pickFromScreenPosition = function(screenPosition) {
    ...
    var imageryLayers
    if (this.terria.showSplitter) {
      var side = splitterSideForPosition(screenPosition, this.terria)
      imageryLayers = findLayersBySide(this.scene.imageryLayers, side)
      
    } else {
      imageryLayers = this.scene.imageryLayers
    }
  
    var pickRasterPromise = imageryLayers.pickImageryLayerFeatures(pickRay, this.scene)
...
function splitterSideForPosition(position, terria) {
  var splitterX = terria.currentViewer.getContainer().clientWidth * terria.splitPosition;
...
function findLayersBySide(layers, side) {
  var result = new ImageryLayerCollection()

  for (var i=0; i < layers.length; i++) {
    var layer = layers.get(i)
    if (layer) {
      if (layer.splitDirection === ImagerySplitDirection.NONE || layer.splitDirection === side) {
        result.add(layer)
      }
    }
  }
