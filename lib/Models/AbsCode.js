...
var AbsCode = function(code, name) {
    ...
    /**
     * Flag to say if this is a code.  This property is observable.
     * @type {Boolean}
     */
    this.isCode = true;  //for ko

    /**
     * Flag to say if parents of this code should be cleared if active.
     * This property is observable.
     * @type {Boolean}
     */
    this.clearParents = true;

    /**
     * Flag to say if children of this code should be cleared if active.
     * This property is observable.
     * @type {Boolean}
     */
    ...
    /**
     * Flag to say if siblings of this code should be cleared if active.
     * This property is observable.
     * @type {Boolean}
     */
    this.clearSiblings = false;
...
AbsCode.prototype.toggleActive = function(test) {
    ...
    function clearSiblings(code) {
        if (defined(code.parent)) {
            clearChildren(code.parent);
            this.isActive = true;
        }
    }
    if (this.isActive) {
        if (this.clearParents) {
            clearParents(this);
        }
        if (this.clearChildren) {
            clearChildren(this);
        }
        if (this.clearSiblings) {
            clearSiblings(this);
        }
    }
