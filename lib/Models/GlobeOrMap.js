 ...
 * @see Leaflet
 */
var GlobeOrMap = function(terria, disclaimerClass) {
    /**
     * Gets or sets the Terria instance.
     * @type {Terria}
     */
    this.terria = terria;
    this._disclaimerClass = disclaimerClass;

    this._tilesLoadingCountMax = 0;
    this._removeHighlightCallback = undefined;
    this._highlightPromise = undefined;
    this._disclaimerShiftSubscription = undefined;

    var shiftProperty = knockout.getObservable(this.terria.commonViewerProps, 'shiftDisclaimerPx');
    if (shiftProperty) {
        this._disclaimerShiftSubscription = shiftProperty.subscribe(this._shiftDisclaimers, this);
    }

    this._shiftDisclaimers(this.terria.commonViewerProps.shiftDisclaimerPx);
};
...
GlobeOrMap.prototype._highlightFeature = function(feature) {
    ...
    if (defined(feature)) {
        ...
        if (!hasGeometry) {
            if (feature.imageryLayer && feature.imageryLayer._imageryProvider instanceof MapboxVectorTileImageryProvider) {
                // Add a new CsvCatalogItem with one row
                // Need to change MapboxVTIP colorFunction so that it only styles 1 region
                // Creating a CsvCatalogItem probably isn't the best way. Maybe just create a MVTIP layer manually and give it the correct colorFunction
                /*var tempCsvItem = new CsvCatalogItem(this.terria);
                tempCsvItem.updateFromJson({data: 'sa4\n101'});*
                if (defined(this.terria.cesium)) {
                    result = new ImageryLayer(feature.imageryLayer, {
                        show : true,
                        alpha : 1,
                        rectangle : feature.imageryLayer.rectangle,
                    });
                    ...
                    this.terria.scene.imageryLayers.add(result);
                } else if (defined(this.terria.leaflet)) {

                }*/

                /*var tempItem = new CatalogItem(this.terria);
                tempItem.keepOnTop = true;*/
                //ImageryLayerCatalogItem.enableLayer(tempItem, feature.imageryLayer._imageryProvider.createHighlightImageryProvider(2), 1);
                if (defined(this.terria.cesium)) {
                    var result = new ImageryLayer(feature.imageryLayer._imageryProvider.createHighlightImageryProvider(feature.data.id), {
                        show : true,
                        alpha : 1,
                        /*rectangle : tempItem.clipToRectangle ? catalogItem.rectangle : undefined,
                        isRequired : tempItem.isRequiredForRendering*/
                    });
                    ...
                    var options = {
                        async: true,
                        opacity: 1,
                        bounds : rectangleToLatLngBounds(feature.imageryLayer._imageryProvider.rectangle),
                    ...
                    var layer = new MapboxVectorCanvasTileLayer(feature.imageryLayer._imageryProvider.createHighlightImageryProvider(feature.data.id), options);
                    layer.addTo(map);
                layer.bringToFront();
...
GlobeOrMap.prototype._shiftDisclaimers = function(px) {
    if (!isNaN(px)) {
        const disclaimerElements = document.querySelectorAll(`.${this._disclaimerClass}`);

        for (let i = 0; i < disclaimerElements.length; i++) {
            disclaimerElements[i].style.bottom = px + 'px';
        }
    }
};
