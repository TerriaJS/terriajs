...
defineProperties(RegionParameter.prototype, {
    ...
    regionProvider: {
        get: function() {
            if (this._regionProvider instanceof RegionProvider) {
                return this._regionProvider;
            } else if (this._regionProvider instanceof RegionTypeParameter) {
                var parameter = this.catalogFunction.parameters.filter(parameter => parameter.id === this._regionProvider.id)[0];
                if (!defined(parameter)) {
                    throw new TerriaError({
                        sender: this,
                        title: 'Invalid parameter',
                        message: 'Parameter ' + this.id + ' references a parameter ' + this._regionProvider.id + ' that does not exist.'
                    });
                }
