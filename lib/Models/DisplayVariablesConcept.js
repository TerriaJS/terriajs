...
DisplayVariablesConcept.prototype.toggleSelected = function(variable) {
    ...
    var makeActive = !variable.isActive;
    this.setSelectedFunction(this.items.indexOf(variable), varName, makeActive);
    // Keep other dependents such as the data source (eg. {@link CsvCatalogItem}) up-to-date.
    if (defined(this.updateFunction)) {
        this.updateFunction(this.items.indexOf(variable), varName, makeActive);
    }
};
 ...
 * @param {String} [color] The color of the variable, if applicable.
 */
DisplayVariablesConcept.prototype.addVariable = function(varName, active, color) {
    this.items.push(new VariableConcept(varName, this, active, color));
};
