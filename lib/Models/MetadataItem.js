...
defineProperties(MetadataItem.prototype, {
  ...
  /**
   * Gets a object representing the metadata item and it's children
   * that is navigable using the name as a key.
   * @type {Object}
   */
  asNestedObject: {
    get: function() {
      const out = {};
      if (this.hasChildren) {
        this.items.forEach(function(metaitem) {
          out[metaitem.name] = metaitem.asNestedObject;
        });
      } else {
