...
WebFeatureServiceCatalogItem.prototype.searchDataForMatchingFeatures = function(searchText) {
    const results = [];
    const re = new RegExp(`.*${searchText}.`, 'gmi');
    if (defined(this._geoJsonItem.data && this._geoJsonItem.data.features.length > 0)) {
        const propertyKeys = Object.keys(this._geoJsonItem.data.features[0].properties);
        this._geoJsonItem.data.features.forEach(f => {
            for (var ii = 0; ii < propertyKeys.length; ii++) {
                if (re.test(f.properties[propertyKeys[ii]])) {
                    results.push({
                        matchType: 'WFS',
                        catalogItem: this,
                        feature: f,
                        matchFieldText: f.properties[propertyKeys[ii]]
                    });
                    break;
                }
            }
