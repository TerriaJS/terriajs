...
WebFeatureServiceCatalogItem.prototype.createLikeFilterTemplate = function(properties, query) {
...
WebFeatureServiceCatalogItem.prototype.searchDataForMatchingFeatures = function(regex) {
    const that = this;
    const workbenchItemResults = [];
    if (defined(that._geoJsonItem.data) && that._geoJsonItem.data.features.length > 0) {
        that._geoJsonItem.data.features.forEach(f => {
            for (var ii = 0; ii < that.fields.length; ii++) {
                if (regex.test(f.properties[that.fields[ii]])) {
                    workbenchItemResults.push({
                        matchType: 'WFS',
                        workbenchItem: that,
                        feature: f,
                        matchFieldText: f.properties[that.fields[ii]]
                    });
                    break;
                }
            }
