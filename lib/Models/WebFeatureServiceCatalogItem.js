...
var WebFeatureServiceCatalogItem = function(terria) {
    ...
    /**
     * Specifies whether we've been able to describe the item to enabled searching.
     * @type {Boolean}
     */
    this.searchableViaQuery = null;
...
WebFeatureServiceCatalogItem.prototype.getDescribeFeatureType = function() {
    ...
    return loadDescribeFeature(this);
};
...
WebFeatureServiceCatalogItem.prototype.searchDataForMatchingFeatures = function(searchText, regex) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        if (defined(that.parent) && defined(that._geoJsonItem.data)) {
            resolve(convertGeoJSONSearchResultsToWfs(that._geoJsonItem.searchDataForMatchingFeatures(searchText, regex), that));

        } else if (!defined(that.searchableViaQuery)) {
            // We're attempting to search a WMS via WFS for the first time
            ...
            .catch(function (err) {
                that.searchableViaQuery = false;
                resolve([]);
            });
        } else if (that.searchableViaQuery) {
...
function convertGeoJSONSearchResultsToWfs (results, wfsItem) {
    return results.map(match => {
        return {
            matchType: wfsItem.type,
...
function loadDescribeFeature(wfsItem) {
    return new Promise(function(resolve, reject) {
        loadXML(wfsItem.describeFeatureUrl)
        .then(function(xml) {
