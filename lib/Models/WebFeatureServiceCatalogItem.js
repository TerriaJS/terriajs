...
var WebFeatureServiceCatalogItem = function(terria) {
    ...
    /**
     * Gets or sets a value indicating that the GML definition uses a non-standard axis order, and that
     * the lat-lon order should be reversed.
     * @type {Boolean}
     */
    this.flipGmlCoords = false;
...
function loadGml(wfsItem) {
    return loadXML(buildGmlUrl(wfsItem)).then(function(xml) {
        return gmlToGeoJson(xml, wfsItem.flipGmlCoords);
    });
}
