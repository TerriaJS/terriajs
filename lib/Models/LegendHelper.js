...
var defaultVariousValuesEnumLabel = 'Various Values';
...
var LegendHelper = function(tableColumn, tableStyle, regionProvider, name) {
    ...
    this.tableColumnStyle.colorBins = defaultValue(this.tableColumnStyle.colorBins, 7);  // Would be better named numberOfColorBins. Not to be confused with binColors.
...
function buildBinColors(legendHelper, colorBins) {
    ...
    if (method === 'cycle' && tableColumn.uniqueValues.length > colorBins) {
...
function buildLegendProps(legendHelper, colorMap) {
    ...
    if (!binColors) {
        ...
        if (legendHelper._cycleEnumValues) {
            items = [{
                title: defaultVariousValuesEnumLabel,
            ...
            items = reversedBinColors.map(function(bin, index) {
                ...
                if (tableColumn.uniqueValues.length > binColors.length && legendHelper._cycleEnumValues) {
                    label = defaultVariousValuesEnumLabel;
                } else if ((index === 0) && (tableColumn.uniqueValues.length > binColors.length)) {
