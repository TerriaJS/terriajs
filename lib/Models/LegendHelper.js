...
var defaultNoColumnColorCodes = [
    ...
    'rgba(204,235,197,.6)',
    'rgba(255,237,111,.6)'
]; // From ColorBrewer2.org, with 60% opacity added. Use 12 to minimize chance of clashes (the index is chosen using a hash of the name).
...
var LegendHelper = function(tableColumn, tableStyle, regionProvider, name) {
    ...
    this._noColumnColorCode = defaultNoColumnColorCodes[noColumnIndex];
    this._noColumnColorArray = getColorArrayFromCssColorString(this._noColumnColorCode);
    ...
    if (!defined(tableColumn) && defined(legendHelper.name)) {
        return {
            items: [{
                ...
                color: legendHelper._noColumnColorCode
            }]
        };
    }
