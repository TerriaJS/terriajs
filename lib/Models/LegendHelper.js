...
function buildLegendProps(legendHelper, colorMap) {
    ...
    // No legend if no active tableColumn
    if (!defined(tableColumn)) {
        return undefined;
    }
    ...
    var title = replaceUnderscores(tableColumn.name);
    ...
    if (!binColors) {
        ...
        var items = reversedBinColors.map(function(bin, index) {
            var label = replaceUnderscores(tableColumn.uniqueValues[reversedBinColors.length - 1 - index]);
            return {
