...
function populateGroupFromResults(ckanGroup, json) {
    ...
    for (var itemIndex = 0; itemIndex < items.length; ++itemIndex) {
        ...
        for (var resourceIndex = 0; resourceIndex < resources.length; ++resourceIndex) {
            ...
            if (resource.format.match(wmsFormatRegex)) {
                isWms = true;
            } else if (resource.format.match(esriRestFormatRegex) || resource.format.match(kmlFormatRegex) || resource.format.match(/aus-geo-csv/)) {
                isWms = false;
            } else {
                continue;
            }
            ...
            } else if (resource.format.match(/aus-geo-csv/)) {
