...
function createItemFromResource(resource, ckanGroup, item, extras) {
    ...
    if (resource.format.match(ckanGroup.wmsResourceFormat)) {
        isWms = true;
    } else if (['esriMapServerResourceFormat', 'kmlResourceFormat', 'geoJsonResourceFormat', 'csvResourceFormat', 'czmlResourceFormat'].some(
        function(rf) { return resource.format.match(ckanGroup[rf]); }
        )) {
        isWms = false;
    } else {
        return;
