...
sending an email to <a href="mailto:nationalmap@lists.nicta.com.au">nationalmap@lists.nicta.com.au</a>.</p>'
        ...
        if (rootLayers) {
            ...
            addLayersRecursively(that, json, rootLayers, that.items, parentLayer, getFeatureInfoFormats, dataCustodian);
        }
...
function addLayersRecursively(wmsGroup, capabilities, layers, items, parent, getFeatureInfoFormats, dataCustodian) {
    ...
    for (var i = 0; i < layers.length; ++i) {
        ...
        if (defined(layer.Layer)) {
            ...
            if (defined(layer.Name) && layer.Name.length > 0) {
                var all = createWmsDataSource(wmsGroup, capabilities, layer, getFeatureInfoFormats, dataCustodian);
            ...
            addLayersRecursively(wmsGroup, capabilities, layer.Layer, recurseItems, layer, getFeatureInfoFormats, dataCustodian);
            ...
            items.push(createWmsDataSource(wmsGroup, capabilities, layer, getFeatureInfoFormats, dataCustodian));
        }
    }
}

function createWmsDataSource(wmsGroup, capabilities, layer, getFeatureInfoFormats, dataCustodian) {
