...
freezeObject(GeoRssCatalogItem.defaultUpdaters);
...
function loadGeoRss(geoRssItem) {
  return loadXML(proxyCatalogItemUrl(geoRssItem, geoRssItem.url)).then(xml => {
  ...
  for (
    ...
    if (child.localName === "title") {
      ...
      if (author.length === 0) {
        ...
        var name;
        var email;
        for (var authorIndex = 0; authorIndex < author.length; ++authorIndex) {
          var authorChild = author[authorIndex];
