...
WebMapServiceCatalogItem.prototype.updateFromCapabilities = function(capabilities, overwrite, thisLayer) {
    ...
    if (thisLayer.Style) {
        ...
        if (style.LegendURL) {
            ...
            if (uri.toString().match(/GetLegendGraphic/i)) {
                if (this._isGeoServer) {
                    uri.setQuery('version', '1.1.0');
                    var legendOptions = 'fontSize:14;forceLabels:on;fontAntiAliasing:true';
                    uri.setQuery('transparent','True');       // remove if our background is not longer light
                    ...
                    uri.setQuery('LEGEND_OPTIONS',legendOptions);
                } else if (this._isEsri) {
                    uri.setQuery('width', '300'); // TODO use Legend default width
                }
            }
