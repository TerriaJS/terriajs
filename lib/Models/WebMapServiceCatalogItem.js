...
var WebMapServiceCatalogItem = function(terria) {
    ...
    /**
     * Gets or sets the name of a property in a feature returned from this layer's `GetFeatureInfo` service
    ...
    this._intervalFilterFeature = undefined;
    ...
    knockout.track(this, [
        ...
        '_allIntervals', '_intervalFilterFeature']);

    overrideProperty(this, '_filteredIntervals', {
        get: function() {
            if (!defined(this._intervalFilterFeature)) {
                return undefined;
            }

            const featureTimesProperty = this._intervalFilterFeature.properties[this.featureTimesProperty];
...
WebMapServiceCatalogItem.prototype.canFilterIntervalsByFeature = function(feature) {
    if (!defined(this.featureTimesProperty) || !defined(feature) || !defined(feature.properties)) {
        return false;
    }

    const featureTimes = feature.properties[this.featureTimesProperty];
    // TODO: verify we can interpret the times.
    return defined(featureTimes);
};

WebMapServiceCatalogItem.prototype.filterIntervalsByFeature = function(feature) {
    if (!this.canFilterIntervalsByFeature(feature)) {
        throw new DeveloperError('This feature can not be used to filter intervals.');
    }

    this._intervalFilterFeature = feature;
