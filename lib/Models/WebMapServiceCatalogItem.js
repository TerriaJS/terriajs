...
var WebMapServiceCatalogItem = function(terria) {
    ...
    /**
     * Gets or sets the URL of a WCS that enables clip-and-ship for this WMS item. This property is observable.
     * @type {String}
     */
...
WebMapServiceCatalogItem.prototype.exportData = function() {
    ...
    const viewExtent = this.terria.currentViewer.getCurrentExtent();
    viewExtent.west = CesiumMath.toDegrees(viewExtent.west);
    viewExtent.south = CesiumMath.toDegrees(viewExtent.south);
    viewExtent.east = CesiumMath.toDegrees(viewExtent.east);
    viewExtent.north = CesiumMath.toDegrees(viewExtent.north);
    ...
    const query = {
        ...
        width: '600',
        height: '400',
        coverage: this.linkedWcsCoverage,
        bbox: `${viewExtent.west},${viewExtent.south},${viewExtent.east},${viewExtent.north}`
    };

    if (defined(this.discreteTime)) {
        query.time = this.discreteTime.toISOString();
    }
    var uri = new URI(this.url).query(query);
