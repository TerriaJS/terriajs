...
var WebMapServiceCatalogItem = function(terria) {
    ...
    knockout.track(this.parameters, ['colorscalerange']);
...
WebMapServiceCatalogItem.prototype.updateFromCapabilities = function(capabilities, overwrite, thisLayer) {
    ...
    if (defined(legendUri)) {
        ...
        if (legendUri.toString().match(/GetLegendGraphic/i)) {
            ...
            objectToUri(this.parameters, legendUri);
        }
...
/**
 * Copies the keys of an object into the provided url with their values as &key=value. Excludes keys that have an
 * undefined value.
 */
function objectToUri(object, uri) {
    for (var key in object) {
        if (object.hasOwnProperty(key) && defined(object[key])) {
            uri.setQuery(key, object[key]);
        }
    }
}
