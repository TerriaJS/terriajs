...
var WebMapServiceCatalogItem = function(terria) {
    ...
    this._isGeoServer = undefined;
    this._isEsri = undefined;
...
WebMapServiceCatalogItem.prototype.updateFromCapabilities = function(capabilities, overwrite, thisLayer) {
    ...
    if (!defined(this._isGeoServer) && capabilities && capabilities.Service && capabilities.Service.KeywordList && capabilities.Service.KeywordList.Keyword && capabilities.Service.KeywordList.Keyword.indexOf('GEOSERVER') >= 0) {
        this._isGeoServer = true;
    }

    if (defined(capabilities["xmlns:esri_wms"])) {
        this._isEsri = true;
    }
    ...
    if (thisLayer.Style) {
        ...
        if (style.LegendURL) {
            ...
            var legendUrl = Array.isArray(style.LegendURL) ? style.LegendURL[0] : style.LegendURL, legend;
            if (this._isGeoServer) {
                var uri = new URI(decodeURIComponent(legendUrl.OnlineResource['xlink:href']));
                uri.setQuery('version', '1.1.0');
                uri.setQuery('transparent','True'); // TODO: figure out what to do about the image style being forced white.
                // TODO also work out what to do with the high DPI version. we somehow need to shrink the image back down.
                uri.setQuery('LEGEND_OPTIONS','fontSize:14;dpi:182;forceLabels:on;fontColor:0xDDDDDD');
                legend = new LegendUrl(uri.toString(), legendUrl.Format);
            } else {
                legend = new LegendUrl(legendUrl.OnlineResource['xlink:href'], legendUrl.Format);
            }
