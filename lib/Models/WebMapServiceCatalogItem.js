...
function computeLegendForLayer(catalogItem, thisLayer, styleName) {
    ...
    if (!defined(legendUri)) {
        const styleParam = defined(style) && defined(style.Name) ? '&styles=' + encodeURIComponent(style.Name) : '';
        // Construct a GetLegendGraphic request.
        legendUri = new URI(cleanUrl(catalogItem.url) +
            '?service=WMS&version=1.1.0&request=GetLegendGraphic&format=image/png&transparent=True&layer=' + encodeURIComponent(thisLayer.Name) + styleParam);
