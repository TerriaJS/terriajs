...
function getFeatureInfoFormats(capabilities) {
    ...
    if (supportsXmlGetFeatureInfo) {
        var xmlGetFeatureInfoFormat = new GetFeatureInfoFormat('xml', xmlContentType);
        xmlGetFeatureInfoFormat.callback = function(xml) {
            var documentElement = xml.documentElement;
            if (documentElement.localName === 'FeatureInfoResponse' && documentElement.getElementsByTagNameNS('*', 'FIELDS').length === 0) {
                // This looks like an Esri WMS response, but it has no FIELDS - handle it with a custom handler
                return fieldlessEsriXmlToFeatureInfo(xml);
            } else if (documentElement.localName === 'msGMLOutput') {
                return msGmlToFeatureInfo(xml);
            }
            // fall back to Cesium's default callback
            this.callback(xml);
        };
        result.push(xmlGetFeatureInfoFormat);
    }
...
function fieldlessEsriXmlToFeatureInfo(xml) {
    // for now, the same as Cesium's unknownXmlToFeatureInfo
    var xmlText = new XMLSerializer().serializeToString(xml);

    var element = document.createElement('div');
    var pre = document.createElement('pre');
    pre.className = 'foobar'; // to test it came through here
...
// similar to gmlToFeatureInfo in Cesium's GetFeatureInfoFormat.js, but assumes different XML structure, eg.
// <msGMLOutput>
//     <ABC_layer>
//         <ABC_feature>
//             <gml:boundedBy>
//                 <gml:Box srsName="EPSG:3857">
//                     <gml:coordinates>13184221,-3899031 14360421,-2684332</gml:coordinates>
//                 </gml:Box>
//             </gml:boundedBy>
//             <objectid>8</objectid>
//             <abs_name>My Name</abs_name>
//             <shape_area>26</shape_area>
//             <shape_len>30</shape_len>
//         </ABC_feature>
//     </ABC_layer>
// </msGMLOutput>
...
// getGmlPropertiesRecursively is the same as in Cesium's GetFeatureInfoFormat.js
