...
var WebMapServiceCatalogItem = function(terria) {
    ...
    /**
     * Lazy load time intervals callback for populating time intervals prior to data use.
     * @type {function}
     */
    this._lazyLoadTimeIntervals = undefined;
...
WebMapServiceCatalogItem.prototype.updateFromCapabilities = function(capabilities, overwrite, thisLayer, availableStyles) {
    ...
    this._lazyLoadTimeIntervals = function() {
      // Time intervals loading is very inefficient for some datasets with lots of time intervals.
      // So we lazy load it.
...
WebMapServiceCatalogItem.prototype._load = function() {
    ...
    if (!defined(this._rawMetadata) && defined(this.getCapabilitiesUrl)) {
        promises.push(loadXML(proxyCatalogItemUrl(this, this.getCapabilitiesUrl, '1d')).then(function(xml) {
            ...
            if (that._lazyLoadTimeIntervals) {
                that._lazyLoadTimeIntervals();
                that._lazyLoadTimeIntervals = undefined;
            }
        }));
    }
