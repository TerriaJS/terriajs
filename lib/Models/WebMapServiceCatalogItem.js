...
function computeLegendForLayer(catalogItem, thisLayer, styleName) {
    ...
    if (!defined(legendUri) && defined(style) && defined(style.Name)) {
        // Construct a GetLegendGraphic request.
        legendUri = new URI(cleanUrl(catalogItem.url) +
            '?service=WMS&version=1.1.0&request=GetLegendGraphic&format=image/png&transparent=True&layer=' + encodeURIComponent(thisLayer.Name) + '&styles=' + encodeURIComponent(style.Name));
