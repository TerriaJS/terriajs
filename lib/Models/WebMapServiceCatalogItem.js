...
WebMapServiceCatalogItem.prototype.updateFromCapabilities = function(capabilities, overwrite, thisLayer) {
    ...
    if (definedNotNull(capabilities.Service) && !containsAny(capabilities.Service.Abstract, WebMapServiceCatalogItem.abstractsToIgnore)) {
        updateInfoSection(this, overwrite, 'Service Description', capabilities.Service.Abstract);
    }
...
function requestMetadata(wmsItem) {
    ...
    result.promise = loadXML(proxyUrl(wmsItem.terria, wmsItem.metadataUrl)).then(function(capabilities) {
        ...
        wmsItem.updateFromCapabilities(json, false, layer);
