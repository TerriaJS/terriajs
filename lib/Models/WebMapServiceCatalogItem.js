...
WebMapServiceCatalogItem.prototype._load = function() {
    if (!defined(this._rawMetadata)) {
        ...
        var p1 = loadXML(proxyCatalogItemUrl(this, this.metadataUrl)).then(function(xml) {
		...
		if (!defined(this.dataUrl)) {
			var describeLayersURL = cleanUrl(this.url) + '?service=WMS&version=1.1.1&request=DescribeLayer&Layers=' + encodeURIComponent(this.layers);
			
			var p2 = loadXML(proxyCatalogItemUrl(this, describeLayersURL)).then(function(xml) {
				var json = xml2json(xml);
				
				if (defined(json.LayerDescription.owsType)) {
					switch (json.LayerDescription.owsType.toLowerCase()) {
						case "wfs":
							if (defined(json.LayerDescription.wfs)) {
								that.dataUrlType = "wfs";
								that.dataUrl = json.LayerDescription.wfs;
							}
							break;
						case "wcs":
							if (defined(json.LayerDescription.wcs)) {
								that.dataUrlType = "wcs";
								that.dataUrl = json.LayerDescription.wcs;
							}
							break;
					}
				}		
