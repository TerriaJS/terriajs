...
function updateIntervalsFromIsoSegments(intervals, isoSegments, time, wmsItem) {
  ...
  const originalDateIsInUTC = isoSegments[0].indexOf("Z") >= 0;
  const localTimeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;

  var start = originalDateIsInUTC
    ? dateFnsTz.toDate(isoSegments[0], { timeZone: localTimeZone })
    : parseISO(isoSegments[0]);
  var stop = originalDateIsInUTC
    ? dateFnsTz.toDate(isoSegments[1], { timeZone: localTimeZone })
    : parseISO(isoSegments[1]);
...
function formatDateToIsoString(
  ...
  if (!originalDateHasTime) return formatDateToString(date, "yyyy-MM-dd");
  return originalDateIsInUTC
    ? dateFnsTz.format(date, "yyyy-MM-dd'T'HH:mm:ss.SSSSxxx", {
        timeZone: localTimeZone
      })
    : dateFnsTz.format(date, "yyyy-MM-dd'T'HH:mm:ss'Z'", { timeZone: "UTC" });
}

function formatDateToStringForWmsRequest(
  date,
  duration,
  originalDateHasTime,
  originalDateIsInUTC,
