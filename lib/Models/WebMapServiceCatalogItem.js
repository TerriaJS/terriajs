...
WebMapServiceCatalogItem.prototype.searchDataForMatchingFeatures = function(searchText, regex) {
    ...
    let workbenchItemResults = [];
    if (this.hasLinkedWfs === false) {
       return workbenchItemResults;
    } else if (this.hasLinkedWfs) {
        return that.linkedWfs.searchDataForMatchingFeatures(searchText, regex);
        ...
        return new Promise(function(resolve, reject) {
            ...
            .then(function(xml) {
                ...
                .then(function (result) {
                    ...
                    const wfsResults = that.linkedWfs.searchDataForMatchingFeatures(searchText, regex);
                    resolve(convertWfsSearchResultsToWms(wfsResults, that));
                })
                .otherwise(function (err) {
                    that.hasLinkedWfs = false;
                    resolve(workbenchItemResults);
                });
            })
            .otherwise(function (err) {
                that.hasLinkedWfs = false;
