...
var WebMapServiceCatalogItem = function(terria) {
    ...
    /**
     * Specifies whether a linked WFS has been found for this WMS item.
     * @type {Boolean}
     */
    this.hasLinkedWfs = undefined;

    /**
     * Gets or sets the URL of a WFS that enables search for this WMS item.
     * @type {String}
     */
    this.linkedWfs = undefined;
...
WebMapServiceCatalogItem.prototype.searchDataForMatchingFeatures = function(searchText, regex) {
    const that = this;
    if (this.hasLinkedWfs === false) {
       return [];
    } else if (!defined(that.hasLinkedWfs)) {
        return new Promise(function(resolve, reject) {
            
            that.getDescribeLayer()
            .then(function(xml) {
                const response = xml2json(xml);
                if (response.LayerDescription.wfs) {
                    constructLinkedWfs(that, response.LayerDescription.wfs);
                } else {
                    throw new Error(`Found a DescribeLayer Url but it doesn't contain a WFS`);
                }
            })
            .otherwise(function (err) {
                const guessedWfsUrl = that.getCapabilitiesUrl.replace(/wms/gi, 'wfs');
                tryGuessingWfsUrl(guessedWfsUrl)
                .then(function (response) {
                    constructLinkedWfs(that, guessedWfsUrl);
                })
                .otherwise(function (err) {
                    // We couldn't find a DesribeLayer endpoint or guess the WFS URL
                    that.hasLinkedWfs = false;
                });
            })
            .always(function (){
                if (!that.hasLinkedWfs) {
                    resolve([]);
                } else {
                    that.linkedWfs.searchDataForMatchingFeatures(searchText, regex)
                    .then(function (results) {
                        resolve(convertWfsSearchResultsToWms(results, that));
                    })
                    .catch(function (err) {
                        console.log(err);
                        resolve([]);
    ...
    } else if (this.hasLinkedWfs && this.linkedWfs.searchableViaQuery !== false) {
        return new Promise(function(resolve, reject) {
            that.linkedWfs.searchDataForMatchingFeatures(searchText, regex)
            .then(function (results) {
                resolve(convertWfsSearchResultsToWms(results, that));
    ...
    } else if (this.linkedWfs.searchableViaQuery === false) {
        return [];
    }
};
...
WebMapServiceCatalogItem.prototype.getDescribeLayer = function() {
    this.describeLayerUrl = buildDescribeLayerUrl(this);
    return loadDescribeLayer(this);
};

function constructLinkedWfs(wmsItem, wfsUrl) {
    wmsItem.hasLinkedWfs = true;
    wmsItem.linkedWfs = new WebFeatureServiceCatalogItem(wmsItem.terria);
    wmsItem.linkedWfs.url = wfsUrl;
    wmsItem.linkedWfs.typeNames = wmsItem.layers;
    wmsItem.linkedWfs.requestGeoJson = true;
    wmsItem.linkedWfs.requestGml = true;
    wmsItem.linkedWfs.updateFromJson({
        parameters: {
            maxFeatures: 10
        }
    });
...
function tryGuessingWfsUrl(guessedWfsUrl) {
    return loadXML(guessedWfsUrl);
}

function buildDescribeLayerUrl(wmsItem) {
    var url = cleanAndProxyUrl(wmsItem, wmsItem.url);
    return new URI(url)
        .setQuery(combine({
        service: 'WMS',
...
function convertWfsSearchResultsToWms (results, wmsItem) {
    return results.map(match => {
        return {
            matchType: 'WMS',
            workbenchItem: wmsItem,
            feature: match.feature,
            matchFieldText: match.matchFieldText
        };
    });
}
...
function loadDescribeLayer(wmsItem) {
    return loadXML(wmsItem.describeLayerUrl);
}
