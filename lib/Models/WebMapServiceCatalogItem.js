...
WebMapServiceCatalogItem.prototype.updateFromCapabilities = function(capabilities, overwrite, thisLayer) {
    ...
    if (!defined(thisLayer)) {
        thisLayer = findLayer(capabilities, getFirstLayer(this.layers));
        if (!defined(thisLayer)) {
            return;
        }
    }
...
WebMapServiceCatalogItem.prototype._load = function() {
    if (!defined(this._rawMetadata)) {
        ...
        return loadXML(proxyUrl(this.terria, this.metadataUrl)).then(function(xml) {
            that._rawMetadata = capabilitiesXmlToJson(xml);
    ...
    for (var i = 0; i < layers.length; i++) {
        if (r === undefined) {
            if (layers[i] !== undefined) {
                r = Rectangle.clone(getRectangleFromLayer(layers[0]));
            }
            continue;
        }


        var r2 = getRectangleFromLayer(layers[i]);
        if (r2.west < r.west) {
            r.west = r2.west;
        }
        if (r2.east > r.east) {
...
function requestMetadata(wmsItem) {
    ...
    var metadata = wmsItem._rawMetadata || loadXML(proxyUrl(wmsItem.terria, wmsItem.metadataUrl)).then(capabilitiesXmlToJson);
    ...
    result.promise = when(metadata, function(json) {
        ...
        if (layers) {
            ...
            wmsItem._rectangleFromMetadata = getRectangleFromLayers(layers);

            if (wmsItem.populateIntervalsFromTimeDimension) {
                wmsItem._intervalsFromMetadata = getIntervalsFromLayer(layers[0]); // TODO merge multiple layers
            }
        } else {
            result.dataSourceErrorMessage = 'Layer information not found in GetCapabilities operation response.';
        }

        wmsItem.updateFromCapabilities(json, false, layers[0]); // TODO merge multiple layers
