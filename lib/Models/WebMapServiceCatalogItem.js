...
function updateIntervalsFromIsoSegments(intervals, isoSegments, wmsItem) {
    ...
    if (isoSegments.length === 2) {
        intervals.addInterval(new TimeInterval({
            start: start,
            stop: stop,
            data: times[j] // http://mapserver.org/ogc/wms_time.html#supported-time-requests
        }));
    } else {
        ...
        var prevStop = start,
            count = 0;
        // add intervals starting at start until we go past the stop date or go past a limit
        while (JulianDate.compare(prevStop, stop) <= 0 && count < wmsItem.maxRefreshIntervals) {
            var thisStop = addDurationFromString(prevStop, isoSegments[2], wmsItem);
            intervals.addInterval(new TimeInterval({
                start: prevStop,
                stop: thisStop,
                data: JulianDate.toIso8601(prevStop) // assume this is how to form the web request
            }));
            prevStop = thisStop;
            count++;
        }
    ...
    for (var i = 0; i < dimensions.length; ++i) {
        ...
        for (var j = 0; j < times.length; ++j) {
            ...
            if (isoSegments.length > 1) {
                updateIntervalsFromIsoSegments(result, isoSegments, wmsItem);
            } else {
