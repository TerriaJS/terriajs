...
WebMapServiceCatalogItem.prototype.searchDataForMatchingFeatures = function(searchText, regex) {
    ...
    if (this.hasLinkedWfs === false) {
       return [];
    } else if (this.hasLinkedWfs) {
        return new Promise(function(resolve, reject) {
            queryRelatedWfs(that.linkedWfs, searchText, regex)
        ...
        return new Promise(function(resolve, reject) {
            loadXML(that.getCapabilitiesUrl.replace(/wms/gi, 'wfs'))
            .then(function(xml) {
                that.hasLinkedWfs = true;

                that.linkedWfs = new WebFeatureServiceCatalogItem(that.terria);
                that.linkedWfs.url = cleanUrl(that.getCapabilitiesUrl.replace(/wms/gi, 'wfs'));
                that.linkedWfs.typeNames = that.layers;
                that.linkedWfs.requestGeoJson = true;
                that.linkedWfs.requestGml = true;
                that.linkedWfs.updateFromJson({
                    parameters: {
                        maxFeatures: 10
                    }
                });

                that.linkedWfs.getDescribeFeatureType()
                .then(function () {
                    queryRelatedWfs(that.linkedWfs, searchText, regex)
                    .then(function (results) {
                        const r = convertWfsSearchResultsToWms(results, that);
                        resolve(r);
                    })
                    .catch(function (err) {
...
function queryRelatedWfs (linkedWfs, searchText, regex) {
    return new Promise(function(resolve, reject) {
        const filterTemplate = linkedWfs.createLikeFilterTemplate(linkedWfs.fields, searchText);

        linkedWfs.updateFromJson({
            parameters: {
        ...
        linkedWfs.load()
        .then(function () {
            resolve(linkedWfs.searchDataForMatchingFeatures(regex));
        })
        .otherwise(function (err) {
            resolve([]);
        });
