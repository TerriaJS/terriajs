...
import { parseISO, isBefore, isEqual, addSeconds } from 'date-fns';
...
function updateIntervalsFromIsoSegments(intervals, isoSegments, time, wmsItem) {
    ...
    if (isoSegments.length === 2) {
        ...
        intervals.addInterval(new TimeInterval({
            start: JulianDate.fromDate(start),
            stop: JulianDate.fromDate(stop),
        ...
        if (duration.seconds > 0 || duration.minutes > 0 ||
            ...
            var durationAsSeconds = isoDuration.toSeconds(duration);
            ...
            while (thisStop && (isEqual(prevStop, stopDate) || isBefore(prevStop, stopDate)) && count < wmsItem.maxRefreshIntervals) {
                thisStop = addSeconds(thisStop, durationAsSeconds);
                intervals.addInterval(new TimeInterval({
                    start: JulianDate.fromDate(prevStop),
                    stop: JulianDate.fromDate(thisStop),
                    data: JulianDate.fromDate(prevStop) // used to form the web request
                }));
                prevStop = thisStop;
                ++count;
            }
