...
WebMapServiceCatalogItem.prototype._createImageryProvider = function(time) {
    ...
    // If we're in 2D mode and the tiling scheme is geographic, we need to do some extra work because
    // Leaflet _only_ works with WebMercatorTilingScheme.  Presumably we're using the geographic tiling
    // scheme because the server doesn't know how to do Mercator.
    if (defined(this.terria.leaflet) && this.tilingScheme instanceof GeographicTilingScheme) {
        // We can work around this Leaflet / server limitation as follows:
        // - Request tiles from the WMS server using the Web Mercator _tiling scheme_ but the Geographic _projection_.
        // - Reproject the Geographic tiles to Web Mercator.

        // This is complicated by the fact that most (all?) WMS servers won't mess with the aspect ratio of the map,
        // even though the WMS spec section 7.3.3.8 says they should.  So if you request a bigger tile in the
        // vertical direction, the pixel dimensions in that direction need to be bigger as well.
        var tilingScheme = new WebMercatorTilingScheme();
        imageryProvider = new UrlTemplateImageryProvider({
            url: imageryProvider._tileProvider.url.replace('{westProjected}', '{westDegrees}').replace('{eastProjected}', '{eastDegrees}').replace('{northProjected}', '{northDegrees}').replace('{southProjected}', '{southDegrees}'),
            pickFeaturesUrl : imageryProvider._tileProvider.pickFeaturesUrl.replace('{westProjected}', '{westDegrees}').replace('{eastProjected}', '{eastDegrees}').replace('{northProjected}', '{northDegrees}').replace('{southProjected}', '{southDegrees}'),
            tilingScheme : tilingScheme,
            rectangle : imageryProvider._tileProvider.rectangle,
            tileWidth : imageryProvider._tileProvider.tileWidth,
            tileHeight : imageryProvider._tileProvider.tileHeight,
            minimumLevel : imageryProvider._tileProvider.minimumLevel,
            maximumLevel : imageryProvider._tileProvider.maximumLevel,
            proxy : imageryProvider._tileProvider.proxy,
            subdomains: imageryProvider._tileProvider.subdomains,
            tileDiscardPolicy : imageryProvider._tileProvider.tileDiscardPolicy,
            credit : imageryProvider._tileProvider.credit,
            getFeatureInfoFormats : imageryProvider._tileProvider.getFeatureInfoFormats
        });
        ...
        var originalRequestImage = imageryProvider.requestImage;
        imageryProvider.requestImage = function(x, y, level) {
            var promise = originalRequestImage.call(this, x, y, level);
            if (!defined(promise)) {
                return undefined;
            }
            ...
            return promise.then(function(image) {
                ...
                var rectangle = tilingScheme.tileXYToRectangle(x, y, level, rectangleScratch);
                var nativeRectangle = tilingScheme.tileXYToNativeRectangle(x, y, level, nativeRectangleScratch);

                var result = new Uint8ClampedArray(image.width * image.height * 4);
                for (var j = 0; j < image.height; ++j) {
                    cartesianScratch.x = nativeRectangle.west;
                    cartesianScratch.y = CesiumMath.lerp(nativeRectangle.north, nativeRectangle.south, j / (image.height - 1.0));
                    var cartographic = tilingScheme.projection.unproject(cartesianScratch, cartographicScratch);
                    var sourceRow = ((image.height - 1.0) * (cartographic.latitude - rectangle.north) / (rectangle.south - rectangle.north)) | 0;
                    var sourceIndex = sourceRow * image.width * 4;
                    var outIndex = j * image.width * 4;

                    for (var i = 0; i < image.width; ++i) {
                        result[outIndex++] = pixels[sourceIndex++];
                        result[outIndex++] = pixels[sourceIndex++];
                        result[outIndex++] = pixels[sourceIndex++];
                        result[outIndex++] = pixels[sourceIndex++];
                    }
