...
function addDurationFromString(start, durationString) {
    // given an ISO8601 duration string such as PT1H or PT30M or P1D
    // add this duration to the start Julian Date
    // with help from https://gist.github.com/jmrocela/7167208
    var matches = durationString.match(/^P(([0-9]+)D)?T?(([0-9]+)H)?(([0-9]+)M)?(([0-9]+)S)?$/);
    var stop;
    var jd = new JulianDate();
    if (matches) {
        if (matches[2]) { stop = JulianDate.addDays(start, +matches[2], jd); }
        if (matches[4]) { stop = JulianDate.addHours(start, +matches[4], jd); }
        if (matches[6]) { stop = JulianDate.addMinutes(start, +matches[6], jd); }
        if (matches[8]) { stop = JulianDate.addSeconds(start, +matches[8], jd); }
    } else {
...
function getIntervalsFromLayer(layer) {
    ...
    for (var i = 0; i < dimensions.length; ++i) {
        ...
        var start, stop, previousInterval, duration;
        ...
        for (var j = 0; j < times.length; ++j) {
            ...
            if (isoSegments.length > 1) {
                start = JulianDate.fromIso8601(isoSegments[0]);
                stop = JulianDate.fromIso8601(isoSegments[1]);
                if (isoSegments.length === 2) {
                    result.addInterval(new TimeInterval({
                        start: start,
                        stop: stop,
                        data: JulianDate.toIso8601(start)
                    }));
                } else {
                    // note that WMS uses extension ISO19128 of ISO8601
                    // ISO 19128 allows start/end/periodicity
                    // it appears not to use the "R[n]/" prefix for repeated intervals
                    // eg. Current data refreshed every 30 min (periodic interval):
                    // 2000-06-18T14:30Z/2000-06-18T14:30Z/PT30M
                    // re 06-042_OpenGIS_Web_Map_Service_WMS_Implementation_Specification.pdf section D.4
                    
                    // so, add intervals starting at start until we go past the stop date
                    var prevStop = start,
                        count = 0,
                        maxCount = 1000;
                    while (JulianDate.compare(prevStop, stop) <= 0 && count < maxCount) {
                        var thisStop = addDurationFromString(prevStop, isoSegments[2]);
                        result.addInterval(new TimeInterval({
                            start: prevStop,
                            stop: thisStop,
                            data: JulianDate.toIso8601(prevStop)
                        }));
                        prevStop = thisStop;
                        count++;
                    }
                ...
                start = JulianDate.fromIso8601(times[j]);
                if (j < times.length - 1) {
                    stop = JulianDate.fromIso8601(times[j + 1]);
                } else if (result.length > 0) {
                    previousInterval = result.get(result.length - 1);
                    duration = JulianDate.secondsDifference(previousInterval.stop, previousInterval.start);
