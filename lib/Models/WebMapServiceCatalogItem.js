...
function updateIntervalsFromIsoSegments(intervals, isoSegments, time, wmsItem) {
    ...
    if (isoSegments.length === 2) {
        ...
        // we detect a bad periodicity (which sets thisStop to false);
        // we go past the stop date;
        // or go past the max limit
        while (thisStop && JulianDate.compare(prevStop, stop) <= 0 && count < wmsItem.maxRefreshIntervals) {
            thisStop = addDurationFromString(prevStop, isoSegments[2], wmsItem);
            intervals.addInterval(new TimeInterval({
                start: prevStop,
                stop: thisStop || prevStop, // so if there was a bad periodicity, use prevStop
