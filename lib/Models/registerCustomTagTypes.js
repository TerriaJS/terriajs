...
var registerCustomTagTypes = function() {
    ...
    var chartTagType = new CustomTagType({
        name: 'chart',
        customSetup: function(customTagInstance) {
            extractTitleFromTable(customTagInstance);
            // assume all charts shown as custom tags are expandable and downloadable
            customTagInstance.attributes.canExpand = true;
            customTagInstance.attributes.canDownload = true;
        },
    ...
    var testTagType = new CustomTagType({
        name: 'test',
    });

    customTags.register(testTagType);
