...
export default class StratumOrder {
    ...
    sortTopToBottom<T>(strata: Map<string, T>): T[] {
        return this.sort(strata, (a, b) => {
            ...
            if (aPriority === undefined) {
                throw new DeveloperError(`Stratum ${a[0]} does not exist in StratumOrder.`);
            }
            const bPriority = this.priorities.get(b[0]);
            if (bPriority === undefined) {
                throw new DeveloperError(`Stratum ${b[0]} does not exist in StratumOrder.`);
            }
    ...
    private sort<T>(strata: Map<string, T>, sortFunction: (a: [string, T], b: [string, T]) => number): T[] {
        return Array.from(strata.entries()).sort(sortFunction).map(e => e[1]);
    }
