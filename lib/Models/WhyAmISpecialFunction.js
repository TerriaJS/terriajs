...
WhyAmISpecialFunction.prototype.invoke = function(parameters) {
    ...
    return invokeTerriaAnalyticsService(this.terria, name, this.url, request).then(function(invocationResult) {
        ...
        catalogItem.url = 'test/bike_racks.geojson';
        ...
        for (var i = 0; i < 5 && i < result.columns.length; ++i) {
            ...
            shortReport += 'Only ' + percentile.toFixed(1) + '% of regions have a value ' + percentileDescription + ' than ' + result.values[i] + '.\n';
            shortReport += '<chart source-data=\'' + JSON.stringify([chartData]) + '\' source-names="' + columnName + '" preview-x-label="Value" column-names="Value,Count" column-units="Value,Count" y-column="1"></chart>\n\n';
