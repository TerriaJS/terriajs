...
var ChartsCatalogItem = require('./ChartsCatalogItem');
...
WhyAmISpecialFunction.prototype.invoke = function(parameters) {
    ...
    return invokeTerriaAnalyticsService(this.terria, name, this.url, request).then(function(invocationResult) {
        ...
        var originalLoad = catalogItem._load;

        catalogItem._load = function() {
            var regionProvider = parameters.regionType;

            var promises = [
                // Call the original load for this catalog item.
                originalLoad.call(this),

                // Also request the geometry of this region so we can get a rectangle.
                regionProvider.getRegionFeature(that.terria, region).then(function(feature) {
                    if (defined(feature) && feature.type === 'Feature') {
                        var geojsonCatalogItem = new GeoJsonCatalogItem(that.terria);
            ...
            // We're done loading when both promises above resolve.
