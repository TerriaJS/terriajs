...
WhyAmISpecialFunction.prototype._load = function() {
};
 ...
 */
WhyAmISpecialFunction.prototype.invoke = function() {
    ...
    return when.all([geoJsonPromise, invocationPromise]).then(function(allResults) {
        ...
        for (var i = 0; i < 5 && i < result.columns.length; ++i) {
            ...
            shortReport += '<chart id="' + columnName + '" data=\'' + JSON.stringify(chartData) + '\' column-names="," y-column="1" styling="histogram" highlight-x="' + result.values[i] + '"></chart>\n';
