...
WhyAmISpecialFunction.prototype.invoke = function(parameters) {
    ...
    return invokeTerriaAnalyticsService(this.terria, name, this.url, request).then(function(invocationResult) {
        ...
        for (var i = 0; i < 5 && i < result.columns.length; ++i) {
            ...
            catalogItem.shortReport += '<collapsible title="' + columnName + '" open="' + (i === 0 ? 'true' : 'false') + '">\n';
            catalogItem.shortReport += 'Only ' + percentile.toFixed(1) + '% of regions are ' + percentileDescription + ' than ' + result.values[i] + '.\n';
            catalogItem.shortReport += '<chart sources="' + toDataUri(chartData) + '" source-names="' + columnName + '" preview-x-label="Value" column-names="Value,Count" column-units="Value,Count" y-column="1"></chart>\n\n';
            catalogItem.shortReport += '</collapsible>\n';

            // <chart sources='{{url}}_1d.csv,{{url}}_2d.csv,{{url}}_5d.csv' source-names='1d,2d,5d' preview-x-label='Last 24 hours' column-names='Time,Signal at 560 GHz,Signal at 890 GHz' column-units='Date,Watts,Watts' y-column='1'></chart>
