...
sending an email to <a href="mailto:nationalmap@lists.nicta.com.au">nationalmap@lists.nicta.com.au</a>.</p>'
        ...
        addLayersRecursively(catalogGroup, serviceJson, -1, layersJson.layers, catalogGroup.items, dataCustodian);
...
function addLayersRecursively(mapServiceGroup, topLevelJson, parentID, layers, items, dataCustodian) {
    ...
    for (var i = 0; i < layers.length; ++i) {
        ...
        } else if (layer.type === 'Feature Layer' || layer.type === 'Raster Layer') {
...
function createGroup(catalogGroup, items, basePath, folderJson) {
    ...
    var newGroup = new ArcGisMapServerCatalogGroup(catalogGroup.terria);
    ...
    if (defined(catalogGroup.itemProperties)) {
        for (var propertyName in catalogGroup.itemProperties) {
            if (catalogGroup.itemProperties.hasOwnProperty(propertyName)) {
                newGroup[propertyName] = catalogGroup.itemProperties[propertyName];
            }
        }
    }
...
function createMapServer(catalogGroup, items, basePath, serviceJson) {
    ...
    var mapServer = new ArcGisMapServerCatalogGroup(catalogGroup.terria);
