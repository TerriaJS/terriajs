...
NowViewing.prototype.add = function(item) {
    ...
    if (item.supportsReordering) {
        ...
        while (index < this.items.length && (!this.items[index].supportsReordering || this.items[index].keepOnTop)) {
            ++index;
        }
...
NowViewing.prototype.raise = function(item, index) {
    ...
    if (item.supportsReordering && !(this.items[index - 1].supportsReordering || this.items[index - 1].keepOnTop)) {
        return;
    }
