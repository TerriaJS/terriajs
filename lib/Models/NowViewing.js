...
NowViewing.prototype.add = function(item) {
    ...
    if (item.supportsReordering) {
        ...
        while (index < this.items.length && (!this.items[index].supportsReordering || this.items[index].clipToRectangle)) {
            ++index;
        }
...
NowViewing.prototype.raise = function(item, index) {
    ...
    if (item.supportsReordering && !(this.items[index - 1].supportsReordering || this.items[index - 1].clipToRectangle)) {
        return;
    }
...
NowViewing.prototype.updateCesiumOrder = function() {
    ...
    for (var l = items.length-1; l >= 0; l-- ) {
        if (items[l].imageryLayer && items[l].clipToRectangle) {
