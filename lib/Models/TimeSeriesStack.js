...
TimeSeriesStack.prototype.addLayerToTop = function(item) {
    ...
    var layerStack = this._layerStack.concat(item); // Make a copy so we can trigger the knockout subscription on _layerStack (in one go).

    if (currentIndex > -1) {
        layerStack.splice(currentIndex, 1);
    }
...
TimeSeriesStack.prototype.removeLayer = function(item) {
    ...
    var layerStack = this._layerStack.slice(); // Make a copy to trigger the knockout subscription on _layerStack.
