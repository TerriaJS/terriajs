...
/**
 * A {@link CatalogFunction} that invokes a Representational State Transfer (REST) process that returns a Terria JSON catalog item.
 *
...
defineProperties(TerriaJSONCatalogFunction.prototype, {
    ...
    /**
     * Gets a human-readable name for this type of data source, 'Terria JSON catalog item'.
     * @memberOf TerriaJSONCatalogFunction.prototype
     * @type {String}
     */
    typeName : {
        get : function() {
            return 'Terria JSON catalog item';
        }
    },

    /**
     * Gets the parameters used to {@link CatalogFunction#invoke} to this process.
     * @memberOf TerriaJSONCatalogFunction
...
/**
 * Gets or sets the list of converters between a TerriaJSON Input and a {@link FunctionParameter}.
 * @type {Array}
 */

TerriaJSONCatalogFunction.parameterConverters = [
    {
...
TerriaJSONCatalogFunction.prototype._load = function() {
    ...
    this._parameters = inputs.map(createParameterFromCatalogInput.bind(undefined, this));
};
...
function createParameterFromCatalogInput(catalogFunction, input) {
    for (var i = 0; i < TerriaJSONCatalogFunction.parameterConverters.length; ++i) {
        var converter = TerriaJSONCatalogFunction.parameterConverters[i];
