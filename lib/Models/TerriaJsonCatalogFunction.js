...
var Request = require("terriajs-cesium/Source/Core/Request");
...
function TerriaJsonCatalogFunction(terria) {
    ...
    //to pass the effect of clicking Remove on a 
    //ResultPendingCatalogItem workbench item, a Request object needs to 
    //exist in the same scope as the ResultPendingCatalogItem, and cancel
    //the request on removal from workbench 
    //(isEnabled change from true to false)

    this._JSONheaders = {
        Accept : 'application/json,*/*;q=0.01'
    };

    //Request cancel function gets overwritten in 
    //terriajs-cesium/Source/Core/Resource.js 
    //set cancel function there
    this._abortPropagatingRequest = new Request();
}
...
TerriaJsonCatalogFunction.prototype.invoke = function() {
    ...
    //connect Removing the ResultPendingCatalogItem Workbench Item to
    //cancelling the request
    var abortCheck = knockout.getObservable(asyncResult, 'isEnabled').subscribe(function(newValue){
        if (newValue === false) {
            this._abortPropagatingRequest.cancelFunction();
        }
    },this);
    ...
    const promise = loadJson(proxiedUrl,this._JSONheaders,this._abortPropagatingRequest).then(json => {
        //stop listening to ResultPendingCatalogItem being removed 
