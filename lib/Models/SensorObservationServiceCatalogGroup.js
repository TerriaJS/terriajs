...
/*
 Fetches the locations of all sensors (possibly more than just sensors). This seems to be the only
 way to get spatial information. We could convert this to a GeoJSON or CSV.

 http://www.bom.gov.au/waterdata/services?service=SOS&version=2.0&request=GetFeatureOfInterest
 
 */
function loadFeaturesOfInterest(sosGroup) {
    return querySos(sosGroup, { 
 ...
 * @private
 */
function loadCapabilities(sosGroup) {
    ...
    return querySos(sosGroup, {
        ...
        temporalFilter: sosGroup.temporalFilter
