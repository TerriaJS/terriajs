...
var WfsFeaturesCatalogGroup = function(terria) {
    ...
    knockout.track(this, ['url', 'typeNames', 'nameProperty', 'dataCustodian', 'blacklist']);
};
...
WfsFeaturesCatalogGroup.prototype._getValuesThatInfluenceLoad = function() {
    return [this.url, this.typeNames, this.blacklist];
};

WfsFeaturesCatalogGroup.prototype._load = function() {
    var url = cleanAndProxyUrl(this.terria, this.url) + '?service=WFS&version=1.1.0&request=GetFeature&typeName=' + this.typeNames + '&outputFormat=JSON&propertyName=' + this.nameProperty;
...
sending an email to <a href="mailto:'+that.terria.supportEmail+'">'+that.terria.supportEmail+'</a>.</p>'
        ...
        for (var i = 0; i < features.length; ++i) {
            ...
            item.name = feature.properties[that.nameProperty];
