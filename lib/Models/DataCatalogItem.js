...
var DataCatalogItem = function(terria, url, options) {
    ...
    knockout.defineProperty(this, 'concepts', {
        get : function() {
            var concept = new DisplayVariablesConcept('', toggleYColumn.bind(that));
...
*
* @param {DataCatalogItem} catalogItem this dataCatalogItem
* @param {Integer} index Index of the selected variable in the list of variables.
*/
function toggleYColumn(catalogItem, varIndex) {
    var column = catalogItem.validYColumns[varIndex];
    var indexInSelected = catalogItem.yColumns.indexOf(column);
    if (indexInSelected >= 0) {
        // it's in the list, so remove it
        catalogItem.yColumns.splice(indexInSelected, 1);
        catalogItem.yColumns = catalogItem.yColumns.slice(); // slice to clone the array so computed properties pick up the change.
    } else {
        // it's not in the list, so add it
        catalogItem.yColumns = catalogItem.yColumns.concat([column]);  // use concat not push to clone the array.
