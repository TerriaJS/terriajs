...
TerrainCatalogItem.prototype._showInCesium = function() {
    ...
    var terrainProvider = this._createTerrainProvider();
...
TerrainCatalogItem.prototype._showInLeaflet = function() {
    throw new TerriaError({
        ...
        message: 'This catalog item cannot be show in the 2D view.  Switch to 3D and try again.'
    });
};
...
TerrainCatalogItem.prototype._disableOtherTerrainItems = function() {
    ...
    for (var i = 0; i < items.length; i++) {
        var item = items[i];
        if (defined(item.terrainProvider) && item !== this) {
