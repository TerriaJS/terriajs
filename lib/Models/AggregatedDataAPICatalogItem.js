        ...
        concept.items = item.fields
            ...
            .map((field, fieldIndex) =>
                new VariableConcept(field.name, {
                    parent: concept,
                    id: field.code,
                    active: topicIndex === 0 && fieldIndex === 1
                })
            );
        return concept;
    });
...
function makeTableStructureFromDataFrameJson(item, json) {
    ...
    if (dateFields.length > 0) {
        ...
        json.fields[dateIndex].name = 'date';
    ...
    if (regionFields.length > 0) {
        ...
        json.fields[regionIndex].name = regionFields[0].region_type;
    ...
    var isCategoryField = (field) => !field.calculated && field.role !== 'DATE' && !defined(field.region_type);
    var categoryFields = json.fields.filter(isCategoryField);
    var categoryIndex = getIndexOfField(json.fields, categoryFields[0]);
    ...
    columns.push(new TableColumn('date', dateValues, item._columnOptions));
    ...
    columns = columns.concat(categoryValuesArrays.map((values, i) => {
        return new TableColumn(uniqueCategories[i], values, item._columnOptions);
    }));
    ...
    tableStructure.setActiveTimeColumn('date');
