...
var numberUtils = require('../Core/numberUtils');
...
var TABLES_URI_COMPONENT = 'datasets';  // May also be present in registerCatalogMembers.js.
...
function loadData(item) {
    ...
    const topLevelRegionTypeConcepts = item._concepts.filter(concept => concept.id === item.regionTypeKey);
    if (topLevelRegionTypeConcepts.length > 0) {
        // Find the (first) active region type. There can be none if it was just changed (triggers deselection, then selection).
        const activeRegionTypes = topLevelRegionTypeConcepts[0].items.filter(concept => concept.isActive);
        if (activeRegionTypes.length === 0) {
            return when();
        }
    ...
    const longitudinalConditions = getLongitudinalConditions(item);
    const selectedConditions = [].concat(longitudinalConditions);  // Start off with a clone of the longitudinal conditions, eg. ['year=2011'].
    ...
    uri.query(query);
    const proxiedUrl = proxyCatalogItemUrl(item, uri.toString());
    ...
    if (!defined(item._regionTotals[JSON.stringify(group_bys)])) {
        ...
        totalUri.query({
            where: longitudinalConditions.join(',') || undefined,
            group_by: group_bys.join(',')  // Note URI would turn an array into group_by=YEAR&group_by=AGE, not group_by=YEAR,AGE.
        });
        const proxiedTotalUrl = proxyCatalogItemUrl(item, totalUri.toString());
    ...
    if (defined(regionTotalsObject)) {
        ...
        tableStructureJson = tableStructureJson.map((row, rowNumber) => {
            if (rowNumber === 0) {
                ...
                let fraction = null;
                if (defined(regionTotal) && regionTotal !== 0 && numberUtils.isNumeric(regionTotal)) {
                    if (numberUtils.isNumeric(row[resultIndex])) {
                        fraction = row[resultIndex] / regionTotal;
                        fraction = (fraction < 0.01) ? (Math.round(fraction * 1000) / 10) : (Math.round(fraction * 10000) / 100);
                    } else {
                        fraction = row[resultIndex];  // Non-numeric values should just be passed through, eg. "suppressed".
                    }
                }
...
/**
 * Get the conditions in the API's syntax for the longitudinal variables.
 * Currently this just requires that each one equals the last available value (eg. the last year in the list), skipping sentinels.
 * If no values are provided in the metadata, it defaults to 2011.
 * TODO: Make it work with the timeslider instead (and remove the arbitrary 2011 default).
 * @param  {AggregatedDataAPICatalogItem} item
 * @return {String[]} A list of conditions, eg. ['year=2011', 'simulation=99'].
 */
function getLongitudinalConditions(item) {
    // TODO: Make it work with the timeslider instead.
    var longitudinal_fields = item._fields.filter(field => field.is_longitudinal);
    return longitudinal_fields.map(field => {
...
//     //             {"is_result": true, "name": "perturbed count", "title": "perturbed count"}],
...
//     var resultFields = json.fields.filter(field => field.is_result);
...
//     var isCategoryField = (field) => !field.is_result && !field.is_longitudinal && !defined(field.region_type);
 ...
 *     testing for those fields with is_longitudinal=true, region_type defined, is_result=true, and none of the above.
 * @private
 */
function getJsonFieldIndices(fields) {
    ...
    return {
        ...
        result: getIndicesWhere(fields, field => field.is_result),
        category: getIndicesWhere(fields, field => !field.is_result && !field.is_longitudinal && !defined(field.region_type))
    };
}
...
function createTotalsObject(totalsJson) {
    ...
    // 3. The result column (which will have is_result=true).
