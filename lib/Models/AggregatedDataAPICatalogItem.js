...
var AggregatedDataAPICatalogItem = function(terria, url) {
     ...
     * @type {Boolean}
     */
    this.displayConceptsBeforeLegend = true;
...
function loadData(item) {
    ...
    var valuesKeyedByFieldId = {};
    item.activeConcepts[topLevelTopicConceptIndex].forEach(activeConcept => {
        if (Object.keys(valuesKeyedByFieldId).indexOf(activeConcept.parent.id) < 0) {
    ...
    uri.query({
        where: selectedConditions.concat('Year=2011').join(','), // TODO: Year=2011 just to get things working.
    ...
    if (!defined(item._regionTotals[JSON.stringify(group_by)])) {
        var totalUri = new URI(item.base_url);
        ...
        var proxiedTotalUrl = proxyCatalogItemUrl(item, uri.toString(), {keyType: item.keyType});
    ...
    return when.all(promises).then(function(jsons) {
        ...
        // Also, rename any region dimension as the region type.
        // And rename the calculated field to "Number of <units> meeting the conditions"
        const getFieldName = (field) => {
            var numberText = 'Number';
            var ofWhatText = defined(item._unit) ? (' of ' + item._unit.plural) : '';
            var conditionsText = selectedConditions.length === 1 ? 'the condition' : 'all conditions';
            var meetingWhatText = selectedConditions.length > 0 ? (' meeting ' + conditionsText) : '';
            if (field.calculated) {
                return numberText + ofWhatText + meetingWhatText;
            }
        ...
        if (jsons.length > 1) {
            var totalsJson = jsons[1];
            var tableStructureTotalsJson = [totalsJson.fields.map(getFieldName)].concat(totalsJson.values);
            item._regionTotals[JSON.stringify(group_by)] = TableStructure.fromJson(tableStructureTotalsJson);
        }
        var json = jsons[0];  // The json of the request that meets the conditions.
        var tableStructureJson = [json.fields.map(getFieldName)].concat(json.values);
        TableStructure.fromJson(tableStructureJson, item._tableStructure);
        item._tableStructure.columns[2].isActive = true; // TODO: make this nicer.
            ...
            var concept = new DisplayVariablesConcept(topicName, {
                ...
                items: item.fields
                    ...
                    .map((field, fieldIndex) => {
                        var keys = Object.keys(field.values).sort(compareStringsAndNumbers);
                        var fieldConcept = new DisplayVariablesConcept(field.title, {
                            id: field.name,
                            isOpen: false,
                            allowMultiple: true,
                            items: keys.map((key, valueIndex) => {
                                var variableConcept = new VariableConcept(field.values[key].title || field.values[key].name || key, {
                                    id: key,
