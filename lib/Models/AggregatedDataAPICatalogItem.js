...
function loadItem(item) {
    ...
    return loadJson(proxiedTableInfoUrl).then(function(json) {
        ...
        if (item.name === item.url) {
            console.log('Changing item name', json.name, json);
...
function loadData(item) {
    ...
    return loadJson(proxiedUrl).then(function(json) {
        item._tableStructure = makeTableStructureFromDataFrameJson(item, json);
...
function getIndexOfField(fields, field) {
    return fields.map(field => field.code).indexOf(field.code);
}

function makeTableStructureFromDataFrameJson(item, json) {
    // json has the format:
    ...
    // This is very close to the json format which TableStructure.fromJson reads -
    // we just need to put the table names at the start of the values array.
    // TODO: actually need to convert from a dataframe format to a 2D csv format.
    var rows = [json.fields.map(field => field.name)].concat(json.values);
    // Get the time dimension, if any, and rename it.
    var dateFields = json.fields.filter(field => field.role === 'DATE');
    if (dateFields.length > 0) {
        var dateIndex = getIndexOfField(json.fields, dateFields[0]);
        rows[0][dateIndex] = 'date';
    }
    ...
    if (regionFields.length > 0) {
        ...
        rows[0][regionIndex] = regionFields[0].region_type;
    }
    var tableStructure = new TableStructure(item.name, item._columnOptions);
    TableStructure.fromJson(rows, tableStructure);
    tableStructure.setActiveTimeColumn(item._tableStyle.timeColumn);
    // Find the calculated column and activate it.
    var resultFields = json.fields.filter(field => field.calculated);
    if (resultFields.length > 0) {
        var resultIndex = getIndexOfField(json.fields, resultFields[0]);
        tableStructure.columns[resultIndex].isActive = true;
    } else {
        console.log('No calculated field found.');
    }
