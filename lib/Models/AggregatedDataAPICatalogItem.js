...
function loadData(item) {
    ...
    return loadJson(proxiedUrl).then(function(json) {
        ...
        item._concepts = [item._tableStructure].concat(item._concepts);
...
function setTableStructureFromDataFrameJson(item, json) {
    ...
    columns = columns.concat(categoryValuesArrays.map((values, i) => {
        // TODO: if column values are available, replace the category codes with nice names.
        return new TableColumn(uniqueCategories[i].toString(), values, item._columnOptions);
    }));
    columns[2].isActive = true;  // TODO: improve

    item._tableStructure.columns = columns;
    item._tableStructure.setActiveTimeColumn('date');
    item._tableStructure.name = categoryField.name;
