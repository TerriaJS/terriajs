...
function buildConcepts(item) {
    ...
    // (Note AdditiveConditionsConcept inherits from DisplayVariablesConcept.)
    var allTopics = getUniqueValues(item._fields.map(field => field.topic));
    var topLevelTopicConcept = new AdditiveConditionsConcept('Conditions', {
        id: 'topic',
        isOpen: false,
                ...
                items: item.fields
                    ...
                    .map((field, fieldIndex) => {
                        ...
                        var fieldConcept = new DisplayVariablesConcept(field.title, {
                            ...
                            isOpen: false,
                            allowMultiple: true,
                            items: keys.map((key, valueIndex) => new VariableConcept(field.values[key].title || field.values[key].name || key, {
                                id: key,
                                active: (topicIndex === 6 && fieldIndex === 5 && valueIndex <= 1) || (topicIndex === 4 && fieldIndex === 3 && valueIndex === 1), // TODO: for testing only
                            }))
                        });
                        return fieldConcept;
                    })
