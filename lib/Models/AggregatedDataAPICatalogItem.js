...
var AdditiveConditionsConcept = require('../Map/AdditiveConditionsConcept');
...
function loadData(item) {
    ...
    var selectedConditions = ['Year=2011'];  // TODO: just to get things working.
    for (var i = 0; i < topLevelTopicConcept.items.length; i++) {
        var topicConcept = topLevelTopicConcept.items[i];
        for (var j = 0; j < topicConcept.items.length; j++) {
            var questionConcept = topicConcept.items[j];
            if (questionConcept.activeItems.length > 0) {
                selectedConditions.push(questionConcept.id + '=' + questionConcept.activeItems.map(concept => concept.id).join(';'));
            }
        }
    }
    ...
    uri.query({
        where: selectedConditions.join(','),
        group_by: group_by.join(',')  // Note URI would turn an array into group_by=YEAR&group_by=AGE, not group_by=YEAR,AGE.
    });

    var proxiedUrl = proxyCatalogItemUrl(item, uri.toString(), {cacheDuration: '0d', keyType: item.keyType});
    item.isLoading = true;
    return loadJson(proxiedUrl).then(function(json) {
        ...
        const getFieldName = (field) => {
            if (field.calculated) {
                return defined(item._unit) ? 'Number of ' + item._unit.plural + ' meeting the conditions' : 'Number meeting the conditions';
            }
...
function buildConcepts(item) {
    ...
    var topLevelTopicConcept = new AdditiveConditionsConcept('Conditions', {isOpen: false});
    ...
    var topicConcepts = allTopics.map((topicName, topicIndex) => {
        var concept = new DisplayVariablesConcept(topicName, {isOpen: false});
        concept.items = item.fields
            ...
            .map((field, fieldIndex) => {
                ...
                fieldConcept.items = keys.map((key, valueIndex) => new VariableConcept(field.values[key].title || field.values[key].name || key, {
                    parent: fieldConcept,
                    id: key,
                    active: (topicIndex === 6 && fieldIndex === 5 && valueIndex <= 1) || (topicIndex === 4 && fieldIndex === 3 && valueIndex === 1), // TODO: improve!
                }));
                return fieldConcept;
            });
        return concept;
