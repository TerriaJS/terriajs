...
function loadData(item) {
    ...
    // const dateFields = item.fields.filter(field => field.longitudinal);
...
function buildTableStructureWithTotals(item, json, numberOfSelectedConditions, regionTotalsObject) {
    ...
    if (defined(regionTotalsObject)) {
        const keyElements = fieldIndices.longitudinal.concat(fieldIndices.region);
 ...
 *     testing for those fields with longitudinal=true, region_type defined, calculated=true, and none of the above.
 * @private
 */
function getJsonFieldIndices(fields) {
    ...
    //  {"calculated": true, "name": "perturbed count", "title": "perturbed count"}]
    return {
        longitudinal: getIndicesWhere(fields, field => field.longitudinal),
        region: getIndicesWhere(fields, field => defined(field.region_type)),
        result: getIndicesWhere(fields, field => field.calculated),
        category: getIndicesWhere(fields, field => !field.calculated && !field.longitudinal && !defined(field.region_type))
    };
}
...
function createTotalsObject(totalsJson) {
    ...
    // 1. Longitudinal columns (which have longitudinal=true; usually just "year", but there could in theory be > 1).
    // 2. Region column (which have a region_type).
    // 3. The result column (which will have calculated=true).
    ...
    const keyIndices = fieldIndices.longitudinal.concat(fieldIndices.region);
    const resultIndex = fieldIndices.result[0];
    const totalsObject = {};
