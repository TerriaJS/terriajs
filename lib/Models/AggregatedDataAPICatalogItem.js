...
function setTableStructureFromDataFrameJson(item, json) {
    ...
    columns = columns.concat(categoryValuesArrays.map((values, i) => {
        ...
        if (defined(originalField.values) && defined(originalField.values[categoryCode])) {
            categoryName = defaultValue(originalField.values[categoryCode].name, categoryCode.toString());
        } else {
    ...
    item._tableStructure.name = categoryField.name + ' (count)';
    ...
    columns.forEach(column => {
        ...
        updatedTableStyle.columns[column.name].legendName = categoryField.name + ': ' + column.name + ' (count)';
    });
