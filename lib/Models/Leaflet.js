...
var combineLatLngBounds = require('../Map/combineLatLngBounds');
...
function zoomToDataSource(leaflet, target, flightDurationSeconds) {
    ...
    var visualizers = leafletVisualizer.visualizersCallback(leaflet.scene, undefined, target);
    ...
    for (var i = 0, len = visualizers.length; i < len; i++) {
        ...
        if (defined(bounds)) {
            latLngBounds.push(bounds);
        }
    }

    if (latLngBounds.length > 0) {
        leaflet.map.flyToBounds(combineLatLngBounds(latLngBounds), {
            animate: flightDurationSeconds > 0.0,
            duration: flightDurationSeconds
        });
    }
