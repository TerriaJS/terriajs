...
Leaflet.prototype.zoomTo = function(target, flightDurationSeconds) {
    ...
    return when().then(function() {
        ...
        that.map.flyToBounds(rectangleToLatLngBounds(extent), {
...
function entitiesToLatLngBounds(entities) {
    var maxBounds;

    for (var i = 0, len = entities.length; i < len; i++) {
        var latLngs = [];

        if (defined(entities[i].polygon)) {
            var positions = entities[i].polygon.hierarchy._value.positions;

            for (var j = 0; j < positions.length; j++) {
                var cartographic = Cartographic.fromCartesian(positions[j]);
                latLngs.push(L.latLng(toDegrees(cartographic.latitude), toDegrees(cartographic.longitude)));
            }

            if (defined(maxBounds)) {
                var latLngBounds = L.latLngBounds(latLngs);
                maxBounds.extend(latLngBounds);
            } else {
                maxBounds = L.latLngBounds(latLngs);
            }
        }
    }
    return maxBounds;
...
function zoomToDataSource(leaflet, target, flightDurationSeconds) {
    var maxBounds = entitiesToLatLngBounds(target.entities.values);

    if (defined(maxBounds)) {
        leaflet.map.flyToBounds(maxBounds, {
            animate: flightDurationSeconds > 0.0,
            duration: flightDurationSeconds
        });
    }
