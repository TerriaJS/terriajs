...
/**
 * A splitter can be shown on this viewer, so define a function to toggle it on and off.
 */
Leaflet.prototype.toggleSplitter = function() {
    this.showSplitter = !this.showSplitter;
    if (this.showSplitter) {
        this._sideBySide = L.control.sideBySide([], [], { thumbSize: 25, padding: 70 });
        this._sideBySide.addTo(this.map);
        this.updateSplitLayers();
    } else {
        if (defined(this._sideBySide)) {
            // First put all layers on both sides, so we don't have to redraw once the splitter is removed.
            this._sideBySide.setLeftLayers([]);
            this._sideBySide.setRightLayers([]);
            this.map.removeControl(this._sideBySide);
            this._sideBySide = undefined;
        }
    }
};
