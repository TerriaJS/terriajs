...
var Leaflet = function(terria, map) {
    ...
    // The split control (aka side-by-side control).
    // This appears in the DOM as:
    // <div class="leaflet-sbs">
    //   <div class="leaflet-sbs-divider"></div>
    //   <input class="leaflet-sbs-range" type="range">
    // </div>
    // and can be styled with custom css. Note the thumb and track css is handled using css pseudo-elements,
    // eg. ::-webkit-slider-thumb - see leaflet-side-by-side/range.css.
    this._sideBySide = undefined;
    ...
    knockout.getObservable(terria, 'showSplitter').subscribe(function() {
        updateShowSplitter(this);
    }, this);
...
Leaflet.prototype.notifyRepaintRequired = function() {
    ...
    // except update the layers on the left/right of the splitter.
 ...
 * @param {ImagerySplitDirection} [options.splitDirection=ImagerySplitDirection.NONE] The {@link ImagerySplitDirection} split to apply to this layer.
...
Leaflet.prototype.addImageryProvider = function(options) {
    ...
    // this.updateSplitLayers();   // Not useful, because at this point result hasn't been added to the map, so there's nothing new to update.
...
Leaflet.prototype.updateSplitLayers = function() {
    if (this.terria.showSplitter) {
        const leftLayers = [];
        const rightLayers = [];
        this.map.eachLayer(layer => {
            // If it's not explicitly on the left or right, put it on both.
            if (layer.splitDirection === ImagerySplitDirection.RIGHT) {
                rightLayers.push(layer);
            } else if (layer.splitDirection === ImagerySplitDirection.LEFT) {
                leftLayers.push(layer);
            }
        });
        if (!defined(this._sideBySide)) {
            createSideBySide(this);
        }
        this._sideBySide.setLeftLayers(leftLayers);
        this._sideBySide.setRightLayers(rightLayers);
    }
};

function createSideBySide(viewer) {
    // viewer._sideBySide = L.control.sideBySide([], [], { thumbSize: 25, padding: 70 });
    viewer._sideBySide = L.control.sideBySide([], []);
    viewer._sideBySide.addTo(viewer.map);
}

/**
 * Do some work when the splitter is turned on or off.
 */
function updateShowSplitter(viewer) {
    if (viewer.terria.showSplitter) {
        createSideBySide(viewer);
        viewer.updateSplitLayers();
    } else {
        if (defined(viewer._sideBySide)) {
            // First put all layers on both sides, so we don't have to redraw once the splitter is removed.
            viewer._sideBySide.setLeftLayers([]);
            viewer._sideBySide.setRightLayers([]);
            viewer.map.removeControl(viewer._sideBySide);
            viewer._sideBySide = undefined;
        }
    }
}
