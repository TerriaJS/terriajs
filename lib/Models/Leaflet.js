...
Leaflet.prototype.addImageryProvider = function(options) {
    ...
    if (options.imageryProvider instanceof MapboxVectorTileImageryProvider) {
        options.async = true;
        options.bounds = rectangleToLatLngBounds(options.imageryProvider.rectangle);
        result = new MapboxVectorCanvasTileLayer(options.imageryProvider, options);
    }
    else {
        result = new CesiumTileLayer(options.imageryProvider, options);
    } 
