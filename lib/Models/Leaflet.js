...
function pickFeatures(leaflet, latlng, tileCoordinates, existingFeatures) {
    ...
    pickedFeatures.allFeaturesAvailablePromise = when.all(promises).then(function(results) {
        ...
        if (leaflet.terria.showSplitter) {
            ...
            var screenPosition = leaflet.computePositionOnScreen(pickedFeatures.pickPosition);
            var pickedSide = leaflet.terria.getSplitterSideForScreenPosition(screenPosition);

            pickedFeatures.features = pickedFeatures.features.filter(function(feature) {
                return (feature.imageryLayer.splitDirection === ImagerySplitDirection.NONE ||
                        feature.imageryLayer.splitDirection === pickedSide);
            });
        }
