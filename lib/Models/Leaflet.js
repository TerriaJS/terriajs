...
var Leaflet = function(terria, map) {
    ...
    map.on('click', function(e) {
        that.terria.pickedFeatures = pickFeatures(that, e.latlng);
    });
...
Leaflet.prototype.pickLocation = function(latlng, imageryLayerCoords) {
    // Need to runLater to make sure the picking comes after leaflet sets itself up.
    prePickFeatures(this);

    this._pickedFeatures.allFeaturesAvailablePromise = runLater(function() {
        return pickFeatures(this, latlng, {x: imageryLayerCoords.x, y: imageryLayerCoords.y}, imageryLayerCoords.level);
    }.bind(this));

    return this._pickedFeatures;
};
...
function featurePicked(leaflet, entity) {
    ...
    prePickFeatures(leaflet, entity.position);
    leaflet._pickedFeatures.features.push(entity);
}

function prePickFeatures(leaflet, position) {
    if (!defined(leaflet._pickedFeatures)) {
        leaflet._pickedFeatures = new PickedFeatures();
        runLater(function() {
            var features = pickFeatures(leaflet);

            if (defined(features)) {
                leaflet.terria.pickedFeatures = features;
            }

            if (defined(position)) {
                leaflet.terria.pickedFeatures.pickPosition = position;
            }
        });
    }
}

function pickFeatures(leaflet, latlng, tileCoordinates, level) {
    if (!defined(leaflet._pickedFeatures)) {
        return;
    }
    ...
    if (defined(latlng)) {
        ...
        for (var i = 0; i < activeItems.length; ++i) {
            ...
            if (dataSource.isEnabled && dataSource.isShown && defined(dataSource.imageryLayer) && defined(dataSource.imageryLayer.pickFeatures)) {
                promises.push(dataSource.imageryLayer.pickFeatures(leaflet.map, CesiumMath.toRadians(latlng.lng), CesiumMath.toRadians(latlng.lat), tileCoordinates, level));
    ...
    leaflet._pickedFeatures.allFeaturesAvailablePromise = when.all(promises).then(function(promiseResult) {
