...
var GoogleUrlShortener = function(options) {
  if (!defined(options) || !defined(options.terria)) {
    throw new DeveloperError("options.terria is required."); // eslint-disable-line i18next/no-literal-string
  }
...
GoogleUrlShortener.prototype.shorten = function(url) {
  if (!this.isUsable) {
    throw new DeveloperError(
      "GoogleUrlShortener is not usable because Terria.configPrameters.googleUrlShortenerKey is not defined." // eslint-disable-line i18next/no-literal-string
    );
  }

  return loadWithXhr({
    url:
      this.url + "?key=" + this.terria.configParameters.googleUrlShortenerKey, // eslint-disable-line i18next/no-literal-string
    method: "POST",
...
GoogleUrlShortener.prototype.expand = function(token) {
  ...
  var url = corsProxy.getURLProxyIfNecessary(
    that.url +
    "?key=" + // eslint-disable-line i18next/no-literal-string
    that.terria.configParameters.googleUrlShortenerKey +
    "&shortUrl=http://goo.gl/" + // eslint-disable-line i18next/no-literal-string
      token
  );
