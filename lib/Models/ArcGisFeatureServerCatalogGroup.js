...
sending an email to <a href="mailto:'+terria.supportEmail+'">'+terria.supportEmail+'</a>.</p>'
        ...
        if (defined(defined(result.documentInfo))) {
        ...
        addLayers(catalogGroup, result, -1, result.layers, catalogGroup, dataCustodian);
...
function addLayers(featureServerGroup, topLevelJson, parentID, layers, thisGroup, dataCustodian) {
    ...
    for (var i = 0; i < layers.length; ++i) {
        ...
        thisGroup.add(createDataSource(featureServerGroup, topLevelJson, layer, dataCustodian));
    }
}

function createDataSource(featureServerGroup, topLevelJson, layer, dataCustodian) {
    var result = new ArcGisFeatureServerCatalogItem(featureServerGroup.terria);

    result.name = replaceUnderscores(layer.name);
    result.dataCustodian = dataCustodian;
    result.url = featureServerGroup.url;
    result.layers = layer.id.toString();
    result.maximumScale = layer.maxScale;

    result.updateFromMetadata(topLevelJson, true, layer);

    if (typeof(featureServerGroup.itemProperties) === 'object') {
        result.updateFromJson(featureServerGroup.itemProperties);
    }
