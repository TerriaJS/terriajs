...
SpatialDetailingFunction.prototype.invoke = function(parameters) {
    ...
    var that = this;
    return invokeTerriaAnalyticsService(this.terria, name, this.url, request).then(function(result) {
        var csv = parameters['regionTypeToPredict'].regionType + ',Predicted ' + predictedCharacteristic + ',95% confidence interval\n';
