...
WebMapTileServiceCatalogGroup.defaultSerializers.items = function(wmsGroup, json, propertyName, options) {
...
WebMapTileServiceCatalogGroup.defaultSerializers.isLoading = function(wmsGroup, json, propertyName, options) {};
...
function createWmtsDataSource(wmtsGroup, capabilities, layer) {
    ...
    if (typeof(wmtsGroup.itemProperties) === 'object') {
        Object.keys(wmtsGroup.itemProperties).forEach(function(k) {
            result[k] = wmtsGroup.itemProperties[k];
        });
    }
