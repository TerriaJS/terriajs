    ...
    var promise = loadXML(url).then(function(xml) {
        ...
        if (!xml || !xml.documentElement || (xml.documentElement.localName !== 'ExecuteResponse')) {
            throw new TerriaError({
                ...
                message: '\
An error occurred while invoking ExecuteProcess on the WPS server for process name '+that.name+'.  The server\'s response does not appear to be a valid ExecuteResponse document.  \
<p>This error may also indicate that the processing server you specified is temporarily unavailable or there is a \
...
sending an email to <a href="mailto:'+that.terria.supportEmail+'">'+that.terria.supportEmail+'</a>.</p>'
        ...
        return handleExecuteResponse(that, asyncResult, xml);
    });
...
function handleExecuteResponse(catalogFunction, asyncResult, xmlResponse) {
    ...
    if (defined(status.ProcessFailed)) {
        ...
        asyncResult.failedMessage = 'Web Processing Service invocation failed.';
        ...
        // TODO!
    } else if (defined(json.statusLocation)) {
        // Continue polling the status location.
...
sending an email to <a href="mailto:'+catalogFunction.terria.supportEmail+'">'+catalogFunction.terria.supportEmail+'</a>.</p>'
                ...
                return handleExecuteResponse(catalogFunction, asyncResult, xml);
            });
        }, 500);
    }
}
