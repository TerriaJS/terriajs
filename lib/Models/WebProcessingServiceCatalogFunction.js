...
function WebProcessingServiceCatalogFunction(terria) {
    ...
    /**
     * Gets or sets the parameterOverrides of this WPS process. This property is observable.
     * @type {Object}
     */
    this.parameterOverrides = undefined;

    /**
     * Set the delimiter for separating parameters in an URL.
     * @type {String}
     */
    this.delimiter = undefined;

    knockout.track(this, ['_parameters', 'url', 'identifier', 'inputs', 'parameterOverrides']);
}
...
WebProcessingServiceCatalogFunction.parameterConverters = [
    ...
    },
    {
        ...
        functionParameterToInput: function(catalogFunction, parameter, value) {
            ...
            return parameter.id + '=' + bboxMinCoord1
                                + "," + bboxMinCoord2
                                + "," + bboxMaxCoord1
                                + "," + bboxMaxCoord2
                                + "," + urn;
...
function createParameterFromWpsInput(catalogFunction, input) {
    for (var i = 0; i < WebProcessingServiceCatalogFunction.parameterConverters.length; ++i) {
        ...
        if (defined(functionParameter) && defined(catalogFunction.parameterOverrides)) {
            if (functionParameter.id in catalogFunction.parameterOverrides) {
                var overrides = catalogFunction.parameterOverrides[functionParameter.id];
                if ("hidden" in overrides && overrides["hidden"]) {
                    functionParameter.display = false;
                }
                if ("value" in overrides) {
                    functionParameter.overrideValue = overrides["value"];
                }
                if ("patternConstraint" in overrides) {
                    functionParameter.constraints["pattern"] = overrides["patternConstraint"];
                }
            }
        }
...
function createWpsDataInputsFromParameters(catalogFunction, parameterValues) {
    var delimiter = ";";
    if (defined(catalogFunction.delimiter))
    {
        delimiter = catalogFunction.delimiter;
    }
    return catalogFunction.parameters.map(function(parameter) {
        var value = parameterValues[parameter.id];
        if (defined(parameter.overrideValue)) {
...
sending an email to <a href="mailto:'+catalogFunction.terria.supportEmail+'">'+catalogFunction.terria.supportEmail+'</a>.</p>'
    ...
    // There seem to be different ways of representing status depending on how the WPS service was implemented. I blame
    // the lack of examples in the OGC standard. I can't tell which way is more compliant. For reference, one way is:
    //   <wps:Status creationTime="2016-09-15T03:38:07Z">
    //      <wps:ProcessSucceeded>PyWPS Process finished</wps:ProcessSucceeded>
    //   </wps:Status>
    // while another way is:
    //	<wps:StatusInfo creationTime="2016-09-15T03:33:22">
	//      <wps:JobID>18043</wps:JobID>
    // 	    <wps:Status>ProcessSucceeded</wps:Status>
	//  </wps:StatusInfo>
    //  Here, we just handle both cases.
    ...
    if (defined(status.ProcessFailed) || status === "ProcessFailed") {
    ...
    } else if (defined(status.ProcessSucceeded) || status === "ProcessSucceeded") {
