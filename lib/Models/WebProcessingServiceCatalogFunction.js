...
WebProcessingServiceCatalogFunction.parameterConverters = [
    {
        ...
        inputToFunctionParameter: function(catalogFunction, input) {
            ...
            if (input.Identifier === "regionType") {
                return undefined;
            }
    ...
    },
    {
        ...
        inputToFunctionParameter: function(catalogFunction, input) {
            ...
            var allowedValues = input.LiteralData.AllowedValues;
            var validRegionTypes;
            if (defined(allowedValues)) {
                var allowed = allowedValues.Value.slice();
                if (typeof(allowed) === "string") {
                    allowed = [allowed];
                }
                validRegionTypes = allowed;
            }
            var regionTypeParameterVals = new RegionTypeParameter({
               terria: catalogFunction.terria,
               catalogFunction: catalogFunction,
               id: 'regionType',
               name: 'Region Type',
               description: 'The type of region to analyze.',
               validRegionTypes: validRegionTypes
            });
            catalogFunction.regionTypeParameter = regionTypeParameterVals;
            return regionTypeParameterVals;
        },
        functionParameterToInput: function(catalogFunction, parameter, value) {
            if (!defined(value) || value === '') {
                return undefined;
            }
            return parameter.id + '=' + value.regionType;
        }
    },
    {
        id: 'DateTime',
    ...
    },
    {
         ...
         inputToFunctionParameter: function(catalogFunction, input) {
             ...
             if (schema.substring(schema.lastIndexOf('#') + 1) !== 'multipolygon') {
                 return undefined;
             }

             if (input.Identifier.indexOf('region') === -1) {
                 return undefined;
             }

             if (!defined(catalogFunction.regionTypeParameter)) {
                 throw new DeveloperError('No regionTypeParameter found, but regionGeometry requires a regionTypeParameter.');
             }
             catalogFunction.regionTypeParameter.showInUi = false;
             return new RegionParameter({
                 ...
                 description: input.Abstract,
                 isRequired: (input.minOccurs | 0) > 0,
                 regionProvider: catalogFunction.regionTypeParameter
             });
         },
         functionParameterToInput: function(catalogFunction, parameter, value) {
             if (!defined(value) || value === '' || !defined(value.id)) {
                 return undefined;
             }

             // Value is an object with id and fid. We want an actual polygon for WPS, but not for analytics.
             return parameter.regionProvider.getRegionFeature(catalogFunction.terria, value, undefined).then(function(feature) {
                 var regionParameterString = parameter.id + '=' + JSON.stringify({
    ...
    },
    {
         ...
         inputToFunctionParameter: function(catalogFunction, input) {
             ...
             // Don't pick up region input as polygons
             if (input.Identifier.indexOf('region') !== -1) {
                 return undefined;
             }
...
WebProcessingServiceCatalogFunction.prototype.invoke = function() {
    ...
    when.all(createWpsDataInputsFromParameters(this)).then(function(dataInputs) {
        ...
        var url = proxyCatalogItemUrl(that, uri.toString(), '1d');
