  ...
  return loadXML(url).then(function(xml) {
    ...
    if (
      ...
      throw new TerriaError({
        ...
        message:
          "\
An error occurred while invoking DescribeProcess on the WPS server for process name " +
          that.name +
          ".  The server's response does not appear to be a valid DescribeProcess document.  \
...
function handleExecuteResponse(
  ...
  if (
    ...
    throw new TerriaError({
      ...
      message:
        "\
An error occurred while accessing the status location on the WPS server for process name " +
        catalogFunction.name +
        ".  The server's response does not appear to be a valid ExecuteResponse document.  \
<p>This error may also indicate that the processing server you specified is temporarily unavailable or there is a \
