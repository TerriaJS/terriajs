 ...
 * @param {Boolean} [options.allValue] Whether or not to have OneForAll.value in this.value
 * @param {Boolean} [options.allCascade] Whether OneForAll being true passes all ParameterList.id: true values through to this.value
...
function formatAsStringOfKeyEqualsBoolean(value) {
  return value.map(param => Object.keys(param).toString() + "=" + 
            param[Object.keys(param)].toString()
           ).join('&');
}
...
function makeBooleanEditorArrayFromJson(objectArray,inTerria,inCatalogFunction) {
    var ParameterObjects = [];
    if (typeof objectArray !== 'undefined' && objectArray.length > 0) {
        objectArray.forEach(function(object) {
            ParameterObjects.push(makeBooleanEditorFromJson(object,inTerria,inCatalogFunction));
        });
    }
...
BooleanParameterGroup.prototype.getValue = function() {
    var param_values = this.ParameterList.map(
    ...
    if (this.allValue === true && this.OneForAll.value === true) {
        if (this.allCascade === true) {
            ...
            param_values = [{[this.OneForAll.id] : this.OneForAll.value}];
        }
    }
