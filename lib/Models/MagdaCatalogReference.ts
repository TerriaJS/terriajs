...
export default class MagdaCatalogReference extends UrlMixin(
  ...
  protected forceLoadReference(
    ...
    const target = this.createTargetFromRecord(existingRecord, previousTarget);
    if (target !== undefined) {
    ...
    return this.loadMagdaRecord({
      ...
      return this.createTargetFromRecord(record, previousTarget);
    });
  }

  protected createTargetFromRecord(record: JsonObject | undefined, previousTarget: BaseModel | undefined): BaseModel | undefined {
    if (record === undefined) {
      return undefined;
    }
    ...
    if (distributions) {
      ...
      if (match !== undefined && match.format.definition && isJsonString(match.format.definition.type)) {
