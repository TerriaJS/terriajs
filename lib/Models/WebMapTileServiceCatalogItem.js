...
WebMapTileServiceCatalogItem.prototype.updateFromCapabilities = function(capabilities, overwrite, thisLayer) {
    ...
    var service = defaultValue(capabilities.Service, {});
    ...
    updateValue(this, overwrite, 'format', getTileFormat(thisLayer.Format));

    var tileMatrixSet = getTileMatrixSet(thisLayer.TileMatrixSetLink, capabilities);
    updateValue(this, overwrite, 'tileMatrixSetID', tileMatrixSet.id);
    updateValue(this, overwrite, 'tileMatrixSetLabels', tileMatrixSet.labels);

    var defaultStyle = getDefaultStyle(thisLayer.Style);
    if (defaultStyle) {
        updateValue(this, overwrite, 'style', defaultValue(defaultStyle.Identifier, ''));
        updateValue(this, overwrite, 'legendUrl', getLegend(defaultStyle));
    } else {
        updateValue(this, overwrite, 'style', '');
    }
};
...
/**
 * Process a returned capabilities document, adding a "usableTileMatrixSets" element of tilesets that meet our needs.
 * @param {String} xml XML "GetCapabilities" response.
 * @returns {Object} json Converted response document.
 */
...
function getTileMatrixSet(tileMatrixSetLinks, capabilities) {
    var set = arrayify(tileMatrixSetLinks)
        .find(l => capabilities.usableTileMatrixSets[l.TileMatrixSet]) || {};

    return {
        id: defaultValue(set.TileMatrixSet, 'urn:ogc:def:wkss:OGC:1.0:GoogleMapsCompatible'),
        labels: capabilities.usableTileMatrixSets[set.TileMatrixSet] // possibly undefined, which is ok
    };
}

/**
 * Find the style marked 'isDefault'.
 * @param {Object|Array} styles 
 */
function getDefaultStyle(styles) {
    return arrayify(styles).find(s => s.isDefault);
}
/**
 * Create a LegendURL from a single style object.
 * @param {Object} style 
 */
function getLegend(style) {
    ...
    var legendData = arrayify(style.legendURL)[0];
    if (defined(legendData)) {
        return new LegendUrl(legendData.href, legendData.format);
    }
}


/**
 * Find a suitable image format.  Prefer PNG but fall back on JPEG if necessary
 * @param {String|Array} format Format element from capabilities document.
 * @returns {String} Preferred MIME type 
 */
function getTileFormat(formats) {
    formats = arrayify(formats);
...
function findLayer(json, name) {
    if (defined(json.Contents) && defined(json.Contents.Layer)) {
        return json.Contents.Layer.find(l => l.Identifier === name || l.Title === name);
    }
}
...
/**
 * Sets a property to a non-undefined value.
 * @param {Object} item 
 * @param {Boolean} overwrite If not truthy, don't change existing values.
 * @param {String} propertyName Property to set.
 * @param {String} propertyValue Value to set it to.
 */
