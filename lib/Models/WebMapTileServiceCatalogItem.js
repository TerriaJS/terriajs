...
var WebMapTileServiceCatalogItem = function(terria) {
    ...
    /**
     * Gets or sets the labels to each for each level in the matrix set.  This property is observable.
     * @type {[type]}
     */
    ...
    knockout.track(this, [
        ...
        'layer', 'style', 'tileMatrixSetID', 'parameters', 'getFeatureInfoFormats',
...
WebMapTileServiceCatalogItem.defaultUpdaters.tilingScheme = function(wmsItem, json, propertyName, options) {
...
WebMapTileServiceCatalogItem.defaultUpdaters.getFeatureInfoFormats = function(wmsItem, json, propertyName, options) {
    var formats = [];
...
WebMapTileServiceCatalogItem.defaultSerializers.dataUrl = function(wmsItem, json, propertyName) {
    json.dataUrl = wmsItem._dataUrl;
};
WebMapTileServiceCatalogItem.defaultSerializers.dataUrlType = function(wmsItem, json, propertyName) {
    json.dataUrlType = wmsItem._dataUrlType;
};
WebMapTileServiceCatalogItem.defaultSerializers.metadataUrl = function(wmsItem, json, propertyName) {
    json.metadataUrl = wmsItem._metadataUrl;
};
WebMapTileServiceCatalogItem.defaultSerializers.legendUrl = function(wmsItem, json, propertyName) {
    json.legendUrl = wmsItem._legendUrl;
};
WebMapTileServiceCatalogItem.defaultSerializers.tilingScheme = function(wmsItem, json, propertyName) {
...
WebMapTileServiceCatalogItem.prototype.updateFromCapabilities = function(capabilities, overwrite, thisLayer) {
    ...
    for (var i = 0; i < tileMatrixSetLinks.length; ++i) {
...
WebMapTileServiceCatalogItem.prototype._createImageryProvider = function() {
    var parameters = combine(this.parameters, WebMapTileServiceCatalogItem.defaultParameters);
...
WebMapTileServiceCatalogItem.defaultParameters = {
    transparent: true,
    format: 'image/png',
...
function requestMetadata(wmtsItem) {
    ...
    result.promise = when(metadata, function(json) {
        ...
        if (defined(json.Capability)) {
            layer = findLayer(json.Content.Layer, wmtsItem.layer);
        }
        if (layer) {
