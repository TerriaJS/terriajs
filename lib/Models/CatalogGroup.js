...
CatalogGroup.prototype.add = function(item) {
    ...
    if (item.isGroup) {
        if (typeof(this.itemProperties) === 'object') {
            promises.push(item.updateFromJson({'itemProperties' : this.itemProperties})); // Or item.itemProperties = this.itemProperties;
        }
        if (typeof(this.childGroupProperties) === 'object') {
            promises.push(item.updateFromJson(this.childGroupProperties));
        }
    }
    else {
        if (typeof(this.itemProperties) === 'object') {
