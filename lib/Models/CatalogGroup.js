...
var CatalogGroup = function(terria) {
    ...
    this.childGroupProperties = undefined;
...
CatalogGroup.prototype.add = function(item) {
    ...
    if (item.isGroup) {
        if (typeof(this.itemProperties) === 'object') {
            promises.push(item.updateFromJson({'itemProperties' : this.itemProperties}));
        }
        if (typeof(this.childGroupProperties) === 'object') {
            promises.push(item.updateFromJson(this.childGroupProperties)); // Each subgroup loses 1 layer of nesting on childGroupProperties
        }
    }
    else {
        if (typeof(this.itemProperties) === 'object') {
            promises.push(item.updateFromJson(this.itemProperties));
        }
    }
