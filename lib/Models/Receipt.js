...
function filterHotspots({ sector }, viewState) {
  getHotspotsCatalogItems(viewState.terria).map(item => {
...
function openStorySummary(params, viewState) {
  const hotspot = getHotspotsCatalogGroup(viewState.terria)
...
async function exitStory(viewState) {
  // Hide & reset story
  viewState.storyShown = false;
  ...
  const hotspotGroup = getHotspotsCatalogGroup(viewState.terria);
  viewState.terria.catalog.group.items.forEach(item => {
    if (item.name === "EU Lines!- Mapbox Style") {
      item.isEnabled = true;
      item.isShown = true;
      item.isHiddden = false;
    } else if (item === hotspotGroup) {
      item.isEnabled = true;
      item.isShown = true;
...
function getHotspotsCatalogItems(terria) {
  return getHotspotsCatalogGroup(terria).items;
}
