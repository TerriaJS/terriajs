...
function RCChangeUrlParams(params = getUrlParams(), viewState) {
  console.log("RCChangeUrlParams 🏋🏻Url Changed:", params, window.location.href);
  ...
  //     viewState.currentStoryId = params.page - 1;
...
function setSelectedHotspot(params, viewState) {
  const hotspotFeature = getCatalogMemberByName(
    viewState.terria.catalog,
    "Hotspot Overview"
  )
    .items.find(item => item.name === params.sectorName)
    ._dataSource.entities.values.find(
      feature =>
        feature.properties["_rc-story-id"]._value === parseInt(params.storyID)
    );
  if (!hotspotFeature) {
    console.log(
...
function setSelectedStory(params, viewState) {
  ...
  if (group) {
    ...
    return loadTerriaStoryFromJSON(params, viewState.terria).then(() => {
...
function loadTerriaStoryFromJSON(params, terria) {
  const storyUrl = getLinkToStoryJSONFromParams(params, terria);
  const hashProperties = { share: storyUrl, playStory: "1" }; // playStory is the pagenumber?
...
function getCurrentStoryCatalogGroup(catalog, params) {
  ...
  var story_group_name = `Story ${String(params.storyID).padStart(3, '0')} data`;
