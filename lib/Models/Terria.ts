...
export default class Terria {
    ...
    addModel(model: BaseModel) {
        if (model.uniqueId === undefined) {
            throw new DeveloperError(
                "A model without a `uniqueId` cannot be added."
            );
        }

        if (this.models.has(model.uniqueId)) {
            throw new RuntimeError(
                "A model with the specified ID already exists."
            );
        }
    ...
    applyInitData({
        ...
        if (Array.isArray(initData.corsDomains)) {
            this.corsProxy.corsDomains.push(
                ...(<string[]>initData.corsDomains)
            );
        }
            ...
            return runInAction(() => {
                    ...
                    workbench.map(modelId => {
                        if (typeof modelId !== "string") {
                            throw new TerriaError({
                                ...
                                message:
                                    "A model ID in the workbench list is not a string."
                            });
                        }
                ...
                this.timelineStack.items = this.workbench.items
                    .filter(item => {
                        return (
                            item.uniqueId &&
                            timeline.indexOf(item.uniqueId) >= 0
                        );
        ...
        return proxyableDomainsPromise.then((proxyableDomains: any | void) => {
            if (proxyableDomains) {
                ...
                proxyableDomains.allowProxyFor =
                    proxyableDomains.allowProxyFor ||
                    proxyableDomains.proxyableDomains;
            }
...
const loadInitSource = createTransformer(
    (initSource: InitSource): Promise<JsonObject | undefined> => {
        ...
        if (isInitUrl(initSource)) {
            ...
            promise = initSource.options.reduce(
                (previousOptionPromise, option) => {
