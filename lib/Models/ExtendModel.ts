...
/**
 * A mixin to extend a model with new traits.
 *
 * @param Base The model to extend
 * @param Traits The traits class for the extended model.
 *        This should be a mix of the new traits and the  base class traits (and not just
 *        the new traits).
 * @return The extended model with all the `Traits` and the methods from base class.
 */
export default function ExtendModel<
  T extends TraitsConstructor<ModelTraits>,
  BaseClass extends ModelConstructor<BaseModel>
>(
  Base: BaseClass,
  Traits: T
): ModelConstructor<InstanceType<BaseClass> & ModelType<InstanceType<T>>> {
  type Traits = InstanceType<T>;
  type StratumTraits = StratumFromTraits<Traits>;

  const MixinBase = Base as new (...a: any[]) => BaseModel;

  abstract class Model extends MixinBase implements ModelInterface<Traits> {
