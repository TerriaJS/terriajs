 ...
 */
CatalogGroup.prototype.getNextColor = function() {
    ...
    // remove colors in use, looking through to the concepts
    for (var itemIndex = this.items.length - 1; itemIndex >= 0; itemIndex--) {
        var item = this.items[itemIndex];
        for (var conceptIndex = item.concepts.length - 1; conceptIndex >= 0; conceptIndex--) {
            var concept = item.concepts[conceptIndex];
            for (var variableConceptIndex = concept.items.length - 1; variableConceptIndex >= 0; variableConceptIndex--) {
                var variableConcept = concept.items[variableConceptIndex];
                if (defined(variableConcept.color)) {
                    var colorIndex = colors.indexOf(variableConcept.color);
                    if (colorIndex > -1) {
