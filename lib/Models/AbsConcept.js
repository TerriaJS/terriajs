...
var AbsConcept = function(options) {
    ...
    if (defined(options.codes)) {
        ...
        buildConceptTree(this, options.filter, options.id, options.codes);
    }
...
function buildConceptTree(parent, filter, conceptId, codes) {
    ...
    for (var i = 0; i < codes.length; ++i) {
        ...
        if (codes[i].parentCode === parentCode) {
            ...
            var codeFilter = conceptId + '.' + absCode.code;
            ...
            buildConceptTree(absCode, filter, conceptId, codes);
        }
    }
}
