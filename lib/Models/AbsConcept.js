 ...
 * @param {String[]} [options.code] The concept's machine-identifying code, eg. "REGIONTYPE".
...
var AbsConcept = function(options) {
    ...
    Concept.call(this, options.name || options.code);
    ...
    if (defined(options.codes)) {
        ...
        buildConceptTree(this, options.filter, options.code, options.codes);
    }
...
function buildConceptTree(parent, filter, conceptCode, codes) {
    ...
    for (var i = 0; i < codes.length; ++i) {
        ...
        if (codes[i].parentCode === parentCode) {
            ...
            buildConceptTree(absCode, filter, conceptCode, codes);
        }
    }
}
