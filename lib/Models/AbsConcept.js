...
var AbsConcept = function(options) {
    ...
    if (defined(options.codes)) {
        ...
        buildConceptTree(this, options.filter, this, options.codes);
    }

    // TODO: Do we need to track all these?
    knockout.track(this, ['id', 'items', 'isOpen', 'isUnique']);  // name, isSelectable already tracked by Concept
    ...
    knockout.getObservable(this, 'activeItems').subscribe(function(newConcepts) {
        console.log('getObservable AbsConcept activeItems triggered');
    }, this);
...
function buildConceptTree(parent, filter, concept, codes) {
    ...
    for (var i = 0; i < codes.length; ++i) {
        ...
        if (codes[i].parentCode === parentCode) {
            ...
            buildConceptTree(absCode, filter, concept, codes);
        }
    }
}
