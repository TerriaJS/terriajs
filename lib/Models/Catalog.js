...
var ChartDataGroup = require("./ChartDataGroup");
...
Catalog.prototype._updateAndEnable = function(shareKey, itemJson) {
  ...
  if (existingMember) {
    ...
    defined(itemJson.url) &&
    defined(itemJson.tableStyle) &&
    itemJson.type === "csv"
  ) {
    // we have a csv with url so regenerate
    const newItem = new CsvCatalogItem(this.terria, itemJson.url, {
      tableStyle: itemJson.tableStyle
    });
    const group = this.chartDataGroup;
    group.add(newItem);
    newItem.name = itemJson.name;
    newItem.isLoading = true;
    newItem.isMappable = false;
    this.addChartableItem(newItem); // Notify the chart panel so it shows "loading".
