...
function makeImageryLayerFromParts(parts: ImageryParts): Cesium.ImageryLayer {
    ...
    // This should only be called on ImageryParts that are observable. I think we can assume
    autorun(() => {
        console.log(`Updating alpha/show from ${layer.alpha} to ${parts.alpha}`);
 ...
 */
// var CesiumOld = function(terria, viewer) {
...
//     this._workbenchMapItemsSubscription = autorun(() => {
//         const catalogItems = [...terria.workbench.items, terria.baseMap];
//         const allMapItems = [].concat(...catalogItems.map(item => (item && item.mapItems) !== undefined ? item.mapItems : []));
//         // TODO: Look up the type in a map and call the associated function.
//         //       That way the supported types of map items is extensible.
//         // This is the Cesium ImageryLayer, not our Typescript one
//         const imageryLayers = allMapItems.filter(mapItem => mapItem instanceof ImageryLayer);
//         //const dataSources = allMapItems.filter(mapItem => mapItem instanceof DataSource);
...
//         // Iterate backwards so that adding multiple layers adds them in increasing cesium index order
//         for (let modelIndex = imageryLayers.length-1; modelIndex >= 0; modelIndex--) {
//             const mapItem = imageryLayers[modelIndex];

//             const targetCesiumIndex = imageryLayers.length - modelIndex - 1;
//             const currentCesiumIndex = this.scene.imageryLayers.indexOf(mapItem);
//             if (currentCesiumIndex === -1) {
//                 this.scene.imageryLayers.add(mapItem, targetCesiumIndex);
//             } else if (currentCesiumIndex > targetCesiumIndex) {
//                 console.log(`Lowering ${mapItem.fromCatalogItem} from ${currentCesiumIndex} to ${targetCesiumIndex}`);
//                 for (let j = currentCesiumIndex; j > targetCesiumIndex; j--) {
//                     this.scene.imageryLayers.lower(mapItem);
//                 }
//             } else if (currentCesiumIndex < targetCesiumIndex) {
//                 console.log(`Raising ${mapItem.fromCatalogItem} from ${currentCesiumIndex} to ${targetCesiumIndex}`);
//                 for (let j = currentCesiumIndex; j < targetCesiumIndex; j++) {
//                     this.scene.imageryLayers.raise(mapItem);
//                 }
//             }
...
//         }
//         // Log imagery layers in Terria and imagery layers in Cesium
//         console.log(`Model layer imagery layers:\n${imageryLayers.map(il => il.fromCatalogItem).join('\n')}`);
//         let logString = 'Cesium imagery layers:\n';
//         for (let i = this.scene.imageryLayers.length-1; i >= 0; i--) {
//             logString += `${this.scene.imageryLayers.get(i).fromCatalogItem}\n`;
//         }
