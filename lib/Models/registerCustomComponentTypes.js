...
const registerCustomComponentTypes = function(terria) {
    ...
    function processChartNode(catalogItem, feature, node, children) {
        ...
        const sourceData = node.attribs['data'] ? JSON.parse(node.attribs['data'].replace(/&quot;/g, '"')) : undefined;
    ...
    function sourceDataAsTableStructure(sourceData) {
        // sourceData can be either json (starts with a '[') or csv format (contains a '\n').
        if (!defined(sourceData) || sourceData.length < 2) {
            return;
        }
        if (sourceData[0] === '[') {
            // Treat as json.
            return TableStructure.fromJson(sourceData);
        }
        if (sourceData.indexOf('\n') >=0) {
            // Treat as csv.
            return TableStructure.fromCsv(sourceData);
        }
    }
