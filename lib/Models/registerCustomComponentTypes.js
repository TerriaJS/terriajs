...
const Chart = require('../ReactViews/Custom/Chart/Chart');
const ChartExpandAndDownloadButtons = require('../ReactViews/Custom/Chart/ChartExpandAndDownloadButtons');
const Collapsible = require('../ReactViews/Custom/Collapsible/Collapsible');
...
import ChartPreviewStyles from '../ReactViews/Custom/Chart/chart-preview.scss';
 ...
 * - [poll-replace-data]: Either 'true' or 'false' (case sensitive). Pass 'true' to completely replace the data, 'false' to update
...
const registerCustomComponentTypes = function(terria) {
    ...
    function processChartNode(context, node, children) {
        ...
        const updateCounter = CustomComponents.getUpdateCounter(context.updateCounters, Chart, keyProps);
        return React.createElement('div', {
            key: (sources && (sources.join('|') + (defined(yColumns) ? yColumns.join('|') : ''))) || 'chart-wrapper',
            ...
            React.createElement(ChartExpandAndDownloadButtons, {
                ...
                pollReplaceData: (node.attribs['poll-replace-data'] === 'true'),
                updateCounter: updateCounter,  // Change this to trigger an update.
                // pollSource: node.attribs['poll-src'],
                // pollSeconds: node.attribs['poll-seconds']
            }),
