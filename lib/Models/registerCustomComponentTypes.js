...
const registerCustomComponentTypes = function(terria) {
    ...
    function processChartNode(catalogItem, feature, node, children) {
        ...
        const sourceData = node.attribs['source-data'] ? JSON.parse(node.attribs['source-data'].replace(/&quot;/g, '"')) : undefined;
        ...
        return React.createElement('div', {
            ...
            React.createElement(Chart, {
                ...
                sourceData: defined(sourceData) ? sourceData[0] : undefined,
                xColumn: node.attribs['x-column'],
                yColumns: yColumns,
                styling: styling,
            ...
            //
            {
                ...
                processNode: function(catalogItem, feature, node, children) {
                    ...
                    const revisedChildren = [
                        React.createElement('div', {key: 'title', className: 'chart-title-from-table'}, title)
                    ].concat(children);
                    return React.createElement(node.name, {key: 'chart', colSpan: 2, className: 'chart'}, node.data, revisedChildren);
                }
            },
            {
                shouldProcessNode: node => (
