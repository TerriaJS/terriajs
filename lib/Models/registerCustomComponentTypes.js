...
const chartAttributes = ['src', 'src-preview', 'sources', 'source-names', 'long-names', 'data', 'source-preview', 'source-default', 'title', 'x-column', 'y-column', 'y-columns'];
...
const registerCustomComponentTypes = function(terria) {
    ...
    function processChartNode(catalogItem, feature, node, children) {
        ...
        let sources = node.attribs.sources;
        let sourceNames = node.attribs['source-names'];
        let longSourceNames = node.attribs['long-names'];
        if (defined(sources)) {
            sources = sources.split(',');
            sourceNames = defined(sourceNames) ? sourceNames.split(',') : undefined;
            longSourceNames = defined(longSourceNames) ? longSourceNames.split(',') : undefined;
