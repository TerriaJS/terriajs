 ...
 * @constructor
 *
 * For user drawings, which includes lines/a polygon
 * @param {Terria} terria The Terria instance.
 * @param {Object} options Object with the following properties, all of which are optional:
 * @param {String} [options.messageHeader] Heading for the dialog this pops up when in user drawing mode.
 * @param {Object} [options.userPointOptions] Properties for an Entity of type point, such as colour and outline width.
 * @param {Object} [options.onPointClicked] Callback for when a point is clicked. Function that takes a CustomDataSource.
 * @param {Object} [options.onCleanup] Callback for when clean up is happening, i.e., for done or cancel.
 * @param {Object} [options.onMakeDialogMessage] Callback for when the dialog is displayed, to provide a custom message.
 */
var UserDrawing = function(terria, options) {

    if (!defined(terria)) {
        throw new DeveloperError('Terria instance is required.');
    }

    // Client object is not required to pass options.
    if (!defined(options)) {
        options = {};
    }

    var defaultUserPointOptions = {
                                       color: Color.WHITE,
                                       pixelSize: 8,
    ...
    /**
     * The text that appears at the top of the dialog for when drawmode is active.
     * @type {String}
     * @default "Draw on Map"
     */
    ...
    /**
     * What the point looks like on the map.
     * @type {Object}
     * @default { color: Color.WHITE, pixelSize: 8, outlineColor: Color.BLACK, outlineWidth: 2 }
     */
    this.userPointOptions = defaultValue(options.userPointOptions, defaultUserPointOptions);
     ...
     * @default undefined
     */
    this.onPointClicked = options.onPointClickedCallback;
     ...
     * @default undefined
     */
    this.onCleanUp = options.onCleanUpCallback;
     ...
     * @default undefined
     */
    this.onMakeDialogMessage = options.onMakeDialogMessageCallback;
        ...
        when(pickedFeatures.allFeaturesAvailablePromise, function() {
            if (defined(pickedFeatures.pickPosition)) {
                ...
                var firstPointEntity = new Entity({
                    name: 'First Point',
                    //point: that.userPointOptions,
                    position: pickedPoint,
                    billboard : {
                            image : '../../images/selected-point.png',
                            scale : 0.65,
...
UserDrawing.prototype._cleanUp = function() {
    ...
    // Allow client class to clean up too
 ...
 * @param {PickedFeatures} Feature/s that are under the point the user picked
