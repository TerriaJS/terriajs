...
var TableColumn = function(name, values, options) {
    ...
    this.meta_definition = this.options.meta_definition;
    this.meta_name = this.options.meta_name;
    this.meta_datatype = this.options.meta_datatype;
    ...
    if (defined(values) && this._type === VarType.TIME) {
        ...
        if (this.dates.length === 0) {
            ...
            // Calculate default end dates and timeIntervals, and define a clock.
            if (!this.options.noFinishDates) {
    ...
    knockout.track(this, ['_type', '_values', 'units']);
};
...
function calculateFinishDates(originalJulianDates) {
    ...
    return result;
    // return originalJulianDates.map(function(startDate, index) {
    //     if (defined(startDate)) {
    //         for (var i = 0; i < sortedUniqueJulianDates.length; i++) {
