...
*/
DataTable.prototype.getDataVariableList = function () {
    return this.getVariableNamesByType(VarType.SCALAR);
};
...
DataTable.prototype.loadJson = function (jsonTable) {
    ...
    this.selected.data = this.getVariableNamesByType(VarType.SCALAR)[0];
...
DataTable.prototype.getPointList = function (maxPoints) {
    ...
    if (defined(this.variables[this.selected.data])) {
        vals = this.variables[this.selected.data].vals;
    }
    if (!defined(maxPoints)) {
        maxPoints = lon.length;
    }

    var ret = [];
    for (var i = 0; i < lon.length && i < maxPoints; i++) {
        var rec = {
            val: defined(vals) ? vals[i] : undefined,
            time: time ? time[i] : undefined,
            pos: [lon ? lon[i] : 0.0, lat ? lat[i] : 0.0, alt ? alt[i] : 0.0],
            row: i
        };
        if (defined(vals) && (this.isNoData(rec.pos[0]) || this.isNoData(rec.pos[1]))) {
            continue;
        }
        ret.push(rec);
    }
