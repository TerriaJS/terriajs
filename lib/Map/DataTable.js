...
var DataTable = function(options) {
    ...
    this._allowMultiple = defaultValue(options && options.allowMultiple, false);
...
*
* @param {String} varName The variable name which has been toggled.
* @param {Boolean} [makeActive=True] Has this been made active or inactive? Note if allowMultiple is True, this is ignored.
*
*/
    ...
    if (defined(this.variables[varName])) {
        if (this.allowMultiple) {
            ...
            if (index === -1) {
                ...
                this.selectedNames = this.selectedNames.splice(index, 1).slice(0);  // final slice clones the array
            }
        } else {
            // If this don't allow multiple selections, just reset the list to the new selection.
            if (makeActive === false) {
                this.selectedNames = [];
            } else {
