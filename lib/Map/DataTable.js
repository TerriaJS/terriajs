...
var defaultVarTypes = [VarType.ALT, VarType.SCALAR, VarType.ENUM];
 ...
 * @param {Boolean} [options.allowMultiple=false] Set to true if more than one variable can be selected at a time.
 * @param {ChartCatalogGroup} [options.chartCatalogGroup] The instance of the chartCatalogGroup with which this data table is associated (if any).
 *        If set, sets the variable's color when toggled.
 */
var DataTable = function(options) {
    ...
    this.allowMultiple = defaultValue(options && options.allowMultiple, false);
    this.chartCatalogGroup = defaultValue(options && options.chartCatalogGroup, undefined);

    var name = options && options.name;

    knockout.track(this, ['variables', 'selected', 'selectedNames']);
        ...
        get : function() {
            ...
            if (varNames.length > 0) {
                ...
                for (var i = 0; i < varNames.length; i++) {
                    var active = that.selectedNames.indexOf(varNames[i]) > -1;
                    concept.addVariable(varNames[i], active, that.variables[varNames[i]].color);
                }
            }
            return concept;
        }
...
DataTable.prototype.getDataMinValue = function (varName) {
    var name = varName;
    if (!defined(varName) && this.selectedNames.length === 1) {
        name = this.selectedNames[0];
    }
    if (defined(name)) {
...
* @param {String} varName The name of the variable concept which has been toggled.
* @param {Boolean} makeActive Whether to make this variable active or not. Only used if the `allowMultiple` flag is NOT set.
*/
DataTable.prototype.setDataVariable = function(varIndex, varName, makeActive) {
    // Note we take care to ensure selectedNames is a new object so it triggers knockout's tracking.
    var thisVariable = this.variables[varName];
    if (defined(thisVariable)) {
        if (this.allowMultiple) {
            // If it's not in the list, add it; if it is, remove it.
            var index = this.selectedNames.indexOf(varName);
            if (index === -1) {
                if (defined(this.chartCatalogGroup) && !defined(thisVariable.color)) {
                    thisVariable.color = this.chartCatalogGroup.getNextColor();
                }
                this.selectedNames = this.selectedNames.concat([varName]);  // use concat not push to clone the array.
            } else {
                this.selectedNames.splice(index, 1);
                this.selectedNames = this.selectedNames.slice();  // slice to clone the array.
            }
        } else {
            // If this doesn't allow multiple selections, just reset the list to the new selection.
            if (makeActive === false) {
                this.selectedNames = [];
            } else {
                if (defined(this.chartCatalogGroup) && !defined(thisVariable.color)) {
...
* @exception {DeveloperError} If more than one variable is selected (which is possible if allowMultiple = true).
*/
DataTable.prototype.getDataVariable = function () {
    ...
    throw new DeveloperError('DataTable.getDataVariable called with more than one variable selected.');
};

/**
* Get all current variable names.
*
* @returns {String[]} The current variable names.
*/
DataTable.prototype.getDataVariables = function () {
...
* @returns {Array} An array of point objects (with keys: val, time, pos=[lon, lat, alt], row) based on the selected variables.
*/
DataTable.prototype.getPointList = function (maxPoints) {
    ...
    var vals = (this.selectedNames.length === 1) ? this.variables[this.selectedNames[0]].vals : undefined;
