...
export function createRegionProviderFromGeoJson(regionProviderList, geoJson, property, terria) {
    let fid = 0;
    geoJson.features.forEach(f => f.properties['FID'] = fid++);
    const tileIndex = geojsonvt(geoJson);
    geoJsonCounter++;
    const rp = new RegionProvider(`geojson_${geoJsonCounter}_${property}`, {
        nameProp: property,
        regionProp: property,
...
export default function createAllRegionProvidersFromGeoJson(geoJson, terria) {
    ...
    if (uniqueProperties.length > 0) {
        ...
        return fromUrl(terria.configParameters.regionMappingDefinitionsUrl, terria.corsProxy).then(rpList => {
            uniqueProperties.forEach(prop => {
                createRegionProviderFromGeoJson(rpList, geoJson, prop, terria);
            });
        });
    }
