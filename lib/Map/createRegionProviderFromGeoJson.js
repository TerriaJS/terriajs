...
export default function createRegionProviderFromGeoJson(
  terria,
  regionProviderList,
  geoJson,
  property,
  ...
  return import(/* webpackChunkName: "geojson-vt" */ "geojson-vt").then(
    ({ default: geojsonvt }) => {
      const tileIndex = geojsonvt(geoJson);
      const rp = new RegionProvider(
        rpType || `geojson_${geoJsonCounter++}_${property}`,
        {
          nameProp: property,
          regionProp: property,
      ...
      processRegionIds(
        rp,
        geoJson.features.map(f => f.properties[property]),
        undefined,
        "serverReplacements"
      );
...
export function createAllRegionProvidersFromGeoJson(terria, geoJson) {
  ...
  var uniqueProperties = properties
    .filter(prop =>
      ...
      geoJson.features.every(f =>
        ["number", "string"].includes(typeof f.properties[prop])
      )
    )
    .filter(
      p =>
        // Filter out properties that aren't unique per row
        new Set(geoJson.features.map(f => f.properties[p])).size ===
