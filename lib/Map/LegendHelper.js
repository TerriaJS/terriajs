...
function generateLegend(legendHelper) {
    ...
    if (!defined(legendHelper.tableStyle.colorMap)) {
        ...
        if (!defined(tableColumn) || (tableColumn.type === VarType.ENUM)) {
            ...
            if (defined(tableColumn)) {
                legendHelper.tableStyle.colorBins = Math.min(tableColumn.values.length, defaultEnumColorCodes.length);
            } else {
                legendHelper.tableStyle.colorBins = defaultEnumColorCodes.length;
            }
    ...
    legendHelper._binColors = buildBinColors(legendHelper);
 ...
 * @param  {Number} [value] The absolute value.
 * @return {Number[]} The color, as an array [r, g, b, a]. If no value is provided, or no color bins are defined, uses a default color.
 */
 ...
 */
function buildBinColors(legendHelper) {
    var tableColumn = legendHelper.tableColumn;
    ...
    if (tableStyle.colorBins <= 0 || tableStyle.colorBinMethod.match(/none/i)) {
        return undefined;
    }
    var binColors = [];
    var i;
    ...
    } else if (!tableColumn.isNumeric) {
        // If there are no numerical values (eg. for type ENUM), use the index into the uniqueValues as the value.
        numericalValues = tableColumn.uniqueValues.map(function(value, index) { return index; });
    } else {
        numericalValues = tableColumn.values.filter(function(value) { return typeof value === 'number'; });
    }
    // Must ask for fewer clusters than the number of items.
    var binCount = Math.min(tableStyle.colorBins, numericalValues.length);
...
function buildLegendProps(tableColumn, tableStyle, colorMap, binColors) {
    ...
    if (!binColors) {
        ...
        // Non-numeric legend labels are centered on each box, and slightly separated.
        return {
            title: tableColumn.name,
            itemSpacing: 2,
            items: binColors.map(function(bin, index) {
                return {
                    title: tableColumn.uniqueValues[index],
