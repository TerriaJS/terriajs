...
function buildBinColors(tableColumn, tableStyle, colorGradient) {
    ...
    var i;
    var numericalValues;
    if (isNaN(tableColumn.minimumValue)) {
...
function buildLegendProps(tableColumn, tableStyle, colorMap, binColors) {
    ...
    } else if (isNaN(tableColumn.minimumValue)) {
        // Non-numeric legend labels are centered on each box, and slightly separated.
        // The test for this (isNaN(tableColumn.minimumValue)) must match the test in buildBinColors.
        return {
            title: tableColumn.name,
            itemSpacing: 2,
