...
function buildBinColors(legendHelper) {
    ...
    if (!defined(tableColumn) || !defined(tableColumn.values)) {
        ...
        // If there are no numerical values (eg. for type ENUM), use the index as the value.
        numericalValues = tableColumn.values.map(function(value, index) { return index; });
    } else {
    ...
    if (!binColors) {
        ...
        return {
            ...
            items: binColors.map(function(bin, index) {
                return { 
                    title: tableColumn.values[index],
