...
// ### When using Jenks etc, does this change? Do we just change the underlying dataImage?
// I think we do have to change this mapping - because potentially there would be a couple of colours squished into
// a single pixel or something in a very extreme case.
...
TableDataSource.prototype.getLegendGraphic = function () {
    ...
    function drawColorMap(ctx, gradH, gradW, colorMap) {
        var bins = 7;
        for (i = 0; i < bins; i++) {
            var color = ''
            //ctx.fillStyle = 'rgba(
