...
var TableDataSource = function(catalogItem) {
    ...
    // Track _region so that when it is discovered that region mapping applies, it updates the legendHelper via activeItems.
    knockout.track(this, ['_tableStructure', '_legendUrl', '_region']);
    ...
    knockout.getObservable(this._tableStructure, 'activeItems').subscribe(function(activeItems) {
        ...
        this._legendHelper = new LegendHelper(activeColumn, this._tableStyle, this._region ? true : false);
    ...
    if (this.hasLatitudeAndLongitude) {
        ...
        this.regionPromise.then(function(region) {
            if (region) {
                ...
                console.log('Found region match based on ' + region.column.name + (defined(region.disambigColumn) ? (' and ' + region.disambigColumn.name) : ''));
                region.column.type = VarType.REGION;
                if (defined(region.disambigColumn)) {
                    region.disambigColumn.type = VarType.REGION;
                }
                if (!defined(columnToActivate)) {
...
function setNewRegionImageryLayer(dataSource, layerIndex) {
    ...
    if (defined(tableStructure.activeItems[0])) {
        // Recolor the regions
        // TODO: currently only if there is a variable to base the recoloring on
        var regionIndices = dataSource.region.regionProvider.getRegionIndices(
            region.column.values,
            defined(region.disambigColumn) && region.disambigColumn.values
        );
        var regionValues = regionIndices;  // This is only appropriate for non-numeric values.
        if (!isNaN(tableStructure.activeItems[0].minimumValue)) {
            // This test (isNaN(tableColumn.minimumValue)) must match the tests in LegendHelper (better to centralise!).
            regionValues = regionIndices.map(function(i) { return tableStructure.activeItems[0].values[i]; });
        }
        colorFunction = dataSource.region.regionProvider.getColorLookupFunc(
            regionValues,
        ...
        ImageryProviderHooks.addPickFeaturesHook(regionImageryProvider, function(results) {
            ...
            for (var i = 0; i < results.length; ++i) {
                var uniqueId = results[i].data.properties[region.regionProvider.uniqueIdProp];
