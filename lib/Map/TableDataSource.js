...
TableDataSource.prototype.getLegendGraphic = function () {
    ...
    function drawLabels(ctx, height, width, top, bins, minVal, maxVal) {
        ...
        // draw min and max values big and dark
        ctx.setTransform(1,0,0,1,0,0);
        ctx.fillStyle = "#2F353C";
        ...
        // label each color a bit lighter and smaller
        ctx.fillStyle = "#666";
        ...
        for (var i = 0; i < bins.length; i++) {
            ...
            var binVal = (bins[i].upperBound - lowerBound) / 2 + lowerBound;
            var binValText = Math.round(binVal * 100 / 100).toString();

            var bi = bins.length - i - 1;
            ctx.fillText(binValText, width + 25, height * (bi + 0.5) / bins.length + top );
        }


        // we should consider showing some intermediate values, but linear interpolations that don't line
        // up with the color bins are not right.
        /* 
        var val;
    ...
    if (dataVar.varType === VarType.ENUM) {
        ...
        drawLabels(ctx, rampHeight, rampWidth, rampTop, this._binColors, minVal, maxVal);
    }
