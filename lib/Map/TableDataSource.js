...
TableDataSource.prototype.loadText = function (text) {
    ...
    //if there are no scalar variables then grab lon variable and set data color - suppresses legend
    if (!defined(this.dataVariable) && !defined(this.dataset.getDataVariable())) {
        this.dataVariable = this.dataset.selected.lon;
        var seed = 0;
        for (var i = 0; i < 100 && i < text.length; i++) {
        ...
        this.setColorGradient([{offset: 0.0, color: color.toCssColorString()}]);
    }
...
TableDataSource.prototype._czmlRecFromPoint = function (point) {
    ...
    if (this.dataset.hasTimeData()) {
        ...
        var finish = JulianDate.addMinutes(point.time, this.displayDuration, endScratch);
...
TableDataSource.prototype.getLegendGraphic = function () {
    ...
    if (!this._colorByValue || !defined(this.dataset.variables[this.dataset.selected.data])) {
        return undefined;
    }
    ...
    var s, ht;
    var segments = this.legendTicks+1;
    ...
    var minText = (minVal) === undefined ? 'und.' : (Math.round(minVal * 100) / 100).toString();
    var maxText = (maxVal) === undefined ? 'und.' : (Math.round(maxVal * 100) / 100).toString();
...
//    ctx.textAlign = 'right';
    ...
    ctx.fillText(maxText, gradW + 25, 15+h-gradH-15);
