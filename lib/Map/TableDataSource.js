...
TableDataSource.prototype.setDisplayStyle = function (style) {
    ...
    //TODO: we did this regardless to force rebuild of czml datasource - do we still need to?
...
function addEntityFromPoint(dataSource, point, name, properties) {
    ...
    var entity = new Entity({
        id: objectId,
        name: name,
        description: dataSource.describe(properties),
        position: Cartographic.fromDegrees(point.pos[0], point.pos[1])
    });
    ...
    if (dataSource.dataset.hasTimeData()) {
        ...
        entity.availability = JulianDate.toIso8601(start) + '/' + JulianDate.toIso8601(finish);
        show[1].interval = entity.availability;
    }
    else {
        show[0].boolean = true;
