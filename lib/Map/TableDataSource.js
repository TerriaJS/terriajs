...
TableDataSource.prototype._setColorBins = function() {
    ...
    var binCount = Math.min(this.colorBins, vals.length); // Must ask for fewer clusters than the number of items.
...
TableDataSource.prototype.getLegendGraphic = function () {
    ...
    function drawColorMap(ctx, height, width, top, left, binColors) {
        ...
        ctx.fillStyle = "black"; // just for testing
        ctx.fillRect(0, 0, ctx.canvas.width, ctx.canvas.height);
    ...
    if (this._binColors) {
        drawColorMap(ctx, rampHeight, rampWidth, rampTop, rampLeft, this._binColors);
    } else {
    ...
    drawVariableName(ctx, this.dataset.getDataVariable());
