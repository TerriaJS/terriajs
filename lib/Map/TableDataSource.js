...
var TableDataSource = function(catalogItem) {
    ...
    this._regionIDsPromises = undefined; // Used to only load regions ids once.
        ...
        get: function() {
            ...
            return regionProviderListPromise.then(function(regionProviderList) {
                ...
                if (rawRegionDetails.length > 0) {
                    if (!defined(that._regionIDsPromises)) {
                        that._regionIDsPromises = rawRegionDetails.map(function(rawRegionDetail) { return rawRegionDetail.regionProvider.loadRegionIDs(); });
                    }
                    return when.all(that._regionIDsPromises).then(function(requiredReload) {
 ...
 */
function calculateRegionIndices(dataSource, clock) {
...
function setNewRegionImageryLayer(dataSource, layerIndex, regionIndices) {
    ...
    if (!defined(regionIndices)) {
        regionIndices = calculateRegionIndices(dataSource, dataSource._catalogItem.terria.clock);
    }
...
// Shallow checks if two arrays of the same length have the same elements.
...
function onClockTick(dataSource, clock) {
    ...
    if (areArraysEqual(dataSource._previousRegionIndices, regionIndices)) {
        return;
    }
