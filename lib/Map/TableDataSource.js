...
TableDataSource.prototype.getLegendGraphic = function () {
    ...
    var segmentCount = 0;
    for (var l = 1; l < segmentCount; l++) {
        var ht = gradH * l / segmentCount;
        ctx.beginPath();
        ctx.moveTo(0, ht);
        ctx.lineTo(gradW, ht);
        ctx.stroke();
    ...
    var minText = (minVal) === undefined ? 'und.' : minVal.toString();
    var maxText = (maxVal) === undefined ? 'und.' : maxVal.toString();
...
//    ctx.textAlign = 'right';
    ...
    if (defined(minVal) && defined(maxVal)) {
        for (var l = 1; l < segmentCount; l++) {
            var ht = gradH * l / segmentCount;
            var val = (maxVal - minVal) * (segmentCount-l) / segmentCount;
