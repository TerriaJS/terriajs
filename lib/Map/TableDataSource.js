...
var TableDataSource = function(catalogItem) {
    ...
    knockout.getObservable(this._tableStructure, 'activeItems').subscribe(function(activeItems) {
        ...
        var regionProvider = defined(this._region) ? this._region.regionProvider : undefined;
...
function setNewRegionImageryLayer(dataSource, layerIndex) {
    ...
    var regionValues = regionIndices;  // This is only appropriate for non-numeric values, or no active column at all.
    if ((tableStructure.activeItems.length > 0) && tableStructure.activeItems[0].isNumeric) {
