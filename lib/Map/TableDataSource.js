...
TableDataSource.prototype.getLegendGraphic = function () {
    ...
    function drawNonBinLabels(ctx, rampHeight, rampWidth, rampTop, segments, minVal, maxVal) {
        ...
        if (defined(minVal) && defined(maxVal)) {
            for (var s = 1; s < segments; s++) {
                ...
                ctx.fillText(valText, rampWidth + 25, textTop);
            }
        }
    }

    /** Label the thresholds between bins for numeric columns */
    function drawBinLabels(ctx, height, left, top, bins, minVal, maxVal) {
        ctx.fillStyle = "#666";
        ctx.font = (bins.length > 6 ? "10px" : "14px") +  " 'Roboto', sans-serif";

        for (var i = -1; i < bins.length; i++) {
            var binVal = (i < 0 ? minVal : bins[i].upperBound );
            var binValText = Math.round(binVal * 100 / 100).toString();

            var bi = bins.length - i - 1;
            var y = height *  bi / bins.length + top;
            ...
            ctx.fillText(binValText, left + 2, y + 3);
        }
    }
    ...
    ctx.fillStyle = "white"; // for consistency with other legends
    ...
    ctx.fillStyle = "lightgrey"; // for consistency with other legends
    ctx.fillRect(rampLeft - 1, rampTop - 1, rampWidth + 2, rampHeight + 2);
    ...
    if (dataVar.varType === VarType.ENUM) {
        drawEnumLabels(ctx, rampHeight, rampWidth, rampTop, dataVar.enumList, bins);
    } else if (bins) {
        //drawLabels(ctx, rampHeight, rampWidth, rampTop, singleValueLegend ? 0 : this.legendTicks + 2, minVal, maxVal);
        drawBinLabels(ctx, rampHeight, rampWidth + 20, rampTop, this._binColors, minVal, maxVal);
    } else {
        drawNonBinLabels(ctx, rampHeight, rampWidth, rampTop, singleValueLegend ? 0 : this.legendTicks + 1, minVal, maxVal);
    }
