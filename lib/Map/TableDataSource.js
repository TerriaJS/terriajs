...
function setNewRegionImageryLayer(dataSource, layerIndex) {
    ...
    var colorFunction;
    // Recolor the regions
    var regionIndices = regionDetail.regionProvider.getRegionIndices(
        regionDetail.column.values,
        defined(regionDetail.disambigColumn) && regionDetail.disambigColumn.values
    );
    ...
    if (tableStructure.activeItems.length > 0) {
        ...
        // Filter the region values by time, if present
        var timeColumn = tableStructure.columnsByType[VarType.TIME][0];
        if (timeColumn && dataSource._availabilities) {
            // this is wrong. should be filtering something else I think.
            regionValues = regionValues.map(function(value, index) {
                return dataSource._availabilities[index].contains(dataSource.clock.currentTime);
            });
        }
    ...
    colorFunction = regionDetail.regionProvider.getColorLookupFunc(
        regionValues,
