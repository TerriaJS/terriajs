    ...
    if (defined(longitudeColumn) && defined(latitudeColumn)) {
        ...
        for (var i = 0; i < rowObjects.length; i++) {
            ...
            if (defined(tableColumn)) {
                value = tableColumn.indicesOrNumericalValues[i];
                if (tableColumn.usesIndicesIntoUniqueValues) {
                    // The "value" is an index into the column's uniqueValues array.
                    // Find the string value it indexes.
                    value = tableColumn.uniqueValues.indexOf(value);
                }
            }
