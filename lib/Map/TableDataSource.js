...
TableDataSource.prototype.setDisplayStyle = function (style) {
    ...
    this.bins = defaultValue(style.bins, this.bins);
...
TableDataSource.prototype._setColorBins = function(binCount) {
    if (!defined(this.dataset.selected.data)) {
    ...
    binCount = Math.min(binCount, vals.length); // Must ask for fewer clusters than the number of items.
...
TableDataSource.prototype.getLegendGraphic = function () {
    var gradientLeft = 15, gradientTop = 5;
    ...
    // #### Change legend to draw discrete colours instead of gradient
    function drawColorMap(ctx, gradH, gradW, colorMap, binColors) {

        // do we really need a panel background? Transparent is surely fine?
        //ctx.fillStyle = "white";
        //ctx.fillRect(0, 0, ctx.canvas.width, ctx.canvas.height);

        
        var bins = binColors.length;
        var binHeight = gradH / bins;
        for (var i = 0; i < bins; i++) {
            ctx.fillStyle = 'rgb(' + binColors[i].color[0] + ',' + binColors[i].color[1] + ',' + binColors[i].color[2] + ')';
            ctx.fillRect(gradientLeft, gradientTop + gradH - binHeight * i, gradW, binHeight);
    ...
    if (this._binColors) {
        drawColorMap(ctx, gradH, gradW, this.colorMap, this._binColors);
    } else {
...
TableDataSource.prototype.setColorGradient = function (colorMap) {
    ...
    // ### If using Jenks etc, we probably don't want a linear gradient?
