...
function overzoom(rings, extent, x, y, level, newX, newY, newLevel) {
    var factor = 1 << (newLevel - level); // 2^n
    var offsetX = extent*(x*factor - newX);
    var offsetY = extent*(y*factor - newY);
    console.log([x,y,level].join('/') +  ' -> ' + [newX,newY,newLevel].join('/'))
    console.log('factor = ' + factor + ', offset = (' + offsetX + ',' + offsetY + ')')
    for (var i = 0; i < rings.length; i++) {
        for (var i2 = 0; i2 < rings[i].length; i2++) {
            rings[i][i2].x = factor * rings[i][i2].x + offsetX;
            rings[i][i2].y = factor * rings[i][i2].y + offsetY;
        }
    }
}
...
MapboxVectorTileImageryProvider.prototype._requestImage = function(x, y, level, canvas) {
    ...
    if (diffZ > 0 && x >> diffZ === 58 && y >> diffZ === 37) {
        console.log([level, x, y].join('/'));
        // Use overzoomed (6/58/37 tile)
        over = true;
        overX = x;
        overY = y;
        overZ = level;
    ...
    return loadArrayBuffer(url).then(function(data) {
        if (over) console.log("Overzooming " + [level, x, y].join('/'));
        ...
        for (var i = 0; i < layer.length; i++) {
            ...
            if (feature.type === POLYGON_FEATURE) {
                ...
                if (over) {
                    overzoom(coordinates, layer.extent, x,y,level, overX,overY,overZ);
                }
