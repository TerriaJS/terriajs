...
var MapboxVectorTileImageryProvider = function(options) {
  if (defined(options.tileIndex)) {
    this._tileIndex = options.tileIndex;
  } else {
...
MapboxVectorTileImageryProvider.prototype._requestImage = function(
  ...
  return this._getTile(nativeTile.x, nativeTile.y, nativeTile.level).then(
    tile => this._drawTile(requestedTile, nativeTile, tile, canvas)
  );
};
