...
var MapboxVectorTileImageryProvider = function (options) {
    if (typeof options.url === 'string') {
        ...
        this._tileIndex = options.url; // url is used to pass tileIndex for prototyping
    }
...
defineProperties(MapboxVectorTileImageryProvider.prototype, {
    url : {
        get : function() {
            // Needed for Terria's Models/Leaflet.js pickFeatures
            return this._tileIndex ? 'some-url' : this._uriTemplate.expression;
        }
    },
...
MapboxVectorTileImageryProvider.prototype._getTile = function(x, y, level) {
    if (this._tileIndex) {
        var layer = this._tileIndex.getTile(level, x, y);
        if (defined(layer)) {
            layer.extent = 4096;
            ...
            layer.feature = (i) => {
                ...
                f.properties = f.tags;
                f.loadGeometry = () => f.geometry.map(ring =>
                    ring.map(coord => ({
                        x: coord[0],
                        y: coord[1]
                    }))
...
MapboxVectorTileImageryProvider.prototype._requestImage = function(x, y, level, canvas) {
    ...
    return this._getTile(nativeTile.x, nativeTile.y, nativeTile.level).then(tile => this._drawTile(requestedTile, nativeTile, tile, canvas));
};
...
MapboxVectorTileImageryProvider.prototype.pickFeatures = function(x, y, level, longitude, latitude) {
    ...
    return this._getTile(nativeTile.x, nativeTile.y, nativeTile.level).then(function(tile) {
...
MapboxVectorTileImageryProvider.prototype.createHighlightImageryProvider = function(regionUniqueID) {
    ...
    var imageryProvider = new MapboxVectorTileImageryProvider({
        url: this._tileIndex || this._uriTemplate.expression,
