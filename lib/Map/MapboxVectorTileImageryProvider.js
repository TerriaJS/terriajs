...
function getCachingFeature(feature, key) {
    // edit feature's loadGeometry function
    var oldLoadGeometry = feature.loadGeometry;
    feature.loadGeometry = function () {
        var geom = getCachingFeature.lrucache.get(key);
        if (defined(geom)) {
            getCachingFeature.cacheHits++;
            return geom;
        } else {
            getCachingFeature.cacheMisses++;
            geom = oldLoadGeometry.call(feature);
            getCachingFeature.lrucache.set(key, geom);
            return geom;
        }
    };
    return feature;
...
function geometryLength(geom, key) {
    return geom.map(function (ring) { return ring.length; }).reduce(function (a, b) { return a + b; });
}

getCachingFeature.cacheHits = 0;
getCachingFeature.cacheMisses = 0;
getCachingFeature.lrucache = LRU({
    max: 1000000, // Assuming 16 bytes per coordinate, this is 16MB
    length: geometryLength, // Sum of ring lengths
    dispose: function (key, geom) { console.log(key, geometryLength(geom, key)); }, // Print size on delete
...
MapboxVectorTileImageryProvider.prototype._requestImage = function(x, y, level, canvas) {
    ...
    return loadArrayBuffer(url).then(function(data) {
        ...
        for (var i = 0; i < layer.length; i++) {
            var feature = getCachingFeature(layer.feature(i), url + ':' + i);
            if (feature.type === POLYGON_FEATURE) {
                ...
                if (overzoom) {
                    ...
                    var levelDelta = level - nativeTile.level;
                    ...
                    var x1 = size * (x - (nativeTile.x << levelDelta)); //
                    var y1 = size * (y - (nativeTile.y << levelDelta));
                    ...
                    coordinates = overzoomGeometry(feature.loadGeometry(), nativeTile, size, newTile);
                } else {
...
MapboxVectorTileImageryProvider.prototype.pickFeatures = function(x, y, level, longitude, latitude) {
    ...
    return loadArrayBuffer(url).then(function(data) {
        ...
        for (var i = 0; i < layer.length; i++) {
           var feature = getCachingFeature(layer.feature(i), url + ':' + i);
            if (feature.type === POLYGON_FEATURE && isFeatureClicked(overzoomGeometry(feature.loadGeometry(), nativeTile, layer.extent >> levelDelta, newTile), point)) {
