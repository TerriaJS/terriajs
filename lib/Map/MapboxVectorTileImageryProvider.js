...
var POINT_FEATURE = 1; // feature.type == 1 for point features
var LINESTRING_FEATURE = 2; // feature.type == 2 for polyline features
...
function calculateOverzoomOffset(nativeTile, newExtent, newTile) {
...
function overzoomGeometry(rings, nativeTile, newExtent, newTile) {
  // offset.(x, y) in the native tile is the (0,0) of the new tile
  var offset = calculateOverzoomOffset(nativeTile, newExtent, newTile);
  for (var i = 0; i < rings.length; i++) {
    for (var i2 = 0; i2 < rings[i].length; i2++) {
      rings[i][i2].x -= offset.x;
      rings[i][i2].y -= offset.y;
    }
  }
}
...
MapboxVectorTileImageryProvider.prototype._drawTile = function(
  ...
  if (nativeTile.level !== requestedTile.level) {
    ...
    // Used in Polygon case to do a quick BBOX check before decoding, overzooming, painting
  ...
  for (var i = 0; i < layer.length; i++) {
    ...
    if (feature.type === POINT_FEATURE) {
      ...
      if (!defined(pointRadius)) {
        pointRadius = 4;
      }
      coordinates = feature.loadGeometry();
      if (nativeTile.level !== requestedTile.level) {
        overzoomGeometry(coordinates, nativeTile, size, requestedTile);
      }
      // coordinates should be of shape coordinates: [Point][] (an aray of Point 1-tuples)
      // Single point
      ...
      pos = coordinates[0][0];
    } else if (feature.type === POLYGON_FEATURE) {
      if (nativeTile.level !== requestedTile.level) {
        // Overzoom feature
        ...
        if (size < 16) {
          // Polygon feature on tile that has less detail than 16x16
          throw new DeveloperError(
        ...
        if (
          ...
          // Feature in native tile lies fully outside requested tile
          continue;
        }
        // Overzooming works by subtracting an offset from each point in every ring
        //  and changing the scale (extentFactor) at which they're rendered to canvas
        coordinates = feature.loadGeometry();

        // Rewrite geometry in place
        overzoomGeometry(coordinates, nativeTile, size, requestedTile);
      } else {
        coordinates = feature.loadGeometry();
      }
