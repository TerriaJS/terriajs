 ...
 * @param {Object} [options] Options, as per DisplayVariablesConcept.  The default for allowMultiple is changed to true here.
 * @param {Boolean} [options.canAddAndRemove]  When false, the SummaryConcept can be substituted directly for a regular displayVariableConcept
 *   (or array of concepts). Each child can have zero, one or more items selected or not selected (depending on their canAddAndRemove setting).
 *   When true (the default), it treats the child concepts as "conditions", any number of which can be added or removed.
 *   See SummaryConcept.jsx for more.
 */
var SummaryConcept = function(name, options) {
    name = defaultValue(name, 'Conditions');

    options = defaultValue(options, defaultValue.EMPTY_OBJECT);
    options.allowMultiple = defaultValue(options.allowMultiple, true);

    DisplayVariablesConcept.call(this, name, options);

    this.canAddAndRemove = defaultValue(options.canAddAndRemove, true);
};
