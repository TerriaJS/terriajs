 ...
 * @param {Function} [condition] A function of a concept, which returns a boolean.  If not provided, all leaf descendants are returned.
 * @return {Concept[]}
 */
DisplayVariablesConcept.prototype.getNodes = function(condition) {
    if (!defined(condition)) {
        condition = node => (!defined(node.items));  // By default, return only the leaf nodes.
    }
...
function getNestedNodes(concept, condition) {
    ...
    if (!concept.items) {
        // Return an empty array, not undefined, so that it is completely removed by flattenNested.
        return [];
    }
...
DisplayVariablesConcept.prototype.toggleActiveItem = function(concept) {
    ...
    // Note this is the case if there are no _other_ active items, _and_ you are trying to de-activate the current one (ie. it is active now).
    // The getNodes call returns all the other nodes which are active, plus this node if it's inactive.
    // When this has length 0 is the case we want to prevent.
    if (isActive && this.requireSomeActive && this.getNodes(child => child === concept ? !child.isActive : child.isActive ).length === 0) {
        return;
    }
    ...
    if (!isActive && !this.allowMultiple || shouldClearActives(this, concept.id)) {
        ...
        // It was not appropriate to clear all other active concepts, so just check if there's a higher parent with rules.
        if (defined(this.parent) && defined(this.parent.toggleActiveItem)) {
            this.parent.toggleActiveItem(concept);
        } else {
            // Otherwise just toggle the concept.
