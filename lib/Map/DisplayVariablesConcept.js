...
DisplayVariablesConcept.prototype.toggleActiveItem = function(concept) {
    ...
    if (this.requireSomeActive && this.items.every(child => child === concept ? child.isActive : !child.isActive )) {
        return;
    }
    // If we only allow one selection, we may need to deselect the others.
    if (!this.allowMultiple || shouldClearActives(this, concept.id)) {
