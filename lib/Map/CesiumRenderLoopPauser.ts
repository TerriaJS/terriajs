...
export default class CesiumRenderLoopPauser {
    ...
    destroy() {
        ...
        if (this._boundNotifyRepaintRequired) {
            this.cesiumWidget.canvas.removeEventListener('mousemove', this._boundNotifyRepaintRequired, false);
            this.cesiumWidget.canvas.removeEventListener('mousedown', this._boundNotifyRepaintRequired, false);
            this.cesiumWidget.canvas.removeEventListener('mouseup', this._boundNotifyRepaintRequired, false);
            this.cesiumWidget.canvas.removeEventListener('touchstart', this._boundNotifyRepaintRequired, false);
            this.cesiumWidget.canvas.removeEventListener('touchend', this._boundNotifyRepaintRequired, false);
            this.cesiumWidget.canvas.removeEventListener('touchmove', this._boundNotifyRepaintRequired, false);

            if (defined(globalThis.PointerEvent)) {
                this.cesiumWidget.canvas.removeEventListener('pointerdown', this._boundNotifyRepaintRequired, false);
                this.cesiumWidget.canvas.removeEventListener('pointerup', this._boundNotifyRepaintRequired, false);
                this.cesiumWidget.canvas.removeEventListener('pointermove', this._boundNotifyRepaintRequired, false);
            }

            if (this._wheelEvent) {
                this.cesiumWidget.canvas.removeEventListener(this._wheelEvent, this._boundNotifyRepaintRequired, false);
            }
    ...
    private postRender(date: JulianDate) {
        ...
        if (!cameraMovedInLastSecond && !terrainTilesWaiting && !this.cesiumWidget.clock.shouldAnimate && tweens.length === 0) {
