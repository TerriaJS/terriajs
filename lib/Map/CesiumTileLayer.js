...
var CesiumTileLayer = L.TileLayer.extend({
    ...
    createTile: function(coords, done) {
        ...
        if (this.dirtyHack) {
            // TODO STORE PREVIOUS ERROR HANDLER...
            L.DomEvent.on(tile, 'error', L.bind(this.tokenError, this, done, tile, coords));

            this.dirtyHack = false;
        }

        return tile;
    },

    // TODO: Possibly remove done as it is not needed from this error handler as it currently sits.
    tokenError: function (done, tile, coords) {
        console.log("Hit tokenError()!");

        var that = this;
        ArcGisMapServerImageryProvider.updateToken(this.imageryProvider).then(function() {
            console.log("Hit token updated!");
            console.log("New Token: " + that.imageryProvider.token);

            that.dirtyHack = true;
            that.validToken = true; // TODO REMOVE THIS DEBUG CONSTRUCT.
            tile.src = that.getTileUrl(coords);
            that.validToken = false; // TODO REMOVE THIS DEBUG CONSTRUCT.
            that.dirtyHack = false;
            // TODO RESTORE PREVIOUS ERROR HANDLER...
        });
    },
    ...
    getTileUrl: function(tilePoint) {
        ...
        if (this.dirtyHack) {
            // TODO REMOVE THIS...FORCE A TOKEN ERROR FOR DEBUGGING TOKEN ERRORS...
            if (!this.validToken) {                         // TODO REMOVE THIS DEBUG CONSTRUCT.
                this.imageryProvider.token = "Invalid";     // TODO REMOVE THIS DEBUG CONSTRUCT.
            }                                               // TODO REMOVE THIS DEBUG CONSTRUCT.

            return ArcGisMapServerImageryProvider.buildImageUrl(this.imageryProvider, tilePoint.x, tilePoint.y, z);
        }
