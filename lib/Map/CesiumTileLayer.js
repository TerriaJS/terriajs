...
var CesiumTileLayer = L.TileLayer.extend({
    ...
    pickFeatures: function(map, longitudeRadians, latitudeRadians, tileCoordinates, level) {
        var ll = new Cartographic(CesiumMath.negativePiToPi(longitudeRadians), latitudeRadians, 0.0);

        var level = defaultValue(level, map.getZoom());
        ...
        return pollToPromise(function() {
            ...
            var tileCoordinates = defaultValue(tileCoordinates, tilingScheme.positionToTileXY(ll, level));
            if (!defined(tileCoordinates)) {
                return undefined;
            }

            return that.imageryProvider.pickFeatures(tileCoordinates.x, tileCoordinates.y, level, longitudeRadians, latitudeRadians).then(function(features) {
                return {
                    features: features,
                    coords: {x: tileCoordinates.x, y: tileCoordinates.y, level: level}
                };
            });
        });
    },
