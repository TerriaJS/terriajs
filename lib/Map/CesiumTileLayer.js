...
var CesiumTileLayer = L.TileLayer.extend({
    ...
    _update: function() {
        ...
        if (!this.initialized) {
            ...
            if (this.imageryProvider.tileDownloadTimeout) {
                this.addEventListener('loading', this._resetTimeoutWatchdog, this);
                this.addEventListener('load', this._stopTimeoutWatchdog, this);
                this.addEventListener('tileload', this._resetTimeoutWatchdog, this);
            }
    ...
    onRemove: function(map) {
        ...
        this._stopTimeoutWatchdog();

        this.removeEventListener('loading', this._resetTimeoutWatchdog, this);
        this.removeEventListener('load', this._stopTimeoutWatchdog, this);
        this.removeEventListener('tileload', this._resetTimeoutWatchdog, this);
    ...
    _resetTimeoutWatchdog: function() {
        this._stopTimeoutWatchdog();

        console.log('start');
        var that = this;
        this._timeoutID = setTimeout(function() {
            that.errorEvent.raiseEvent(that, that.imageryProvider.tileDownloadTimeoutMessage);
            that._timeoutID = undefined;
        }, this.imageryProvider.tileDownloadTimeout);
    }
});
