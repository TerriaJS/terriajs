...
const combineLatLngBounds = require('./combineLatLngBounds');
 ...
 *
 * @param {EntityCollection} entities The collection of entities whose bounds to compute.
 * @returns {LatLngBounds} The computed bounds.
 */
LeafletDataSourceDisplay.prototype.getLatLngBounds = function(entities) {
    var dataSources = this._dataSourceCollection;
    var latLngBounds = [];
    ...
    for (var i = 0, len = dataSources.length; i < len; i++) {
        ...
        if (entities.id === dataSource._entityCollection.id) {
    ...
    if (latLngBounds.length > 0) {
        return combineLatLngBounds(latLngBounds);
    }
}
