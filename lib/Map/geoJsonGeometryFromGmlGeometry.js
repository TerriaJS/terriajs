...
function createLineStringFromGmlGeometry(geometry) {
  ...
  var posNodes = geometry.getElementsByTagName("gml:posList");
...
function createPointFromGmlGeometry(geometry) {
  var posNodes = geometry.getElementsByTagName("gml:pos");
  if (posNodes < 1) {
...
function createMultiLineStringFromGmlGeometry(geometry) {
  var curveMembers = geometry.getElementsByTagName("gml:posList");
  var curves = [];
...
function createMultiPolygonFromGmlGeomtry(geometry) {
  ...
  var polygons = geometry.getElementsByTagName("gml:Polygon");
  for (var i = 0; i < polygons.length; ++i) {
    ...
    var posListNodes = exterior.getElementsByTagName("gml:posList");
    ...
    var interiors = polygon.getElementsByTagName("gml:interior");
...
function createPolygonFromGmlGeometry(geometry) {
  ...
  var exteriorNodes = geometry.getElementsByTagName("gml:exterior");
  if (exteriorNodes.length < 1) {
    throw new RuntimeError(i18next.t("map.gmlToGeoJson.missingExteriorRing"));
  }
  var exterior = exteriorNodes[0];
  ...
  var interiors = geometry.getElementsByTagName("gml:interior");
  for (var j = 0; j < interiors.length; ++j) {
    var interior = interiors[j];
    var interiorPosListNodes = interior.getElementsByTagNameNS(
      gmlNamespace,
