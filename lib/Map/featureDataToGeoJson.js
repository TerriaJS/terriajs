...
function getEsriGeometry(featureData, geometryType, spatialReference) {
  if (defined(featureData.features)) {
    ...
    return {
      ...
      features: filterOutUndefined(
        featureData.features.map(function(subFeatureData) {
  ...
  if (geometryType === "esriGeometryPolygon" && defined(featureData.geometry)) {
...
function esriSpatialReferenceToCrs(spatialReference) {
  ...
  const wkidCode = spatialReference.latestWkid ?? spatialReference.wkid;
  ...
  } else if (defined(wkidCode)) {
    return {
      type: "name",
      properties: {
        name: "urn:ogc:def:crs:EPSG::" + wkidCode
      }
    };
  }
  return undefined;
