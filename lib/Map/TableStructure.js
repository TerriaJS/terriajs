...
defineProperties(TableStructure.prototype, {
    ...
    activeTimeColumnNameIdOrIndex: {
        ...
        set: function(nameIdOrIndex) {
            ...
            this.sortBy(this.activeTimeColumn);
            var activeTimeColumn = this.activeTimeColumn;  // sortBy changes all the columns, so this.activeTimeColumn is now different.
            if (defined(activeTimeColumn) && (!defined(activeTimeColumn.finishJulianDates))) {
                // Calculate default end dates and timeIntervals, and define a clock on the active time column.
                activeTimeColumn.finishJulianDates = calculateFinishDates(this, activeTimeColumn);
                activeTimeColumn._timeIntervals = calculateTimeIntervals(activeTimeColumn);
                activeTimeColumn._clock = createClock(activeTimeColumn);
            }
        }
    },
    ...
    activeTimeColumn: {
        get: function() {
            if (defined(this._activeTimeColumnNameIdOrIndex)) {
                return this.getColumnWithNameIdOrIndex(this._activeTimeColumnNameIdOrIndex);
            }
        }
    },
...
function calculateSortedUniqueJulianDates(originalDates) {
    var sortedUniqueJulianDates = originalDates.filter(function(d) { return defined(d); });
    sortedUniqueJulianDates.sort(JulianDate.compare);
    sortedUniqueJulianDates = sortedUniqueJulianDates.filter(function(element, index, array) {
        return (index === 0) || (!JulianDate.equals(array[index - 1], element));
    });
    return sortedUniqueJulianDates;
}

// For each row, find the next different date (minus 1 second).
// TODO: Restrict to only those rows with this value of the idColumnNames, if present.
...
function calculateFinishDates(tableStructure, timeColumn) {
    var originalJulianDates = timeColumn.julianDates;
    // First calculate a set of unique, sorted dates.
    var sortedUniqueJulianDates = calculateSortedUniqueJulianDates(originalJulianDates);
    // indices[k] has the property that originalJulianDates[indices[k]] = sortedUniqueJulianDates[k].
    var indices = calculateInverseIndicies(originalJulianDates, sortedUniqueJulianDates);
    ...
    var endDates = sortedUniqueJulianDates.slice(1).map(function(rawEndDate, index) {
        var secondsDifference = JulianDate.secondsDifference(rawEndDate, sortedUniqueJulianDates[index]);
        if (secondsDifference < 20) {
            return JulianDate.addSeconds(sortedUniqueJulianDates[index], secondsDifference * 0.95, new JulianDate());
        } else {
...
function calculateAvailability(timeColumn, index) {
    ...
    if (defined(startJulianDate)) {
        ...
        return new TimeInterval({start: timeColumn.julianDates[index], stop: finishJulianDate});
    }
}
...
* @param {Boolean} [quoteStringsWithCommas] False by default; if true, any strings which contain commas will be quoted (including column names).
...
*/
TableStructure.prototype.toArrayOfRows = function(dateFormatString, rowNumbers, formatScalars, quoteStringsWithCommas) {
    ...
    function getRow(rowNumber) {
        return that.columns.map(column => {
            ...
            // Put quotes around any value that contains commas, so csv format doesn't go nuts.
            return (quoteStringsWithCommas && formattedValue.toString().indexOf(',') >= 0) ? '"' + formattedValue + '"' : formattedValue;
        });
    }
    var rows;
    ...
    if (quoteStringsWithCommas) {
        columnNames = columnNames.map(s => s.indexOf(',') >=0 ? '"' + s + '"' : s);
    }
...
TableStructure.prototype.getIdStringForRowNumber = function(rowNumber, idColumnNames) {
    return getIdStringForRowNumber(getIdColumns(this, idColumnNames), rowNumber);
};
...
TableStructure.prototype.sortBy = function(sortColumn, compareFunction) {
    ...
    var sortedColumns = this.columns.map(column => {
