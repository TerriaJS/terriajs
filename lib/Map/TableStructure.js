...
/**
* Return data as an array of rows of objects,
* with both column ids and names present if they differ, eg.
* If the y column has id 'y' and name 'value ($)',
* [{'x': 1, 'value ($)': 10, 'y': 10}, {'x': 2, 'value ($)': 20, 'y': 20}, ...].
*
* @returns {Object[]} Array of objects containing a property for each unique column name and id
*          of the row. If the table has no data, returns [].
*/
TableStructure.prototype.toNameAndIdRowObjects = function() {
    var asRows = this.toArrayOfRows();
    if (!defined(asRows) || asRows.length < 1) {
        return [];
    }
    ...
    for (var i = 1; i < asRows.length; i++) {
        ...
        for (var j = 0; j < columnNames.length; j++) {
            rowObject[columnNames[j]] = asRows[i][j];
            if (this.columns[j].id !== columnNames[j]) {
                rowObject[this.columns[j].id] = asRows[i][j];
            }
        }
        result.push(rowObject);
    }
