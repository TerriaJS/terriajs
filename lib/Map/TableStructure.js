...
*/
TableStructure.fromJson = function(json, aristotleMetadata, result) {
    ...
    var cols;
    if (defined(aristotleMetadata.columns) && aristotleMetadata.columns.length > 0) {
        cols = aristotleMetadata.columns;
    ...
    for (var columnNumber = 0; columnNumber < columnNames.length; columnNumber++) {
        ...
        if (defined(cols) && cols[columnNumber].column_name === columnNames[columnNumber]) {
            nameAndcolumnOptions[1].meta_definition = cols[columnNumber].data_element.definition;
            nameAndcolumnOptions[1].meta_name = cols[columnNumber].data_element.name;
            nameAndcolumnOptions[1].meta_datatype = cols[columnNumber].data_type;
        }
...
* @param {TableStructure} [result] A pre-existing TableStructure object; if not present, creates a new one.
*/
TableStructure.fromCsv = function(csvString, aristotleMetadata, result) {
    ...
    return TableStructure.fromJson(json, aristotleMetadata, result);
};
...
TableStructure.prototype.loadFromJson = function(json) {
    return TableStructure.fromJson(json, undefined, this);
};
...
*
* @param {String} csvString String in csv format.
* @param {Promise} aristotleMetadata A promise resolving to aristotle metadata.
*/
TableStructure.prototype.loadFromCsv = function(csvString, aristotleMetadata) {
    return TableStructure.fromCsv(csvString, aristotleMetadata, this);
};
 ...
 */
function calculateFinishDatesFromStartDates(startJulianDates, localDefaultFinalDurationSeconds, shaveSeconds) {
...
function calculateFinishDates(columns, nameIdOrIndex, tableStructure) {
    ...
    if (!defined(tableStructure.idColumnNames) || tableStructure.idColumnNames.length === 0) {
        return calculateFinishDatesFromStartDates(startJulianDates, defaultFinalDurationSeconds, tableStructure.shaveSeconds);
    }
    ...
    for (featureIdString in idMapping) {
        if (idMapping.hasOwnProperty(featureIdString)) {
            ...
            var theseEndDates = calculateFinishDatesFromStartDates(theseStartDates, finalDurationSeconds, tableStructure.shaveSeconds);
...
TableStructure.prototype.toArrayOfRows = function(dateFormatString, rowNumbers, formatScalars, quoteStringsWithCommas) {
    ...
    function getRow(rowNumber) {
        return that.columns.map(column => {
            ...
            return (quoteStringsWithCommas && formattedValue && formattedValue.toString().indexOf(',') >= 0) ? '"' + formattedValue + '"' : formattedValue;
        });
    }
    var rows;
