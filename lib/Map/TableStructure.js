...
var sortedIndices = require('../Core/sortedIndices');
...
TableStructure.fromColumnArrays = function(columnArrays, result) {
    ...
    for (var j = 0; j < firstArray.length; j++) {
        ...
        combinedValueArrays.push(thisColumn.values.slice());
    ...
    for (var i = 1; i < columnArrays.length; i++) {
        ...
        combinedValueArrays[0] = combinedValueArrays[0].concat(currentFirstColumn.values);
        ...
        for (j = 1; j < currentColumns.length; j++) {
            ...
            combinedValueArrays.push(empty2.concat(currentColumn.values));
    ...
    combineRepeatedInPlace(combinedValueArrays, firstArray[0].type);
...
function sortByFirst(valueArrays) {
    ...
    var indices = sortedIndices(firstValues);
 ...
 * @param  {Array[]} sortedValueArrays The array of arrays of values to combine. These must be sortedByFirst.
 * @param  {Integer} [firstColumnType] Eg. VarType.TIME.
 * @return {Array[]} The values, with any repeats in the first column combined into one.
 */
function combineRepeatedInPlace(sortedValueArrays, firstColumnType) {
    if (firstColumnType === VarType.TIME) {
        // Replace this column with the string form of the julian dates they represent, so different representations match.
        // Just use the table column's juliandates calculation, so we don't need the more complex 'finishdates' calculations.
        var timeColumn = new TableColumn('', sortedValueArrays[0], {type: firstColumnType, noFinishDates: true});
        sortedValueArrays[0] = timeColumn.julianDates.map(function(jdate) { return JulianDate.toIso8601(jdate); });
    }
    for (var j = sortedValueArrays[0].length - 1; j >= 1; j--) {
        if (sortedValueArrays[0][j] === sortedValueArrays[0][j - 1]) {
            sortedValueArrays[0].splice(j, 1);
            for (var i = 1; i < sortedValueArrays.length; i++) {
                if (sortedValueArrays[i][j] === undefined) {
                    sortedValueArrays[i].splice(j, 1);
                } else {
                    sortedValueArrays[i].splice(j - 1, 1);
                }
            }
        }
    }
