...
*/
TableStructure.prototype.toArrayOfRows = function(dateFormatString, rowNumbers, formatScalars) {
    ...
    function getRow(rowNumber) {
        return that.columns.map(column => {
            ...
            return (formattedValue.toString().indexOf(',') >= 0) ? '"' + formattedValue + '"' : formattedValue;
        });
    }
    var rows;
...
TableStructure.prototype.toCsvString = function(dateFormatString, rowNumbers, formatScalars) {
    var arraysOfRows = this.toArrayOfRows(dateFormatString, rowNumbers, formatScalars);
    var joinedRows = arraysOfRows.map(function(row, i) {
        if (i===0) {
            // Put quotes around the column headings if they contain commas
            return row.map(s => s.indexOf(',') >=0 ? '"' + s + '"' : s).join(",");
        }
        return row.join(',');
    });
