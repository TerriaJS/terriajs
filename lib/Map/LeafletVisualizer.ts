...
class LeafletGeomVisualizer {
    ...
    private _updateBillboard(
        ...
        const height: number | undefined = getValue(
            markerGraphics.height,
            time
        );
        const width: number | undefined = getValue(markerGraphics.width, time);
        const color = getValueOrDefault(
            markerGraphics.color,
            time,
            defaultColor
        );
        ...
        if (!isDefined(geomLayer)) {
            ...
            for (const prop in iconOptions) {
                if (
                    ...
                    iconOptions[prop] !==
                        (<any>marker.options.icon.options)[prop]
                ) {
                    redrawIcon = true;
                    break;
                }
    ...
    private _updatePolyline(
        ...
        const carts = Ellipsoid.WGS84.cartesianArrayToCartographicArray(
            positions
        );
        const latlngs = [];
        ...
        if (!isDefined(geomLayer)) {
            if (latlngs.length > 0) {
                ...
                polyline.on(
                    "click",
                    featureClicked.bind(undefined, this, entity)
                );
                polyline.on(
                    "mousedown",
                    featureMousedown.bind(undefined, this, entity)
            ...
            for (let prop in polylineOptions) {
                if (
                    (<any>polylineOptions)[prop] !==
                    (<any>polyline.options)[prop]
                ) {
