...
function prettifyCoordinates(longitude, latitude, options) {
    ...
    const digits = defaultValue(optionsDefaulted.digits, 3);

    result.latitude = Math.abs(latitude).toFixed(digits) +
        '°' + (latitude < 0.0 ? 'S' : 'N');
    result.longitude = Math.abs(longitude).toFixed(digits) +
        '°' + (longitude < 0.0 ? 'W' : 'E');

    if (defined(optionsDefaulted.height)) {
        result.elevation = Math.round(optionsDefaulted.height) + (defined(optionsDefaulted.errorBar) ? '±' +
            Math.round(optionsDefaulted.errorBar) : '') + 'm';
    } else {
