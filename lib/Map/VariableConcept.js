...
VariableConcept.prototype.toggleActive = function() {
    // This code somewhat duplicates code found in DisplayVariablesConcept.
    // If it weren't for the call to this.parent.toggleActiveCallback, we could rationalise this.
    if (defined(this.parent)) {
        // We may be trying to deactivate the last active one.
        if (this.isActive) {
            if (this.parent.requireSomeActive) {
                this.parent.toggleActiveItem(this);
                return;
            }
        } else {
            // Are we trying to activate multiple?
            if (!this.parent.allowMultiple || defined(this.parent.exclusiveChildIds)) {
    ...
    // toggleActiveCallback is defined on TableStructure, not DisplayVariablesConcept.
