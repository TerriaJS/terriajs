...
function createRegionImageryProvider(dataSource) {
    ...
    if (defined(tableStructure.activeItems[0])) {
        ...
        var regionValues = dataSource.region.regionProvider.getRegionValues(tableStructure.activeItems[0].values, region.column.values, defined(region.disambigColumn) && region.disambigColumn.values);
        var colorFunction = dataSource.region.regionProvider.getColorLookupFunc(regionValues, legendHelper.getColorArrayFromValue.bind(legendHelper));
        // colorFunction = function(id) { return [Math.trunc(Math.random() * 255), Math.trunc(Math.random() * 255), Math.trunc(Math.random() * 255), 255]; };
