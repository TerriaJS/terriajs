...
var TableDataSource = function(catalogItem) {
    ...
    knockout.getObservable(this._tableStructure, 'activeItems').subscribe(function(activeItems) {
        ...
        this.regionPromise.then(function(region) {
            if (defined(that._tableStructure.activeItems[0])) {
                // only recolor the regions if there is a variable to base the recoloring on.
                // TODO: rationalise which parts are here , which in csvCatalogItem.
                // var regionValues = region.regionProvider.getRegionValues(that._tableStructure.activeItems[0].values, region.column.values, defined(region.disambigColumn) && region.disambigColumn.values);
                // var colorFunction = region.regionProvider.getColorLookupFunc(regionValues, that._legendHelper.getColorFromValue.bind(that._legendHelper));
                ImageryProviderHooks.addRecolorFunc(that._regionImageryProvider, function(id) { return [0, 200, 100, 255]; });  // colorFunction
            }
        });
    }, this);
};
...
TableDataSource.prototype.enable = function(layerIndex) {
    ...
    this.regionPromise.then(function(region) {
        if (region) {
            var catalogItem = that._catalogItem;
            that._regionImageryProvider = createRegionImageryProvider(that);
            that._imageryLayer = ImageryLayerCatalogItem.enableLayer(catalogItem, that._regionImageryProvider, catalogItem.opacity, layerIndex);
