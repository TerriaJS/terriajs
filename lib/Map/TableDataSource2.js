...
var TableStructure = require('./TableStructure');
...
TableDataSource.prototype.load = function(csvString, tableStyle) {
    if (defined(this._tableStructure)) {
        this._tableStructure.destroy();
    }
    this._tableStyle = tableStyle;
    this._tableStructure = TableStructure.fromCsv(csvString);
    var selectedColumn = this._tableStructure.yColumns[0];  // TODO: can this go on csvCatalogItem? It may be better here.
    this._legend = TableColumnLegend.fromTableColumnAndStyle(selectedColumn, tableStyle);
    updateEntities(this, selectedColumn, tableStyle, this._legend);
};

function firstElementIfDefined(a) {
    // if a and a[0] are both defined, return a[0]
    // otherwise undefined.
    if (defined(a)) {
        return a[0];
    }
...
function updateEntities(dataSource, tableColumn, tableStyle, legend) {
    ...
    if (dataSource.hasLocationData) {
        ...
        var longitudeColumn = firstElementIfDefined(tableStructure.columnsByType[VarType.LON]);
        var latitudeColumn = firstElementIfDefined(tableStructure.columnsByType[VarType.LAT]);
        var heightColumn = firstElementIfDefined(tableStructure.columnsByType[VarType.ALT]);
        ...
        for (var i = 0; i < rowObjects.length; i++) {
            ...
            var color = colorArrayToColor(legend.getColorFromValue(tableColumn.values[i]));
            var scale = 1; // TODO: // dataSource._mapValue2Scale(point.val);

            var show = new TimeIntervalCollectionProperty();

            // TODO: add back time stuff
            ...
            if (!defined(tableStyle.imageUrl) || tableStyle.imageUrl === '') {
                entity.point = {
...
function colorArrayToColor(color) {
    // TODO: better to move this to TableColumnLegend?
