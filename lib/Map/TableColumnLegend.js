 ...
 * 
 * TODO: I thought it made sense to inherit from Legend, but since this is in fact the only place where
 * Legend is used, it may be better to refactor this so that it just uses the Legend class as a helper
 * to generate the legendUrl.
 *
...
var defaultColor = [32, 0, 200, 255];  // used if no selected variable.

TableColumnLegend.fromTableColumnAndStyle = function(tableColumn, tableStyle) {
    if (!defined(tableColumn)) {
        return undefined;
    }
    tableStyle = defined(tableStyle) ? tableStyle : new TableStyle();  // instead of defaultValue, so new object only created if needed.
    tableStyle.colorBins = defaultValue(tableStyle.colorBins, 7);
    tableStyle.legendTicks = defaultValue(tableStyle.legendTicks, 0);

    var colorMap = defaultValue(tableStyle.colorMap, defaultColorMap);
    ...
    return new TableColumnLegend(legendProps, tableColumn, tableStyle, colorGradient, binColors);
};
 ...
 */
TableColumnLegend.colorArrayToColor = function(colorArray) {
    if (!defined(colorArray)) {
...
TableColumnLegend.prototype.getColorFromFractionalPosition = function(fractionalPosition) {
    return getColorFromColorGradient(this._colorGradient, fractionalPosition);
};

TableColumnLegend.prototype.getColorFromValue = function(value) {
    var i = 0;
    while (i < this._binColors.length - 1 && value > this._binColors[i].upperBound) {
        i++;
    }
    if (!defined(this._binColors[i])) { // TODO: is this even possible given the checks above?
