...
var combineLatLngBounds = require('./combineLatLngBounds');
...
LeafletGeomVisualizer.prototype.getLatLngBounds = function() {
    ...
    for (var i = 0, len = entities.length; i < len; i++) {
        ...
        if (defined(entity._point) || defined(entity._billboard) || defined(entity._label)) {
            latLngs = [positionToLatLng(Property.getValueOrUndefined(entity._position))];
        }
        if (defined(entity._polyline)) {
            var positions = Property.getValueOrUndefined(entity._polyline._positions);
            latLngs = positions.map(positionToLatLng);
        }
        if (defined(entity._polygon)) {
            latLngs = hierarchyToLatLngs(Property.getValueOrUndefined(entity._polygon._hierarchy));
        }

        if (defined(latLngs)) {
            bounds.push(L.latLngBounds(latLngs));
        }
    }

    if (bounds.length > 0) {
        return combineLatLngBounds(bounds);
    }
};
