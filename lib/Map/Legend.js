...
var Legend = function(props) {
     ...
     * @type {String}
     */
    this.cssClass = defaultValue(props.cssClass, 'legend__legend');
...
function finishSvg(legend, background, height) {
    background.setAttributeNS(legend._svgns, 'height', height);
    legend._svg.setAttributeNS(legend._svgns, 'height', height);
...
Legend.prototype.drawSvg = function() {
    ...
    if (defined(this.gradientColorMap)) {
        y = drawGradient(this, barGroup, y);
    }
    if (this.items.length > 0) {
        drawItemBoxes(this, barGroup);
        y += drawItemLabels(this, barGroup);
    }

    return finishSvg(this, background, y + this.itemHeight / 2);
};
