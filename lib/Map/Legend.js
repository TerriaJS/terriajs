...
function drawColorRamp(legend) {
    legend._items.forEach(function(item, i) {
        ...
        legend._context.fillRect(
            legend._itemLeft, 
            legend._itemTop + legend._itemHeight * (legend._items.length - i - 1), 
            legend._itemWidth, 
            legend._itemHeight);
...
function drawBinLabels(legend) {
    ...
    var textOffsetY = 3;
    legend._items.forEach(function(item, i) {
        var y = (legend._items.length - i - 0.5) * legend._itemHeight + legend._itemTop;
        if (item.title) {
            legend._context.fillText(item.title, left + textOffsetX, y + textOffsetY);
        }
        if (item.titleAbove) {
            legend._context.fillText(item.titleAbove, left + textOffsetX, y  - legend._itemHeight / 2 + textOffsetY);
            drawTick(y - legend._itemHeight / 2);
        }
        if (item.titleBelow) {
            legend._context.fillText(item.titleBelow, left + textOffsetX, y  + legend._itemHeight / 2 + textOffsetY);
            drawTick(y + legend._itemHeight / 2);
        }
    });
}
