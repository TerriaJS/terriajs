...
var measureText = require('terriajs-cesium/Source/ThirdParty/measureText');
...
var Legend = function(props) {
    ...
    /**
     * Gets or sets the font to use to render text as a CSS font string (e.g. "10px sans-serif")
     * @type {String}
     */
    this.font = defaultValue(props.font, '14px monospace, sans-serif');
...
defineProperties(Legend.prototype, {
    ...
    width: {
        get: function() {
            return defaultValue(this._width, 315);
        },
        set: function(w) {
            this._width = w;
        }
...
function drawVariableName(legend) {
    // Create a canvas to use for measuring text.
    var canvas = document.createElement('canvas');
    canvas.width = 1;
    canvas.height = 1;

    if (defined(legend.font)) {
        canvas.style.font = legend.font;
    }
    ...
    while (start < parts.length) {
        ...
        var dimensions = measureText(context2D, text, false, true);
        if (dimensions.width <= legend.width) {
            ...
            if (end === start) {
                // TODO: handle the case where the first remaining word doesn't fit on the line.
                break;
            }
        }
    }
 ...
 */
function drawItemBoxes(legend, barGroup, y) {
...
Legend.prototype.drawSvg = function() {
    ...
    var barGroup = addSvgElement(this, 'g', {
        transform: 'translate(' + this.barLeft + ',' + (this.barTop + y) + ')'
    }, 'legend-bar-group');
    if (defined(this.gradientColorMap)) {
        drawGradient(this, barGroup, y);
    }
