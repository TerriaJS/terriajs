...
function initSvg(legend) {
    legend._svg = '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="' + legend.width + '" height="' + legend.height + '">';
}

function finishSvg(legend) {
    return legend._svg + '</svg>';
}    


function addSvgElement(legend, element, attributes, innerText) {
    // TODO support gradient again
    legend._svg += svgElement(legend, element, attributes, innerText);
}

function svgElement(legend, element, attributes, innerText) {
    var ele = '<' + element;
    Object.keys(attributes).forEach(function(att) {
        ele += ' ' + att + '="' + esc(attributes[att]) + '"';
    });
    if (innerText) {
        ele += '>' + defaultValue(esc(innerText), '') + '</' + element + '>';
    } else {
        ele += '/>';
    }
    return ele;
