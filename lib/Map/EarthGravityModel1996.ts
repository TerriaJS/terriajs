...
class EarthGravityModel1996 {
  gridFileUrl: string;
  data?: Promise<Int16Array> | Int16Array;
  ...
  /**
   * Determines if this class will work in the current environment.  It will
   * return false on older browsers without support for typed arrays.
   *
   * @return True if this class may be used in this environment; otherwise, false.
   */
  static isSupported() {
    return (
   ...
   * @param baseUrl The base URL for TerriaJS resources.
   * @param longitude The longitude.
   * @param latitude The latitude
   * @return A promise, that, when it results The height of
   * mean sea level above the ellipsoid at the specified location.  Negative numbers
   * indicate that mean sea level is below the ellipsoid.
   */
  getHeight(longitude: number, latitude: number) {
  ...
  getHeights(cartographicArray: Cartographic[]) {
    return getHeightData(this).then(function(data) {
      if (data === undefined) {
...
function getHeightData(model: EarthGravityModel1996) {
  if (!defined(model.data)) {
    model.data = loadArrayBuffer(model.gridFileUrl) as any;
  }

  return Promise.resolve(model.data).then(function(data) {
    if (!(model.data instanceof Int16Array) && data !== undefined) {
