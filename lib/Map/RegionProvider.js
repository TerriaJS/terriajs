...
RegionProvider.prototype.loadRegionIDs = function() {
    ...
    var that = this;
    var p;
    if (defined(this.regionIdList)) {
        p = loadJson(this.regionIdList);
    } else {
        p = this.loadRegionsFromWfs(this.regionProp);
    }
    p.then(function(json) {
        return that.processRegionIds(json.values, undefined, 'serverReplacements');
    ...
    if (this.disambigProp) {
        ...
        dp.then(function(json) {
            return that.processRegionIds(json.values, that.disambigProp, 'disambigServerReplacements');
        });
        promises.push(dp);
    }
