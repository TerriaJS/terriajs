    ...
    properties.serverReplacements instanceof Array
      ? properties.serverReplacements.map(function(r) {
          return [
            r[0],
            r[1].toLowerCase(),
            new RegExp(r[0].toLowerCase(), "gi") // eslint-disable-line i18next/no-literal-string
          ];
        })
      : [];
...
RegionProvider.prototype.loadRegionIDs = function() {
  ...
  if (this.server === undefined) {
    ...
    throw new DeveloperError(
      "No server for region mapping defined: " + this.regionType // eslint-disable-line i18next/no-literal-string
    );
  }
  ...
  if (!defined(this._loadRegionIDsPromises)) {
    this._loadRegionIDsPromises = [
      fetchAndProcess(
        ...
        this.regionProp,
        undefined,
        "serverReplacements" // eslint-disable-line i18next/no-literal-string
      ),
      fetchAndProcess(
        this.regionDisambigIdsFile,
        this.disambigProp,
        this.disambigProp,
        "disambigServerReplacements" // eslint-disable-line i18next/no-literal-string
      )
    ];
  }
...
RegionProvider.prototype.loadRegionNames = function() {
  ...
  var baseuri = URI(this.analyticsWmsServer).addQuery({
    service: "wfs", // eslint-disable-line i18next/no-literal-string
    version: "2.0",
    request: "getPropertyValue", // eslint-disable-line i18next/no-literal-string
    typenames: this.analyticsWmsLayerName
  });

  // get the list of IDs that we will attempt to match against for this column
  var url = baseuri.setQuery("valueReference", nameProp).toString(); // eslint-disable-line i18next/no-literal-string
  ...
  this._loadRegionNamesPromise = loadText(url).then(function(xml) {
    ...
    if (!defined(obj.member)) {
      ...
      var exception = defined(obj.Exception)
        ? "<br/><br/>" + obj.Exception.ExceptionText // eslint-disable-line i18next/no-literal-string
        : "";
      throw new TerriaError({
        title: i18next.t("map.regionProvider.csvRegionMappingTitle"),
        message: i18next.t(
          "map.regionProvider.csvRegionNamesMessageLoadError",
          { regionType: that.regionType, exception: exception }
        )
      });
    }
...
RegionProvider.prototype.mapRegionsToIndicesInto = function(
  ...
  if (this.regions.length < 1) {
    throw new DeveloperError("Region provider is not ready to match regions."); // eslint-disable-line i18next/no-literal-string
  }
...
RegionProvider.prototype.getRegionFeature = function(
  ...
  url = new URI(url)
    ...
    .addQuery({
      service: "WFS",
      version: "1.1.0",
      request: "GetFeature", // eslint-disable-line i18next/no-literal-string
      typeName: this.layerName,
      outputFormat: "JSON",
      cql_filter: "FID=" + region.FID // eslint-disable-line i18next/no-literal-string
    })
    .toString();
...
function loadRegionsFromWfs(regionProvider, propName) {
  if (regionProvider.serverType !== "WMS") {
    throw new DeveloperError(
      "Cannot fetch region ids for region providers that are not WMS" // eslint-disable-line i18next/no-literal-string
    );
  }
  ...
  var url = regionProvider.corsProxy.getURLProxyIfNecessary(
    baseuri.setQuery("valueReference", propName).toString() // eslint-disable-line i18next/no-literal-string
  );
...
function processRegionIds(
  ...
  if (!isDisambiguation) {
    propertyName = "id"; // eslint-disable-line i18next/no-literal-string
  }
...
function findRegionIndex(regionProvider, code, disambigCode) {
  ...
  var processedCode = applyReplacements(
    regionProvider,
    code,
    "dataReplacements" // eslint-disable-line i18next/no-literal-string
  );
  ...
  if (!defined(id)) {
    ...
    var processedDisambigCode = applyReplacements(
      regionProvider,
      disambigCode,
      "disambigDataReplacements" // eslint-disable-line i18next/no-literal-string
    );
...
function findVariableForAliases(varNames, aliases) {
  for (var j = 0; j < aliases.length; j++) {
    var re = new RegExp("^" + aliases[j] + "$", "i"); // eslint-disable-line i18next/no-literal-string
