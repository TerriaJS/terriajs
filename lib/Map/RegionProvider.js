...
/**
 * Gets a list of region values in the same order as this.regions.
 * Eg. converts arrays ['Vic', 'NSW'] and [10, 20] into [20, 10] if regions = ['NSW', 'Vic']. 
 *
 * @param  {Array} values An array of values to show for each region.
 * @param  {Array} regionValues An array of the regions (eg. the column of State values from a csv file). Could be Strings or Numbers.
 ...
 * @return {TBD} TBD.
 */
RegionProvider.prototype.getRegionValues = function(values, regionValues, disambigValues) {
    // TODO: In some cases region mapping without a data variable is meaningful. ???
    if (this.regions.length < 1) {
        throw new DeveloperError('Region provider is not ready to match regions.');
    }
    if (values.length !== regionValues.length) {
        throw new DeveloperError('Values and regions do not have the same length (' + values.length + ' and ' + regionValues.length + '.');
    }
    ...
    for (var i = 0; i < values.length; i++) {
        var index = this.findRegionIndex(regionValues[i], disambigValues[i]);
        if (index < 0) {
        ...
        // TODO: set reverse lookup from variable back to region index.
