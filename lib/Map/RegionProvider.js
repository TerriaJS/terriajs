...
var RegionProvider = function(regionType, properties) {
    ...
    */
    ...
    // TODO doc
    this.fidList = properties.fidList;
...
/** 
 *  If we have a pre-computed JSON list of indices available, use that - it's faster to retrieve and parse.
 */
RegionProvider.prototype.loadFidList = function() {
    var that = this;
    return loadJson(this.fidList).then(function(json) {
        json[Object.keys(json)[0]].map(function(id, i) {
            that.regions[i] = {
                id: id,
                FID: i
            };
...
RegionProvider.prototype.loadRegionIDs = function() {
    ...
    if (defined(this.fidList)) {
        return this.loadFidList();
    }
...
RegionProvider.prototype.codeMatchesRegionID = function(id, processedCode, disambigId, processedDisambigCode) {
    ...
    if (!defined(processedDisambigCode)) {
...
RegionProvider.prototype.findRegionIndex = function(code, disambigCode) {
    ...
    if (defined(disambigCode)) {
        ...
        // For simple cases, we can simply look up the processed value in a hash. So fast!
        if (defined(this._idIndex[processedCode])) {
            return this._idIndex[processedCode];
        }
    }
    return -1;
};
