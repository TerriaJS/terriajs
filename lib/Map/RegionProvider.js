...
var RegionProvider = function(regionType, properties, regionProviderList) {
    ...
    */
    ...
    /**
     * Array of references to other region providers which will be used to resolve ambiguous names such as local governent areas (LGAs).
     */
...
- we need a few disambiguation options
- each has possible column names, field replacements, server replacements, server field name
- it is assumed that the server field name is on the same layer as this provider - we don't
  have any way to disambiguate otherwise
- we can look at a set of variables and say whether any of them match the disambigs we have
- at this point we have: variable, provider


[ maybe regionproviderlist should generate new disambig providers since it's not really the actual
  region provider ]

Option 1 (current): dab points to a real provider, we also store its region prop
Option 2: dab points to new dab provider, and we set the region prop on it at creation
Option 3: we copy aliases, serverReplacements, dataReplacements properties from dab provider to region provider
...
4. CSV calls RPL.chooseRegionProvider, which asks each RP to identify a region variable
5. Based on response, it assigns RP to the right variable, sets this.selected.region.
(should it also assign the disambig RP to the right variable... but where would regionProp go?)
...
RegionProvider.prototype.findRegionIndex = function(code, disambigCode) {
    for (var i = 0; i < this.regions.length; i++) {
        var dabId;
        if (defined(this.disambigColumns) && defined(this.disambigProp)) {
