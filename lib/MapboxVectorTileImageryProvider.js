    ...
    return loadArrayBuffer(url).then(function(data) {
        ...
        for (var i = 0; i < layer.length; i++) {
            ...
            if (feature.type == 3) {
                ...
                //if (!defined(json.properties.ALBERS_SQM) || json.properties.ALBERS_SQM > pixelArea) {
                    ...
                    for (var i2 = 0; i2 < coordinates.length; i2++) {
                        ...
                        //pos = map(pos, x_range, y_range, [0, canvas.width-1], [canvas.height-1, 0]); // Southern border is at y=canvas.height-1
                ...
                /*}
                    ...
                    console.log('Skipping: ' + json.properties[that._featurePicking.uniqueIdProp] + ' because it\'s area (' + json.properties.ALBERS_SQM + ') is less than ' + pixelArea);
                }*/
            }
            else {
                console.log('Unexpected geometry type: ' + json.geometry.type + ' in region map');
            }
        }
        return canvas;
    });
...
function ringIsExterior(ring) {
    var area = 0;
    for (var i = 0; i < ring.length; i++) {



        // Check https://en.wikipedia.org/wiki/Shoelace_formula
    }
}
            ...
            if (json.geometry.type == 'Polygon') {
                for (var i2 = 0; i2 < json.geometry.coordinates.length; i2++) {
                    ...
                    if (inside(point, json.geometry.coordinates[i2])) {
                        ...
                        // A point can be inside an exterior ring of a polygon and inside an interior ring within the exterior ring
                        // In this case, the point is not in the region defined by the polygon (for example Canberra is within the exterior ring of NSW,
                        //  but also in the interior ring ACT, so isn't in NSW)

                        // According to the Mapbox Vector Tile specifications, a polygon consists of one exterior ring followed by 0 or more interior rings
                        // Therefore we need something like:
                        // for each polygon:
                        //   if point in polygon:
                        //     check polygon is exterior ring <- This should always be true because otherwise the interior ring isn't fully enclosed by it's exterior ring
                        //     for each interior ring:
                        //       if point in interior ring:
                        //         point not in polygon (go to next polygon)
                        ...
                        var uniqueId = json.properties[that._featurePicking.uniqueIdProp];
                        var rowObject = that._featurePicking.regionRowObjects[uniqueId];
                        if (defined(rowObject)) {
                            feature.properties = rowObject;
                            feature.description = that._featurePicking.regionRowDescriptions[uniqueId];
                        } else {
