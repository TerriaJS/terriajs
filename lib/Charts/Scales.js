...
const Scales = {
    ...
    calculate(size, domain, data) {
        ...
        if (!defined(domain)) {
            ...
            domain = {
                x: [d3Array.min(domains.map(l=>l.domain.x), x=>x[0]), d3Array.max(domains.map(l=>l.domain.x), x=>x[1])],
                y: {}
            };
            allUnits.forEach(theUnits=>{
                const domainsWithTheseUnits = domains.filter(l=>(l.units === theUnits));
                domain.y[theUnits] = [
                    d3Array.min(domainsWithTheseUnits, d=>d.domain.y[0]),
                    d3Array.max(domainsWithTheseUnits, d=>d.domain.y[1])
                ];
            });
        ...
        if (yContainsZero) {
            const yPositiveOnly = d3Scale.scaleLinear()
        ...
        for (theseUnits in domain.y) {
           if (domain.y.hasOwnProperty(theseUnits)) {
                const thisYDomain = domain.y[theseUnits];
                yScales[theseUnits] = d3Scale.scaleLinear()
                    .range([size.plotHeight, 0])
                    .domain(thisYDomain);
            }
        }
