...
const Scales = {
    ...
    calculate(size, domain, data, xPadding=0) {
        ...
        function computeDomain() {
            ...
            // if there is at least one usable domain with "showAll", filter the others out.
            const keepAll = data.filter(series => series.showAll && series.points.length > 0).length === 0;
            ...
            const domain = {
                x: [d3ArrayMin(importantDomains, d => d.domain.x[0]), 
                    d3ArrayMax(importantDomains, d => d.domain.x[1])
                ], y: {}
            };
            allUnits.forEach(theUnits=>{
                const domainsWithTheseUnits = importantDomains.filter(d => d.units === theUnits);
                domain.y[theUnits] = [
        ...
        function computeXScale() {
            const xScale = domain.x[0] instanceof Date ? d3ScaleTime() : d3ScaleLinear();
            return xScale
