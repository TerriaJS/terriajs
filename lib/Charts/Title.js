...
const Title = {
    ...
    enterUpdateAndExit(d3Element, titleSettings, margin, data, transitionDuration) {
        ...
        if (defined(titleSettings)) {
            ...
            const numberOfCategories = d3Collection.nest().key(d=>d.categoryName).entries(titleData).length;
            const numberOfColumnNames = d3Collection.nest().key(d=>d.name).entries(titleData).length;
