...
const Title = {
    ...
    enterUpdateAndExit(d3Element, titleSettings, margin, data, transitionDuration) {
        ...
        if (defined(titleSettings)) {
            ...
            const numberOfCategories = d3Collection.nest().key(d=>d.categoryName).length;
            const numberOfColumnNames = d3Collection.nest().key(d=>d.name).length;
            ...
            addedTitleComponents.append('span')
                .attr('class', 'name');
            // Enter and update.
            addedTitleComponents.select('.color').style('background-color', d=>d.color);
            addedTitleComponents.select('.name').text(getName);
            // Exit.
