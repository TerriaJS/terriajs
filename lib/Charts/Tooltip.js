...
import {select as d3Select, event as d3Event, clientPoint as d3ClientPoint} from 'd3-selection';
    ...
    show(html, tooltipElement, tooltipSettings, boundingRect) {
        ...
        switch (tooltipSettings.align) {
            ...
            case 'prefer-right':
            {
                ...
                const leftEdgeWhenPositionedRight = boundingRect.width - tooltipOffset.right - tooltipWidth;
                const rightEdgeWhenPositionedLeft = tooltipOffset.left + tooltipWidth;
                if ((x >= leftEdgeWhenPositionedRight) && (x > rightEdgeWhenPositionedLeft)) {
                    left = tooltipOffset.left;
                } else {
                    right = tooltipOffset.right;
                }
