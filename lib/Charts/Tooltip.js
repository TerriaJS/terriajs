...
import * as d3Transition from 'd3-transition'; // eslint-disable-line no-unused-vars
...
const Tooltip = {
    ...
    select(tooltipSettings) {
        return d3Selection.select('#' + Tooltip.id(tooltipSettings));
    },
    ...
    destroy(tooltipSettings) {
        ...
        if (defined(tooltipSettings)) {
            ...
            const tooltipElement = d3Selection.select('#' + id)['_groups'][0];
            if (tooltipElement) {
                d3Selection.select('#' + id).remove();
    ...
    html(selectedData) {
        ...
        const dataGroupedByCategory = d3Collection.nest().key(d=>d.categoryName).entries(selectedData);
        // And similarly for the column names.
        const dataGroupedByName = d3Collection.nest().key(d=>d.name).entries(selectedData);
    ...
    show(html, tooltipElement, tooltipSettings, boundingRect) {
        ...
        const tooltipHeight = +tooltipElement['_groups'][0][0].offsetHeight;
        const tooltipWidth = +tooltipElement['_groups'][0][0].offsetWidth;
        ...
        switch (tooltipSettings.align) {
            ...
            case 'prefer-right':
            {
                ...
                if ((d3Selection.event.pageX >= left) && (d3Selection.event.pageX > rightEdgeWhenPositionedLeft)) {
                    left = +boundingRect.left + tooltipOffset.left;
                }
                break;
            }
            case 'hover':
                top = d3Selection.event.pageY - tooltipOffset.top;
                left = d3Selection.event.pageX + (-tooltipWidth - tooltipOffset.left);
                break;
