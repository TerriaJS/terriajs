...
const Tooltip = {
  ...
  singleRowHtml(color, name, value, units) {
    ...
    html +=
      '<td><span class="color" ' +
      styleAttribute +
      '></span><span class="name" style="padding-right: 3px;">' +
      name +
      "</span></td>";
    html +=
      '<td class="value">' +
  ...
  html(selectedData) {
    ...
    let html;
    const x =
      typeof selectedData[0].point.x.getMonth === "function"
        ? dateformat(selectedData[0].point.x, "dd/mm/yyyy, HH:MMTT Z")
        : selectedData[0].point.x; // All the data has the same x.
    ...
    if (
      ...
      html += this.singleRowHtml(
        ...
        onlyLine.type === "moment" ? x : onlyLine.point.y,
        onlyLine.units
      );
      html += "</tbody></table>";
      return html;
