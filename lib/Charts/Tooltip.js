...
const transition = d3Transition.transition().duration(showHideDuration);
...
const Tooltip = {
    ...
    destroy(tooltipSettings) {
        ...
        if (defined(tooltipSettings)) {
            ...
            const tooltipElement = d3Selection.select('#' + id).nodes();
            if (tooltipElement) {
    ...
    hide(tooltipElement) {
        tooltipElement.transition(transition).style('opacity', 1e-6);
