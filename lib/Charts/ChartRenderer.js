...
import {mouse as d3Mouse, select as d3Select, event as d3Event} from 'd3-selection';
...
class ChartRenderer {
    ...
    static render(container, state) {
        ...
        const totalOffset = yOffsets.length > 0 ? yOffsets.reduce((a, b)=> a+b) : Size.yAxisWidth;
    ...
    static renderToolTip(renderContext, hasData) {
        ...
        if (defined(state.tooltipSettings)) {
            ...
            chartSVGContainer
                ...
                .on('click', () => highlightAndTooltip(...htParams(), false, false, true))
                .on('mouseout', (e) => {
                    // filter out mouseout events caused by mouse over an internal element like our highlight
                    if (d3Event.relatedTarget === chartSVGContainer.node() || !chartSVGContainer.node().contains(d3Event.relatedTarget)) {
    ...
    static renderAxis(renderContext, hasData) {
        ...
        // -----------------
        // y axis group
        d3Sync(chart.select('.y-axis'), units, 'g', (axis, enter, index) => {
        ...
        chart.selectAll('.y.axis').nodes().forEach((node, yNodeIndex) => {
            ...
            const tickValues = state.mini ? scale.domain() : Scales.truncatedTickValues(scale, numYTicks);
            ...
            if(renderContext.yOffsets.length > 0 && yNodeIndex > 0) {
                offset = renderContext.yOffsets.slice(0, yNodeIndex).reduce((a, b)=> a+b);
            }
            ...
            const axisWidth = !isNaN(renderContext.yOffsets[yNodeIndex]) ? renderContext.yOffsets[yNodeIndex] : 30;
 
            // for units label, rotate and position text appropriately for showing unit label
            yNode.selectAll('.units-label').attr('transform', `translate(${-axisWidth + yAxisLabelWidth}, ${size.yChartOffset + 42 })rotate(270)`).text(unit || '');
...
function highlightAndTooltip(hasData, data, state, scales, g, tooltip, showTooltip = true, showHighlight = true, fireClick = false) {
    if (!hasData) {
        return;
    }
...
function computeYOffset(state, units, scales, size) {
  ...
  units.map((unit, index)=>{
    const scale = scales.y[unit];
    const tickValues = state.mini ? scale.domain() : Scales.truncatedTickValues(scale, numYTicks);
    const maxTickLength = Math.max(...(tickValues.map(v=> v.toString().length)));
