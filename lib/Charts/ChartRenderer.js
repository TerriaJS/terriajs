...
class ChartRenderer {
    ...
    static render(container, state) {
        ...
        size.yAxesWidth = size.yAxesWidth + totalOffset;
    ...
    static renderAxis(renderContext, hasData) {
        ...
        chart.selectAll('.y.axis').nodes().forEach((node, yNodeIndex) => {
          ...
          let offset = 0;
          if(yNodeIndex > 0){
            offset = renderContext.yOffsets.slice(0, yNodeIndex + 1).reduce((a, b)=> a+b);
          }
...
function computeYOffset(state, units, scales, size) {
  ...
  state.data.map((data, index)=>{
    ...
    const tickValues = state.mini ? scale.domain() : Scales.truncatedTickValues(scale, numYTicks);
    const maxTickLength = Math.max(...tickValues).toString().length;
    const offset = maxTickLength > 3 ? (maxTickLength - 2) * 5 : 20;
