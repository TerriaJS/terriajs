...
class ChartRenderer {
    ...
    static renderAxis(renderContext, hasData) {
        ...
        d3Sync(chart.select('.y-axis'), units, 'g', (axis, axes, enter) => {
            ...
            axis = axis
                .attr('class', 'y axis')
                .attr('id', (unit, i) => {index = i; return `${unit}`;});
            ...
            d3Sync(axis, ['ticks', 'colors', 'units-label-shadow-group', 'units-label-group'], 'g', sub => {
                ...
                sub.each(function(item) {
                    if (item.match(/^units/)) {
                        ...
                        const axisWidth = !isNaN(renderContext.yOffsets[index]) ? renderContext.yOffsets[index] : 30;
                        node.attr('transform', `translate(${-axisWidth + yAxisLabelWidth}, ${size.yChartOffset + 42 })rotate(270)`); 
