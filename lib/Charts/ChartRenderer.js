...
class ChartRenderer {
    ...
    static render(container, state) {
        ...
        const totalOffset = yOffsets.reduce((a, b)=> a+b);
        size.yAxesWidth = totalOffset > 0 ? totalOffset : size.yAxesWidth;
    ...
    static renderAxis(renderContext, hasData) {
            ...
            d3Sync(axis, ['ticks', 'colors', 'units-label-shadow-group', 'units-label-group'], 'g', sub => {
                ...
                sub.each(function(item) {
                    if (item.match(/^units/)) {
                        ...
                        node.attr('transform', `translate(${-Size.yAxisWidth + yAxisLabelWidth}, ${size.yChartOffset + 42 })rotate(270)`);
        ...
        chart.selectAll('.y.axis').nodes().forEach((node, yNodeIndex) => {
          ...
          let offset = 0;
          if(yNodeIndex > 0 && renderContext.yOffsets.length > 0){
...
function computeYOffset(state, units, scales, size) {
  ...
  state.data.map((data, index)=>{
    const unit = data.type !== 'moment' && data.units;
    const scale = scales.y[unit];
    const tickValues = state.mini ? scale.domain() : Scales.truncatedTickValues(scale, numYTicks);
    const maxTickLength = Math.max(...tickValues).toString().length;
    const offset = maxTickLength * 10;
