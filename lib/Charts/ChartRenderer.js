  ...
  static render(container, state) {
    ...
    function zoomed() {
      ...
      data.forEach(function(d) {
        d.renderer.zoomOnAxisX(chart, new_xScale);
      });
...
function findSelectedData(data, x) {
  ...
  // Push moment data to the back of array as it's less useful for a range of things
  // eg generating tooltip titles
  selectedData.sort(function(a, b) {
    if (a.type === "moment" && b.type !== "moment") return 1;
    if (b.type === "moment" && a.type !== "moment") return -1;
    return 0;
  });
  return selectedData;
}
...
function hilightData(selectedData, scales, g) {
  ...
  verticalLine
    ...
    .attr("x1", d =>
      scales.zoomedX ? scales.zoomedX(d.point.x) : scales.x(d.point.x)
    )
    ...
    if (state.dblClickTimer) {
      ...
      state.dblClickTimer = setTimeout(function() {
        ...
        selectedData.forEach(chartData => {
          ...
          if (!defined(chartData.type)) {
            return;
          }

          const datasetG = d3Select(`#${chartData.id}`);

          if (chartData.type === "momentPoints") {
            datasetG.selectAll("circle").style("fill-opacity", 0.3);
            datasetG
              .selectAll("circle")
              .filter(`.dataIndex-${chartData.selectedIndex}`)
              .style("fill-opacity", 1);
          } else if (chartData.type === "moment") {
            // Clear the existing highlighted point
            datasetG
              .selectAll("rect")
