...
class ChartRenderer {
    ...
    static renderToolTip(renderContext)
    {
        ...
        if (defined(state.tooltipSettings)) {
            ...
            const highlightAndShow = isClick => highlightDataAndShowTooltip(hasData, state.data, state, scales, chart, tooltip, isClick);
            // Whenever the chart updates, remove the hilighted points and tooltips.
            unhilightDataAndHideTooltip(chart, tooltip);
            chartSVGContainer.on('mouseover', highlightAndShow)
                .on('mousemove', highlightAndShow)
                .on('click', () => highlightAndShow(true))
                .on('mouseout', () => unhilightDataAndHideTooltip(chart, tooltip));
        }
    }
...
function highlightDataAndShowTooltip(hasData, data, state, scales, g, tooltip, isClick) {
    if (!hasData) {
        return;
    }
