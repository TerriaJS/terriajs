  ...
  static renderAxis(renderContext, hasData) {
    ...
    chart
      ...
      .forEach((node, yNodeIndex) => {
        ...
        if (
          unit === undefined &&
          state.data[yNodeIndex].type === "momentPoints"
        ) {
        ...
        if (renderContext.yOffsets.length > 0 && realYNodeIndex > 0) {
          offset = renderContext.yOffsets[yNodeIndex];
        ...
        if (realYNodeIndex > 0) {
          tickSizeInner += realYNodeIndex * -Size.yAxisWidth;
        }
...
function findFirstChartWithYdata(datasets) {
  ...
  for (var i = 0; i < datasets.length; i++) {
    if (datasets[i].type === "momentPoints" || datasets[i].type === "moment")
      continue;
    if (out.firstY === null) out.firstY = datasets[i];
    if (datasets[i].yAxisMin < out.yMin) out.yMin = datasets[i].yAxisMin;
    if (datasets[i].yAxisMax > out.yMax) out.yMax = datasets[i].yAxisMax;
  }
  return out.firstY === null ? null : out;
}
