...
class ChartRenderer {
    ...
    render(container, state) {
        ...
        // Update chart area
        const chartTransform = 'translate(' + (margin.left + size.yAxesWidth) + ',' + (margin.top + Title.getHeight(state.titleSettings)) + ')';
    ...
    renderChart(renderContext) {
        d3Sync(renderContext.chartPlotContainer, renderContext.state.data, 'g', chart => {
            chart.each(function (chartData, index, charts) {
                const chart = d3Select(this).attr('class', 'render')
                    .attr('id', chartData.id);
                chartData.renderer.render(chart, chartData, renderContext);
            });
        }, renderContext.chartTransition);
    }
    
    renderToolTip(renderContext)
    {
    ...
    renderAxis(renderContext)
    {
            ...
            d3Sync(axis, ['ticks', 'colors', 'units-label-shadow-group', 'units-label-group'], 'g', sub => {
                ...
                sub.each(function(item) {
                    if (item.match(/^units/)) {
                        ...
                        node.attr('transform', `translate(${-Size.yAxisWidth + yAxisLabelWidth}, 0)rotate(270)`);
        ...
        chart.selectAll('.y.axis').nodes().forEach((node, yNodeIndex) => {
            ...
            // draw axis
            const axis = yNode.selectAll('.ticks').transition(chartTransition).call(yAxis);
            ...
            if (units.length > 1) {
                ...
                axis.selectAll('line').style('stroke', null).style('opacity', null);
                axis.selectAll('text').style('fill', null);
                axis.selectAll('path.domain').style('stroke', null);
            }
