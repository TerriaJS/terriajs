...
function render(element, state) {
    ...
    function makeMouseOverHtml(selectedPoints, selectedData, selectedColors) {
        if (selectedPoints.length === 0) {
            return '';
        }
        let html = '<p>' + selectedPoints[0].x + '</p>';  // All the data has the same x.
        html += '<table class="mouseover">';
    ...
    function showTooltip() {
        ...
        if (!state.mini) {
            tooltip
                .html(makeMouseOverHtml(selectedPoints, selectedData, selectedColors))
                .style('top', function() {
