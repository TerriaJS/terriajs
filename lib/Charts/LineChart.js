...
const defaultTooltipClassName = 'd3-linechart-tooltip';
const defaultTooltipId = 'd3-linechart-tooltip-id';
...
const LineChart = {
     ...
     * @param  {Object} [state.tooltip] Settings for the tooltip.
     * @param  {String} [state.tooltip.id] The id to use for the tooltip DOM element, defaults to 'd3-linechart-tooltip-id'. Do not change this after creation.
     * @param  {String} [state.tooltip.className] The className to use for the tooltip DOM element, defaults to 'd3-linechart-tooltip'. Do not change this after creation.
     * @param  {String} [state.tooltip.align] One of 'hover' (hover at the mouse position), 'left', 'right', 'prefer-right' (chooses left or right depending on mouse position).
     * @param  {Object} [state.tooltip.offset] An object with top, left and right properties; these properties' meanings depend on the alignment above.
     */
    create(element, state) {
        ...
        if (defined(state.tooltip)) {
            d3.select('body')
                .append('div')
                .attr('id', defaultValue(state.tooltip.id, defaultTooltipId))
                .attr('class', defaultValue(state.tooltip.className, defaultTooltipClassName))
                .style('opacity', 1e-6)
    ...
    destroy(element, state) {
        ...
        if (defined(state.tooltip)) {
            const id = defaultValue(state.tooltip.id, defaultTooltipId);
...
function render(element, state) {
    ...
    if (defined(state.tooltip)) {
        const tooltipId = defaultValue(state.tooltip.id, defaultTooltipId);
...
function makeMouseOverHtml(selected) {
    ...
    const groupedData = d3.nest().key(d=>d.categoryName).entries(selected.data);
    html = '<p class="x-value">' + x + '</p>';
    groupedData.forEach(group=>{
        html += '<p class="category-name">' + group.key + '</p>';
        group.values.forEach((line, index)=>{
            ...
            html += '<td class="name">' + line.name + '</td>';
            html += '<td class="value">' + selected.points[index].y + ' <span class="units">' + line.units + '</span></td>';
            html += '</tr>';
            html += '</tbody></table>';
        });
    });
...
function showTooltip(selected, tooltipElement, tooltipSettings, g) {
    tooltipElement
    ...
    const chartBounds = g[0][0].getBoundingClientRect();
    const tooltipOffset = defaultValue(tooltipSettings.offset, defaultTooltipOffset);
    let top, left;

    switch (tooltipSettings.align) {
        case 'left':
            top = chartBounds.top + tooltipOffset.top;
            left = chartBounds.left + tooltipOffset.left;
            break;
        case 'right':
            top = chartBounds.top + tooltipOffset.top;
            left = chartBounds.right - tooltipOffset.right - tooltipWidth;
            break;
        case 'prefer-right':
            // Only show on the left if we would be under the tooltip on the right, but not on the left.
            top = chartBounds.top + tooltipOffset.top;
            // console.log(chartBounds.top);  // Occasionally the top jerks around a little?
            left = chartBounds.right - tooltipOffset.right - tooltipWidth;
            const rightEdgeWhenPositionedLeft = chartBounds.left + tooltipOffset.left + tooltipWidth;
        ...
        case 'hover':
        default:
            top = Math.min((d3.event.pageY - tooltipOffset.top), Math.max(5, window.innerHeight - tooltipHeight));
...
function highlightDataAndShowTooltip(hasData, data, state, scales, g, tooltip) {
    ...
    showTooltip(selected, tooltip, state.tooltip, g);
}
