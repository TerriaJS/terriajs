...
function render(element, state) {
    ...
    const xTickValues = Scales.truncatedTickValues(scales.x, Math.min(12, Math.floor(size.width / 120) + 1));
    ...
    yAxisElements[0].forEach(yAxisElement=>{
        ...
        yAxis
            .tickValues(Scales.truncatedTickValues(thisYScale, state.mini ? 2 : 8))
            .scale(thisYScale);
        yAxisD3
            .transition('2nd').duration(transitionDuration)
            .style('opacity', hasData ? 1 : 1e-6)
