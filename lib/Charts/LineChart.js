...
const LineChart = {
    ...
    create(element, state, tooltipClassName) {
        ...
        if (tooltipClassName) {
            tooltip = d3.select('body').append('div').attr('class', tooltipClassName).style('opacity', 1e-6).style('position', 'absolute');
        }
    ...
    destroy(element) {
        ...
        if (tooltip) {
            const body = d3.select('body')[0][0];
            body.removeChild(tooltip[0][0]);
        }
    }
...
function render(element, state) {
    ...
    function showTooltip() {
        ...
        const closestXPts = data.map(datum=>datum.reduce((previous, current)=>
        ...
        const selectedData = closestXPts.filter(point=>(Math.abs(point.x - closestXPt.x) < threshold));
        const selectedColors = defined(colors) ? colors.filter((color, index)=>(Math.abs(closestXPts[index].x - closestXPt.x) < threshold)) : undefined;
        const selected = g.select('.selection').selectAll('circle').data(selectedData);
    ...
    function hideTooltip() {
        ...
        tooltip.transition().style('opacity', 1e-6);
    }
