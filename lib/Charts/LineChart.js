...
function render(element, state) {
    ...
    const hasData = (data.pointArrays.length > 0 && data.pointArrays[0].length > 0);
    ...
    const lines = g.selectAll('.line').data(data.pointArrays, d=>d.id).attr('class', 'line');
    ...
    lines.enter().append('path')
        ...
        .style('fill', 'none')
        .style('opacity', 1e-6)
        .style('stroke', (d, i)=>defined(data.colors) ? data.colors[i] : '')
    ...
    function showTooltip() {
        ...
        const closestXPts = data.pointArrays.map(point=>point.reduce((previous, current)=>
        ...
        const nearlyEqualX = (_, index)=>(Math.abs(closestXPts[index].x - closestXPt.x) < threshold);
        // Only select the chart lines (pointArrays) which have their closest x to the mouse = the overall closest.
        const isSelectedArray = closestXPts.map(nearlyEqualX);
        const selectedPoints = closestXPts.filter(nearlyEqualX);
        const selectedColors = defined(data.colors) ? data.colors.filter(nearlyEqualX) : undefined;  // or data.colors.filter((d, i)=>isSelectedArray[i]);
        ...
        tooltip
            .html(makeMouseOverHtml(data, selectedPoints, isSelectedArray, state.mini))
            .style('top', function() {
...
function makeMouseOverHtml(data, selectedPoints, isSelectedArray, mini) {
    ...
    if (mini) {
        if (selectedPoints.length === 1) {
            ...
            html += '</td>';
            html += '<td>' + selectedPoints[0].y + ' ' + units[0] + '</td>';
            html += '</tr>';
            html += '</tbody></table>';
        }
        ...
        selectedPoints.forEach((point, index)=>{
            const styleAttribute = defined(colors[index]) ? ('style="background-color: ' + colors[index] + '" ') : '';
            html += '<tr>';
            html += '<td class="color" ' + styleAttribute + '></td>';
            html += '<td>' + names[index] + '</td>';
            html += '<td>' + categoryNames[index] + '</td>';
            html += '<td>' + point.y + ' ' + units[index] + '</td>';
            html += '</tr>';
        });
        html += '</tbody></table>';
    }
