...
const LineChart = {
     ...
     * @param  {Array[]} state.data The data series. Each subarray contains elements {x: X, y: Y}.
...
function render(element, state) {
    ...
    if (!defined(state.margin) && state.mini) {
        margin.left = miniLeftMargin;
    }
    ...
    g.select('.y.axis')
        .attr('transform', 'translate(' + (state.mini ? -miniLeftMargin : 0) + ', 0)')
        .transition().duration(transitionDuration)
        .call(yAxis);
}
