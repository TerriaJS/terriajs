...
const LineChart = {
    ...
    create(element, state) {
        ...
        if (state.tooltipId) {
            d3.select('body').append('div').attr('id', state.tooltipId).style('opacity', 1e-6).style('position', 'absolute').style('display', 'none');
        }
...
function render(element, state) {
    ...
    const lines = g.selectAll('.line').data(data.map(line=>line.points), (points, index)=>data[index].id).attr('class', 'line');
    ...
    lines.enter().append('path')
        ...
        .style('fill', 'none')
        .style('opacity', 1e-6)
        .style('stroke', (d, i)=>defined(data[i].color) ? data[i].color : '')
    ...
    if (state.tooltipId) {
        ...
        console.log('Highlighting activated on svg', svg);
        const boundHilightData = hilightData.bind(null, hasData, data, scales, g, tooltip);
        unhilightData(g, tooltip);
        svg.on('mouseover', boundHilightData)
            .on('mousemove', boundHilightData)
            .on('click', boundHilightData)
            .on('mouseout', unhilightData.bind(null, g, tooltip));
    }
...
function makeMouseOverHtml(selectedData, selectedPoints) {
    ...
    selectedData.forEach((line, index)=>{
        ...
        html += '<td>' + selectedPoints[index].y + ' ' + line.units + '</td>';
        html += '</tr>';
    });
    html += '</tbody></table>';
    return html;
...
function hilightData(hasData, data, scales, g, tooltip) {
    if (!hasData) {
        return;
    }
    ...
    const selectedColors = selectedData.map(d=>d.color);
    ...
    tooltip
        .html(makeMouseOverHtml(selectedData, selectedPoints))
        .style('top', function() {
...
function unhilightData(g, tooltip) {
