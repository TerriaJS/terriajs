...
const LineChart = {
    ...
    create(element, state) {
        ...
        const d3Element = d3Selection.select(element);
...
function render(element, state) {
    ...
    const g = d3Transition.transition().select(()=> element).selectAll('.line-chart');

    const lines = g.selection().selectAll('.line').data(data, d => d.id).attr('class', 'line');
    // NOTE
    const isFirstLine = (!defined(lines['_groups'][0]));
    ...
    lines.enter().append('path')
        ...
        .style('opacity', 1)
        .style('stroke', d => defined(d.color) ? d.color : '#ffffff');

    // Enter and update.
    lines
        // NOTE: need to put hover back
        // The problem with the below line is d3-transition too late
    ...
    const yAxisElements = g.selection().select('.y.axes').selectAll('.y.axis').data(allUnits, d => d);
    ...
    var noData = g.selection().select('.no-data')
    ...
    if (isFirstLine) {
        g.selection().select('.x.axis').attr('transform', 'translate(0,' + y0 + ')');
    }

    g.selection().select('.x.axis')
    ...
    if (defined(state.grid) && state.grid.x) {
        ...
        g.selection().selectAll('.x.axis line').attr('transform', 'translate(0,' + (size.plotHeight - y0) + ')');
    }
    ...
    if (defined(state.axisLabel) && defined(state.axisLabel.x)) {
        g.selection().select('.x.axis .label')
            .style('text-anchor', 'middle')
    ...
    yAxisElements['_groups'][0].forEach(yAxisElement => {
        const yAxisD3 = d3Selection.select(yAxisElement);
...
function hilightData(selectedData, scales, g) {
    const verticalLine = g.selection().select('.selection').selectAll('line').data(selectedData.length > 0 ? [selectedData[0]] : []);
    ...
    const selection = g.selection().select('.selection').selectAll('circle').data(selectedData);
...
function highlightDataAndShowTooltip(hasData, data, state, scales, g, tooltip) {
    ...
    const localCoords = d3Selection.mouse(g['_groups'][0][0]);
    ...
    Tooltip.show(Tooltip.html(selectedData), tooltip.selection(), state.tooltipSettings, chartBoundingRect);
}

function unhilightDataAndHideTooltip(g, tooltip) {
    g.selection().select('.selection').selectAll('circle').data([]).exit().remove();
    g.selection().select('.selection').selectAll('line').data([]).exit().remove();
...
function fade(d3Element, opacity) {
    return function(selectedLine) {
        d3Element.selection().selectAll('.line')
