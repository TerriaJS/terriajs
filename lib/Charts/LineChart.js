...
function render(element, state) {
    ...
    const svg = d3Element.select('svg')
        .attr('width', state.width)
        .attr('height', state.height);
    d3Element.select('rect.backing')
        .attr('width', size.width)
        .attr('height', size.plotHeight);
    const title = d3Element.select('.chart-title')
    ...
    title
        ...
        .text(defined(state.titleSettings) ? state.titleSettings.title : 'none');
...
function showTooltip(selectedData, tooltipElement, tooltipSettings, g) {
    tooltipElement
        .html(makeMouseOverHtml(selectedData))
        .style('opacity', 1e-6)
        .style('display', 'block')
        .transition()
            .style('opacity', 1);
...
function highlightDataAndShowTooltip(hasData, data, state, scales, g, tooltip) {
    ...
    showTooltip(selectedData, tooltip, state.tooltip, g);
}
