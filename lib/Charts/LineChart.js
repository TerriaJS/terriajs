...
function render(element, state) {
    ...
    const xTickValues = scales.x.ticks(Math.min(12, Math.floor(size.width / 120) + 1));
    const lastXTickValue = xTickValues[xTickValues.length - 1];
    if ((lastXTickValue - scales.x.domain()[0]) / (scales.x.domain()[1] - scales.x.domain()[0]) > (1 - 0.4 / xTickValues.length)) {
        xTickValues.pop();
    }
    ...
    // const yTickValues = scales.y.ticks(8);
    // const lastYTickValue = yTickValues[yTickValues.length - 1];
    // if ((lastYTickValue - scales.y.domain()[0]) / (scales.y.domain()[1] - scales.y.domain()[0]) > (1 - 0.4 / yTickValues.length)) {
    //     yTickValues.pop();
    // }
    ...
    if (state.mini) {
        yAxis.ticks(2)
            .tickSize(0, 0)
            .orient('right');
        // Start it off in the right place so it doesn't animate shifting left.
        yAxisElements.attr('transform', 'translate(' + (-miniYAxisShift) + ', 0)');
    }
    yAxisElements
        .transition('1st').duration(transitionDuration)
