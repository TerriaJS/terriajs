 ...
 * @param {Function} updateCallBack Function called with (d3object, isNewElement)
...
function d3Sync(parent, arrayData, childElementTagName, updateCallback, transition=null) {
    ...
    parent
        ...
        .filter((data, dataIndex) => this.tagName !== childElementTagName) //## data not used??
        .each(() => this.parentElement.appendChild(this))
        ;
    // synchronise intended elements
    const existing = parent
        .selectAll(childNodesSelector)
        .data(arrayData, d => d ? d.id || d : this.id); // id hack for ChartData objects which get re-created each time there are any changes
