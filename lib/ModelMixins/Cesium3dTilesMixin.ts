...
export function showExpressionFromFilters(
  instance: Model<Cesium3dTilesTraits>
) {
  if (!isDefined(instance.filters)) {
    return;
  }
  const terms = instance.filters.map((filter) => {
...
export function createCesium3DTileStyle(
  instance: Model<Cesium3dTilesTraits>
): Cesium3DTileStyle | undefined {
  if (
    !isDefined(instance.style) &&
    (!isDefined(instance.opacity) || instance.opacity === 1)
  ) {
    return;
  }

  const style = clone(toJS(instance.style) || {});
  const opacity = clone(toJS(instance.opacity));
  ...
  // if (isDefined(instance.showExpressionFromFilters)) {
  style.show = showExpressionFromFilters(instance);
  // style.show = toJS(instance.showExpressionFromFilters);
  // }
