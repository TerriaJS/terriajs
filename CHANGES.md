...
#### next release (8.1.23)
* `IDEAL ZOOM` can be customised by providing `lookAt` or `initialCamera` for `idealZoom` in `MappableTraits`. The `lookAt` takes precedence of `initialCamera` if both exist. The values for `initialCamera` can be easily obtained from shared link api.
