...
- **Root group**: The group of a catalog that contains all other groups and catalog items in a catalog.  
...
- If we keep it as `/`, then this catalog will behave the same as a v8 JSON catalog
...
If we use `/`, then we will only need to deal with adding `shareKeys` for the "v8 JSON catalog -> Magda v8 catalog" transformation.
