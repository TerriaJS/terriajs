...
|`regionMappingDefinitionsUrl`|yes|**string | string[]**|`"build/TerriaJS/data/regionMapping.json"`|URL of the JSON file that defines region mapping for CSV files. This option only needs to be changed in unusual deployments. It has to be changed if deploying as static site, for instance. It supports multiple URLs - if provided then the first matching region will be used (in order of URLs) |
